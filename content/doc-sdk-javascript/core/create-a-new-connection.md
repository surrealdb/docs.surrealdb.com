...
<table>
	...
	<tbody>
		...
		<tr>
			<td scope="row" data-label="Method"><a href="/docs/2.x/sdk/javascript/api/core/surreal-session#use"> <code> db.use(namespace,database)</code></a></td>
...
This means that the `.use()` method is not required if you specify the [namespace and database](/docs/surrealdb/introduction/concepts#system-structure) in the `options` object.  However, if you want to switch to a different namespace or database, you should use the [`.use()`](/docs/2.x/sdk/javascript/api/core/surreal-session#use) method. Depending on the use case, you can also specify advanced custom logic for the connection.
