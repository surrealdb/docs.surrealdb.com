...
If you are only interested in accessing the [SDK API reference](/docs/2.x/sdk/javascript/api), you can use `useSurrealClient()` to directly obtain a reference to the `Surreal` client instance. In the example below, we use the `.query()` method which allows you write raw SurrealQL queries:
...
- Explore the [JavaScript SDK API reference](/docs/2.x/sdk/javascript/api) for a complete list of available methods and features
