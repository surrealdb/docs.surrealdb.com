...
Next, we should configure SurrealDB so that it can verify tokens sent to it through the [HTTP REST API](/docs/integration/http) via the “Authorization” header or through any of the [SDKs](/docs/integration/sdks) via the “Authenticate” methods.
...
Although what is described in this guide is the recommended approach, if you require an alternative to using public key cryptography in this manner, you may also use an HMAC algorithm to sign tokens as explained in the “Alternative: Using HMAC” [Annex](https://www.notion.so/Auth0-Integration-Guide-ff06d7dbb03d424b9ef4e1a83ee407e5?pvs=21) section below.
