...
To get started using Docker, you can use the `latest` tag. To start a server use the [`start`](/docs/CLI%20tool/start_command) command. By default SurrealDB runs on port `8000`. To view all the available versions and tags, or to use a specific tag visit the [Docker Hub](https://hub.docker.com/r/surrealdb/surrealdb) page.
...
For details on the [`start`](/docs/CLI%20tool/start_command) command, and all of the available configuration options and arguments, view the [`start command documentation`](/docs/CLI%20tool/start_command).
...
For details on the different commands available, visit the [CLI tool documentation](/docs/CLI%20tool).