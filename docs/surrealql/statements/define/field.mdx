...
The object type allows to define arbitrary nested objects:
...
Flexible types allow you to have `SCHEMALESS` functionality on a `SCHEMAFULL` table. This is especially useful for working with nested schemaless objects.
...
One common pattern is to have a field that is set once and never change.
This can be achieved by setting the latest `VALUE` of a field a.k.a. `$before` the current change.
