...
You can follow the following instructions to import the data:
...
surreal start --username root --password root --allow-all
```

2. Download the file and load it into the database

```bash
# Download the file

mkdir -p surrealdemo && cd surrealdemo
...
curl -v -X POST -u "root:root" -H "NS: experiments" -H "DB: demo" -H "Accept: application/json"   --data-binary @surreal_deal_v1.surql http://localhost:8000/import
```
