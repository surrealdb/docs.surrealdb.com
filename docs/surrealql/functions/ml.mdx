...
<tbody>
  <tr>
    <td><a href="#count"><code>ml::&lt;name-of-model&gt;<version>()</code></a></td>
...
```surql
SELECT *, ml::Prediction<0.0.1>({ squarefoot: squarefoot_col, num_floors: num_floors_col }) AS price_prediction FROM house_listing;
```
...
```surql
SELECT * FROM (SELECT *, ml::Prediction<0.0.1>({ squarefoot: squarefoot_col, num_floors: num_floors_col }) AS price_prediction FROM house_listing) WHERE price_prediction > 177206.21875;
```
