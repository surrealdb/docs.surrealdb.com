...
To quickly test out SurrealDB and SurrealQL functionality, we've included demo data here in `.surql` files which you can download and [`import`](/docs/CLI%20tool/import_command) into SurrealDB using the CLI
...
[Download the dataset here](https://drive.google.com/uc?id=1AlSMBJrncEuqOtbbEXLqtenUb0Z5BjlD&export=download), then [start the server](/docs/CLI%20tool/start_command) and use the [import command](/docs/CLI%20tool/import_command) to add the dataset from your "Downloads" folder. Please be aware that the import process might take a few seconds.
