...
SurrealDB also uses a few [predefined variables](/docs/SurrealQL/parameters).
...
SurrealDB supports the ability to query a range of records, using the record ID. The record ID ranges, retrieve records using the natural sorting order of the record IDs. These range queries can be used to query a range of records in a timeseries context. You can see more here about [array-based Record IDs](/docs/SurrealQL/Data_Model/record_ids#array-based-record-ids).
...
One of the most powerful functions in SurrealDB are the [record links](/docs/SurrealQL/Data_Model/record_links) and [graph connections](/docs/SurrealQL/statements/relate).
