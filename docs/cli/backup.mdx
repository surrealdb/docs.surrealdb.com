---
sidebar_position: 5
---

import CommandHelp from '../../src/components/TableData/CLI/Backup/commandHelp';

# Backup command

# Backup command

The Backup command backs up data into or from an existing database.

:::note
 <em>BEFORE YOU START</em> Make sure you’ve installed SurrealDB — it should only take a second! 
:::

## Command options

<table>
    <thead>
        <tr>
            <th colspan="2">Arguments</th>
            <th>Description</th>
        </tr>
    </thead>  
    <tbody>
        <tr>
            <td colspan="2">
                <code>-c / --help</code>
                <l className='grey'>OPTIONAL</l>
            </td>
            <td>
            Prints help information and instructions on the command-line tool and its arguments
            </td>
        </tr>
        <tr>
            <td colspan="2">
                <code>-u / --user</code>
                <l className='grey'>OPTIONAL</l>
            </td>
            <td>
                Sets master username for the database
            </td>
        </tr>
        <tr>
            <td colspan="2">
                <code>-p / --pass</code>
                <l className='grey'>OPTIONAL</l>
            </td>
            <td>
                Sets master password for the database
            </td>
        </tr>
    </tbody>
</table>

## Positional argument

<table>
    <thead>
        <tr>
            <th colspan="2">Arguments</th>
            <th>Description</th>
        </tr>
    </thead>  
    <tbody>
        <tr>
            <td colspan="2">
                <code>from</code>
                <l className='yellow'>REQUIRED</l>
            </td>
            <td>
            Path to the remote database or file from which to export
            </td>
        </tr>
        <tr>
            <td colspan="2">
                <code>into</code>
                <l className='yellow'>REQUIRED</l>
            </td>
            <td>
            Path to the remote database or file into which to import
            </td>
        </tr>
    </tbody>
</table>

### Example usage
To perform a SurrealQL database Backup, in a terminal run the `surreal backup` command with the required arguments. If you are using a file, the filename should have a `.db` extention


```bash title="Backup a database"
surreal backup [OPTIONS] <from> <into>
``` 

#### Example:

```bash
surreal backup http://localhost:8123 your_backup_name.db 
```

```bash
surreal backup http://localhost:8123 http://localhost:9000 
```

## Command help
To see the help information and usage instructions, in a terminal run the `surreal backup --help` command without any further arguments. This command gives general information on the arguments, inputs, and additional options for the `backup` command.

```bash
surreal backup --help
```

The output of the above command :

```
Backup data to or from an existing database

USAGE:
surreal backup [OPTIONS] <from> <into>

ARGS:
 <file> Path to the sql file to export ,     <into>    Path to the remote database or file into which to import

OPTIONS:
-h, --help                Print help information

-p, --pass <pass>         Database authentication password to use when connecting [default: root]

-u, --user <user>         Database authentication username to use when connecting [default: root]

```