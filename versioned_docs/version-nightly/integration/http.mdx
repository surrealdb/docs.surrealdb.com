...
:::note
__Note:__ Ensure that your URL is set correctly, if running locally, the URL should be `http://localhost:8000`.By default, the URL is set to local. Start your server using the Start command in the [CLI](/docs/nightly/cli/start), before querying the endpoints.
:::
...
### Example usuage via Postman

After you have defined the users permissions, you can use the `POST /signin` endpoint to sign in as a user.

Using the [user credentials](/docs/nightly/security/authentication#scope-users) created add the following to the request body:
```json
{
    "ns": "test",
    "db": "test",
...
Before you sigup a new [Scope user](/docs/nightly/security/authentication#scope-users), you must first [define a scope](/docs/nightly/surrealql/statements/define/scope) for the user. To do so, follow these steps:

:::note
You can also define [System users](/docs/nightly/security/authentication#system-users) and [Define User](/docs/nightly/surrealql/statements/define/user) credentials using the [`POST /sql`](/docs/nightly/integration/http#sql) endpoint.
:::
