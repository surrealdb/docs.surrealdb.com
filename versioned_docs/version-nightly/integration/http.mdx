---
sidebar_position: 3
---

# HTTP & Rest

# HTTP & Rest

<br />

The HTTP endpoints enable simple selection and modifications of all records or a single record in a table, in addition to support for custom SurrealQL queries with multiple statements, using traditional RESTful HTTP url endpoints.

<br />

<table>
    <thead>
        <tr>
            <th>Function</th>
            <th>Description</th>
        </tr>
    </thead>
    <tbody>
        <tr>
            <td><a href="#export"><code>export</code></a></td>
            <td>Exports all data for a specific Namespace and Database</td>
        </tr>
        <tr>
            <td><a href="#health"><code>health</code></a></td>
            <td>Checks the status of the database server and storage engine</td>
        </tr>
        <tr>
            <td><a href="#import"><code>import</code></a></td>
            <td>Imports data into a specific Namespace and Database</td>
        </tr>
        <tr>
            <td><a href="#get-table"><code>Get table records</code></a></td>
            <td>Selects all records in a table from the database</td>
        </tr>
        <tr>
            <td><a href="#post-table"><code>Create table record</code></a></td>
            <td>Creates a records in a table in the database</td>
        </tr>
        <tr>
            <td><a href="#delete-table"><code>Delete table records</code></a></td>
            <td>Deletes all records in a table from the database</td>
        </tr>
        <tr>
            <td><a href="#get-table-id"><code>Get record by ID</code></a></td>
            <td>Selects the specific record from the database</td>
        </tr>
        <tr>
            <td><a href="#post-table-id"><code>Create record with ID</code></a></td>
            <td>Creates the specific record in the database</td>
        </tr>
        <tr>
            <td><a href="#put-table-id"><code>Replace record by ID</code></a></td>
            <td>Updates the specified record in the database</td>
        </tr>
        <tr>
            <td><a href="#patch-table-id"><code>Patch record with ID</code></a></td>
            <td>Modifies the specified record in the database</td>
        </tr>
        <tr>
            <td><a href="#delete-table-id"><code>Delete record by ID</code></a></td>
            <td>Deletes the specified record from the database</td>
        </tr>
        <tr>
            <td><a href="#signup"><code>signup</code></a></td>
            <td>Signs-up as a scope user to a specific scope</td>
        </tr>
        <tr>
            <td><a href="#signin"><code>signin</code></a></td>
            <td>Signs-in as a root, namespace, database, or scope user</td>
        </tr>
        <tr>
            <td><a href="#sql"><code>Send a query</code></a></td>
            <td>Allows custom SurrealQL queries</td>
        </tr>
        <tr>
            <td><a href="#status"><code>status</code></a></td>
            <td>Applies JSON Patch changes to all records in a table, or a specific record</td>
        </tr>
        <tr>
            <td><a href="#version"><code>version</code></a></td>
            <td>Returns the version of the SurrealDB database server</td>
        </tr>
        <tr>
            <td><a href="#import_ml"><code>import ML</code></a></td>
            <td>Uploads your trained machine learning model that is packaged in the `.surml` file format.</td>
        </tr>
    </tbody>
</table>

<br />

## `export`

This HTTP RESTful endpoint exports all data for a specific Namespace and Database.

```json title="Method and URL"
GET /export
```

### Headers
<table>
    <thead>
        <tr>
            <th colspan="2">Header</th>
            <th>Description</th>
        </tr>
    </thead>  
    <tbody>
        <tr>
            <td colspan="2">
                <code>Authorization</code>
                <l className='grey'>OPTIONAL</l>
            </td>
            <td>
                Sets the root, namespace, or database authentication data
            </td>
        </tr>
        <tr>
            <td colspan="2">
                <code>Accept</code>
                <l className='yellow'>REQUIRED</l>
            </td>
            <td>
                Sets the desired content-type of the response
            </td>
        </tr>
        <tr>
            <td colspan="2">
                <code>NS</code>
                <l className='grey'>OPTIONAL</l>
            </td>
            <td>
                Sets the selected Namespace for queries
            </td>
        </tr>
        <tr>
            <td colspan="2">
                <code>DB</code>
                <l className='grey'>OPTIONAL</l>
            </td>
            <td>
                Sets the selected Database for queries
            </td>
        </tr>
    </tbody>
</table>

<br />

## `health`

This HTTP RESTful endpoint checks whether the database server and storage engine are running.

The endpoint returns a `200` status code on success and a `500` status code on failure.

```json title="Method and URL"
GET /health
```

<br />

## `import`

This HTTP RESTful endpoint imports a set of SurrealQL queries into a specific Namespace and Database.

```json title="Method and URL"
GET /import
```

### Headers
<table>
    <thead>
        <tr>
            <th colspan="2">Header</th>
            <th>Description</th>
        </tr>
    </thead>  
    <tbody>
        <tr>
            <td colspan="2">
                <code>Authorization</code>
                <l className='grey'>OPTIONAL</l>
            </td>
            <td>
                Sets the root, namespace, or database authentication data
            </td>
        </tr>
        <tr>
            <td colspan="2">
                <code>Accept</code>
                <l className='yellow'>REQUIRED</l>
            </td>
            <td>
                Sets the desired content-type of the response
            </td>
        </tr>
        <tr>
            <td colspan="2">
                <code>NS</code>
                <l className='grey'>OPTIONAL</l>
            </td>
            <td>
                Sets the selected Namespace for queries
            </td>
        </tr>
        <tr>
            <td colspan="2">
                <code>DB</code>
                <l className='grey'>OPTIONAL</l>
            </td>
            <td>
                Sets the selected Database for queries
            </td>
        </tr>
    </tbody>
</table>

<br />

## `Get table records` {#get-table}

This HTTP RESTful endpoint selects all records in a specific table in the database.

```json title="Method and URL"
GET /key/:table
```

### Headers
<table>
    <thead>
        <tr>
            <th colspan="2">Header</th>
            <th>Description</th>
        </tr>
    </thead>  
    <tbody>
        <tr>
            <td colspan="2">
                <code>Authorization</code>
                <l className='grey'>OPTIONAL</l>
            </td>
            <td>
            Sets the root, namespace, database, or scope authentication data
            </td>
        </tr>
        <tr>
            <td colspan="2">
                <code>Accept</code>
                <l className='yellow'>REQUIRED</l>
            </td>
            <td>
                Sets the desired content-type of the response
            </td>
        </tr>
        <tr>
            <td colspan="2">
                <code>NS</code>
                <l className='grey'>OPTIONAL</l>
            </td>
            <td>
                Sets the selected Namespace for queries
            </td>
        </tr>
        <tr>
            <td colspan="2">
                <code>DB</code>
                <l className='grey'>OPTIONAL</l>
            </td>
            <td>
                Sets the selected Database for queries
            </td>
        </tr>
    </tbody>
</table>

### Translated query
```surql
SELECT * FROM type::table($table);
```

<br />

## `Create a record` {#post-table}

This HTTP RESTful endpoint creates a record in a specific table in the database.

```json title="Method and URL"
POST /key/:table
```

:::note
This HTTP endpoint expects the HTTP body to be a JSON or SurrealQL `object`.
:::

### Headers
<table>
    <thead>
        <tr>
            <th colspan="2">Header</th>
            <th>Description</th>
        </tr>
    </thead>  
    <tbody>
        <tr>
            <td colspan="2">
                <code>Authorization</code>
                <l className='grey'>OPTIONAL</l>
            </td>
            <td>
            Sets the root, namespace, database, or scope authentication data
            </td>
        </tr>
        <tr>
            <td colspan="2">
                <code>Accept</code>
                <l className='yellow'>REQUIRED</l>
            </td>
            <td>
                Sets the desired content-type of the response
            </td>
        </tr>
        <tr>
            <td colspan="2">
                <code>NS</code>
                <l className='grey'>OPTIONAL</l>
            </td>
            <td>
                Sets the selected Namespace for queries
            </td>
        </tr>
        <tr>
            <td colspan="2">
                <code>DB</code>
                <l className='grey'>OPTIONAL</l>
            </td>
            <td>
                Sets the selected Database for queries
            </td>
        </tr>
    </tbody>
</table>

### Translated query
```surql
CREATE type::table($table) CONTENT $body;
```

<br />

## `Delete table records` {#delete-table}

This HTTP RESTful endpoint deletes all records from the specified table in the database.

```json title="Method and URL"
DELETE /key/:table
```

### Headers
<table>
    <thead>
        <tr>
            <th colspan="2">Header</th>
            <th>Description</th>
        </tr>
    </thead>  
    <tbody>
        <tr>
            <td colspan="2">
                <code>Authorization</code>
                <l className='grey'>OPTIONAL</l>
            </td>
            <td>
            Sets the root, namespace, database, or scope authentication data
            </td>
        </tr>
        <tr>
            <td colspan="2">
                <code>Accept</code>
                <l className='yellow'>REQUIRED</l>
            </td>
            <td>
                Sets the desired content-type of the response
            </td>
        </tr>
        <tr>
            <td colspan="2">
                <code>NS</code>
                <l className='grey'>OPTIONAL</l>
            </td>
            <td>
                Sets the selected Namespace for queries
            </td>
        </tr>
        <tr>
            <td colspan="2">
                <code>DB</code>
                <l className='grey'>OPTIONAL</l>
            </td>
            <td>
                Sets the selected Database for queries
            </td>
        </tr>
    </tbody>
</table>

### Translated query
```surql
DELETE FROM type::table($table);
```

<br />

## `Get record by ID` {#get-table-id}

This HTTP RESTful endpoint selects a specific record from the database.

```json title="Method and URL"
GET /key/:table/:id
```

### Headers
<table>
    <thead>
        <tr>
            <th colspan="2">Header</th>
            <th>Description</th>
        </tr>
    </thead>  
    <tbody>
        <tr>
            <td colspan="2">
                <code>Authorization</code>
                <l className='grey'>OPTIONAL</l>
            </td>
            <td>
            Sets the root, namespace, database, or scope authentication data
            </td>
        </tr>
        <tr>
            <td colspan="2">
                <code>Accept</code>
                <l className='yellow'>REQUIRED</l>
            </td>
            <td>
                Sets the desired content-type of the response
            </td>
        </tr>
        <tr>
            <td colspan="2">
                <code>NS</code>
                <l className='grey'>OPTIONAL</l>
            </td>
            <td>
                Sets the selected Namespace for queries
            </td>
        </tr>
        <tr>
            <td colspan="2">
                <code>DB</code>
                <l className='grey'>OPTIONAL</l>
            </td>
            <td>
                Sets the selected Database for queries
            </td>
        </tr>
    </tbody>
</table>

### Translated query
```surql
SELECT * FROM type::thing($table, $id);
```

<br />

## `Create record with ID` {#post-table-id}

This HTTP RESTful endpoint selects a specific record from the database.

```json title="Method and URL"
POST /key/:table/:id
```

### Headers
<table>
    <thead>
        <tr>
            <th colspan="2">Header</th>
            <th>Description</th>
        </tr>
    </thead>  
    <tbody>
        <tr>
            <td colspan="2">
                <code>Authorization</code>
                <l className='grey'>OPTIONAL</l>
            </td>
            <td>
            Sets the root, namespace, database, or scope authentication data
            </td>
        </tr>
        <tr>
            <td colspan="2">
                <code>Accept</code>
                <l className='yellow'>REQUIRED</l>
            </td>
            <td>
                Sets the desired content-type of the response
            </td>
        </tr>
        <tr>
            <td colspan="2">
                <code>NS</code>
                <l className='grey'>OPTIONAL</l>
            </td>
            <td>
                Sets the selected Namespace for queries
            </td>
        </tr>
        <tr>
            <td colspan="2">
                <code>DB</code>
                <l className='grey'>OPTIONAL</l>
            </td>
            <td>
                Sets the selected Database for queries
            </td>
        </tr>
    </tbody>
</table>

### Translated query
```surql
CREATE type::thing($table, $id) CONTENT $body;
```

<br />

## `Replace record by ID` {#put-table-id}

This HTTP RESTful endpoint selects a specific record from the database.

```json title="Method and URL"
PUT /key/:table/:id
```

:::note
This HTTP endpoint expects the HTTP body to be a JSON or SurrealQL object.
:::

### Headers
<table>
    <thead>
        <tr>
            <th colspan="2">Header</th>
            <th>Description</th>
        </tr>
    </thead>  
    <tbody>
        <tr>
            <td colspan="2">
                <code>Authorization</code>
                <l className='grey'>OPTIONAL</l>
            </td>
            <td>
            Sets the root, namespace, database, or scope authentication data
            </td>
        </tr>
        <tr>
            <td colspan="2">
                <code>Accept</code>
                <l className='yellow'>REQUIRED</l>
            </td>
            <td>
                Sets the desired content-type of the response
            </td>
        </tr>
        <tr>
            <td colspan="2">
                <code>NS</code>
                <l className='grey'>OPTIONAL</l>
            </td>
            <td>
                Sets the selected Namespace for queries
            </td>
        </tr>
        <tr>
            <td colspan="2">
                <code>DB</code>
                <l className='grey'>OPTIONAL</l>
            </td>
            <td>
                Sets the selected Database for queries
            </td>
        </tr>
    </tbody>
</table>

### Translated query
```surql
UPDATE type::thing($table, $id) CONTENT $body;
```

<br />

## `Patch record with ID` {#patch-table-id}

This HTTP RESTful endpoint selects a specific record from the database.

```json title="Method and URL"
PATCH /key/:table/:id
```

:::note
This HTTP endpoint expects the HTTP body to be a JSON or SurrealQL object.
:::

### Headers
<table>
    <thead>
        <tr>
            <th colspan="2">Header</th>
            <th>Description</th>
        </tr>
    </thead>  
    <tbody>
        <tr>
            <td colspan="2">
                <code>Authorization</code>
                <l className='grey'>OPTIONAL</l>
            </td>
            <td>
            Sets the root, namespace, database, or scope authentication data
            </td>
        </tr>
        <tr>
            <td colspan="2">
                <code>Accept</code>
                <l className='yellow'>REQUIRED</l>
            </td>
            <td>
                Sets the desired content-type of the response
            </td>
        </tr>
        <tr>
            <td colspan="2">
                <code>NS</code>
                <l className='grey'>OPTIONAL</l>
            </td>
            <td>
                Sets the selected Namespace for queries
            </td>
        </tr>
        <tr>
            <td colspan="2">
                <code>DB</code>
                <l className='grey'>OPTIONAL</l>
            </td>
            <td>
                Sets the selected Database for queries
            </td>
        </tr>
    </tbody>
</table>

### Translated query
```surql
UPDATE type::thing($table, $id) MERGE $body;
```

<br />

## `Delete record by ID` {#delete-table-id}

This HTTP RESTful endpoint deletes a single specific record from the database.

```json title="Method and URL"
DELETE /key/:table/:id
```

### Headers
<table>
    <thead>
        <tr>
            <th colspan="2">Header</th>
            <th>Description</th>
        </tr>
    </thead>  
    <tbody>
        <tr>
            <td colspan="2">
                <code>Authorization</code>
                <l className='grey'>OPTIONAL</l>
            </td>
            <td>
            Sets the root, namespace, database, or scope authentication data
            </td>
        </tr>
        <tr>
            <td colspan="2">
                <code>Accept</code>
                <l className='yellow'>REQUIRED</l>
            </td>
            <td>
                Sets the desired content-type of the response
            </td>
        </tr>
        <tr>
            <td colspan="2">
                <code>NS</code>
                <l className='grey'>OPTIONAL</l>
            </td>
            <td>
                Sets the selected Namespace for queries
            </td>
        </tr>
        <tr>
            <td colspan="2">
                <code>DB</code>
                <l className='grey'>OPTIONAL</l>
            </td>
            <td>
                Sets the selected Database for queries
            </td>
        </tr>
    </tbody>
</table>

### Translated query
```surql
DELETE FROM type::thing($table, $id);
```

<br />

## `signup`

This HTTP RESTful endpoint is used to create an account inside the SurrealDB database server.

```json title="Method and URL"
POST /signup
```

### Headers
<table>
    <thead>
        <tr>
            <th colspan="2">Header</th>
            <th>Description</th>
        </tr>
    </thead>  
    <tbody>
        <tr>
            <td colspan="2">
                <code>Accept</code>
                <l className='yellow'>REQUIRED</l>
            </td>
            <td>
                Sets the desired content-type of the response
            </td>
        </tr>
    </tbody>
</table>

### Example usage
```bash title="Request"
curl -X POST -H "Accept: application/json" -d '{"ns":"test","db":"test","sc":"user_scope","username":"john.doe","password":"123456"}' http://localhost:8000/signup
```

```json title="Response"
{
	"code": 200,
	"details": "Authentication succeeded",
	"token": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIiwibmFtZSI6IkpvaG4gRG9lIiwiaWF0IjoxNTE2MjM5MDIyfQ.SflKxwRJSMeKKF2QT4fwpMeJf36POk6yJV_adQssw5c"
}
```

<br />

## `signin`
```json title="Method and URL"
POST /signin
```
This HTTP RESTful endpoint is used to create an account inside the SurrealDB database server.

The following headers can be used to customise the query and the response.

<table>
    <thead>
        <tr>
            <th colspan="2">Header</th>
            <th>Description</th>
        </tr>
    </thead>  
    <tbody>
        <tr>
            <td colspan="2">
                <code>Accept</code>
                <l className='yellow'>REQUIRED</l>
            </td>
            <td>
                Sets the desired content-type of the response
            </td>
        </tr>
    </tbody>
</table>

### Example with Scope user
```bash title="Request"
curl -X POST -H "Accept: application/json" -d '{"ns":"test","db":"test","sc":"user_scope","user":"john.doe","pass":"123456"}' http://localhost:8000/signin
```

```json title="Response"
{
	"code": 200,
	"details": "Authentication succeeded",
	"token": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIiwibmFtZSI6IkpvaG4gRG9lIiwiaWF0IjoxNTE2MjM5MDIyfQ.SflKxwRJSMeKKF2QT4fwpMeJf36POk6yJV_adQssw5c"
}
```

### Example with Namespace user
```bash title="Request"
curl -X POST -H "Accept: application/json" -d '{"ns":"test","user":"john.doe","pass":"123456"}' http://localhost:8000/signin
```

```json title="Response"
{
	"code": 200,
	"details": "Authentication succeeded",
	"token": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIiwibmFtZSI6IkpvaG4gRG9lIiwiaWF0IjoxNTE2MjM5MDIyfQ.SflKxwRJSMeKKF2QT4fwpMeJf36POk6yJV_adQssw5c"
}
```

### Example with Root user
```bash title="Request"
curl -X POST -H "Accept: application/json" -d '{"user":"john.doe","pass":"123456"}' http://localhost:8000/signin
```

```json title="Response"
{
	"code": 200,
	"details": "Authentication succeeded",
	"token": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIiwibmFtZSI6IkpvaG4gRG9lIiwiaWF0IjoxNTE2MjM5MDIyfQ.SflKxwRJSMeKKF2QT4fwpMeJf36POk6yJV_adQssw5c"
}
```

<br />

## `Send a query` {#sql}

The SQL endpoint enables advanced SurrealQL queries.

```json title="Method and URL"
POST /sql
```

:::note
This HTTP endpoint expects the HTTP body to be a set of SurrealQL statements.
:::

### Headers
<table>
    <thead>
        <tr>
            <th colspan="2">Header</th>
            <th>Description</th>
        </tr>
    </thead>  
    <tbody>
        <tr>
            <td colspan="2">
                <code>Authorization</code>
                <l className='grey'>OPTIONAL</l>
            </td>
            <td>
            Sets the root, namespace, database, or scope authentication data
            </td>
        </tr>
        <tr>
            <td colspan="2">
                <code>Accept</code>
                <l className='yellow'>REQUIRED</l>
            </td>
            <td>
                Sets the desired content-type of the response
            </td>
        </tr>
        <tr>
            <td colspan="2">
                <code>NS</code>
                <l className='grey'>OPTIONAL</l>
            </td>
            <td>
                Sets the selected Namespace for queries
            </td>
        </tr>
        <tr>
            <td colspan="2">
                <code>DB</code>
                <l className='grey'>OPTIONAL</l>
            </td>
            <td>
                Sets the selected Database for queries
            </td>
        </tr>
    </tbody>
</table>

### Example usage
```bash title="Request"
curl -X POST -u "root:root" -H "NS: myapplication" -H "DB: myapplication" -H "Accept: application/json" -d "SELECT * FROM person WHERE age > 18" http://localhost:8000/sql
```

```json title="Response"
[
	{
		"time": "14.357166ms",
		"status": "OK",
		"result": [
			{
				"age": "23",
				"id": "person:6r7wif0uufrp22h0jr0o"
				"name": "Simon",
			},
			{
				"age": "28",
				"id": "person:6r7wif0uufrp22h0jr0o"
				"name": "Marcus",
			},
		]
	}
]
```

<br />

## `status` {#status}

This HTTP RESTful endpoint checks whether the database web server is running, returning a 200 status code.

```json title="Method and URL"
POST /status
```

<br />

## `version` {#version}

This HTTP RESTful endpoint returns the version of the SurrealDB database server.

```json title="Method and URL"
POST /version
```

<br />

## `import ML` {#import_ml}

This HTTP RESTful endpoint uploads your trained machine learning model that is packaged in the `.surml` file format.

```json title="Method and URL"
POST /ml/import
```

As machine learning files can be large, the endpoint expects a stream where chunks of the file are read from the `.surml` file
in an async manner. The [Stream](https://docs.rs/futures-core-preview/latest/futures_core/stream/trait.Stream.html) trait is a
good example on how to stream data. If you don't want to implement your own stream the [surreaml](https://github.com/surrealdb/surrealml/tree/onnx-build)
package can be used to upload the model with the following python code:

```python
from surrealml import SurMlFile

url = "http://0.0.0.0:8000/ml/import"
SurMlFile.upload("./linear_test.surml", url, 5)
```