...
- You have [SurrealDB installed](/docs/surrealdb/nightly/installation/overview) and a [fresh instance of SurrealDB running.](/docs/surrealdb/nightly/introduction/start)
...
Open your command line or terminal and run the following command to [start SurrealDB](/docs/surrealdb/nightly/introduction/start).
...
Now, let's [connect to your SurrealDB instance](/docs/surrealdb/nightly/cli/sql/) using the forwarding address provided by Ngrok.
...
With the help of the `CREATE` statement, [create a new record](/docs/surrealdb/nightly/surrealql/statements/create). 
...
country = 'England';
```
And let's [query the data](/docs/surrealdb/nightly/surrealql/statements/select) from the record now. 
```surql
