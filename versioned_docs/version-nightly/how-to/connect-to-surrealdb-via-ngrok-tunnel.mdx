...
- You have [SurrealDB installed](https://surrealdb.com/install) and a [fresh instance of SurrealDB running.](/docs/nightly/introduction/start)
...
Open your command line or terminal and run the following command to [start SurrealDB](/docs/nightly/introduction/start).
...
Now, let's [connect to your SurrealDB instance](/docs/nightly/cli/sql/) using the forwarding address provided by Ngrok.
...
With the help of the `CREATE` statement, [create a new record](/docs/nightly/surrealdb.com/docs/surrealql/statements/create). 
...
country = 'England';
```
And let's [query the data](/docs/nightly/surrealql/statements/select) from the record now. 
```surql
