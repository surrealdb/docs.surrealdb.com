...
Pointing to a remote JWKS object ensures that token verification will work seamlessly even in the case that AWS rotates their encryption keys and that tokens signed with revoked keys will no longer be accepted by SurrealDB. Ensure that you read the [relevant documentation](/docs/surrealql/statements/define/token#json-web-key-set-jwks) to understand exactly how revocation is handled by SurrealDB.
