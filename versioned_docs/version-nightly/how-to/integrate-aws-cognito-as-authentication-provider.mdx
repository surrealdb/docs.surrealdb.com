...
Depending on how you congigured your user pool, users will be able to register by different means. To ensure this guide can be followed easily, we will create a test user that to authenticate in our web application. Open your user pool and click on the "Create user" button under the "Users" section in the main user pool view. Provide an email address for that user and check "Mark email address as verified" to avoid having to do so manaually. Set a temporary password that matches the requirements that you configured.
...
Pointing to a JWKS file ensures that token verification will work seamlessly even in the case that AWS rotates their encryption keys and that tokens signed with revoked keys will no longer be accepted by SurrealDB. Ensure that you read the [relevant documentation](/docs/surrealql/statements/define/token#json-web-key-set-jwks) to understand exactly how revocation is handled by SurrealDB.
...
In order to allow SurrealDB to establish a connection with AWS Cognito to download the JWKS object, you will require running it with the network <a href="/docs/security/capabilities">capability</a>. For the strongest security, provide your specific Cognito user pool domain when starting SurrealDB with <code>--allow-net</code>. For example: <code>--allow-net cognito-idp.eu-west-1.amazonaws.com</code>.
...
Once the example web application is working correctly, you can inspect the simple code under `app.js` to understand how it works.

## Annex

## Example Single Page Application

You can view and download a minimal example of an web application using AWS Cognito and SurrealDB [here](https://github.com/surrealdb/examples/tree/main/aws-cognito).
