...
- You have a [fresh instance of SurrealDB running](/docs/surrealdb/nightly/introduction/start) with version `1.2.0` or later.

- You can [use a local Docker container](/docs/surrealdb/nightly/installation/running/docker) without volumes for the purposes of this guide.
...
To understand how revocation is handled by SurrealDB, read the [JSON Web Key Set documentation](/docs/surrealdb/nightly/surrealql/statements/define/token#json-web-key-set-jwks) under `DEFINE TOKEN`.
...
In order to allow SurrealDB to establish a connection with AWS Cognito to download the JWKS object, you will require running it with the network <a href="/docs/surrealdb/nightly/security/capabilities">capability</a>.
