...
__BEFORE YOU START :__ Make sure you’ve [installed SurrealDB](/docs/surrealdb/nightly/installation/overview/) — it should only take a second!
...
### [`CREATE`](/docs/surrealdb/nightly/surrealql/statements/create)

The [create statement](/docs/surrealdb/nightly/surrealql/statements/create) is used to add records to the database.
...
;
```
### Querying data with [`SELECT`](/docs/surrealdb/nightly/surrealql/statements/select)

The querying functionality in SurrealDB works similarly to a traditional SQL, but with many of the added benefits of NoSQL query languages. To retrieve data, we will use a [`SELECT` statement](/docs/surrealdb/nightly/surrealql/statements/select). You can query all the articles in your records and this will also return the record links.
...
### [`UPDATE`](/docs/surrealdb/nightly/surrealql/statements/update)

Similar to [UPDATE](/docs/surrealdb/nightly/surrealql/statements/update) in SQL you can also update specific IDs, for example say you wanted to update the first name of the author you can do so:
...
### [`DELETE`](/docs/surrealdb/nightly/surrealql/statements/delete)

You can also delete specific records from the recordID `DELETE author:john` with the [DELETE statement](/docs/surrealdb/nightly/surrealql/statements/delete) or, you could delete a record with specific conditions:

```surql
DELETE article WHERE author.name.first = 'David';
```

### [`REMOVE`](/docs/surrealdb/nightly/surrealql/statements/remove)
...
## Learn more
By completing this guide you have successfully set up a SurrealDB database and ran some SurrealQL queries. To learn more about [SurrealQL, refer to the SurrealQL guides](https://surrealdb.com/docs/nightly/surrealql).
