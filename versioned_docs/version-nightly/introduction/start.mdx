...
__BEFORE YOU START :__ Make sure you’ve [installed SurrealDB](/docs/nightly/installation/overview/) — it should only take a second!
...
### [`CREATE`](/docs/nightly/surrealql/statements/create)

The [create statement](/docs/nightly/surrealql/statements/create) is used to add records to the database.
...
;
```
### Querying data with [`SELECT`](/docs/nightly/surrealql/statements/select)

The querying functionality in SurrealDB works similarly to a traditional SQL, but with many of the added benefits of NoSQL query languages. To retrieve data, we will use a [`SELECT` statement](/docs/nightly/surrealql/statements/select). You can query all the articles in your records and this will also return the record links.
...
### [`UPDATE`](/docs/nightly/surrealql/statements/update)

Similar to [UPDATE](/docs/nightly/surrealql/statements/update) in SQL you can also update specific IDs, for example say you wanted to update the first name of the author you can do so:
...
### [`DELETE`](/docs/nightly/surrealql/statements/delete)

You can also delete specific records `DELETE author:david` using the [DELETE statement](/docs/nightly/surrealql/statements/delete). You could also delete a record with specific conditions:
```surql
DELETE article WHERE author = 'David';
```

### [`REMOVE`](/docs/nightly/surrealql/statements/remove)
