...
The CREATE statement can be used to add records to the database, if those records do not already exist. However, it can not be used to create graph relationships. For that, use the [`RELATE`](/docs/nightly/surrealql/statements/relate) statement.
