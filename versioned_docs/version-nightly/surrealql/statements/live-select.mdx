...
You can also use this UUID to [KILL (stop)](/docs/nightly/surrealql/statements/kill) the Live Query.

The protocol will then send messages that are of a Notification format.
You can find an example of such a message in the [Live Query WebSocket protocol](/docs/nightly/integration/websocket#live) description.
