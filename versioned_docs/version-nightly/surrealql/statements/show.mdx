...
## Requirements
- You must first [`DEFINE`](/docs/surrealql/statements/define/table#example-usage) a [Change Feed](https://surrealdb.com/cf).
...
:::note
__Note:__ An important thing to note here is that the `SINCE <time>` needs to be after the time the `CHANGEFEED` was defined.
:::