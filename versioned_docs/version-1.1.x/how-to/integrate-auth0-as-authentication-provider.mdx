...
- You have a [fresh instance of SurrealDB running.](/docs/1.1.x/introduction/start)

- You can [use a local Docker container](/docs/1.1.x/installation/running/docker) without volumes for the purposes of this guide.
...
Next, we should configure SurrealDB so that it can verify tokens sent to it through the [HTTP REST API](/docs/1.1.x/integration/http) via the “Authorization” header or through any of the [SDKs](/docs/1.1.x/integration/sdks/Overview) via the “Authenticate” methods.
...
To learn about other potential uses for the SurrealDB token functionality, you can read the [`DEFINE TOKEN` documentation page](/docs/1.1.x/surrealql/statements/define/token).
