...
```
> Text: "helloWorld" , Tokens: ["hello", "World"]
...
```
> Text: "123abc!XYZ", Tokens: ["123", "abc", "!", "XYZ"]
...
```
> Text: "résumé café" , Tokens: ["resume", "cafe"]

- `ngram(min,max)`: The ngram filter is used to create a sequence of 'n' tokens from a given sample of text or speech. These items can be syllables, letters, words or base pairs according to the application. It accepts two parameters `min` and `max` which indicates that you want to create n-grams starting from min to size of max. 
```surql
DEFINE ANALYZER example_ngram TOKENIZERS class FILTERS ngram(1,3);
```
> Text: "apple banana", Tokens: ["a", "p", "p", "l", "e", "ap", "pp", "pl", "le", "app", "ppl", "ple", "b", "a", "n", "a", "n", "a", "ba", "an", "na", "an", "na", "ban", "ana", "nan", "ana"]
...
```
> Text: "apple banana", Tokens: ["a", "ap", "app", "b", "ba", "ban"]
...
```
> Text: "running cats", Tokens: ["run", "cat"]
