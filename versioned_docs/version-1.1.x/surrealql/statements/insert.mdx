...
The `INSERT` statement can be used to insert or update data into the database, using the same statement syntax as the traditional SQL Insert statement. However, it can not be used to create graph relationships. For that, use the [`RELATE`](/docs/surrealql/statements/relate) statement.
