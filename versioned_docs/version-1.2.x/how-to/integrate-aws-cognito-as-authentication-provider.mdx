...
To understand how revocation is handled by SurrealDB, read the [JSON Web Key Set documentation](/docs/1.2.x/surrealql/statements/define/token#json-web-key-set-jwks) under `DEFINE TOKEN`.
...
In order to allow SurrealDB to establish a connection with AWS Cognito to download the JWKS object, you will require running it with the network <a href="/docs/1.2.x/security/capabilities">capability</a>. 
