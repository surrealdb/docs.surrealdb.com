...
When creating a record with the create statment, you can specify the [record ID](/docs/surrealdb/surrealql/datamodel/ids) right after the `CREATE` using the `:` separator. The name following the `CREATE` keyword is the Table name, and the ID following the `:` is the ID. These two togther form the full recordID. The ID can be a string or a number. If no ID is specified, a random ID will be generated.
...
The above will create a new Person table and also create a new record with a random ID.
...
```surql
CREATE person:26 SET age = 46, username = "john-smith" PARALLEL;
```
