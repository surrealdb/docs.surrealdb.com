...
Check out the [HTTP & Rest integration documentation](/docs/1.2.x/integration/http) for a detailed list of all the endpoints supported.
...
- SurrealDB installed - If you do not, see the [installation guide](/docs/1.2.x/installation/overview) specific to your machine.
...
To do so run the [Start command](/docs/1.2.x/cli/start) in your terminal: 

```surql
surreal start --auth --user root --pass root
```

The above command [enables authentication](/docs/1.2.x/cli/start#authentication) with `--auth` and specifies that the user and password are `root` - Note that this is just for demonstration purposes. You can replace it with any other word.

After running locally, head over to Postman and create a fork for your workspace. You should see all the endpoints listed. You can also [check the endpoints in the documentation](/docs/1.2.x/integration/http). 


## Using [`INFO` Statement](/docs/1.2.x/surrealql/statements/info)

You can get information about your workspace by using the INFO statement. However, you need the right permissions in other to see the output. See the documentation for the [INFO statement](/docs/1.2.x/surrealql/statements/info).
...
You can see the JSON output using the [INFO statement](/docs/1.2.x/surrealql/statements/info) in the body of the request 
...
DB:{{database}}
```
You can also [define a user](/docs/1.2.x/surrealql/statements/define/user) with other credentials such as Root or Database user. For this tutorial, we will be using only Scope users.
...
In the request body, [define the scope](/docs/1.2.x/surrealql/statements/define/scope) `human` with a session of 24 hours: 
...
The code above allows a user to sign up as a scope user with their email and password. Then hash the password with the [`crypto::argon2::generate`](/docs/1.2.x/surrealql/functions/crypto/#cryptoargon2generate) function 

The sign in logic gets all the users where the emails match and uses the [`crypto::argon2::compare`](/docs/1.2.x/surrealql/functions/crypto/#cryptoargon2compare) function to check the hash value to the unhashed. 
...
The Postman collection has a couple of endpoints for `POST` `GET` `PUT` `PATCH` `DEL` operations. For example, If you want to add a new entry to the table `Person` (Which is the default table in the collection) go to the [`POST /key/:table`](/docs/1.2.x/integration/http#post-table) endpoint then in a body of the request and add the table content in the body of the request. E.g:
...
You should get a response as seen below, notice that there is a [record ID](/docs/1.2.x/surrealql/datamodel/ids), with this record ID you can now use any of the [`/key/:table/:id`](/docs/1.2.x/integration/http#get-record) endpoints for  `POST` `GET` `PUT` `PATCH` `DEL` operations. 
...
The Postman collection is still in active development for more information on using  surrealDB via the HTTP endpoints. [Check out our documentation on HTTP and rest endpoints](/docs/1.2.x/integration/http)