...
__BEFORE YOU START :__ Make sure you’ve [installed SurrealDB](/docs/1.2.x/installation/overview/) — it should only take a second!
...
### [`CREATE`](/docs/1.2.x/surrealql/statements/create)

The [create statement](/docs/1.2.x/surrealql/statements/create) is used to add records to the database.
...
;
```
### Querying data with [`SELECT`](/docs/1.2.x/surrealql/statements/select)

The querying functionality in SurrealDB works similarly to a traditional SQL, but with many of the added benefits of NoSQL query languages. To retrieve data, we will use a [`SELECT` statement](/docs/1.2.x/surrealql/statements/select). You can query all the articles in your records and this will also return the record links.
...
### [`UPDATE`](/docs/1.2.x/surrealql/statements/update)

Similar to [UPDATE](/docs/1.2.x/surrealql/statements/update) in SQL you can also update specific IDs, for example say you wanted to update the first name of the author you can do so:
...
### [`DELETE`](/docs/1.2.x/surrealql/statements/delete)

You can also delete specific records `DELETE author:david` using the [DELETE statement](/docs/1.2.x/surrealql/statements/delete). You could also delete a record with specific conditions:
```surql
DELETE article WHERE author = 'David';
```

### [`REMOVE`](/docs/1.2.x/surrealql/statements/remove)
...
## Learn more
By completing this guide you have successfully set up a SurrealDB database and ran some SurrealQL queries. To learn more about [SurrealQL, refer to the SurrealQL guides](https://surrealdb.com/docs/1.2.x/surrealql).
