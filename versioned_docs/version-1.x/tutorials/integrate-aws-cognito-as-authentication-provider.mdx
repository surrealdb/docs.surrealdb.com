...
- You have a [fresh instance of SurrealDB running](/docs/surrealdb/1.x/introduction/start) with version `1.2.0` or later.

- You can [use a local Docker container](/docs/surrealdb/1.x/installation/running/docker) without volumes for the purposes of this guide.
...
Next, we should configure SurrealDB so that it can verify tokens sent to it through the [HTTP REST API](/docs/surrealdb/1.x/integration/http) via the “Authorization” header or through any of the [SDKs](/docs/surrealdb/1.x/integration/sdks) via the “Authenticate” methods.
...
To understand how revocation is handled by SurrealDB, read the [JSON Web Key Set documentation](/docs/surrealdb/1.x/surrealql/statements/define/token#json-web-key-set-jwks-since-120) under `DEFINE TOKEN`.
...
In order to allow SurrealDB to establish a connection with AWS Cognito to download the JWKS object, you will require running it with the network <a href="/docs/surrealdb/1.x/security/capabilities">capability</a>. 
