...
In SurrealDB, you can use an M-Tree based index to perform the search. 

### M-Tree index
Suitable for the task of finding exact nearest neighbors based on a distance metric (like Euclidean distance)
 ```surql
DEFINE INDEX index_name ON table_name FIELDS column_name MTREE DIMENSION n;
```

Consider a record with 4 dimensional vectors.
...
Let's define a M-tree type of index named `mt_pt` on the `point` field of the `pts` record.

```surql
DEFINE INDEX mt_pt ON pts FIELDS point MTREE DIMENSION 4;
```

With the M-tree index in place, you can now perform a vector search to find records based on the distance from a given point.

In this example, the query searches for points closest to the vector [2,3,4,5] and uses the [vector functions](https://surrealdb.com/docs/surrealdb/surrealql/functions/vector) to calculate the distance between the two points. Mtree currently supports [Euclidean](https://surrealdb.com/docs/surrealdb/surrealql/functions/vector#vectordistanceeuclidean), [Cosine](https://surrealdb.com/docs/surrealdb/surrealql/functions/vector#vectorsimilaritycosine), [Manhattan](https://surrealdb.com/docs/surrealdb/surrealql/functions/vector#vectordistancemanhattan) and [Minkowski](https://surrealdb.com/docs/surrealdb/surrealql/functions/vector#vectordistanceminkowski) distance functions. When no function is specified it chooses Euclidean for the default distance function. 
