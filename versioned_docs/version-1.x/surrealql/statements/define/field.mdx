...
- [You must select your namespace and database](/docs/surrealdb/1.x/surrealql/statements/use) before you can use the `DEFINE FIELD` statement.
...
The `DEFINE FIELD` statement allows you to set the data type of a field. For a full list of supported data types, see [Data types](/docs/surrealdb/1.x/surrealql/datamodel).
...
The example below shows how to define a field `user` on a `POST` table. The field is of type [record](/docs/surrealdb/1.x/surrealql/datamodel/records). This means that the field can store a `record<user>` or `NONE`.
...
In the example below, the `VALUE` clause is used to ensure that the email address is always stored in lowercase characters by using the [`string::lowercase`](/docs/surrealdb/1.x/surrealql/functions/string#stringlowercase) function.
