...
The example below shows how to define a field `user` on a `POST` table. The field is of type [record](/docs/surrealdb/surrealql/datamodel). This means that the field can store a `record<user>` or `NONE`.
