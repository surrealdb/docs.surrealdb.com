...
title: Integrate AWS Cognito as an Authentication Provider | Authentication | SurrealDB
description: In this guide you will learn how to configure AWS Cognito to issue tokens that can be used with SurrealDB, configure SurrealDB to accept tokens issued by AWS Cognito, define user-level authorization using SurrealDB scopes, authenticate users with AWS Cognito in a client-side web application, and retrieve and update information from SurrealDB using the authenticated user.
---
