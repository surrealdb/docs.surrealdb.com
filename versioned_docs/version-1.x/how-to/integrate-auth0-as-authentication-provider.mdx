...
title: Integrate Auth0 as an Authentication Provider| Authentication | SurrealDB | Auth0
description: This guide will cover using Auth0 as the authentication provider for single-page web applications using SurrealDB as the only backend. In this guide you will learn how to configure Auth0 to issue tokens that can be used with SurrealDB, configure SurrealDB to accept tokens issued by Auth0, define user-level authorization using SurrealDB scopes, authenticate users with Auth0 in a single-page application, and retrieve and update information from SurrealDB using the authenticated user.
---
