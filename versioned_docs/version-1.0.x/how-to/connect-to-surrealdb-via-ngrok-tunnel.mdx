...
- You have [SurrealDB installed](/docs/1.0.x/installation/overview) and a [fresh instance of SurrealDB running.](/docs/1.0.x/introduction/start)
...
Open your command line or terminal and run the following command to [start SurrealDB](/docs/1.0.x/introduction/start).
...
Now, let's [connect to your SurrealDB instance](/docs/1.0.x/cli/sql/) using the forwarding address provided by Ngrok.
...
With the help of the `CREATE` statement, [create a new record](/docs/1.0.x/surrealql/statements/create). 
...
country = 'England';
```
And let's [query the data](/docs/1.0.x/surrealql/statements/select) from the record now. 
```surql
