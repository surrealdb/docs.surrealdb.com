...
- [A Command line interface (CLI)](/docs/surrealdb/cli/surql) for interacting with SurrealDB or a [Surrealist sandbox](/docs/surrealist/#connecting-to-the-sandbox).
...
In the code above you may notice the `ASSERT` clause. This can be used to validate any restrictions you want on a field. In the example above the [`string::is::email`](/docs/surrealdb/surrealql/functions/database/string#stringisemail) function is used to check whether the value is an email.
...
In SurrealDB there are two ways you can define a schemaless table. You can either use any of the data definition statements such as [`CREATE`](/docs/surrealdb/surrealql/statements/create) or [`UPDATE`](/docs/surrealdb/surrealql/statements/update)  and that will make a table based on the [record ID](/docs/surrealdb/surrealql/datamodel/ids#object-based-record-ids) specified. For example:
...
In the example above, you used the `CREATE` statement to make an `IC_directory` table and created an Object ID for this table. ID can be anything you want and is randomly generated if not defined. You have also used the UPDATE statement to make a `manager_directory`, which has `janesmith` as the ID. Learn more about [Record IDs in the documentation](/docs/surrealdb/surrealql/datamodel/ids#object-based-record-ids).
...
In SurrealDB, you can add [flexible fields](/docs/surrealdb/surrealql/statements/define/field#flexible-data-types) to a schemafull table using the `DEFINE FIELD` statement using the `FLEXIBLE` clause. This allows you to have `schemaless` functionality in a `schemafull` table. For example
...
Learn more about setting up a schema in the [`DEFINE` statement documentation](/docs/surrealdb/surrealql/statements/define/).