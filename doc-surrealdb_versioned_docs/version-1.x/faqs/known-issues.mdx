...
In the [`DELETE` statement](/docs/surrealdb/surrealql/statements/delete) the `ONLY` clause doesn't work as expected because it checks for a length of 1 and `DELETE` doesn't return anything by default at this time. So it always gives the following error:
