...
In the [`DELETE` statement](/docs/surrealdb/surrealql/statements/delete) the `ONLY` clause doesn't work as expected because it checks for a length of 1 and `DELETE` doesn't return anything by default at this time. So it always gives the following error: 'Expected a single result output when using the ONLY keyword'. However, adding `RETURN $before` will force `DELETE` to return the deleted record, which will be the expected length of 1.
