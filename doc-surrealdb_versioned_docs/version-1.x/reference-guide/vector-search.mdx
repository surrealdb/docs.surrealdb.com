...
As cosine similarity measures the orientation of the vectors, rather than their absolute magnitudes, this also makes it suitable to help capture the overall similarity in facial  recognition example for features and proportions of faces, while being less sensitive to factors like scale or image brightness.
...
You can also use the  [`REBUILD` statement](/docs/surrealdb/surrealql/statements/rebuild), which allows for the manual rebuilding of indexes as needed (e.g. if you restart the server). This approach ensures that while we explore persistence options, we maintain the optimal performance that users expect from HNSW and MTree, providing flexibility and control over the indexing process.
