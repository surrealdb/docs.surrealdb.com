...
title: Built-in functions | SurrealQL
...
Besides basic JavaScript utilities and [classes for SurrealQL types](/docs/surrealdb/surrealql/functions/script/type-conversion), there are a handful of utilities built into the embedded scripting functions.
...
:::note
___NOTE:___ For complete documentation, please refer to the MDN documentation.
:::
...
:::note
___NOTE:___ Only subqueries can be executed with the query() function. This means that only a single query can currently be executed, and that only CRUD operations are allowed.
:::
