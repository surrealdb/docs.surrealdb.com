...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td scope="row" data-label="Function"><a href="#typething"><code>type::range()</code></a></td>
      <td scope="row" data-label="Description">Converts a values into a record range</td>
    </tr>
    <tr>
...
```surql title="API DEFINITION"
type::range(any, any, any, object) -> thing
```
This function takes on up two four argument. 
The first is the table of range. 
The second and third are the beginning and end of the range. If either is NULL or not specified, the range will be unbounded in the direction that is not specified.
The fourth argument is an object which can be used to define how the range is bounded. 
If an object is specified as the fourth argument with the property `begin` or `end` containing the value `"included"` or `"excluded"` the ranges boundd will be updated accordingly.
...
RETURN type::thing($tb, $begin, $end, { begin: "excluded", end: "included" });
```
