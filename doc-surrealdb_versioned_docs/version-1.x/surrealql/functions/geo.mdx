...
The following example shows this function, and its output, when used in a [`RETURN`](/docs/surrealdb/surrealql/statements/return) statement for four approximate points found on a map for the US state of Wyoming which has an area of 253,340 km<sup>2</sup> and a mostly rectangular shape.

![A map of Wyoming in the United States with four approximate points on each corner used to approximate its total surface area in SurrealDB's geo area function.](images/wyoming.png)
...
The `geo::bearing` function calculates the bearing between two geolocation points. Bearing is a number between 0 and 360 degrees, beginning from the north.

![A circle showing how bearing is defined from 0 degrees to 360 degrees.](images/bearing.png)
...
![A map showing the path from Paris, the capital of France, to a French town called Le Puy En Velay. The bearing is south southeast.](images/paris_to_le_puy_en_velay.png)
...
RETURN geo::centroid({
	type: "Polygon",
	coordinates: [[
		[-0.03921743611083755, 51.88106875736589], -- London
        [30.48112752349519, 50.683770897949124], -- Kyiv
        [23.661745240015446, 42.94500782833793], -- Sofia
        [1.9248153436185966, 41.69698118125476] -- Barcelona
	]]
});
```
...
![A map showing the centroid between four points in Europe: London, Kyiv, Sofia, and Barcelona. The centroid itself is located in Austria.](images/centroid.png)
...
![A map showing the distance in a straight line from London, the capital of the United Kingdom, to Harare, the capital of Zimbabwe.](images/london_to_harare.png)
