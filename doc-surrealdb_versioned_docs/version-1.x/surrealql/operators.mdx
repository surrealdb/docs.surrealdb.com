...
SurrealDB supports different K-Nearest Neighbors methods, each with unique requirements for syntax.
Below are the details for each method, including how to format your query and an example to guide you.
...
In practice, the efficiency and scalability of the KNN algorithm are crucial, especially when dealing with large datasets.
Different implementations of KNN are tailored to optimize these aspects without compromising the accuracy of the results.
In SurrealDB, we offer several methods to perform KNN searches, each with unique characteristics:

### Brute Force Method <l className='purple'>Since 1.3.0</l>

Best for smaller datasets or when the highest accuracy is required.

Syntax: ```<|K,DISTANCE_METRIC|>```
...
SELECT id FROM pts WHERE point <|2,EUCLIDEAN|> [2,3,4,5] EXPLAIN;
```

### MTREE Index Method <l className='purple'>Since 1.3.0</l>
...
```surql
CREATE pts:3 SET point = [8,9,10,11];
DEFINE INDEX mt_pts ON pts FIELDS point MTREE DIMENSION 4 DISTANCE EUCLIDEAN;
...
### HNSW Method <l className='purple'>Since 1.5.0</l>
...
SELECT id, vector::distance::euclidean(point, [2,3,4,5]) AS dist FROM pts WHERE point <|10,40|> [2,3,4,5];
SELECT id FROM pts WHERE point <|10,40|> [2,3,4,5] EXPLAIN;
```
