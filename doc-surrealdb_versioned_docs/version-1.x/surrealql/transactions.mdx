...
:::note
_Note_: SurrealDB only supports transaction statements as top-level statements, not as a part of function bodies, blocks, or subqueries.
:::
...
While transactions are automatically rolled back if an error occurs in any of its statements, [THROW](/docs/surrealdb/surrealql/statements/throw) can also be used to explicitly break out of a transaction at any point.
