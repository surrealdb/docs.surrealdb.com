...
The `REBUILD` statement is used to rebuild resources in SurrealDB. It is usually used in relation to a specified [Index](/docs/surrealdb/surrealql/statements/define/indexes) to optimize performance. It is useful to rebuild indexes because sometimes [M-TREE](/docs/surrealdb/surrealql/statements/define/indexes#m-tree-index-since-130) and [HNSW](/docs/surrealdb/surrealql/statements/define/indexes#hnsw-hierarchical-navigable-small-world-since-150) index performances can degrade due to frequent updates.
...
```
:::note
__Note:__ The `IF EXISTS` and TABLE clauses are optional.
:::
