...
As Record IDs work like pointers, an update to a non-existing record will create the record. Note the difference in behaviour between an update on all records and an update on a single specified record ID:

```surql
-- Checks for `city` records, none found, returns nothing
UPDATE city SET time_modified = time::now();
-- Goes to location `city:london` and sets `time_modified` field
UPDATE city:london SET time_modified = time::now();
```
...
[
	{
		id: city:london,
		time_modified: '2024-06-05T01:36:15.913Z'
	}
]
```
...
-- `click_count` set to 1 even if no `webpage` records exist before update
-- as SurrealDB infers `click_count` to be a number at its default value of 0
