...
`@target` refers to either record output including an `id` field, or even just a record ID on its own.
...
An `UPDATE` statement will work on any value as long as is an `id` or contains an `id` field holding a record ID:
