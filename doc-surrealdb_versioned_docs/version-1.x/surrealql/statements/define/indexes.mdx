...
DEFINE INDEX [ IF NOT EXISTS ] @name ON [ TABLE ] @table [ FIELDS | COLUMNS ] @fields
	[ UNIQUE
        ...
        | MTREE DIMENSION @dimension [ TYPE @type ] [ DISTANCE @distance ] [ CAPACITY @capacity]
        | HNSW DIMENSION @dimension [ TYPE @type ] [DISTANCE @distance] [ EFC @efc ] [ M @m ]
    ]
```
