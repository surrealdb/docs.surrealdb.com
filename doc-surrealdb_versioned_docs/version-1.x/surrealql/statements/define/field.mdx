...
```
$value: The value of the field itself.
$this: The table that the field is being defined on.
```
...
## Interacting with the parent table

The $this parameter gives access inside a DEFINE FIELD statement to the parent table.

```surql
DEFINE FIELD first_name ON TABLE person TYPE string;
DEFINE FIELD surname ON TABLE person TYPE string;
