...
The original `IF ELSE` syntax involves one or more conditions, `THEN`, an expression if the condition is true, and the `END` keyword to finalize it.
...
Another `IF ELSE` syntax does not use `THEN` or `END`, instead using `{}` to open up a new scope which will be run when a condition is true.

```surql title="SurrealQL IF with new scope syntax"
