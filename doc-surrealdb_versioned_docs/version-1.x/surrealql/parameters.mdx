...
`$this` represents the current context in an operation.

```surql title="$this as the current item in an iterative operation"
RETURN [1,2,3,4,5,6,7,8,9][WHERE $this % 2 = 0];
```
...
When inside a subquery, `$parent` can be used to access the parent context.
...
Thus, `$this` and `$parent` can be used together to compare items in both the current and parent context.
