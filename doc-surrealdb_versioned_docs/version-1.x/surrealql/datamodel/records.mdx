...
A [DEFINE FIELD](/docs/surrealdb/surrealql/statements/define/field) statement can be used to ensure that a field can only be a record link. Record links are of type `array<record<record_name>>`, or `option<array<record<record_name>>>` for records that might not have record links to other records upon creation.
