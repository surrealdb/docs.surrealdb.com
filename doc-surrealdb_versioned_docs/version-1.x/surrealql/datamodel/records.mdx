...
## Record links in defined schema

A [DEFINE FIELD](/docs/surrealdb/surrealql/statements/define/field) statement can be used to ensure that a field can only be a record link. A single record link is of type `record<record_name>`. In practice, `option<array<record<record_name>>>` offers the most flexibility by allowing a type to have any number of record links, including none at all.
