...
Whenever authentication is performed with any kind of user against SurrealDB, a session is established between the client and the SurrealDB server with which the connection was established. These sessions persist on the server for the duration of the connection, whether it is a single request through the HTTP REST API or through multiple requests in the same connection using the WebSocket API and any of the SDK that leverage it.

### Expiration <l className='purple'>Since 1.3.0</l>

Authenticated sessions are established using a token in the JWT format and will remain authenticated for as long as that token is valid according to its `exp` claim. Whenever a token that was used to establish a session expires, so will the session. This is specially relevant in the case of WebSocket connections such as those used by the SurrealDB SDK. Expired sessions can no longer be used to call authenticated methods and attempting to do so will result in a specific error indicating that the session has expired.
...
Although SurrealDB sessions will always match the expiration of tokens used to authenticate, SurrealDB will not set a session expiration in sessions established when authenticating as a system user with credentials. This is due to the fact that all tokens issued by SurrealDB have a default duration of 24 hours which cannot currently be modified in the case of system users. If a system user authenticates using a token, the expiration of the token will be used in the session, regardless of whether this token was issued by SurrealDB or a third party.
