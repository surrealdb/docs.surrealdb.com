...
The code above allows a user to sign up as a scope user with their email and password. Then hash the password with the [`crypto::argon2::generate`](/docs/surrealdb/2.x/surrealql/functions/crypto#cryptoargon2generate) function 

The sign in logic gets all the users where the emails match and uses the [`crypto::argon2::compare`](/docs/surrealdb/2.x/surrealql/functions/crypto#cryptoargon2compare) function to check the hash value to the unhashed. 
