...
DB:{{database}}
```
You can also [define a user](/docs/surrealdb/2.x/surrealql/statements/define/user) with other credentials such as Root or Database user. For this tutorial, we will be using only Scope users.
...
In the request body, [define the scope](/docs/surrealdb/2.x/surrealql/statements/define/scope) `human` with a session of 24 hours: 
...
The code above allows a user to sign up as a scope user with their email and password. Then hash the password with the [`crypto::argon2::generate`](/docs/surrealdb/2.x/surrealql/functions/database/crypto#cryptoargon2generate) function 
