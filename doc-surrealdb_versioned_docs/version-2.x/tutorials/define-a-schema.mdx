...
When starting a new database project, there are a couple of early decisions to be made, such as creating tables and defining the records that will be in these tables and also the datatypes of the records, and how the tables you describe relate to each other which includes data sharing or manipulation.
...
- SurrealDB is installed on your machine. You can download and install SurrealDB from the [installation page](/docs/surrealdb/2.x/installation/).
- [A Command line interface (CLI)](/docs/surrealdb/2.x/cli/surql) for interacting with SurrealDB or a [Surrealist sandbox](/docs/surrealist/#connecting-to-the-sandbox).
...
In the code above you may notice the `ASSERT` clause. This can be used to validate any restrictions you want on a field. In the example above the [`string::is::email`](/docs/surrealdb/2.x/surrealql/functions/database/string#stringisemail) function is used to check whether the value is an email.

### Adding data to a Schemafull table

Now that you have defined all the fields needed, you can start populating them. To do this, you can use the [CREATE statement](https://www.notion.so/Schemaless-vs-Schemafull-2f60f2c00b0c409fa69d7200f779c8c5?pvs=21). For example, add a new user:
...
In SurrealDB there are two ways you can define a schemaless table. You can either use any of the data definition statements such as [`CREATE`](/docs/surrealdb/2.x/surrealql/statements/create) or [`UPDATE`](/docs/surrealdb/2.x/surrealql/statements/update)  and that will make a table based on the [record ID](/docs/surrealdb/2.x/surrealql/datamodel/ids#object-based-record-ids) specified. For example:
...
In the example above, you used the `CREATE` statement to make an `IC_directory` table and created an Object ID for this table. ID can be anything you want and is randomly generated if not defined. You have also used the UPDATE statement to make a `manager_directory`, which has `janesmith` as the ID. Learn more about [Record IDs in the documentation](/docs/surrealdb/2.x/surrealql/datamodel/ids#object-based-record-ids).
...
In SurrealDB, you can add [flexible fields](/docs/surrealdb/2.x/surrealql/statements/define/field#flexible-data-types) to a schemafull table using the `DEFINE FIELD` statement using the `FLEXIBLE` clause. This allows you to have `schemaless` functionality in a `schemafull` table. For example
...
Learn more about setting up a schema in the [`DEFINE` statement documentation](/docs/surrealdb/2.x/surrealql/statements/define/).