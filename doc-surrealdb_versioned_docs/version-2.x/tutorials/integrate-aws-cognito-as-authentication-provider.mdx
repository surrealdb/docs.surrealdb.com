...
- Define user-level authorization using SurrealDB [record users](/docs/surrealdb/2.x/security/authentication#record-users).
...
- You have a [fresh instance of SurrealDB running](/docs/surrealdb/2.x/introduction/start) with version `1.2.0` or later.

- You can [use a local Docker container](/docs/surrealdb/2.x/installation/running/docker) without volumes for the purposes of this guide.
...
DEFINE ACCESS cognito ON DATABASE TYPE RECORD
  ...
  AUTHENTICATE {
    ...
    -- The email in the token must be verified as belonging to the user.
    AND $token.email_verified = true {
      -- We return the only user that matches the email address found in the token.
...
In order to allow SurrealDB to establish a connection with AWS Cognito to download the JWKS object, you will require running it with the network <a href="/docs/surrealdb/2.x/security/capabilities">capability</a>. 
