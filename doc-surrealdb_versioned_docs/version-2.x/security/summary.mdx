...
SurrealDB offers the ability to limit its functionality to what is strictly required to reduce its attack surface. All capabilities (e.g. scripting, networking…) are disabled by default. Even when enabled, capabilities can be restricted to specific targets such as functions that can be executed or network addresses that outbound connections can be made to.
    - Relevant documentation:
        - [Security: Capabilities](https://surrealdb.com/docs/surrealdb/2.x/security/capabilities)
        - [Security Best Practices: Capabilities](https://surrealdb.com/docs/surrealdb/2.x/reference-guide/security-best-practices#capabilities)

## System Users
SurrealDB is managed by system users. Such users can be defined at the root, namespace and database level, which they can sign into with a username and password. The password for those users is stored hashed and salted using the [Argon2id](https://github.com/P-H-C/phc-winner-argon2/blob/master/argon2-specs.pdf) algorithm with default parameters, which ensures robust resistance against modern attacks. Passwords can also be provided to SurrealDB already hashed in the form of a passhash, ensuring that the SurrealDB server never has knowledge of the original password. When defining a user, the maximum duration for authentication tokens and authenticated sessions can be explicitly defined to mitigate the impact of compromised credentials.
    - Relevant documentation:
        - [Statement: DEFINE USER](https://surrealdb.com/docs/surrealdb/2.x/surrealql/statements/define/user)
        - [Security Best Practices: Expiration](https://surrealdb.com/docs/surrealdb/2.x/reference-guide/security-best-practices#expiration)
...
SurrealDB implements Role-Based Access Control (RBAC) for system users at any level. This means that even if a person or system needs to authenticate with a SurrealDB user at the root, namespace or database level, its access can be restricted within that level by the owner, editor and viewer roles to minimize the impact of an incident involving the user.
    - Relevant documentation:
        - [Statement: DEFINE USER (Roles)](https://surrealdb.com/docs/surrealdb/2.x/surrealql/statements/define/user#roles)
        - [Security: Authentication (System Users)](https://surrealdb.com/docs/surrealdb/2.x/security/authentication#system-users)
        - [Security Best Practices: Least Privilege](https://surrealdb.com/docs/surrealdb/2.x/reference-guide/security-best-practices#least-privilege)
## Record Users
SurrealDB supports being used as a web database by allowing end users limited access to the database. This allows clients like single-page applications or mobile applications to directly connect to the database and access certain data or even run arbitrary queries. When accessing the database as a record user, users will be restricted by table and field permissions, which deny all operations by default. End users can independently sign up and sign in following logic that can be customized with SurrealQL.
    - Relevant documentation:
        - [Statement: DEFINE ACCESS ... TYPE RECORD](https://surrealdb.com/docs/surrealdb/2.x/surrealql/statements/define/access/record)
        - [Security: Authentication (Record Users)](https://surrealdb.com/docs/surrealdb/2.x/security/authentication#record-users)
...
SurrealDB enforces table and field permissions for record users. Those permissions ensure that record users can only perform explicitly defined actions over explicitly defined data. Permissions are specified when defining a table or a field and use SurrealQL syntax to establish the conditions under which the table or the field can be queried with SELECT, UPDATE, CREATE and DELETE operations individually. No table permissions are defined by default, thus preventing a record user from querying any data unless explicitly allowed.
    - Relevant documentation:
        - [Statement: DEFINE TABLE](https://surrealdb.com/docs/surrealdb/2.x/surrealql/statements/define/table)
        - [Statement: DEFINE FIELD](https://surrealdb.com/docs/surrealdb/2.x/surrealql/statements/define/field)

## Tokens
SurrealDB internally uses JWT to perform and manage authentication for both system and record users. It also supports accepting tokens issued by third party authentication providers in order to authenticate as a system user on any level as well as a record user for an application. This ensures that advanced authentication features not present in SurrealDB can be integrated through a third party provider. This integration is simple and reliable thanks to JWKS support implemented by SurrealDB.
    - Relevant documentation:
        - [Statement: DEFINE ACCESS ... TYPE JWT](https://surrealdb.com/docs/surrealdb/2.x/surrealql/statements/define/access/jwt)
        - [Statement: DEFINE ACCESS ... TYPE RECORD ... WITH JWT](https://surrealdb.com/docs/surrealdb/2.x/surrealql/statements/define/access/record#with-json-web-token)
        - [Tutorial: Integrate Auth0 as Authentication Provider](https://surrealdb.com/docs/surrealdb/2.x/tutorials/integrate-auth0-as-authentication-provider)
        - [Tutorial: Integrate AWS Cognito as Authentication Provider](https://surrealdb.com/docs/surrealdb/2.x/tutorials/integrate-aws-cognito-as-authentication-provider)
        - [Security Best Practices: JSON Web Tokens](https://surrealdb.com/docs/surrealdb/2.x/reference-guide/security-best-practices#json-web-tokens)
        - [Security Best Practices: Expiration](https://surrealdb.com/docs/surrealdb/2.x/reference-guide/security-best-practices#expiration)

## Custom Authentication
SurrealDB allows record users to authenticate using a token that can be issued by a third party or SurrealDB itself after successful authentication. When verifying tokens, custom verification logic can be implemented using SurrealQL to abort authentication while returning a custom error if certain conditions are not met. This logic can also used to implement various kinds of token auditing and revocation mechanisms. Additionally, tokens issued by SurrealDB can be customized to be signed with specific keys or using a specific algorithms so that other services can rely on the authentication provided by SurrealDB.
        - [Statement: DEFINE ACCESS ... TYPE RECORD ... AUTHENTICATE](https://surrealdb.com/docs/surrealdb/2.x/surrealql/statements/define/access/record#with-authenticate-clause)
        - [Statement: DEFINE ACCESS ... TYPE RECORD ... WITH ISSUER](https://surrealdb.com/docs/surrealdb/2.x/surrealql/statements/define/access/record#with-issuer)
        - [Security Best Practices: Expiration](https://surrealdb.com/docs/surrealdb/2.x/reference-guide/security-best-practices#expiration)

## Parametrized Queries
    - SurrealDB is usually queried through [multiple SDKs](https://surrealdb.com/docs/surrealdb/2.x/integration/sdks) and a powerful [RPC interface](https://surrealdb.com/docs/surrealdb/2.x/integration/rpc). The default query method for both of those interfaces is designed to accept query logic and variables separately to prevent attacks similar to SQL injection. This separation ensures that user-controlled inputs are not mixed with any business logic defined in SurrealQL.
    - Relevant documentation:
        - [Interfaces: RPC (Query Method)](https://surrealdb.com/docs/surrealdb/2.x/integration/rpc#query)
        - [Interfaces: Rust SDK (Query Method)](https://surrealdb.com/docs/sdk/rust/setup#query)
        - [Interfaces: JavaScript SDK (Query Method)](https://surrealdb.com/docs/sdk/javascript/core/data-querying#query)
        - [Security Best Practices: Query Safety](https://surrealdb.com/docs/surrealdb/2.x/reference-guide/security-best-practices#query-safety)
...
SurrealDB accepts persistent connections through its RPC interface in the form of sessions. Sessions will usually be associated with an authentication token that represents a system user or a record user. Sessions and tokens can be configured to have different expiration times. Thanks to this, tokens can be issued to last the minimum time required to mitigate the impact of an attacker stealing the token while ensuring that sessions can last as long as required for the service or application.
    - Relevant documentation:
        - [Security: Sessions](https://surrealdb.com/docs/surrealdb/2.x/security/authentication#sessions)
        - [Statements: DEFINE USER (Duration)](https://surrealdb.com/docs/surrealdb/2.x/surrealql/statements/define/user#duration)
        - [Statements: DEFINE ACCESS (Duration)](https://surrealdb.com/docs/surrealdb/2.x/surrealql/statements/define/access/#duration)
        - [Security Best Practices: Expiration](https://surrealdb.com/docs/surrealdb/2.x/reference-guide/security-best-practices#expiration)
        - [Interfaces: RPC (Authenticate)](https://surrealdb.com/docs/surrealdb/2.x/integration/rpc#authenticate)
...
SurrealDB provides a series of cryptographic functions that can be called from within SurrealQL in order to implement modern and robust security practices in your application. This includes state of the art password hashing algorithms such as Argon2, Bcrypt, Scrypt and PBKDF2. Traditional hashing algorithms like SHA-256 and SHA-512 are also provided for other applications such as integrity verification.
    - Relevant documentation:
        - [Functions: Crypto](https://surrealdb.com/docs/surrealdb/2.x/surrealql/functions/database/crypto)
        - [Security Best Practices: Passwords](https://surrealdb.com/docs/surrealdb/2.x/reference-guide/security-best-practices#passwords)
...
SurrealDB also aims to provide other functions that support developers with building secure applications. An example of this are functions which encode and sanitize HTML content that is stored in the database to prevent code injection (which can lead to cross-site scripting, clickjacking or content injection) when displaying the content in an HTML page.
    - [Functions: String (HTML Encode)](https://surrealdb.com/docs/surrealdb/2.x/surrealql/functions/database/string#stringhtmlencode)
    - [Functions: String (HTML Sanitize)](https://surrealdb.com/docs/surrealdb/2.x/surrealql/functions/database/string#stringhtmlsanitize)
    - [Security Best Practices: Cont](https://surrealdb.com/docs/surrealdb/2.x/reference-guide/security-best-practices#content-safety)[Interfaces: JavaScript SDK (Query Method)](https://surrealdb.com/docs/surrealdb/integration/sdks/javascript#query)[ent Safety](https://surrealdb.com/docs/surrealdb/2.x/reference-guide/security-best-practices#content-safety)
...
SurrealDB has the ability to provide a secure TLS connection to its HTTP server without the need for a reverse proxy or load balancer. A certificate and a private key can be provided when starting the server in order to provide a secure connection.
    - Relevant documentation:
        - [CLI: Start](https://surrealdb.com/docs/surrealdb/2.x/cli/start)
        - [Security Best Practices: Encryption in Transit](https://surrealdb.com/docs/surrealdb/2.x/reference-guide/security-best-practices#encryption-in-transit)
