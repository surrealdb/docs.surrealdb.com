...
- [System users](/docs/surrealdb/2.x/security/authentication#system-users): Created by the SurrealDB administrator and used for managing and consuming the database
- [Record users](/docs/surrealdb/2.x/security/authentication#record-users): Used for consuming the database within permissions logic, they allow custom signup and signin
...
Users may belong to different levels (root, namespace or database) and have different roles assigned to limit what they can do to the system. Users are defined with the [DEFINE USER](/docs/surrealdb/2.x/surrealql/statements/define/user) statement.
...
Go to [DEFINE USER](/docs/surrealdb/2.x/surrealql/statements/define/user) for more information.
...
Record users represent users that are defined as a record in a database instead of through the `DEFINE USER` statement. Since these users exist as regular database records, they can have associated fields containing any information required for authentication and authorization. For this reason, record users allow SurrealDB to offer mechanisms to define your own signin and signup logic as well as custom table and field permissions. This feature contributes to making SurrealDB an all-in-one BaaS (Backend-as-a-Service).

Record users are defined with the [DEFINE ACCESS](/docs/surrealdb/2.x/surrealql/statements/define/access) statement of `TYPE RECORD`.

A record access is configured with the following specific clauses:

- `SIGNUP`: Defines the logic for when a user signs up as a record user. It usually creates a new entry to a table
- `SIGNIN`: Defines the logic for when a user signs in as a record user. It usually check the provided credentials against the data in a table

By default, record users have no permissions. They don't use the RBAC system and can only access data if allowed by a `PERMISSIONS` clause, which is defined on every data resource (i.e. tables and fields) and defaults to `NONE`.

Go to [DEFINE ACCESS ... TYPE RECORD](/docs/surrealdb/2.x/surrealql/statements/define/access/record) for more information.
...
Clients can reuse a connection with an expired session to refresh the session using valid credentials. This includes calling the [`signin`](/docs/surrealdb/2.x/integration/rpc#signin) method to obtain a new token with credentials, which will replace the expired session token, or calling the [`authenticate`](/docs/surrealdb/2.x/integration/rpc#authenticate) method to replace the expired session token with a new token.
