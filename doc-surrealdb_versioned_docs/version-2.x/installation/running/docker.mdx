...
To get started using Docker, you can use the `latest` tag. To start a server use the [`start`](/docs/surrealdb/2.x/cli/start) command. By default SurrealDB runs on port `8000`. To view all the available versions and tags, or to use a specific tag visit the [Docker Hub](https://hub.docker.com/r/surrealdb/surrealdb) page.
...
:::note
__Note:__ For local development, use the `latest-dev` image variant (i.e., docker run --rm --pull always -p 8000:8000 surrealdb/surrealdb:latest-dev start). This version includes a shell and package manager, allowing you to install tools and interact with the container's internals.
:::
...
For details on the [`start`](/docs/surrealdb/2.x/cli/start) command, and all of the available configuration options and arguments, view the [`start command documentation`](/docs/surrealdb/2.x/cli/start).
...
For details on the different commands available, visit the [CLI tool documentation](/docs/surrealdb/2.x/cli).