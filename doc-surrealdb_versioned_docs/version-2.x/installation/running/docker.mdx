...
To get started using Docker, you can use the `latest` tag. To start a server use the [`start`](/docs/surrealdb/2.x/cli/start) command. By default SurrealDB listens on port `8000` in all interfaces so that the host can connect to the container in the default bridge networking mode. To view all the available versions and tags, or to use a specific tag visit the [Docker Hub](https://hub.docker.com/r/surrealdb/surrealdb) page.
