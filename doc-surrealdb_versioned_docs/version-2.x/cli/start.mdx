...
Authenticated is enabled by default in SurrealDB. When disabled with `--unauthenticated`, any guest user is considered to have the same permissions as a root user with the `OWNER` role.

:::note
 ___NOTE:___ It is strongly discouraged to use `--unauthenticated` when running SurrealDB in production or publicly exposed in any way. Doing so may result in unauthorized access.
:::

```bash
surreal start --user [ROOT_USER_NAME] --pass [ROOT_USER_PASSWORD]
```
