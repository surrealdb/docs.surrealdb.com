...
Authentication is enabled by default in SurrealDB and credentials for an existing user will be required to connect.

When starting SurrealDB, a new root user with the `OWNER` role can be created by providing a username and password for it.
...
Authentication can be disabled with `--unauthenticated`. In this mode, any guest user is considered to have the same permissions as a root user with the `OWNER` role.

:::note
 ___NOTE:___ It is strongly discouraged to allow unauthenticated access when running SurrealDB in production or publicly exposed in any way. Doing so may result in unauthorized access.
:::
