...
When running a SurrealDB server, you can configure the [capabilities](/docs/surrealdb/2.x/security/capabilities) for your SurrealQL queries. These capabilities are disabled by default to expose as little attack surface as possible to malicious actors. We strongly discourage running SurrealDB with all capabilities enabled without a good reason. Instead, we recommend enabling only the capabilities necessary for your service.

Whenever you need to enable a specific capability, we recommend doing it on a case-by-case basis instead of generally. For example, suppose you know that your queries need to be able to parse emails using functions. In that case, we recommend you run SurrealDB with the ` --allow-funcs parse::email::* ` flag instead of allowing all functions with` --allow-funcs` without arguments. 
Doing this ensures that your SurrealDB instance will not be affected by vulnerabilities in the code of any other functions that a malicious actor could leverage to increase their privileges or execute unauthorized code.
...
### Example: Allow HTTP GET requests to a specific API
...
When defining [users](/docs/surrealdb/2.x/surrealql/statements/define/user) and [access methods](/docs/surrealdb/2.x/surrealql/statements/define/access/), ensure that you set a specific [session and token duration](/docs/surrealdb/2.x/security/authentication#expiration) whenever possible using the `DURATION` clause.
...
### Example: Bind parameters in Rust SDK
...
### Example: Bind parameters in JavaScript SDK 
```jsx
// JavaScript SDK
// Do this:
