...
When running a SurrealDB server, you can configure the [capabilities](/docs/surrealdb/security/capabilities) for your SurrealQL queries. These capabilities are disabled by default to expose as little attack surface as possible to malicious actors. We strongly discourage running SurrealDB with all capabilities enabled without a good reason. Instead, we recommend enabling only the capabilities necessary for your service.
...
When defining [users](/docs/surrealdb/surrealql/statements/define/user) and [access methods](/docs/surrealdb/surrealql/statements/define/access/), ensure that you set a specific [session and token duration](/docs/surrealdb/security/authentication#expiration) whenever possible using the `DURATION` clause.
...
When retrieving content that may be rendered in an HTML document, we strongly recommend that you use the [`string::html::encode`](/docs/surrealdb/surrealql/functions/database/string#stringhtmlencode) function, which will encode any characters that have special meaning in HTML syntax (e.g. `<`, `>`, `&`...) into HTML entities (e.g. `&lt;`, `&gt;`, `&amp;`...) that will be rendered as the actual original character instead of interpreted as HTML syntax.
