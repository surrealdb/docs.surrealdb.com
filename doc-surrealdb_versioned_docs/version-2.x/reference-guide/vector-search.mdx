...
SurrealDB supports [Full-Text Search](https://surrealdb.com/docs/surrealdb/reference-guide/full-text-search) and [Vector Search](https://www.youtube.com/watch?v=MqddPmgKSCs). Full-text search(FTS) involves indexing documents using the FTS index and breaking down the content of the document into smaller tokens with the help of [analysers](https://surrealdb.com/docs/surrealdb/surrealql/statements/define/analyzer) and [tokenizers](https://surrealdb.com/docs/surrealdb/surrealql/statements/define/analyzer#tokenizers). 
...
You might be familiar with our [Reference Guide for Full-Text Search](https://surrealdb.com/docs/surrealdb/reference-guide/full-text-search), but you might wonder how it differs from Vector Search. Let's consider an example:
...
If you read through the content of the tap water sample, you’ll notice that it does not contain any lead in it but it has the mention of the word `lead` under “The team lead by Dr. Rose…” which means that the team was guided by Dr. Rose. The search pulled up both the records although the tap water sample had no lead in it. This example shows us that while [Full-Text Search](https://surrealdb.com/docs/surrealdb/reference-guide/full-text-search) does a great job at matching query terms with indexed documents, it may not be the best solution for use cases where the query terms have deeper context and scope for ambiguity. 
...
SurrealDB provides [Vector Functions](/docs/surrealdb/2.x/surrealql/functions/vector) for most of the major numerical computations done on vectors. They include functions for element-wise addition, division and even normalisation. 
