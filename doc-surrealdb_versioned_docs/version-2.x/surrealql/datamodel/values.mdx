...
This operator can be used in assertions as well. The `data` field defined in the example below will accept any type, as long as it is truthy.

```surql
DEFINE FIELD data ON TABLE person ASSERT !!$value;
```