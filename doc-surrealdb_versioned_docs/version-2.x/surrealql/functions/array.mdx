...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td scope="row" data-label="Function"><a href="#arrayshuffle"><code>array::shuffle()</code></a> <l className='purple'>Since 1.5.0</l></td>
      <td scope="row" data-label="Description">Sorts the values in an array in random order</td>
    </tr>
    <tr>
...
The `array::concat` function merges two arrays together, returning an array which may contain duplicate values. If you want to remove duplicate values from the resulting array, then use the [`array::union()`](/docs/surrealdb/2.x/surrealql/functions/array#arrayunion) function
...
The `array::len` function calculates the length of an array, returning a number. This function includes all items when counting the number of items in the array. If you want to only count truthy values, then use the [count()](/docs/surrealdb/2.x/surrealql/functions/count) function.
```surql title="API DEFINITION"
...
## `array::shuffle` <l className='purple'>Since 1.5.0</l> {#arrayshuffle}

The `array::shuffle` function sorts the values in an array in random order.
...
```surql title="Output - 1st iteration"
RETURN array::shuffle([ 1, 2, 3, 4, 5 ]);

[ 1, 4, 5, 3, 2 ]
```

```surql title="Output - 2nd iteration"
RETURN array::shuffle([ 1, 2, 3, 4, 5 ]);
