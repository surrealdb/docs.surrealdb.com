...
249552791347.80392
```
If the argument is not a geometry type, then an [`EMPTY`](/docs/surrealdb/2.x/surrealql/datamodel/none_and_null) value will be returned:
...
If either argument is not a geolocation point, then an [`EMPTY`](/docs/surrealdb/2.x/surrealql/datamodel/none_and_null) value will be returned:
...
```
The following example shows this function, and its output, when used in a [`RETURN`](/docs/surrealdb/2.x/surrealql/statements/return) statement. Note: the doubled square brackets are because the function takes an array of an array to allow for more complex types such as MultiPolygon.
...
If either argument is not a geometry type, then an [`EMPTY`](/docs/surrealdb/2.x/surrealql/datamodel/none_and_null) value will be returned:
...
```
If the argument is not a geolocation point, then an [`EMPTY`](/docs/surrealdb/2.x/surrealql/datamodel/none_and_null) value will be returned:
...
"mpuxk"
```
If the first argument is not a geolocation point, then an [`EMPTY`](/docs/surrealdb/2.x/surrealql/datamodel/none_and_null) value will be returned:
