...
The `string::html::sanitize` function sanitizes HTML code to prevent the most dangerous subset of HTML injection that can lead to cross-site scripting, layout breaking, and clickjacking. This function will keep any other HTML syntax intact in order to support user-generated content that needs to contain HTML formatting. It is only recommended to rely on this function if you want to allow the creators of the content to have control over its HTML formatting. You can learn more about its behavior from the [original implementation](https://docs.rs/ammonia/latest/ammonia/fn.clean.html).

```surql title="API DEFINITION"
string::html::sanitize(html) -> string
```
The following example shows this function, and its output, when used in a [`RETURN`](/docs/surrealdb/surrealql/statements/return) statement:
