...
* `$after`: The value after the records are updated.
* `$before`: The value before the records are updated.
* `$parent`: The value of the parent query, if inside a subquery, otherwise NONE.
* `$this`: The record being updated.
...
Let's look at some examples of how to use the `UPDATE` statement: First we'll create a `person` table with the [`CREATE`](/docs/surrealdb/2.x/surrealql/statements/create) Statement so that the examples can be run.
