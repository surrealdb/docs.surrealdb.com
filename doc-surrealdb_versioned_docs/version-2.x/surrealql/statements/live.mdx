...
sidebar_position: 15
sidebar_label: LIVE
title: LIVE statement | SurrealQL
description: The LIVE statement can be used to initiate a real-time selection from a table, including the option to apply filters.
---

# `LIVE` statement

The `LIVE` statement can be used to initiate a real-time selection from a table, including the option to apply filters.
...
You can also use this UUID to [KILL (stop)](/docs/surrealdb/2.x/surrealql/statements/kill) the Live Query.

The protocol will then send messages that are of a Notification format.
You can find an example of such a message in the [Live Query WebSocket protocol](/docs/surrealdb/2.x/integration/rpc#live-websocket-only) description.
...
```surql
LET $age = 18;
LIVE SELECT * FROM person WHERE age > $age;
```
