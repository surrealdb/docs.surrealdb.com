...
- `FULL`: When Full permissions are granted [record](/docs/surrealdb/2.x/security/authentication#record-users) users have access to the function. This is the default permission when not specified.
- `NONE`: When this permission is granted, [record](/docs/surrealdb/2.x/security/authentication#record-users) users have no access to the defined function.
...
The `NONE` permission denies all [record](/docs/surrealdb/2.x/security/authentication#record-users) users access to the function. The following example defines a function that fetches all products from the `product` table 
...
## Using `IF NOT EXISTS` clause <l className='purple'><Since v="v1.3.0" /></l>
