...
- [You must select your namespace and database](/docs/surrealdb/2.x/surrealql/statements/use) before you can use the `DEFINE EVENT` statement.
...
* `$after`: The value of the record after the operation.
* `$before`: The value of the record before the operation.
* `$event`: The event in question. One of "CREATE", "DELETE", or "UPDATE".
* `$value`: The value of the record in question.
