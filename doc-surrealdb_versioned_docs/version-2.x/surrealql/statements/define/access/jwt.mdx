...
- You must be authenticated as a [system user](/docs/surrealdb/2.x/security/authentication#system-users) at the same level or higher than the level to which you want to provide JWT access.
- [You must select a namespace or database](/docs/surrealdb/2.x/surrealql/statements/use) before you can define a JWT access method.
...
For tokens defined for [system users](/docs/surrealdb/2.x/security/authentication#system-users), the optional `rl` claim containing an array of capitalized [system user roles](/docs/surrealdb/2.x/surrealql/statements/define/user#roles) (e.g. `["Viewer", "Editor", "Owner"]`) can be provided. Doing so will apply the access policy for those roles to any action made using the token. By default, sessions stablished with tokens without the `rl` claim will only have the `Viewer` role.
