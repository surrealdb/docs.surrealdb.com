...
The authenticate clause can be used to change or set the record id for a user, but can also be used to fail authentication for record users as it's consistently ran across signin, signup and token authentication. The clause always expects a record id to be returned, and will otherwise generally fail.

#### External auth provider

Rewriting or setting the record id for a session is especially useful in scenario's where the token used to authenticate the session does not contain one. This is a realistic situation when using an external authentication provider, which may only provider a user identifier from their platform, such as a UUID or an email address.

In the below example, we:
- First check if the session may already be tied to an existing user. `$auth` is a reserved variable, containing the record id of a user. If we can select the `id` field from `$auth`, it means that that record exists in the database.
- If that is not the case, we can check if the token may contain a different identifiable property. In this case we check for an email address, after which we try to lookup the user from the user table by their email address.
- Lastly, if everything fails, the clause won't return a record id, either by the `IF` statement not matching, or by the user lookup failing. In this scenario, the `AUTHENTICATE` clause will fail with a generic error. You can also opt to `THROW` a custom error if you so wish.
...
Because the `AUTHENTICATE` clause runs across signin, signup and token authentication, it's in a unique position to centralize logic _after_ a user's credentials are deemed valid, but _before_ the user is completely authenticated.

Below, we show a simple example, validating if a user is enabled. If this is not the case, we can simply `THROW` an error stating why the user cannot authenticate (or you can opt to not return anything which results in a generic error).

If everything checks out, we can `RETURN` the record id which confirms that authentication is successful, and that that is the user which will operate the session.
