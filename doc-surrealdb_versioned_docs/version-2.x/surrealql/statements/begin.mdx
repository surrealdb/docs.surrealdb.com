...
description: The BEGIN statement can be used to run a group of statements together, either succeeding as a whole, or failing.
---
...
The `BEGIN` statement can be used to run a group of statements together, either succeeding as a whole, or failing. If all of the statements within a transaction succeed, and the transaction is successful, then all of the data modifications made during the transaction are committed ([COMMIT](/docs/surrealdb/2.x/surrealql/statements/commit)) and become a permanent part of the database. If a transaction encounters errors and must be cancelled or rolled back ([CANCEL](/docs/surrealdb/2.x/surrealql/statements/cancel)), then any data modification made within the transaction is rolled back, and will not be visible within the database.
