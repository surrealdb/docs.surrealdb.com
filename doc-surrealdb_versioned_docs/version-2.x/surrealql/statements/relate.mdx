...
Otherwise, edge tables behave like normal tables in terms of [updating](/docs/surrealdb/2.x/surrealql/statements/update), [defining a schema](/docs/surrealdb/2.x/surrealql/statements/define/table) or [indexes](/docs/surrealdb/2.x/surrealql/statements/define/indexes).

[Record links](/docs/surrealdb/2.x/surrealql/datamodel/records) are the alternative option when connecting data, and simply consist of a field with record IDs that serve as uni-directional links. The key differences are that graph relations
...
By default, the edge table gets created as a schemaless table when you execute the `RELATE` statement. You can however make the table schemafull by either [defining a schema](/docs/surrealdb/2.x/surrealql/statements/define/table) before or after executing the RELATE statement.

A common use case is to make sure only unique relationships get created. You can do that by [defining an index](/docs/surrealdb/2.x/surrealql/statements/define/indexes).
...
Let's look at how the above gets queried, for more examples see the [SELECT docs](/docs/surrealdb/2.x/surrealql/statements/select).
...
## Using [`LET`](/docs/surrealdb/2.x/surrealql/statements/let) parameters in RELATE statements

You can also use [parameters](/docs/surrealdb/2.x/surrealql/parameters) to specify the record IDs.
...
## Using [`RETURN`](/docs/surrealdb/2.x/surrealql/statements/return) clause
...
You can also delete graph edges between two records in the database by using the [DELETE statement](/docs/surrealdb/2.x/surrealql/statements/delete).
