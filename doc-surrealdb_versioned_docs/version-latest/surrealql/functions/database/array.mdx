...
The types for the closure arguments and output can be annotated for extra type safety. Take the following
...
```surql
[" was first used in the year 876", " is the number of moons in the sky", " is also called a pair"]
  .map(|$item, $index| $item + <string>$index);
```

```bash title="Response"
[
	'0 was first used in the year 876',
	'1 is the number of moons in the sky',
	'2 is also called a pair'
]
```
