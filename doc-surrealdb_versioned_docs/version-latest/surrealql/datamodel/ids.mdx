...
:::note
___NOTE:___ As of version 2.0.0, SurrealDB no longer eagerly converts a string into a record. An implicit `r` prefix or cast using `<record>` (or `<record<recordname>>`) is required instead.
:::
