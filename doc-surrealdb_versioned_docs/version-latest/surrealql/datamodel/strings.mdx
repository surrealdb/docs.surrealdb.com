...
The `r` prefix tells the parser that the contents of the string represent a [`record ID`](/docs/surrealdb/surrealql/datamodel/ids). The parser expects record IDs to have the following format: `table_name:record ID`.
...
The `d` prefix tells the parser that the contents of the string represent a [`datetime`](/docs/surrealdb/surrealql/datamodel/datetimes). The parser expects `datetime` values to have a valid ISO 8601 format. Here are a few examples:
...
The `u` prefix tells the parser that the contents of the string represent a [`uuid`](/docs/surrealdb/surrealql/datamodel/uuid). The parser expects `uuid` values to follow the format of an UUID, `ffffffff-ffff-ffff-ffff-ffffffffffff`, where each non-hyphen character can be a digit (0-9) or a letter between `a` and `f` (representing a single hexadecimal digit).
