...
In SurrealQL, there are several types of string like, quoted values each with a different prefix indicating its type. Previously, in the `1.0` SurrealQL these types would eagerly convert strings to a different type depending on its contents.
...
The most commonly used string prefix, `s` tells the parser that the contents of this string are just a string which also shows up in the database output to make it clear to the viewer as well that the output is a string.

Since `2.0`, all strings without a prefix will be plain strings. Including other datatypes that aren't clearly defined. So the following two queries are equivalent.
...
The `r` prefix tells the parser that the contents of this string represent a record ID.
...
In the example below, using the [`type::is::string()`](/docs/surrealdb/surrealql/functions/database/type#typeisstring) and [`type::is::record()`](/docs/surrealdb/surrealql/functions/database/type#typeisrecord) functions respectively, you can check the type of the string.
...
The `d` prefix tells the parser that the contents of this string represent a datetime.
...
The `u` prefix tells the parser that the contents of this string represent a UUID.
