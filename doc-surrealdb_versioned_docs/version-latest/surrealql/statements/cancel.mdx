...
`CANCEL` is not used to automatically cancel a transaction based on a condition such as inside an [IF..ELSE](/docs/surrealdb/surrealql/statements/ifelse) block. Instead, a [THROW](/docs/surrealdb/surrealql/statements/throw) statement is used. THROW can be followed by any value, usually a string containing context behind the error.
