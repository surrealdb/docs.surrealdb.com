...
The default random ID can be generated in different ways (such as a ULID) using the [built-in ID generation functions](/docs/surrealdb/surrealql/datamodel/ids#generating-record-ids).
...
```surql
-- Use r prefix to parse as a record
CREATE r"person:one";
...
### Unseen behaviour inside a CREATE statement
...
While a connection to SurrealDB via Surrealist or the [surreal sql](/docs/surrealdb/cli/sql) command can include a defined namespace and database, the namespace and database names do not exist upon creation. At this point, they are only held inside the pre-defined [$session](/docs/surrealdb/surrealql/parameters#session) parameter. This can be seen through the [INFO](/docs/surrealdb/surrealql/statements/info) statements, which will show no definitions at all inside a new database.
...
This is to allow the chance to [define](/docs/surrealdb/surrealql/statements/define/database) them manually, such as by including a comment.
...
To disallow this behaviour, you can pass the `--strict` flag when [starting the database](/docs/surrealdb/cli/start). In strict mode, everything must be explicitly defined before it can be used.
