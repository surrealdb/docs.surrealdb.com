...
- [You must select a namespace or database](/docs/surrealdb/surrealql/statements/use) before you can define a JWT access method.
...
The `AUTHENTICATE` clause allows you to define a custom expression that will be executed when the token is verified. This expression will be executed in the context of the token, allowing you to perform additional checks on the token claims before the token is accepted. If the expression returns `false`, the token will be rejected.

#### Example: Using an HMAC algorithm with an `AUTHENTICATE` clause.

This example defines an access policy for a user on a database using JSON Web Tokens (JWT) with the HS512 hashing algorithm. The secret key for this algorithm is provided explicitly. The `AUTHENTICATE` block outlines the authentication logic: it checks if the expiration time (`exp`) of the token is later than the current time ([`time::now()`](/docs/surrealdb/surrealql/functions/database/time#timenow)). If the token is still valid (i.e., its expiration time is in the future), the authentication process returns `true`, indicating successful authentication. If the token has expired, the process returns `false`, indicating authentication failure.

```surql
DEFINE ACCESS user ON DATABASE TYPE JWT ALGORITHM HS512 KEY "sNSYneezcr8kqphfOC6NwwraUHJCVAt0XjsRSNmssBaBRh3WyMa9TRfq8ST7fsU2H2kGiOpU4GbAF1bCiXmM1b3JGgleBzz7rsrz6VvYEM4q3CLkcO8" 
AUTHENTICATE {
  IF $token.exp > time::now() RETURN true;
  RETURN false;
};
```
...
The `AUTHENTICATE` clause can be used to log or stop authentication attempts. The clause always expects a boolean value to be returned and will otherwise fail.

This example defines an access policy for a user on a database using JSON Web Tokens (JWT) with the HS512 algorithm for hashing. The secret key for the hashing algorithm is provided. The `AUTHENTICATE` block contains a series of actions to be performed during authentication: it first checks if the token has been revoked by querying the database with the token's ID (`jti`). If the token is revoked, an error is thrown. If not, the token's details are inserted into the `token` table, marking it as not revoked. Additionally, an audit entry is created with the token's ID and the current timestamp. The token is valid for 30 days, and each session is valid for 1 hour.

```surql
DEFINE ACCESS user ON DATABASE TYPE JWT
  ALGORITHM HS512
  KEY "sNSYneezcr8kqphfOC6NwwraUHJCVAt0XjsRSNmssBaBRh3WyMa9TRfq8ST7fsU2H2kGiOpU4GbAF1bCiXmM1b3JGgleBzz7rsrz6VvYEM4q3CLkcO8"
    AUTHENTICATE {
