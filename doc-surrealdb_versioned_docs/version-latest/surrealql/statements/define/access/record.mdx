...
In the context of [`DEFINE ACCESS ... TYPE RECORD`](/docs/surrealdb/surrealql/statements/define/access/record), the `AUTHENTICATE` clause is always executed across signin, signup and token authentication. The clause always expects a record identifier to be returned and will otherwise fail.
