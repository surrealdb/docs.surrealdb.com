...
- `ANALYZER ascii`: Uses a custom [analyzer](/docs/surrealdb/surrealql/statements/define/analyzer) called `ascii` for tokenizing and analyzing the text.
- `BM25`: Ranking algorithm used for relevance scoring.
- `HIGHLIGHTS`: Allows keyword highlighting in search results output when using the [`search::highlight`](/docs/surrealdb/surrealql/functions/database/search#searchhighlight) function
...
:::note
_Note:_ In SurrealDB the default type for vectors is`F64`.
:::
...
Mtree currently supports [Euclidean](/docs/surrealdb/surrealql/functions/database/vector#vectordistanceeuclidean), [Cosine](/docs/surrealdb/surrealql/functions/database/vector#vectorsimilaritycosine), [Manhattan](/docs/surrealdb/surrealql/functions/database/vector#vectordistancemanhattan) and [Minkowski](/docs/surrealdb/surrealql/functions/database/vector#vectordistanceminkowski) distance functions.
...
:::note
_Note:_ You can only provide TYPE, M, and EFC. SurrealDB automatically computes M0 and LM with the most appropriate value. If not specified, M AND EFC are set to 12 and 150, respectively.
:::

### Brute Force method

The Brute Force method is suitable for tasks with smaller datasets or when the highest accuracy is required.
Brute Force currently supports [Euclidean](/docs/surrealdb/surrealql/functions/database/vector#vectordistanceeuclidean), [Cosine](/docs/surrealdb/surrealql/functions/database/vector#vectorsimilaritycosine), [Manhattan](/docs/surrealdb/surrealql/functions/database/vector#vectordistancemanhattan) and [Minkowski](/docs/surrealdb/surrealql/functions/database/vector#vectordistanceminkowski) distance functions.
...
The `IF NOT EXISTS` clause can be used to define an index only if it does not already exist. If the index already exists, the `DEFINE INDEX` statement will return an error.
...
The `OVERWRITE` clause can be used to define an index and overwrite an existing one if it already exists.
...
Use the `CONCURRENTLY` option to build the index without blocking operations. The statement will return immediately, allowing you to monitor the index-building progress by executing the [INFO](/docs/surrealdb/surrealql/statements/info/) statement.

```surql
-- Create an INDEX concurrently
DEFINE INDEX example ON example FIELDS example CONCURRENTLY;
```
