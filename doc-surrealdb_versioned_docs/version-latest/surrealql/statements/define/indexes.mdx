...
Use `CONCURRENTLY` to build the index concurrently. The statement will return immediatly, and it is then possible to track the progress of the index building by using the statement [INFO](/docs/surrealdb/surrealql/statements/info/).
