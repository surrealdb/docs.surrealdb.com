...
In the example below, the `VALUE` clause is used to ensure that the email address is always stored in lowercase characters by using the [`string::lowercase`](/docs/surrealdb/surrealql/functions/database/string#stringlowercase) function.
...
A field can also be defined as a [literal type](/docs/surrealdb/surrealql/datamodel/literals), by specifying one or more possible values and/or permitted types.
