...
:::note
__Note:__  Since `v2.0.0`, schemafull tables are implicitly type [`NORMAL`](/docs/surrealdb/surrealql/statements/define/table#table-with-specialized-type-clause) tables by default. 
:::
...
As relations are stand-alone tables, they can be constructed before any linked records exist.
...
As such, a query on the relation will return nothing until the records it has been defined upon exist.
