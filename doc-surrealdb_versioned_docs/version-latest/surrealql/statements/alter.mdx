...
:::note
__Note:__ Currently, the `ALTER` statement only supports modifying tables and their properties - `ALTER TABLE`, but will be expanded to include other resources.
:::
...
:::note
__Note:__ To see the already defined items that can be modified with `ALTER`, use the [INFO](/docs/surrealdb/surrealql/statements/info) statement (`INFO FOR ROOT`, `INFO FOR NAMESPACE`, or `INFO FOR DATABASE`).
:::
