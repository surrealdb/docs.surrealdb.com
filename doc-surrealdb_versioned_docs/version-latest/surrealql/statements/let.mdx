...
Multiple possible types can be specified in a `LET` statement by adding a `|` (pipe) in between each possible type.
...
For more information on this pattern, see the page on [literals](/docs/surrealdb/surrealql/datamodel/literals).
