...
:::note
_NOTE_ : In the case where the record ID doesn't exist, the statement will fail and no records will be updated. Be sure to create the record using the [`CREATE`](/docs/surrealdb/surrealql/statements/create) statement before updating it. Alternatively, you can use the [`UPSERT`](/docs/surrealdb/surrealql/statements/upsert) statement to create the record if it doesn't exist.
:::

Let's look at some examples of how to use the `UPDATE` statement: First we'll create a `person` table with the [`CREATE`](/docs/surrealdb/surrealql/statements/create) Statement so that the examples can be run.
