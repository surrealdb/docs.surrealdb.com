...
For the strongest security, we recommend denying _all_ capabilities by default and only allowing the specific capabilities necessary for your service, following an allowlisting approach. We strongly discourage running SurrealDB with all capabilities allowed in production.

### Example: Deny all capabilities except for those required by your service

```bash
# Allow SurrealDB to call any functions from the array and string families,
# generate and compare Argon2 hashes and make HTTP GET requests over HTTPS to the address of a specific API.
...
Following a denylisting approach as described above should only be used as a last resort, since it is common to miss some risky cases (e.g. [169.254.169.254](https://book.hacktricks.xyz/pentesting-web/ssrf-server-side-request-forgery/cloud-ssrf)), which will become allowed by default.
