...
:::note
__Note:__ By running SurrealDB with the GraphQL module enabled, you are opting into an experimental feature. In SurrealDB GraphQL is still considered experimental and may not be as stable as querying via SurrealQL.  

GraphQL is a pre-release feature with known security flaws. This is not recommended for production use. We welcome your feedback and contributions to help improve the feature and make it more robust.
:::
...
:::note
__Note:__ In a sandbox connection, querying via GraphQL is not supported. 
:::
...
Additionally, you can start [a serving in Surrealist](/docs/surrealist/concepts/local-database-serving) which also enables graphql automatically, this starts a server on `http://localhost:8000` by default for a root user with username and password `root`. 
...
Next, using the [SurrealQL query editor](/docs/surrealist/concepts/sending-queries) create some data. For example, create a new `user` table with fields for `firstName`, `lastName`, and `email` and add a new user to the database.
...
__Note:__ You can also use GraphQL to query data from explicitly defined fields using the [`DEFINE FIELD` statement](/docs/surrealql/statements/define/field). Meaning that in schemaless mode, since the fields are not explicitly defined, you cannot query them using GraphQL.
:::
