...
-A, --allow-all                              Allow all capabilities [env: SURREAL_CAPS_ALLOW_ALL=]
...
-D, --deny-all                               Deny all capabilities except for those that are specifically allowed [env: SURREAL_CAPS_DENY_ALL=]
...
--deny-net [<DENY_NET>...]                   Deny all outbound network access except for network targets that are specifically allowed. Alternatively, you can provide a comma-separated list of network targets to deny [env: SURREAL_CAPS_DENY_NET=]
```
