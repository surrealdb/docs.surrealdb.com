...
When using a path argument with SurrealDB, we recommend not using the `file://` prefix. Instead, please use [`surrealkv://`](/docs/surrealdb/cli/start#surrealkv-beta) or [`rocksdb://`](/docs/surrealdb/cli/start#surrealkv-beta) as the path argument. If you are using the `file://` prefix, you will see the following warning:
...
SurrealKV is an embedded ACID-compliant key-value storage engine with built-in versioning, that allows for historical or temporal querying. Built entirely in Rust - like the rest of SurrealDB.

Since SurrealKV supports historical/temporal querying, you can user the `VERSION` clause when [SELECTING](/docs/surrealql/statements/select#the-version-clause) or CREATING data to access historical data. Learn more in the [SurrealQL documentation](/docs/surrealql/statements/select#version). 
