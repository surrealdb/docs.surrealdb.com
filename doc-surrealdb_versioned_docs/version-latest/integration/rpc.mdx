...
:::note
__NOTE:__ The actual values in the response will depend on your specific database/server instance.
:::
...
```json title="Request"
{
    "id": 1,
    "method": "run",
    "params": [ "fn::calculate_discount", [ 100, 15 ] ]
}
```
...
:::note
<em> NOTE:</em> When using a machine learning model (prefixed with `ml::`), the `version` parameter is **required**.
:::
...
<em>Important:</em> Ensure that your GraphQL queries and variables are correctly formatted to avoid parsing errors.
:::
...
:::note
<em> NOTE:</em> This method is particularly useful in databases that support graph-like relations, enabling complex data modeling and querying capabilities.
:::
