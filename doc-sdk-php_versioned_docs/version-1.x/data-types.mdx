...
$line->jsonSerializable();          // { type: "LineString", coordinates: [ [1, 2], [3, 4] ] }
...
```php title="Signature"
new GeometryPoint([number|Decimal $lat, number|Decimal $lon]);
```
...
```php title="Signature"
new Decimal(string|number|Decimal $decimal);
```
...
```php
$decimal = new Decimal("123.456");
$decimal->jsonSerializable();        // "123.456"
