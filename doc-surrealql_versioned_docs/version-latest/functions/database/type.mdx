...
While some of these functions work on any and all value types and many take an input of type `any`, the majority of such functions only work on certain types and generate an error otherwise. In addition, an argument that is the same type as the output type will simply be passed through as the output.

```surql
-- Anything can be stringified
RETURN type::string(["SurrealDB", {is_nice: true}]);

-- No way to make a UUID out of this
RETURN type::uuid(["SurrealDB", {is_nice: true}]);

-- Already an array, returns the input value unchanged
RETURN type::array(["SurrealDB", {is_nice: true}]);
```

```bash title="Output"
-------- Query 1 --------

"['SurrealDB', { is_nice: true }]"

-------- Query 2 --------

"Expected a uuid but cannot convert ['SurrealDB', { is_nice: true }] into a uuid"
...
This is the equivalent of using [`<array>`](/docs/surrealql/datamodel/casting#array) to cast a value to a boolean.
...
-- encoding::base64::decode("QSBmZXcgYnl0ZXM")
```
...
```surql title="API DEFINITION"
type::record(record | string) -> range<record>
```
...
The `type::string` function converts an value except `NONE`, `NULL`, and `bytes` into a string.
...
:::note
___NOTE:___ As of version 2.0, SurrealDB no longer eagerly parses strings into record IDs.
:::
