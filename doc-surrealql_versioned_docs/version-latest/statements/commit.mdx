...
* [THROW](/docs/surrealdb/surrealql/statements/throw) to cancel a transaction with an optional error message. THROW is the only way to cancel a transaction based on a condition, such as inside an [IF..ELSE](/docs/surrealdb/surrealql/statements/ifelse) block.