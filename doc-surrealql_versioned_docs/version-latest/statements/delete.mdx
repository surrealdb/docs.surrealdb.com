...
Note that as a `DELETE` statement returns an empty array by dafult, and the `ONLY` keyword assumes that a single object will be returned, it will return an error if `RETURN BEFORE` is not included.
