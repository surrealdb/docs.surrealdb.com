...
function findSinceMarkers(): FileInventory[] {
    ...
    for (const file of files) {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            const sinceMatch = line.match(/<Since\s+v=["'](v[\d.]+(?:-[a-zA-Z]+)?(?:\.\d+)?)["']\s*\/?>/);
            if (sinceMatch) {
            ...
            const textMatch = line.match(/(?:Since|Available since)[:\s]+(v[\d.]+(?:-[a-zA-Z]+)?(?:\.\d+)?)/i);
...
function generateInventory() {
    ...
    const v1Markers = allMarkers.filter(m => categorizeVersion(m.version) === 'v1');
    const v2Markers = allMarkers.filter(m => categorizeVersion(m.version) === 'v2');
    const v3Markers = allMarkers.filter(m => categorizeVersion(m.version) === 'v3');
    ...
    const needsSeparation = inventories.filter(i => (i.hasV2 && i.hasV3) || (i.hasV1 && (i.hasV2 || i.hasV3)));
    report += `### Files with Mixed Versions (${needsSeparation.length} files)\n\n`;
    report += `These files contain markers from multiple version ranges and need version gating:\n\n`;
    
    for (const inv of needsSeparation.sort((a, b) => a.file.localeCompare(b.file))) {
        report += `#### \`${inv.file}\`\n\n`;
    ...
    for (const inv of v2Only.slice(0, 20).sort((a, b) => a.file.localeCompare(b.file))) {
    ...
    for (const inv of v3Only.slice(0, 20).sort((a, b) => a.file.localeCompare(b.file))) {
        report += `- \`${inv.file}\` (${inv.markers.length} marker(s))\n`;
    }
    if (v3Only.length > 20) {
    ...
    for (const inv of inventories.sort((a, b) => a.file.localeCompare(b.file))) {
        report += `### \`${inv.file}\`\n\n`;
    ...
    // Write report
    const reportPath = path.join(process.cwd(), 'docs/surrealql-version-audit.md');
