...
export const IGNORE_DIRECTORIES_REGEX = [
	"_astro"
];

export const ALLOW_FILES_EXTENSIONS = [
	"html"
];
...
async function listDirectories(path: string) {
	...
	return entries
		...
		.filter(d => !IGNORE_DIRECTORIES_REGEX.some(rx => new RegExp(rx).test(d.name)))
		.map(d => d.name);
}
...
async function listHtmlFilesRecursively(path: string): Promise<string[]> {
	...
	for (const entry of entries) {
		if (entry.isDirectory()) {
			if (IGNORE_DIRECTORIES_REGEX.some(rx => new RegExp(rx).test(entry.name))) continue;
			results.push(...await listHtmlFilesRecursively(join(path, entry.name)));
			continue;
		}
		const isAllowed = ALLOW_FILES_EXTENSIONS.some(ext => entry.name.toLowerCase().endsWith(`.${ext}`));
...
async function parseHtmlForLink(filePath: string): Promise<MapEntryLink | null> {
	try {
		const content = await readFile(filePath, "utf-8");
		const html = parse(content);
		const rawTitle = html.querySelector("title")?.text || "";
		const titleParts = rawTitle.split(" | ").map(p => p.trim()).filter(Boolean);
		let title = titleParts[0] ?? "";
		const relPathFromDocs = relative(DOCS_PATH, filePath).split(sep).join("/");
		...
		const description = html.querySelector("meta[name='description']")?.getAttribute("content")?.trim() || "";
		const paragraphs = html.querySelectorAll(".flag-markdown p").slice(0, 3).map(p => p.text.trim()).filter(Boolean).join(" ");
...
function shouldIncludeInMinimal(relPathFromDocs: string, title: string): boolean {
	...
	if (segments[0] === "surrealql" && segments.length === 3 && segments[2] === "index.html") {
		if (["statements", "clauses", "functions", "datamodel"].includes(segments[1])) return true;
	}
	...
	if (segments[0] === "sdk") {
		...
		const allowSdkKeywords = /(installation|install|start|getting-started|quick[- ]?start|setup|concepts|core|data-types|frameworks|properties|api-documentation|create-a-new-connection)/i;
		if (allowSdkKeywords.test(path) || allowSdkKeywords.test(name)) return true;
		// Exclude method-heavy or API detail pages by default
		const denySdkKeywords = /(methods|api|class|function|examples?)/i;
		if (denySdkKeywords.test(path) || denySdkKeywords.test(name)) return false;
		return false;
	}

	// 4) Cloud: include index and key onboarding pages
	if (segments[0] === "cloud") {
		if (/(getting-started|connect|architecture|faqs|billing|support|create-an-account|create-an-instance)/i.test(path)) return true;
	}

	// 5) SurrealDB: include intro, installation, querying hubs
	if (segments[0] === "surrealdb") {
		if (/(introduction|installation|querying|models|embedding|cli)/i.test(path)) return true;
	}

	// 6) Integrations: include key category indexes
	if (segments[0] === "integrations") {
		if (/(^integrations\/index\.html$|embeddings\/index\.html$|frameworks\/index\.html$|data-management\/index\.html$|surreal-sync\/index\.html$)/i.test(path)) return true;
	}

	// 7) Surrealist: include landing and getting started
	if (segments[0] === "surrealist") {
		if (/(index\.html$|getting-started|installation)/i.test(path)) return true;
	}
...
function titleFromFolder(folder: string) {
	switch (folder.toLowerCase()) {
		...
		case "surrealml": return "SurrealML";
		case "surrealkv": return "SurrealKV";
		case "surrealist": return "Surrealist";
...
function linksToMarkdownLines(links: MapEntryLink[]): string[] {
	return links.map(l => {
		...
		return summary ? `- [${l.text}](${l.url}): ${summary}` : `- [${l.text}](${l.url})`;
	});
}

function entryToMarkdown(entry: MapEntry, includeExcluded: boolean, headingLevel: number): string {
	const lines: string[] = [];
	...
	if (entry.children?.length) {
		for (const child of entry.children) {
			lines.push("");
			lines.push(entryToMarkdown(child, includeExcluded, headingLevel + 1));
		}
	}
...
function groupedMarkdown(entries: MapEntry[], includeExcluded: boolean): string {
	const blocks: string[] = [];
	for (const entry of entries) {
...
function collectLinks(entries: MapEntry[], includeExcluded: boolean): MapEntryLink[] {
	const out: MapEntryLink[] = [];
	for (const e of entries) {
		for (const l of e.links) if (includeExcluded || !l.exclude) out.push(l);
		if (e.children?.length) out.push(...collectLinks(e.children, includeExcluded));
	}
	return out;
}
...
const autoYes = argv.includes("--yes") || argv.includes("-y") || process.env.GENERATE_WRITE === "1";
...
if (!autoYes && (autoNo || bad.length > 0)) {
	if (autoNo) {
		...
		const answer = (await rl.question("Write outputs to llms.txt and llms-full.txt despite broken links? [y/N] ")).trim().toLowerCase();
		rl.close();
...
if (proceed) {
	...
	await writeFile("llms-full.txt", withHeaderFooter(groupedMarkdown(MAP, true)));
} else {
