...
const topicKeywords = {
  'AI': ['ai', 'llm', 'gpt', 'openai', 'langchain', 'rag', 'machine learning', 'ml', 'nlp', 'chatbot', 'gemini', 'titan', 'agents', 'vector', 'autonomous', 'embedding'],
  'Data Management': ['database', 'query', 'migration', 'schema', 'surql', 'orm', 'crud', 'data', 'management', 'storage', 'persistence'],
  'Embedding': ['vector', 'embedding', 'similarity', 'search', 'semantic', 'llm', 'ai', 'rag', 'gemini', 'titan', 'openai'],
  'Security': ['security', 'auth', 'authentication', 'jwt', 'rbac', 'permission', 'access control', 'encrypt'],
  'Examples': ['example', 'demo', 'sample', 'showcase', 'template', 'starter'],
  'Optimisation': ['optimisation', 'optimization', 'performance', 'benchmark', 'efficient', 'fast', 'speed'],
  'Beginner': ['starter', 'tutorial', 'guide', 'learn', 'introduction', 'getting started', 'beginner', 'basic']
};
...
function determineTopics(url, title, currentTopics) {
  ...
  if (currentTopics && Array.isArray(currentTopics)) {
    currentTopics.forEach(topic => {
      if (VALID_TOPICS.includes(topic) && !matchedTopics.includes(topic)) {
        matchedTopics.push(topic);
      }
    });
  }
...
mdFiles.forEach(file => {
  ...
  if (topicsMatch) {
    ...
    updatedContent = content.replace(/topics:\s*\n((\s*-\s*.*\n)*)/, newTopicsSection);
  } else {
