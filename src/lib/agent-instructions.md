...
| --- | --- |
| [Claude Code](/docs/agents/claude-code) | `claude mcp add` |
| Claude Desktop, or the Claude app | Settings → Connectors |
| [Codex](/docs/agents/codex) | `codex mcp add`, or `~/.codex/config.toml` |
| [Cursor](/docs/agents/cursor) | `mcpServers` with `url` |
| [GitHub Copilot](/docs/agents/github-copilot), or [VS Code](/docs/agents/vscode) | `servers` with `"type": "http"` |
| [Windsurf](/docs/agents/windsurf), or Antigravity | `mcpServers` with `serverUrl` |
| [Zed](/docs/agents/zed) | `context_servers` with `url` |
