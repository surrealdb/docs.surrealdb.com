...
Once a model is loaded, you will want to execute it to perform a calculation. First we will cover the high-level execution of models in Python and Rust, then we will explore what happens internally for these executions. Before we go through some code examples, let us define the different types of computation that we can perform on a model.
