...
SurrealML is an engine for storing and executing trained ML models. It leaves training to existing frameworks and works with them to ease the storage, loading, and execution of models. Someone using SurrealML will be able to train their model in a chosen framework in Python, save their model, and load and execute the model in either Python or Rust. The inference engine is composed of python bindings interacting with a core written in Rust. This means that the exact same code that the Python client calls will be running on a production node by itself, or in the database. While the SurrealML engine runs in the database, it is developed in a completely isolated GitHub repository, leaving the user free to choose how they deploy and interact with the SurrealML engine.

The first step is to install the package and check that SurrealML works on your machine.
...
This will give us a trained model, which we can now save with `SurrealML`. First we declare a `SurMlFile` object instance with the inputs, name, model object, and engine with the following code:
...
The next step is optional, but it would be nice to map our inputs to some keys. We must be careful with the order that we declare our columns as they need to be mapped with the order of inputs from the vector that our model was trained on. If you click on `HOUSE_LINEAR` you will see the following declaration:
...
The file is stored in the `.surml` format meaning that there is a header with the data that we defined, and the weights are stored in the `ONNX` format. This means that there are no language-dependent dependencies. We are now ready to load and perform calculations on our model. We can load our model with the following code:
...
Both types of executions are executing the ML model using the Rust engine internally. As a result, the exact same code will be running your model in SurrealDB or your inference server regardless of whether you choose to build a server in Python or Rust. This `.surml` file can also be loaded by Rust, or by any Python version that has surrealML installed, to execute inference.

With the model loading and executing locally, the next section covers deploying it onto SurrealDB by uploading it.

## Model deployment

Before we try and upload our model, we need to have a node running. We can do this with the `docker-compose.yml` file below:
...
Here we are feeding the fields from the table `house_listing` into a model we uploaded called `house-price-prediction` with a version of `0.0.1`. We then get the results of that trained ML model as the field `price_prediction`. We then use the calculated predictions to filter the records, giving us the following result:
...
- [Computation](/docs/explore/ml-models/surrealml/computation) - where a model runs, and what the database does with it
- [Storage](/docs/explore/ml-models/surrealml/storage) - how a trained model is stored and versioned
