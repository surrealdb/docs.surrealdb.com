---
position: 9
title: Integrate AWS Cognito with SurrealDB
description: "Use AWS Cognito as the authentication provider for a client-side application backed only by SurrealDB."
---
...
- Define user-level authorization using SurrealDB [record users](/docs/learn/security/authentication/users#record-users).
...
- You have a [fresh instance of SurrealDB running](/docs/running/overview).

- You can [use a local Docker container](/docs/running/docker) without volumes for the purposes of this guide.
...
Once your user pool has been created, open it in the AWS Console and take note of the following values:
...
```
  ...
  <TabItem label="Using Scope and Token (1.x only)">
  ```js
  const handler = async (event) => {
  event.response = {
    claimsOverrideDetails: {
...
Depending on how you configured your user pool, users will be able to register by different means. To ensure this guide can be followed easily, we will create a test user to authenticate in our web application. Open your user pool and click on the "Create user" button under the "Users" section in the main user pool view. Provide an email address for that user and check "Mark email address as verified" to avoid having to do so manually. Set a password that matches the requirements that you configured.
...
In order to allow SurrealDB to establish a connection with AWS Cognito to download the JWKS object, you will require running it with the network <a href="/docs/learn/security/authorization/capabilities">capability</a>.
...
To understand how revocation is handled by SurrealDB, read the [JSON Web Key Set documentation](/docs/reference/query-language/statements/define/token#json-web-key-set-jwks) under `DEFINE TOKEN`.
...
Note that the `$auth` variable accessible from SurrealQL will not contain any values in this case, as it requires the `id` claim to be added to the JWT, containing the value of the identifier of a SurrealDB record. For the current example, the `$auth` variable will not be necessary.
...
For this guide, we will create a simple client-side web application that will direct the user to log in using the Cognito Hosted UI, redirect the user back to our application with a query parameter containing an authorization code and exchange that code to Cognito for the user identity token which we will use to authenticate with SurrealDB. The application will create or update a SurrealDB user using data from the token claims. This user will later be able to visit our web application and retrieve their information from SurrealDB.


> [!NOTE]
> As most other authentication providers using OpenID Connect (OIDC), AWS Cognito issues both identity and access tokens. In this example, we will be using the identity token as it can include custom claims (which are required by SurrealDB) by default. Note that identity tokens should only be used to assert identity claims as opposed to access claims. In this case, we trust the identity token to provide information about the identity of the user. If we wanted the token to contain authorization claims (e.g. OAuth scopes) we should instead rely on the access token. Customising access token claims has been <a href="https://aws.amazon.com/about-aws/whats-new/2023/12/amazon-cognito-user-pools-customize-access-tokens/">recently supported by AWS</a> and requires enabling <a href="https://docs.aws.amazon.com/cognito/latest/developerguide/cognito-user-pool-settings-advanced-security.html#cognito-user-pool-settings-advanced-security.title">advanced security features</a>. This guide makes the deliberate decision of using the identity token to simplify the process.
...
Once the example web application is working, you can inspect the simple code under `app.js` to understand how it works.
...
You can view and download a minimal example of a web application using AWS Cognito and SurrealDB [in the AWS cognito example project](https://github.com/surrealdb/examples/tree/main/aws-cognito).
