---
position: 8
title: Implement semantic search in Rust
...
The purpose of this post is to demonstrate how to use the Rust SDK to store Mistral AI embeddings as [SurrealDB vectors](/docs/learn/data-models/vector-search/overview), which can then be queried natively in SurrealQL to perform semantic search.
...
```
cargo add anyhow mistralai-client tokio surrealdb --features \
  surrealdb/kv-mem
```
<br />
...
DEFINE INDEX hnsw_embed
  ON document FIELDS embedding HNSW DIMENSION 1024 DIST COSINE;
```
<br />
...
let client = Client::new(Some(KEY.to_string()),
    Some("OTHER_ENV_VAR".to_string()), None, None)?;
```
...
There are a [number of ways](/docs/languages/rust/concepts/flexible-typing) to work with or avoid structs when using the Rust SDK, but we'll just go with two basic structs: one to represent the input into a `.create()` statement, and one to display the results.
...
```rust
let input = "Octopuses solve puzzles and escape enclosures,
    showing advanced intelligence.";
...
```rust
let embeds = [
        "Octopuses solve puzzles and escape enclosures,
        showing advanced intelligence.",
        "Sharks exhibit learning behavior,
        but their intelligence is instinct-driven.",
    ...
    "Busan a major South Korean port located in the far southeast.",
        ...
        known for innovation and dense population.",
    //
...
You can customise this [with other algorithms](/docs/learn/data-models/vector-search/similarity-search#computation-on-vectors-vector-package-of-functions) such as Euclidean, Hamming, and so on.
...
```rust
async fn ask_question(input: &str, db: &Surreal<Any>,
...
```rust
ask_question("Which Korean city is just across the sea from Japan?",
    &db, &client).await?;
ask_question("Who was Germany's last Kaiser?", &db, &client).await?;
ask_question("Which sea animal is most intelligent?", &db,
    &client).await?;
...
async fn main() -> Result<(), Error> {
    ...
    for (index, error) in res.take_errors() {
        ...
        let client = Client::new(Some(KEY.to_string()), None, None,
        None)?;
    ...
    ask_question("Which Korean city is just across the sea from Japan?", &db, &client).await?;
        ask_question("Who was Germany's last Kaiser?", &db,
        &client).await?;
        ask_question("Which sea animal is most intelligent?", &db,
        &client).await?;
    ask_question("Which planet's atmosphere has a part with the same temperature as Earth?", &db, &client).await?;
...
This guide demonstrates how to store OpenAI embeddings as [SurrealDB vectors](/docs/learn/data-models/vector-search/overview) via the Rust SDK for the purposes of semantic search. It uses Rust's [async-openai](https://crates.io/crates/async-openai/0.28.3) crate to generate embeddings.
...
```
cargo add anyhow async-openai tokio surrealdb --features \
  surrealdb/kv-mem
```
<br />
...
DEFINE FIELD embedding ON document TYPE array<float>;
DEFINE INDEX hnsw_embed
  ON document FIELDS embedding HNSW DIMENSION 1536;
```
<br />
...
```
Error: invalid_request_error: Incorrect API key provided: blah. You \
  can find your API key at \
  https://platform.openai.com/account/api-keys. (code: \
  invalid_api_key)
```
<br />
...
There are a [number of ways](/docs/languages/rust/concepts/flexible-typing) to work with or avoid structs when using the Rust SDK, but we'll just go with two basic structs: one to represent the input into a `.create()` statement, and another to show the results.
...
```
Venus is closest to:
[{ distance: 0.6495068000978139f, text: \
  'Which planet rotates slowly on its axis?' }, { distance: \
  0.8388033444017572f, text: \
  'Which Greek general helped Cyrus the Younger?' }]

Xenophon is closest to:
[{ distance: 0.4421917772479055f, text: \
  'Which Greek general helped Cyrus the Younger?' }, { distance: \
  0.873354690471173f, text: 'What does the cat chase?' }]
...
[{ distance: 0.49966454922547254f, text: \
  'What is the largest inland sea?' }, { distance: \
  0.8096568276647603f, text: \
  'Which Greek general helped Cyrus the Younger?' }]
```
<br />
...
use surrealdb::{
    Surreal,
        engine::any::{Any, connect}, types::{RecordId, SurrealValue,
        ToSql, Value},
};
