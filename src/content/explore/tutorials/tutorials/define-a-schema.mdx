...
- SurrealDB is installed on your machine. You can download and install SurrealDB from the [installation page](/docs/start/self-hosted/installation).
- [A Command line interface (CLI)](/docs/reference/cli/surrealdb-cli/commands/sql) for interacting with SurrealDB or a [Surrealist sandbox](/docs/explore/surrealist/getting-started#using-the-sandbox).
...
In SurrealDB, you can add [flexible fields](/docs/reference/query-language/statements/define/field#flexible-data-types) to a schemafull table using the `DEFINE FIELD` statement using the `FLEXIBLE` clause. This allows you to have `schemaless` functionality in a `schemafull` table. For example:
