...
description: "In this guide, you'll learn how to implement real-time presence tracking that can be integrated in any application including chat applications, multiplayer games, and more."
---

# Build a real-time presence web application using SurrealDB live queries
...
a. [SurrealDB](/docs/running/installation) installed on your machine (Make sure you upgrade to the latest version if you already have SurrealDB installed on your machine)
...
Now, this component can be easily integrated into other components, like this one:
...
- `useCurrentUserPresenceLive` - the base hook that is notified by each change in the database (pure real-time capability)
...
One can notice the presence of the `useLiveQuery` hook. This hook is a custom hook that we created to simplify the lifecycle of a Live Query. It automatically subscribes to the Live Query when enabled and kills the Live Query on cleanup (when the component is unmounted). Cleaning up Live Queries correctly prevents memory leaks.
...
- [SurrealDB Live Queries](/docs/reference/query-language/statements/live-select) - The SurrealDB Live Queries documentation
- [SurrealQL Documentation](/docs/reference/query-language) - The SurrealQL documentation
- [Javascript SDK documentation](/docs/reference/javascript) - The Javascript SDK documentation
