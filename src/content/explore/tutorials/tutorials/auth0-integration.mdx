---
position: 10
title: Integrate Auth0 with SurrealDB
description: "Use Auth0 as the authentication provider for a single-page application backed only by SurrealDB."
---
...
To do that, we will use the JWKS support in SurrealDB in order to define a token verification mechanism pointing to a JWKS object served by Auth0. This JWKS object can be found in a [dedicated endpoint for your Auth0 domain](https://auth0.com/docs/secure/tokens/json-web-tokens/locate-json-web-key-sets).
...
Because of the diversity of SDKs that both SurrealDB and Auth0 support, it is difficult to provide examples that will work for everyone. In this guide, we will provide code examples for the most general use case of using the [SurrealDB HTTP REST API](/docs/reference/rest-api/http-protocol) and [Auth0 SPA JS](https://github.com/auth0/auth0-spa-js).
...
To learn about other potential uses for the SurrealDB token functionality, you can read the [`DEFINE ACCESS ... TYPE JWT` documentation page](/docs/reference/query-language/statements/define/access/jwt).
