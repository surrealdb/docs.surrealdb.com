...
Depending on the version of SurrealDB that you are on: `v1.x` or `v2.x`, you may have different options available with respect to using [Scope](/docs/reference/query-language/statements/define/scope) and [Token](/docs/reference/query-language/statements/define/token) or [DEFINE ACCESS](/docs/reference/query-language/statements/define/access) methods to you for integrating Auth0 as an authentication provider.
...
- Define user-level authorization using SurrealDB [record users](/docs/learn/security/authentication/authentication#record-users) or [scopes](/docs/reference/query-language/statements/define/scope) if you are on `v1.x`.
...
- You have a [fresh instance of SurrealDB running.](/docs/start)

- You can [use a local Docker container](/docs/self-hosted/docker) without volumes for the purposes of this guide.
...
exports.onExecutePostLogin = async (event, api) => {
  if (event.authorization) {
    ...
    api.accessToken.setCustomClaim(`https://surrealdb.com/ns`, "main");
    api.accessToken.setCustomClaim(`https://surrealdb.com/db`, "main");
...
Next, we should configure SurrealDB so that it can verify tokens sent to it through the [HTTP REST API](/docs/reference/rest-api/http-protocol) via the “Authorization” header or through any of the [SDKs](/docs/start) via the “Authenticate” methods.
