...
To do so run the [Start command](/docs/reference/cli/surrealdb-cli/start) in your terminal: 

```surql
surreal start --user root --pass secret
```

The above command starts SurrealDB with [authentication](/docs/reference/cli/surrealdb-cli/start#authentication) and specifies that the user and password are `root`. Note that this is just for demonstration purposes. You can replace `root` in both instances with any other value.
