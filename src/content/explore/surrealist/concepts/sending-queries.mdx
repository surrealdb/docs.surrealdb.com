...
description: "Run SurrealQL from Surrealist's Query view over multiple tabs, see saved queries, history, variables, and result layouts."
---
...
> [!NOTE]
> The example above shows writing a query to a sandbox connection where no credentials are required. Sandbox data is not persistent; to store data in a free cloud instance, use **Deploy to Cloud** in Surrealist. When connecting to a SurrealDB Cloud instance or a local connection, you will need to enter your namespace and database credentials in the connection dialog before you can send queries.
...
To save a query, press the save query button at the top right part of the query editor. You will be prompted to enter a name for the query, after which it will be saved and listed in the saved queries panel.
...
- **Live mode**: will display incoming messages received by any [LIVE SELECT](/docs/reference/query-language/statements/live-select) queries sent in your request. You can also terminate active live queries here as well.
