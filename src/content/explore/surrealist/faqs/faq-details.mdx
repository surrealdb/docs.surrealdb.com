...
There may be reasons why Surrealist is unable to connect to a remote database. Some common reasons include specifying the wrong port, using the wrong protocol, or entering incorrect authentication details.

There may be more complex reasons such as network issues, firewalls, or the remote database not being available. You can always try connecting via the [CLI](/docs/reference/cli/surrealdb-cli/overview) to see if the issue is with Surrealist or the remote database.
