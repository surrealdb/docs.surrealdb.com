...
You can then connect to the database either by creating a connection [inside Surrealist](https://surrealist.app/), or by using the following command.

```bash
surreal sql --user root --pass root --ns test --db test --pretty
```

Next, create a new Rust project with the command `cargo new project_name`, go into the newly created directory and use `cargo add` to add the following dependencies:
...
The `serde` and `tokio` crates will also need a few features. Your `cargo.toml` dependencies should look like this:
...
The first thing to do is a bit of groundwork to convert database errors into an error type of our own, which will then implement Axum's `IntoResponse` trait so that it can be used as output for the server. This can be done manually, but the `thiserror` crate saves a certain amount of typing.
...
Next, we'll put the database client together. Axum provides a [`.with_state()`](https://docs.rs/axum/latest/axum/routing/struct.Router.html#method.with_state) method when starting its router that would give us access to the database inside its functions. However, for simplicity we can wrap the client inside a `LazyLock` to make it into a global static.
...
* Run the database at localhost:8000
* Sign in as the root user created through the `surreal start` command
* Use the namespace "test" and database "test"
...
Let's go over each of the definitions to see what they do.

The first three define a `person` table. This table is schemaless, but has one required field `name`, which must be present and must be a string. This table has defined permissions by which a [record user](https://surrealdb.com/docs/surrealql/statements/define/access/record) is able to use `CREATE`, `SELECT`, and `UPDATE` on the `person` table, but can only `DELETE` records that it has created.

The way this is done is by using the `$auth` parameter. This parameter is set whenever a record user is set as the authorized used for the database. The `WHERE $auth` clause simply means "whenever a value exists for the parameter `$auth`". But for `DELETE` queries, it is not enough for `$auth` to just be present, the `created_by` field of a `person` record must also match the ID of the currently authenticated user.

The final `DEFINE FIELD` statement sets an automatically generated `created_by` field which is set to the value of `$auth`, and is `READONLY` and thus cannot be changed. When logged in as a system user (like a root user), its value will be `NONE`. But when logged in as a record user, its value will be something like `user:qx2apv5oc8mh03wtah0q`.
...
So where does an ID like `user:qx2apv5oc8mh03wtah0q` come from? This is thanks to the following definitions that set the signup and signin behaviour of the record users. A typical [`DEFINE ACCESS`](https://surrealdb.com/docs/surrealql/statements/define/access/record) statement will create some sort of record on signup (in this case, a `user`) record, and will compare it against a password during signin. Note that the access has its own name (`account`). A `DEFINE INDEX` statement is also used to ensure that user names must be unique.
...
For an actual user in production, you would probably want to require an email and some other fields for a user. Functions like [`string::is::email`](https://surrealdb.com/docs/surrealql/functions/database/string#stringisemail) can be used to ensure that the value passed in is valid.

```surql
DEFINE FIELD email ON TABLE user TYPE string ASSERT $value.is_email();
```

However, for this simple example, each user will simply have a unique username.

The last step is where the majority of the work takes place: setting up the paths for Axum to handle, and writing the functions that handle the endpoints and (usually) access the database to handle the request. To start, we'll create a function for the root path to display a helpful message to any users wanting to give the server a try via the browser or an app like curl or Postman. These paths and curl examples can all be seen on [the page for SurrealDB's HTTP endpoints](/docs/surrealdb/integration/http).
...
Each of these functions are pretty straightforward.
...
As is the `session()` function, which is just a convenience for a user curious about the current session data. As `.query()` can take more than one statement, it returns each of these responses in order with an index for each. The `.take()` method can then be used to access the response at that index, and turn it into anything that can be deserialized back into a Rust type. In our case, a `String` is all we need here as the output will only be used to show the user the current session info.
...
* The `.signup()` method returns a redacted `Jwt` by default. To make the token visible, you can use the `.into_insecure_token()` method as we have done here. As a small guide to getting started, this example is not concerned about security. However, if you are looking to create something more production-worthy, do take a look at the [security](https://surrealdb.com/docs/surrealdb/security) section of the documentation and the [security best practices](https://surrealdb.com/docs/surrealdb/reference-guide/security-best-practices) page.
...
As the database client is logged in as a root user, the `/person/` routes can be used to do perform any operation on the `person` records of the database.

You can also log in to the CLI or Surrealist as a root user and separately as a record user using the `/new_user` endpoint to compare the experience between the two.
...
But as a record user, it will include a `created_by` field.
...
Now that you have a running database using Axum, here are some other ideas that you might want to explore.

* Using the [`AUTHENTICATE`](https://surrealdb.com/docs/surrealql/statements/define/access/record#example-failing-authentication) clause inside the DEFINE ACCESS statement. This will result in increased performance 