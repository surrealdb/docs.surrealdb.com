...
This configuration on the SDK side is not needed if connecting to a remote instance that has the [`--allow-experimental`](/docs/surrealdb/cli/start#experimental-capabilities) flag passed in, but is if using an embedded instance like in the example below.
