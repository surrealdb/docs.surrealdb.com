...
Then use the `cargo add` command to add three crates: `surrealdb` and `tokio`, and with `serde` with the "serde\_derive" feature (`cargo add serde --features serde_derive`). The dependencies inside `Cargo.toml` should look something like this:
