...
```rust
use surrealdb::engine::any::connect;
use surrealdb_types::{Datetime, Error, Kind, SurrealValue, Value};
...
This attribute can be used on the fields with a type that implements serde's `Serialize` and `Deserialize` traits, but *not* `SurrealValue`.
This is meant for interopability and shouldn't be used if it can be helped.
