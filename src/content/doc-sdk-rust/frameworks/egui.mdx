...
* `serde` and `serde_json`, for deserializing into Rust structs from the JSON format submitted from Egui,
...
* `Sign in as record user` button: if you paste in the output from the `New user` button, you will see an output like `Signed in as Lonnie!`. Note that this query will take a fraction of a second to compute. This is because the algorithms behind cryptographic functions like [`crypto::argon2::compare()`](/docs/surrealql/functions/database/crypto#cryptoargon2compare) are [meant to be computationally expensive](/docs/surrealdb/security/security-best-practices#passwords) so that comparing real passwords to hashed and salted passwords takes as long as possible - but just quick enough that a single comparison is barely noticed by a legitimate user.
