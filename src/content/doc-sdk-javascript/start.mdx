...
The demo project for this guide can be found in the [surrealdb-examples](https://github.com/surrealdb/examples/tree/main/ts-bun-starter) repository.
...
- You have an install version of SurrealDB `>1.5.0` installed on your machine.
...
Next, initialize a new project. You can initialize a new project with any package manager. For this guide, we will use bun.
...
Before you can use this SDK for JavaScript in your environment, you need to install and import it into your created project. In your project directory, install the [SDK](https://npmjs.com/package/surrealdb) using any of the following package managers (we recommend using bun for this guide):
...
After installing, you can then import the SDK into your project. Depending on your setup and environment, we supported multiple options. For the sake of this guide, we will use ES6 syntax.
...
Usually, you can start a new instance of SurrealDB using the `surreal start` command. For the sake of this guide, we will use a function to start the database so that we can interact with the database in the browser. Create a new file named `start-db.ts` in the root of your project and add the following code:
...
Now that you have initialized the SDK, you can use it to connect to the database, use a specific namespace and database, and perform queries anywhere in your application by calling the `getDb` function. 

In this section, we will look at how you can interact with the database via CRUD operations.
...
### Create a new user
Create a new `src` folder and add `create-user.ts` file. This file will contain the logic to create a new user in the database. We will use the [`create`](/docs/sdk/javascript/methods/create) method to create a new user in the database. 

First, import the `getDb` function from the `surreal.ts` utility file. You can also import the `jsonify` class from the SDK to format the output.
...
In the `src` folder, add `delete-user.ts` file. This file will contain the logic to delete a user in the database. We will use the [`delete`](/docs/sdk/javascript/methods/delete) method to delete a user in the database. Since we are working with a single user, we will use the `delete` method to delete the user. For a more specific use case, you can use the `RecordId` datatype to delete a specific user.
...
For the sake of this guide we will simulate a simple UI to interact with the scripts we created above. Usually this can be replaced with any framework of your choice. In the root of your project, create a `src` folder and add `index.ts` file.
