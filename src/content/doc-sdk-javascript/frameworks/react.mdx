...
In your top level component, wrap your root component with `SurrealProvider`:
...
If you are only interested in accessing SDK methods, you can use `useSurrealClient()` to directly obtain a reference to the `Surreal` client instance.
...
export function ExampleComponent() {
  ...
  return (
    <div>
      {/* Example button to execute a query */}
