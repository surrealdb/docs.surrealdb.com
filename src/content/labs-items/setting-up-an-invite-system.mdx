...
Usually when building modern applications, there are instances where you want to allow users to invite others to join, whether it's a project, a team or whatever shared thing. Traditional invitation systems need you to handle a lot of infrastructure: token management, validation logic, cron jobs, email services, etc etc. But what if we told you that SurrealDB does most of this natively?

## The Challenge 

Most invitation systems follow a similar pattern: generate a unique token, store it with an expiration date, and so on. Simple in theory but a lot of separate pieces in practice, middleware to validate tokens, race condition handling and so on.
...
This flow is quite elegant we would say, authenticate with the invitation token, which basically validates everything automatically. Then in a simple transaction, find or create the member, establish the relationship, mark the invitation as used, done. Boom no race conditions and no manual validation logic.
