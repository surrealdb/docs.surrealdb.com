...
Spectron exposes seven tools over the Model Context Protocol at `/mcp`. All tools use the same **`Authorization: Bearer`** authentication as the REST API and respect the `X-Spectron-Context` and `X-Spectron-Scope` headers set during installation.
