...
> [!NOTE]
> `X-Spectron-Context` is a **client-side convenience** for templating. Each MCP tool call must pass `context_id`. The server selects the context from the tool argument, not from this header alone.
