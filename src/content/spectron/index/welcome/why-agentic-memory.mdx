...
| No provenance | Every row carries a **`source`** object (kind, ref, spans, trust, derivation) – see [Provenance](/docs/spectron/mental-model/provenance-and-traceability) |
| No correction tracking | Supersession chains plus explicit **`uncertainty`** for cross-provenance clashes |
| No temporal validity | **Tri-temporal** model – system, known, and valid time – [Tri-temporal model](/docs/spectron/architecture/tri-temporal-model) |
| No verifiability | **Traces** (`retrieval_trace`, `decision_trace`, `response_trace`) as substrate nodes, not disposable logs |
| Unreliable retrieval | **Hybrid** structural retrieval plus **tiered** resolution – [Coherence, retrieval, and cost tiers](/docs/spectron/architecture/coherence-retrieval-and-tiers) |
