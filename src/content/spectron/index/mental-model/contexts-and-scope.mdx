...
- **`scope_view`** — controls how broadly results are folded **within** the caller’s grant:
  ...
  - **`merged`** — intended to fold in same-fact rows stored at narrower scopes while preserving provenance. Accepted on the wire; behaviour may still match `strict` until fully enabled.
