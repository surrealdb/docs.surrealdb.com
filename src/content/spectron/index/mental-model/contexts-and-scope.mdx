...
| --- | --- | --- |
| **API key** | Who is calling? | You rotate or revoke a credential |
...
A key authenticates, but does not itself carry permission. It identifies a
principal, the principal holds grants, and the grants name scope paths. So a
write resolves as **key → principal → grants → scope**, and the fact is tagged
with the scope region that chain lands on.
