...
On SurrealDB Studio, a new context's Overview count of **one scope** is usually that root write region. Register additional paths (for example `surrealdb/employee_1` and `surrealdb/employee_2`) when you want explicit folders for grants and uploads — see [Spectron on SurrealDB Cloud](/docs/spectron/quickstarts/surrealdb-cloud).
...
[scope floor](/docs/spectron/reference/glossary#scope-floor) sets the minimum
...
trailing `/*` is what separates them, so paths are hierarchical only where a
pattern says so:
...
> together with `memory:write=org/acme/team/eng` writes rows the same principal
> cannot read back, and the symptom is quiet: scoped reads return **empty**, not
> refused, so a client polling for its own writes waits instead of failing. Use
> `org/acme/*` when you mean the region.
...
See [Surface, models, and security](/docs/spectron/architecture/surface-security-and-models#security-and-privacy) for how keys and grants are enforced at the request surface.
...
- [Principal](/docs/spectron/reference/glossary#principal)
