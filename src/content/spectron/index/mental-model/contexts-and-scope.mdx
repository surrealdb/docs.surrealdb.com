...
- **`scope_view`** — controls how broadly results are folded **within** the caller’s grant:
  ...
  - **`crossTeam`** — intended to include facts shared inbound from other principals where shared-read grants exist. Accepted on the wire; behaviour currently matches **`strict`** until cross-principal sharing is fully enabled.
  - **`merged`** — intended to fold in same-fact records stored at narrower scopes while preserving provenance. Accepted on the wire; behaviour currently matches **`strict`** until fully enabled.
