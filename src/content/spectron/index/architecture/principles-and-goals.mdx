...
Spectron is a **memory and knowledge layer for AI agents** built so operators and users can **trust** what the system remembers and retrieves. It sits as a stateless application tier in front of SurrealDB; durable state – graph, vectors, documents, structured records – lives in the database, typically **one ACID transaction per write**.
...
- **Memory is tri-temporal and non-deleting.** Supersession and aging replace blind overwrite. **System time** (MVCC record history), **known time** (when Spectron first learnt a fact), and **valid time** (when the fact held in the world) are queryable independently. See [Tri-temporal model](/docs/spectron/architecture/tri-temporal-model).
...
- [Eight pillars and six categories](/docs/spectron/architecture/eight-pillars-and-categories) – the operational model in primitives.
- [Architecture glossary](/docs/spectron/architecture/glossary) – short definitions of key terms.
- [Unified substrate and authority](/docs/spectron/mental-model/two-layer-architecture) – how authoritative and experiential material meet in one graph.
