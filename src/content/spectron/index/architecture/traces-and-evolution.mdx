...
| **`decision_trace`** | Every reconciliation | Input extraction, confidence, trust, per-record outcome (created, updated, superseded, flagged) | `considered`, `created`, `superseded`, `flagged`, `parent_trace` |
...
Traces are queryable (for example `GET /api/v1/{ctx}/traces` and `GET /api/v1/{ctx}/traces/{id}`) and inspectable from the CLI (`spectron inspect trace:…`). See [Surface, models, and security](/docs/spectron/architecture/surface-security-and-models) for HTTP and CLI detail.
