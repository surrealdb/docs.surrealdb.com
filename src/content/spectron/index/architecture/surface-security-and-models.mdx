...
| `POST /api/v1/{ctx}/facts/batch` | Bulk conversation ingest (`messages: [{ role, content, ts }, …]`). Optional **`extract`**: `per_message` (default) or `whole_conversation`. Idempotent with `Idempotency-Key`. |
