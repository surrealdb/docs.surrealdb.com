...
| `POST /api/v1/{ctx}/query` | Unified read over facts and passages. Accepts optional **`labels`**, **`lens`** (filters by scope-path **involvement** within the grant — does not shrink access to exact clause matches), and **`scope_view`** (`strict`, `merged`, or `crossTeam`) to control how broadly results are folded within what the caller is already allowed to see. **`merged`** and **`crossTeam`** are accepted on the wire; behaviour currently matches **`strict`**. |
