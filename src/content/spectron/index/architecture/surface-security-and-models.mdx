...
| `POST /api/v1/{ctx}/query` | Unified read over facts and passages. |
...
- **Generated SDKs** – Python (`surrealdb-spectron`), TypeScript (`@surrealdb/spectron`).
...
- **Graph-resident audit** – writes emit `decision_trace`; reads emit `retrieval_trace`; `/chat` and `/reflect` emit `response_trace`.
