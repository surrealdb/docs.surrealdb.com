...
Use the [management API](/docs/spectron/reference/management-api) or `spectronctl` to create a Context per tenant, then mint an agent key with the appropriate scope floor:

```bash
spectronctl contexts create tenant-acme \
  --namespace tenants --database acme \
  --api-key "$SPECTRON_MGMT_KEY"

spectronctl contexts tenant-acme keys create app-agent \
  --principal agent \
  --scope-floor 'org=acme' \
  --api-key "$SPECTRON_MGMT_KEY"
```
...
Under scope-based isolation, a key with scope floor `{org: "alpha"}` physically cannot read data written by a key with scope floor `{org: "beta"}`. The server enforces this at the query level ([Scope as a security boundary](/docs/spectron/self-hosting/security/scope-as-security-boundary)).
