...
Under scope-based isolation, a key with scope floor `{org: "alpha"}` physically cannot read data written by a key with scope floor `{org: "beta"}`. The server enforces this at the query level — see [Scope as a security boundary](/docs/spectron/self-hosting/security/scope-as-security-boundary) for the mechanism.
