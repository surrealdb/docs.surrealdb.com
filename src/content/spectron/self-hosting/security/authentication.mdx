---
position: 1
title: Authentication
description: "API key authentication and management in Spectron."
---

# Authentication

Every request to Spectron must carry a context or management API key as an HTTP Bearer token:

```http
Authorization: Bearer <secret>
```

There are no session cookies and no OAuth on the data plane. Authentication is stateless: the server validates the key, resolves the principal, and enforces scope before handling the request.

## Keys and principals

| Key | Bound to | Authority |
| --- | --- | --- |
| **Management key** | Nothing — it sits outside the scope model | Context lifecycle, principal creation, key provisioning, management REST |
| **Data-plane key** | Exactly one principal in one Context | Whatever that principal's grants allow, optionally attenuated further on the key |

A principal's `kind` (`human` / `agent` / `service` / `unknown`) records what it is
and never affects access. Roles such as "supervisor" are not a type — they are a
principal granted a wide `memory:read` region.

See [Principals](/docs/spectron/self-hosting/security/principals) and [Scope as security boundary](/docs/spectron/self-hosting/security/scope-as-security-boundary).

## Bootstrapping the first keys

On a fresh install, run **bootstrap** once (via Docker or your operator runbook):

```bash
docker compose exec spectron spectrond bootstrap \
  --connection-string "ws://surrealdb:8000;root;root"
```

Stdout prints `SPECTRON_MANAGEMENT_API_KEY` and `SPECTRON_API_KEY` plus the new Context id. Store them immediately; secrets are not shown again.

For local single-process bring-up, **`spectrond dev start --bootstrap`** seeds the same keys on an empty database then serves (idempotent on restart). Pin a fixed Context key with **`--context-api-key`** / **`SPECTRON_API_KEY`** so clean databases do not mint a new secret each time — see [CLI — bootstrap](/docs/spectron/reference/cli#bootstrap).

Additional Context keys are minted via:

- Management REST: `POST /api/v1/contexts/{id}/keys/{name}`
- CLI: `spectrond keys generate-key` ([CLI reference](/docs/spectron/reference/cli))
## Environment variables (clients)

The public **`spectron`** CLI and generated SDKs expect:

| Variable | Purpose |
| --- | --- |
| `SPECTRON_URL` | Base URL (for example `http://localhost:9090`) |
| `SPECTRON_API_KEY` | Context-scoped end-user key |
| `SPECTRON_CONTEXT_ID` | Context id in the path |

Harness adapters also accept `SPECTRON_BASE_URL` and `SPECTRON_CONTEXT` per package READMEs in the Spectron repo `clients/` tree.

## MCP

MCP clients (Claude Desktop, Cursor, Claude Code) send the same Bearer token in the MCP transport config (`Authorization: Bearer …`). Point the client at your instance's `/mcp` endpoint — see the [MCP server install guide](/docs/spectron/integrations/mcp-server/install).

## Key rotation

Rotate a key in place without changing its id, principal binding, or grants:

```http
POST /api/v1/contexts/{context_id}/keys/{key_name}/rotate?ttl_seconds=2592000
Authorization: Bearer <management-key>
```

The response includes a new secret (shown once). The previous secret stops working immediately — there is no overlap window. Omit `ttl_seconds` to inherit the key’s current expiry; pass it to reset expiry from now.

```bash
spectrond keys rotate <context_id> <key_name> --expires-in 2592000
```

Alternatively, mint a replacement key and delete the old one — see [API keys and delegation](/docs/spectron/self-hosting/security/api-keys-and-delegation#key-rotation).
