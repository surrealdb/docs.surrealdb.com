...
See [SurrealDB file-backed storage](https://surrealdb.com/docs/running/file-backed) for the full path rules.
...
# Optional — allow browser clients (e.g. SurrealDB Studio) to call the user API cross-origin
# SPECTRON_CORS_ALLOWED_ORIGINS=http://localhost:1420,https://app.surrealdb.com
```

See [Configuration — CORS](/docs/spectron/reference/configuration#cors-browser-clients) for allowlist rules and management-API settings.
...
curl -s -X POST http://localhost:9090/api/v1/contexts/dev/keys/my-agent \
  ...
  -d '{
    "principal": "agent",
    "grants": [ { "path": "org/my-org/agent/my-agent", "verbs": ["memory:read", "memory:write"] } ]
  }'
```
