...
Pull the new container image, extract `spectrond` again (same steps as above), replace `/usr/local/bin/spectrond`, and restart the service. Pending per-Context schema migrations are applied asynchronously by the scheduler and workers — see [Migrations and upgrades](/docs/spectron/self-hosting/operations/migrations-and-upgrades).
