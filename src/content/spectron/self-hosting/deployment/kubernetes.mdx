...
If SurrealDB fails with *Failed to create RocksDB directory* or a read-only filesystem error, check that **`workingDir` matches `mountPath`** and that the path slashes are correct — see [Docker storage paths](/docs/spectron/self-hosting/deployment/docker#storage-paths).
