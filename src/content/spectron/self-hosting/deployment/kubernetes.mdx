...
Update the image tag in your `Deployment` manifest and apply it. The rolling update strategy ensures zero-downtime upgrades. Pending per-Context schema migrations are applied asynchronously by the scheduler and workers — see [Migrations and upgrades](/docs/spectron/self-hosting/operations/migrations-and-upgrades).

```bash
# Update the image tag (replace 0.2.0 with the target version)
kubectl set image deployment/spectron \
  spectron=ghcr.io/surrealdb/spectron:0.2.0 -n spectron
kubectl rollout status deployment/spectron -n spectron
```
