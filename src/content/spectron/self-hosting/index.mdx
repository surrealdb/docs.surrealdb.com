...
Spectron can be deployed on your own infrastructure so memory and knowledge stay under your control. The self-hosted distribution is a single stateless Rust binary – `spectrond` – that connects to a SurrealDB instance and an object store. All durable state lives in those two external systems. The binary itself holds no state and can be scaled horizontally by adding replicas.
