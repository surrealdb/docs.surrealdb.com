...
When the limit is reached, LLM-backed requests return **`429 Too Many Requests`** — including **`/chat`**, **`/facts?infer=full`**, **`/reflect`**, and **`/consolidate`**. Read-only operations (direct attribute lookups, cache hits) that do not involve LLM calls are not blocked.
