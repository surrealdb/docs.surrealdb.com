...
**`/traces/stats`** returns operational aggregates for the requested scope and time window — use **`windowHours`** for rolling windows (defaults apply when omitted). Scope-enforcing keys receive **zeroed** aggregates outside their grant (same rule as trace listing); only **`grant:manage`** or unscoped keys (when principal binding is not enforced) see Context-wide totals.
