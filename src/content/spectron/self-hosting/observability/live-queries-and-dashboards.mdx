...
## SurrealDB Studio (SurrealDB Cloud UI)

[SurrealDB Studio](https://app.surrealdb.com) is the SurrealDB Cloud dashboard. For **Spectron on Cloud**, use SurrealDB Studio to create contexts and mint **API keys** — see [Spectron on SurrealDB Cloud](/docs/spectron/quickstarts/surrealdb-cloud). That is separate from this operator-focused observability page.

For **self-hosted** deployments, SurrealDB Studio can also connect to the **raw SurrealDB** instance behind Spectron (advanced debugging — not the Spectron product UI):
