...
Trace listing is scope-gated like memory reads: **`manage`** holders see all traces; other callers see traces within their read grant. See [Tracing](/docs/spectron/self-hosting/observability/tracing).
...
Each API key records **`last_used_at`** on every authenticated request. For write-heavy auditing, query **`decision_trace`** rows by principal or session scope. For read auditing, use **`retrieval_trace`** listing or exported audit events.
