...
- Document deduplication co-ownership — **`document.scope_widen`** when a second uploader's scope clause is unioned onto an existing content-addressed document
...
Wire these events into your SIEM or log pipeline for long-term retention beyond trace table pruning. For deduplicated uploads, also watch the **`documents.scope_clause_count`** histogram — many clauses on one document may indicate over-broad co-ownership.
...
Each API key exposes **`lastUsedAt`** on the self-service key listing, derived from an append-only use log and throttled to at most one entry per key per minute — it shows recent activity, not a per-request trail. For write-heavy auditing, query **`decision_trace`** records by principal or session scope. For read auditing, use **`retrieval_trace`** listing or exported audit events.
