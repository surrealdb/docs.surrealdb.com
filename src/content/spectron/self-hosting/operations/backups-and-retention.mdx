...
There is no API for declarative per-scope retention rules. Expiry is driven by the
lifecycle sweeps, and the only configurable retention window is for trace rows,
set on the Context config:
...
You can trigger the sweep manually:

```python
result = await memory.lifecycle.expire()
print(result)
```
...
> [!NOTE]
> **`spectron lifecycle expire --older-than`** is not supported — the CLI rejects it because the REST route has no per-call threshold. Configure TTL and expiry policy per Context via management API or Context config instead.
...
```python
await memory.entities.delete("person", "alice")
```
