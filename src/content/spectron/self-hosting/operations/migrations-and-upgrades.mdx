...
Each Context database is created from a single greenfield schema file bundled into the server binary. Additional schema versions ship as `V2__…`, `V3__…`, and so on in an append-only sequence when included in the binary.
