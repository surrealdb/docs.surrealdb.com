...
| `add()` | `remember()` | Spectron also deduplicates and reconciles on write |
| `search()` | `recall()` | Spectron adds graph-density reranking |
...
await client.remember("I prefer vegetarian food.", scope=["user/alice"])
results = await client.recall("What are Alice's food preferences?", k=5, scope=["user/alice"])
...
await client.remember("I prefer vegetarian food.", { scope: ["user/alice"] });
const results = await client.recall("What are Alice's food preferences?", { k: 5, scope: ["user/alice"] });
...
**Scoped multi-tenancy**: Mem0 uses separate `user_id` and `agent_id` parameters. Spectron uses **slash-path scope** (for example `["org/acme"]`). A query at `org/acme` can retrieve org-wide memory across users.
...
class MemoryAdapter:
    ...
    async def add(self, content: str, user_id: str):
        ...
        async with self.spectron.sessions.create(scope=[f"user/{user_id}"]) as s:
...
async with client.sessions.create(scope=["user/alice"]) as session:
    for memory in existing_memories:
        await memory.remember(memory["memory"])
```
...
async with client.sessions.create(scope=[f"user/{user_id}"]) as session:
    await memory.remember(content, session_id=session.id, role="user")
