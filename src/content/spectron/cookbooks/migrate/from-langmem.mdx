...
**Namespace vs scope**: LangMem uses a tuple namespace `("user", "alice")`. Spectron uses hierarchical slash paths like `["user/alice"]`. For single-clause scopes, org-wide queries still surface org-tagged memory while hiding user-specific records — see [Contexts and scope](/docs/spectron/mental-model/contexts-and-scope).
