...
| `search_memory()` | `recall()` | Spectron adds graph-density reranking |
...
**Namespace vs scope**: LangMem uses a tuple namespace `("user", "alice")`. Spectron uses hierarchical slash paths like `["user/alice"]`. For single-clause scopes, org-wide queries surface org-tagged memory while hiding user-specific records — see [Contexts and scope](/docs/spectron/mental-model/contexts-and-scope).
...
async with client.sessions.create(scope=[f"user/{user_id}"]) as session:
    ...
    context = await memory.recall(query=user_message, top_k=5)
    ...
    await memory.remember(response.content, session_id=session.id, role="assistant")
```
