...
description: "Ticket-linked memory with authoritative knowledge policies."
---
...
When a customer says "your return policy is actually 60 days", Spectron stores their belief under the **Experiential** pillar with a `contradicts_l0` flag (legacy API name) rather than updating the curated policy under the **Authoritative** pillar. The agent is informed of the discrepancy and can correct the customer without the curated record being silently overwritten.

This is the core guarantee: **Authoritative** content is protected from conversational drift regardless of how many users assert conflicting information. See [Eight pillars and six categories](/docs/spectron/architecture/eight-pillars-and-categories) and [Authority when pillars meet](/docs/spectron/memory/reasoning-model/authority-hierarchy).
