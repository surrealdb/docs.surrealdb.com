...
When the user asks a question that might be answered by project documentation ingested into Layer 0, use `knowledge_search`:
...
This searches the Layer 0 knowledge base for relevant passages from ingested documents (architecture decision records, API specs, README files).

To ingest project documentation into Layer 0:
...
When multiple developers work on the same project, they share memory at the project scope. Each developer's session contributes to and reads from the same Layer 1 memory.
