...
Spectron serves MCP at `/mcp` on your instance (SurrealDB Cloud: your context host from SurrealDB Studio **API keys**; self-hosted: your server's base URL). Install it into your editor with [`install-mcp`](https://github.com/supermemoryai/install-mcp) — the `/mcp` URL is the first argument, auth goes through `--header`, and `--oauth no` skips the OAuth prompt:
