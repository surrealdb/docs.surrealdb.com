...
Research papers and documents should be ingested into Layer 0 for high-fidelity retrieval:
...
After ingestion, the agent can retrieve from Layer 0 via `knowledge_search` (MCP) or `session.knowledge_search()` (SDK):
