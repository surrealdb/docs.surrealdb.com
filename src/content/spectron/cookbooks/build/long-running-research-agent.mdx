...
session = await client.sessions.create(scope=["org/acme/user/researcher-alice"])
...
**Scope pruning**: When a research sub-topic is complete, use `memory.forget(scope=["org/acme/project/sub-topic"])` to prune stale context that would otherwise add noise to future recalls.
