...
If you want Spectron to manage the agent call rather than just recording turns, use the `chat()` endpoint. Spectron retrieves context, calls the Context's configured synthesis model, persists both turns, and runs extraction — there is no callback to supply. Override the model for a single call with `model` if you need to.
...
natural language and Spectron matches the facts within the caller's
