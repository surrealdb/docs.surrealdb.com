...
async def grounded_response(session, user_message: str) -> str:
    ...
    knowledge = await memory.documents.search(
        query=user_message,
        top_k=4,
    )
...
async function groundedResponse(session: Session, userMessage: string): Promise<string> {
    const [knowledge, ctx] = await Promise.all([
        memory.documents.search({ query: userMessage, topK: 4 }),
...
```python
node = await memory.documents.get(
    entity_type="Product",
...
```typescript
const node = await memory.documents.get({
    entityType: "Product",
