...
Many RPGs expose a small talk menu (Name, Job, Bye, and a few topics). Track those slots in the listener and restate them on talk end so Spectron can answer “people whose names I never asked” from **absence**, not only from fuzzy role words:
...
Keep engine **actor ids** on Spectron **labels** only (`npc_id=12`). Never put `npc_id=…` in Avatar-facing `/facts` or `/chat` prompts — synthesis will echo the identifier in first person. Appearance strings (“Spark”, “a shopkeeper”) stay in prose; durable ids stay in labels for filtering and briefing.
