...
Spectron can be used to answer questions that no single historical record answers on its own.

Land surveys, inscriptions, census returns, excavation catalogues, and parish registers share a shape, but tend to be short and fragmentary and may lack a stable identifier. The useful answers in this case come from **stitching**, in which Spectron composes many scattered records into one claim.
...
Do not resolve entities in your own pipeline. If you upload `holding → held_by → william_of_warenne`, you have already done the identity work, and Spectron cannot help with the hard part.
...
Upload records as documents rather than as many synchronous facts. See [Documents (upload)](/docs/spectron/reference/rest-api#documents-upload).
...
- **Leave similar names alone.** `Holconius`, `Marcus Holconius`, and `Marcus Holconius Priscus` may be one person or three. Let resolution and [reconciliation](/docs/spectron/agent-memory/reasoning/reconciliation-and-supersession) decide. A manual merge destroys the evidence for the decision.
...
Reads then accept `asOf`. You can ask what the corpus attests for one moment rather than for all time. See [Temporal validity](/docs/spectron/agent-memory/reasoning/temporal-validity) and [spoiler-safe narrative memory](/docs/spectron/cookbooks/patterns/spoiler-safe-narrative-memory) for more on this subject.
...
`k` on `/query` defaults to **10** and deployments cap it at **50**. The internal retrieval pool is set separately from `k`. See [Answer size vs search breadth](/docs/spectron/agent-memory/retrieve/hybrid-search#answer-size-vs-search-breadth).
...
- **Turn on query expansion for document retrieval.** `/documents/query` accepts `useHyde` and `decomposeQuery`. Both help when query wording and corpus wording diverge. See [Hybrid search](/docs/spectron/agent-memory/retrieve/hybrid-search#advanced-options).
...
3. **`/reflect`** for interpretive claims across many records. Reflection always calls a model. It returns synthesised text with its evidence. See [Reflection](/docs/spectron/agent-memory/operations/reflect).
...
- **Use a fresh Context per experiment.** Operator questions and prior answers become hits, so a reused Context contaminates the next run. See [Contexts and scope](/docs/spectron/mental-model/contexts-and-scope).
