...
# Each user message
result = await memory.chat(content=user_message)
...
const result = await memory.chat({ content: userMessage });
...
async def handle_message(user_id: str, session_id: str | None,
    ...
    if session_id:
        session = memory.sessions.create(session_id)
    else:
        session = await memory.sessions.create(scope=[f"user/{user_id}"])
...
async function handleMessage(
    ...
    const session = sessionId
        ? memory.sessions.create(sessionId)
...
```python
entities = await memory.entities.list(scope=[f"user/{user_id}"])
