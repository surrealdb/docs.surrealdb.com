...
**`/chat` bypass:** when a session already has prior turns, `/chat` skips tier-2 reuse and always synthesises a fresh reply — the transcript window makes the answer session-dependent even when the semantic query matches a cached entry. First-turn `/chat` calls and sessionless reads remain cache-eligible.
...
await memory.config.retention([
    # Keep context facts for 30 days for paying customers
        {"scope": ["plan/enterprise"], "memory_category": "context",
        "ttl": "30d"},
