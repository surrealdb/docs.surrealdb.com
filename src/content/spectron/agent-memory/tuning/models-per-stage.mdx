...
For provider keys and Gemini defaults, see [Models and providers](/docs/spectron/self-hosting/operations/models-and-providers).
...
Embeddings are Gemini-only at 3072 dimensions. Switching the deployment embedding model requires a reindex so stored vectors and query vectors share one space. See [Management API — force reindex](/docs/spectron/reference/management-api#force-reindex).
