...
The embedding stage converts text into dense vectors for similarity search. Spectron uses these vectors for the semantic response cache, HNSW chunk retrieval, and query expansion. The embedding model is **fixed at deployment** (via `SPECTRON_MODEL_EMBEDDING`) — it is not assignable per Context. Vector reads filter by the active **`embedding_model_id`** so query vectors are never compared against a different model’s space. After changing the deployment embedding model, run a [reindex](/docs/spectron/reference/management-api#force-reindex) before serving traffic.
...
Embedding is configured on the server (`SPECTRON_MODEL_EMBEDDING`), not via this API.
