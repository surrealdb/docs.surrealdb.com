...
print(session.scope)    # {"user": "alice", "org": "acme"}
```
...
console.log(session.scope); // { user: "alice", org: "acme" }
```
...
As messages flow through your application, record each one by posting to the turns endpoint. Spectron extracts structured facts from each turn synchronously and returns the extraction result immediately. See [Adding turns](/docs/spectron/agent-memory/sessions/adding-turns) for the full extraction result shape.
...
Creating a **new** session (no pinned id) still uses the caller's resolved write region for the new session's scope tags.

The ownership restriction applies to **writes** only. Reading state and retrieving context from a session may be permitted to other principals depending on the access control configuration of the context. See [Authentication and principals](/docs/spectron/self-hosting/security/principals) for details.
