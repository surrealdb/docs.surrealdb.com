...
Optional read modifiers: **`labels`** (descriptor filter only), **`lens`** (filter by scope-path involvement), **`scope_view`** (`strict`, `crossTeam`, `merged`), **`include`** (`facts`, `passages`, or both — narrows the response only; retrieval still considers all families).
