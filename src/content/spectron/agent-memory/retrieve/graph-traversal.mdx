...
Optional **`graph_edges`** on **`/documents/query`** select which of these signals contribute during **`hybrid_graph`** reranking (`knowledge_has_keyword`, `section_match`, `document_link`, `document_summary`, `keyword_cooccurrence`). See [Hybrid search](/docs/spectron/agent-memory/retrieve/hybrid-search).
...
- [Hybrid search](/docs/spectron/agent-memory/retrieve/hybrid-search)
- [Keywords and BM25](/docs/spectron/agent-memory/retrieve/keywords-and-bm25)
- [Cross-layer linking](/docs/spectron/agent-memory/reasoning/cross-layer-linking)
