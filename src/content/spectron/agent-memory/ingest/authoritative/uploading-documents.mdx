...
file=<binary>
metadata={"title":"Returns Policy","scope":["org/acme/team/eng"]}
```

The `metadata` part is JSON. **`scope`** is a `ScopeSet` (slash-path strings). Omit it to tag the document with the caller's full `memory:write` region.
...
> [!NOTE]
> **MCP `upload`** threads the caller’s write scope the same way as REST — scoped keys produce scope-tagged documents and chunks visible under `memory:read` within that region.
