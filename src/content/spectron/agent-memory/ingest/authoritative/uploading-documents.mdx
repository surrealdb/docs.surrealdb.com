...
Document extraction uses the same structured schema and reconciler as conversational ingest ([Storing memories](/docs/spectron/agent-memory/ingest/experiential/remember)). Facts from a PDF and facts from a turn land in one substrate; prefer documents for long curated sources and `/facts` for short lived moments.
