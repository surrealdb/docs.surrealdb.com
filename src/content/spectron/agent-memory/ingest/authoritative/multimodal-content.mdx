...
Spectron posts raw file bytes and expects a JSON response with extracted text or embedding vectors (exact contract depends on modality). Endpoints must be reachable from the worker and respond within the pipeline timeout.

**Vision LLMs:** image understanding during ingest also works when the Context’s extraction LLM supports vision (OpenAI and Anthropic providers send image content blocks when media is attached).
