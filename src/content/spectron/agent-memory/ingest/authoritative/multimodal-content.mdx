...
PII redaction can be enabled per-Context and applies to all ingested content, including OCR-recognised text and speech transcripts, before chunking. See [Extraction tuning](/docs/spectron/reference/configuration#extraction-tuning) for the **`pii_redaction_enabled`** field.
