...
Document-only ranked retrieval accepts the same **`mode`** values as unified recall (`vector`, `bm25`, `hybrid`, `hybrid_graph`). Optional flags **`useHyde`** and **`decomposeQuery`** are honoured when a request-path LLM is configured: HyDE embeds a hypothetical answer for the vector leg (BM25 and reranking still use the raw query); decomposition fans out sub-questions and merges deduplicated hits. Both degrade gracefully to single-query retrieval on failure.
