...
**Scope gate:** callers with **`grant:manage`** or unscoped keys (when principal binding is not enforced) see Context-wide aggregates. Scope-enforcing keys receive a **zeroed** stats view for regions outside their grant, mirroring trace listing so that cross-principal usage metadata does not leak.
