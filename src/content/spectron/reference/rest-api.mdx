...
| `scope_view` | How broadly to fold results within the grant: `strict` (default — caller’s region only), `crossTeam` (intended to include inbound cross-principal shared reads), or `merged` (intended to fold in same-fact records at narrower scopes). **`crossTeam`** and **`merged`** are accepted on the wire; behaviour currently matches **`strict`** until those views are fully enabled. None widen past the caller’s grant. |
