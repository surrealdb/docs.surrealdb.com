...
`extract` controls batch extraction strategy: **`per_message`** (default) runs extraction per turn; **`whole_conversation`** sends the full transcript in one LLM pass.
...
Response includes `tier` (`direct`, `cache`, `hybrid`, or `full_context`), `hits` (each with `source` as a **`ResultKind`** — for example `entity`, `attribute`, `passage`, `document`), `query_ms`, and `trace_id`.
...
`reflect` runs on-demand synthesis and returns a **`trace_id`** (`response_trace`); `persist` on the body stores lower-trust derived facts when permitted by key type.

`consolidate` responses reference **`decision_trace`** outcomes with per-record **`DecisionKind`** values: `create`, `update`, or `supersede`.
...
**`/audit`** returns structured audit records with a **`TraceKind`** discriminator (`retrieval_trace`, `decision_trace`, or `response_trace`) linking back to graph-resident traces where applicable.
