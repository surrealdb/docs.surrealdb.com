...
The identity an API key acts as. A principal has a `kind` (`human`, `agent`, `service`, or `unknown`) that describes what it is, and a per-verb `grants` map that decides what it may do and where. Kind never affects access. See [Permissions and delegation](/docs/spectron/mental-model/contexts-and-scope#permissions-and-delegation).
