...
If provisioning fails **after** the database was created by this request, Spectron **discards that database** so a retry starts from a clean migration state. Pre-existing databases (operator-provided or left over from a crash) are never removed — only a database this request created is eligible for discard. See [Provisioning failures](/docs/spectron/self-hosting/operations/contexts#provisioning-failures).
...
Returns a fleet-wide view of per-Context schema catch-up after a binary upgrade: version histogram, Contexts still behind the embedded latest, and DLQ entries. See [Migrations and upgrades](/docs/spectron/self-hosting/operations/migrations-and-upgrades).
...
Top-level fields such as **`enforcement_blocked`** (pay-as-you-go vs hard block — see [Configuration](/docs/spectron/reference/configuration#top-level-context-fields-outside-config)) may be set on the same `PATCH` body outside `config`.
