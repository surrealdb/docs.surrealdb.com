...
Generates a new secret for the named key. The key **id**, principal binding, grants, and `created_at` are preserved — only the stored hash changes. The old secret is invalidated immediately. Returns the new secret (shown once).

| Query param | Behaviour |
| --- | --- |
| `ttl_seconds` omitted | Inherit current expiry |
