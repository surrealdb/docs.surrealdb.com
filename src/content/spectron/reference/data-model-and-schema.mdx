...
Scoped tables carry **`scope_sets`** — content-addressed OR-of-AND clauses referencing hierarchical **`scopes`** paths. Entity and keyword index records also store derived **`visible_scope_sets`** so name/type lookups cannot leak facts the caller cannot read whole. See [Contexts and scope](/docs/spectron/mental-model/contexts-and-scope) for the visibility rules; the snippets below use simplified `scope` field names where the wire shape is still `key=value` tags in examples.
...
Structured authoritative facts extracted from documents (or ingested via `POST /facts` with `infer: "triples"`) are stored as **`entity`**, **`attribute`**, and **`relates_to`** records with `source.kind = "document"`. Connectors are not yet implemented; see [Connectors overview](/docs/spectron/knowledge/connectors/overview).
