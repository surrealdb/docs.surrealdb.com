...
DEFINE INDEX embedding_index 
  ON keyword FIELDS embedding HNSW DIMENSION 1536 DIST COSINE TYPE F32;
...
Optional **`role`** and **`duplicate_of`** support role-aware retrieval and near-duplicate suppression (see **`includeDuplicates`** on `/query`).
