...
Scoped tables carry **`scope_sets`** — content-addressed OR-of-AND clauses referencing hierarchical **`scopes`** paths. Entity and keyword index records also store derived **`visible_scope_sets`** so name/type lookups cannot leak facts the caller cannot read whole. See [Contexts and scope](/docs/spectron/mental-model/contexts-and-scope) for the visibility rules.
...
| `created_at` | datetime | |
| `last_used_at` | option\<datetime\> | Vestigial — no longer written. Last use is derived as `max(used_at)` over `context_api_key_usage`; the column stays defined so older rows deserialize |
...
Schema is bundled into the Spectron binary and applied automatically when a Context is created or upgraded. See [Migration status](/docs/spectron/reference/management-api#migration-status) for the fleet-wide catch-up view.
