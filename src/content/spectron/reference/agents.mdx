...
`X-Spectron-Context` is a client-side convenience — each tool call must still pass `context_id`. Scope is per tool via a **`scope`** argument (slash paths, for example `["org/acme/user/alice"]`), not install-time headers.
