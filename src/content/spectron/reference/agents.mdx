...
Use **`remember()` + `sessions.context()`** when you need your own LLM, tools, or streaming. Use **`chat()`** when Spectron should retrieve, call the response model, and persist the reply.
...
Seven tools: `remember`, `recall`, `context`, `reflect`, `forget`, `upload`, `inspect` — see [MCP tools](/docs/spectron/reference/mcp-tools).
