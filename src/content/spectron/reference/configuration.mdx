...
| `reject_unbound_keys` | boolean | `false` | When `true`, API keys with **no bound principal** are rejected (`401`) instead of using the legacy unscoped passthrough. Set this once every data-plane key in the Context is principal-bound. |
...
| `SPECTRON_MAX_QUERY_K` | 1000 | Maximum `limit` / `k` on recall, context, document query, and MCP `recall` / `context` |
