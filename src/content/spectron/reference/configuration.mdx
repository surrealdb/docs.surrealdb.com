...
| `reject_unbound_keys` | boolean | `false` | When `true`, API keys with **no bound principal** are rejected (`401`) instead of using unscoped passthrough. Set this once every data-plane key in the Context is principal-bound. |
