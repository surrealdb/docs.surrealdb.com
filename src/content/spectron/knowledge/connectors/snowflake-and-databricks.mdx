...
Data warehouse connectors let you pull structured records from Snowflake or Databricks directly into Spectron's Layer 0 knowledge store. Each row in the query result becomes a typed knowledge node, giving your agents instant access to product catalogues, customer records, pricing tables, or any other structured data your warehouse holds.
...
The node ID is stable across syncs: if row `P001` changes in the warehouse, the existing node is updated rather than replaced, preserving any cross-references from Layer 1 memory.
