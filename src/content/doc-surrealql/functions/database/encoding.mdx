---
sidebar_position: 8
sidebar_label: Encoding functions
title: Encoding functions | SurrealQL
description: These functions can be used to encode and decode data in base64. It is particularly used when that data needs to be stored and transferred over media that are designed to deal with text. This encoding and decoding helps to ensure that the data remains intact without modification during transport.
---

# Encoding functions

These functions can be used to encode and decode data in `base64`. It is particularly used when that data needs to be stored and transferred over media that are designed to deal with text. This encoding and decoding helps to ensure that the data remains intact without modification during transport.

<table>
  <thead>
    <tr>
      <th scope="col">Function</th>
      <th scope="col">Description</th>
    </tr>
  </thead>
  <tbody>
    <tr>
      <td scope="row" data-label="Function"><a href="#encodingbase64encode"><code>encoding::base64::encode()</code></a></td>
      <td scope="row" data-label="Description"> This function is used to encode data without padding.</td>
    </tr>
    <tr>
      <td scope="row" data-label="Function"><a href="#encodingbase64encodepadded"><code>encoding::base64::encode_padded()</code></a></td>
      <td scope="row" data-label="Description"> This function is used to encode data with padding. </td>
    </tr>
    <tr>
      <td scope="row" data-label="Function"><a href="#encodingbase64decode"><code>encoding::base64::decode()</code></a></td>
      <td scope="row" data-label="Description">This function is used to decode base64 data.</td>
    </tr>
  </tbody>
</table>

## `encoding::base64::encode()`

The `encoding::base64::encode()` function encodes a bytes to base64 without padding.

```surql title="API DEFINITION"
encoding::base64::encode(bytes) -> string
```
The following example shows this function, and its output, when used in a [`RETURN`](/docs/surrealql/statements/return) statement:

```surql
RETURN encoding::base64::encode(<bytes>"");

""
```
```surql
RETURN encoding::base64::encode(<bytes>"2323");

"MjMyMw"
```

```surql
RETURN encoding::base64::encode(<bytes>"hello");

"aGVsbG8"
```

<br />

## `encoding::base64::encode_padded()`

The `encoding::base64::encode_padded()` function encodes a bytes to base64 with padding.

```surql title="API DEFINITION"
encoding::base64::encode_padded(bytes) -> string
```
The following example shows this function, and its output, when used in a [`RETURN`](/docs/surrealql/statements/return) statement:

```surql
RETURN encoding::base64::encode_padded(<bytes>"");

""
```
```surql
RETURN encoding::base64::encode_padded(<bytes>"2323");

"MjMyMw=="
```

```surql
RETURN encoding::base64::encode_padded(<bytes>"hello");

"aGVsbG8="
```

<br />

## `encoding::base64::decode()`

The `encoding::base64::decode()` function decodes a base64 to a string.

```surql title="API DEFINITION"
encoding::base64::decode(string) -> bytes
```
The following example shows this function, and its output, when used in a [`RETURN`](/docs/surrealql/statements/return) statement:

```surql
RETURN encoding::base64::decode("MjMyMw");

[
    50,
    51,
    50,
    51,
]
```

You can also verify the output of the encoded value matches the original value. 

```surql
RETURN encoding::base64::decode("aGVsbG8") = <bytes>"hello";

true
```

<br /><br />