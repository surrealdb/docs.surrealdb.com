...
[[test.results]]
error = ""Expected `record<person>` but found a `'purrson:one'`""
...
"Expected `record<person>` but found a `'purrson:one'`"
```
...
The `type::is_collection` function checks if the passed value is a geometry of type `collection`.
...
The `type::is_multiline` function checks if the passed value is a geometry of type `multiline`.
...
The `type::is_multipoint` function checks if the passed value is a geometry of type `multipoint`.
...
The `type::is_multipolygon` function checks if the passed value is a geometry of type `multipolygon`.
...
The `type::is_point` function checks if the passed value is a geometry of type `point`.
...
The `type::is_polygon` function checks if the passed value is a geometry of type `polygon`.
