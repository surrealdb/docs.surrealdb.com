...
A [number of functions](/docs/surrealql/functions/database/#aggregate-functions) can be used inside a `GROUP BY` query to perform an operation on the data as a whole as opposed to per record.

For example, the [`math::sum()`](/docs/surrealql/functions/database/math#mathsum) function can be used on an array of numbers to calculate their final sum.
