...
As such, a field defined as a `set` can take an `array` as its input and vice versa. This next example shows the `object::keys()` function, which returns an array, used to populate a field defined as an `option<set<string>>`.
