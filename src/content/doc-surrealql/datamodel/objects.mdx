...
As a record is essentially an object with a required [`id` field](/docs/surrealql/datamodel/ids) that can be created, updated, or deleted, they can be worked with in almost exactly the same way.
...
Inside a freeform object, non-ASCII field names can also be set by using a string.
