...
<img src="~/assets/img/image/light/surrealql-parse-error.avif" darkSrc="~/assets/img/image/dark/surrealql-parse-error.avif" alt="A screenshot showing how a string prefix allows incorrect UUID input to be identified before a query can be run. In this case, the parser is able to inform the user that an underscore at column 18 is the issue." />
