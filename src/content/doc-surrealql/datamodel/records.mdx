...
## Drawbacks of embedding records links

In SurrealDB, embedding records links within records is a powerful feature that allows for efficient querying and data retrieval. However, it also has a drawback: When an embedded record is deleted or changed, the parent record may not automatically reflect that change. This leads to stale references or “NONE” entries that can accumulate over time.

For referencal integrity between records, we recommend using the [`RELATE`](/docs/surrealql/statements/relate) statement to create a relationships which will automatically update when the referenced record is deleted or changed.
