...
CREATE
	...
	// Eight cities
	city:los_angeles, city:san_francisco, city:houston, city:dallas, city:toronto, city:ottawa, city:vancouver, city:victoria
	// Give them each names like 'earth', 'us', 'california', etc.
	SET name = id.id();

// Record links from planet to country
UPDATE planet:earth SET next = [country:us, country:canada];
// Record links from country to state/province
UPDATE country:us SET next = [state:california, state:texas];
UPDATE country:canada SET next = [province:ontario, province:bc];
// Record links from state/province to city
...
Traversing each of these paths can be done manually, but takes a good deal of typing and requires knowing the exact depth to traverse.

Here is an example using record links:

```surql
planet:earth.next;
planet:earth.next.next;
planet:earth.next.next.next;
```
...
Or by using graph links. The `(?)` inside the next example means to return anything at the `out` field of a relation

```surql
-- Show all `country` records located at `out`
planet:earth->has->country;
-- Show all `province` or `state` records located at `out`
planet:earth->has->country->has->(province, state);
-- Or use (?) to show any type of record located at `out`
planet:earth->has->(?)->has->(?)->has->(?);
```
...
This syntax can be inserted into both record link and graph queries.
