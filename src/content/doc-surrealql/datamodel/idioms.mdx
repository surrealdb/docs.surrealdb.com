...
CREATE planet:earth SET name = "Earth", all_people = [person:billy, person:billina];
...
For a standard `SELECT` query like these, the difference between a regular query and a query using destructuring is mostly cosmetic - aside from the note on aliases mentioned just above. However, being able to access the current record becomes necessary when using [recursive paths](#recursive-paths).
...
However, if the idiom path is enclosed in parentheses, it will return the record as expected (i.e in full).
...
The following query combinations also return the record 
...
-- Minimum 2, maximum 6 iterations of recursion (or either)
...
* If we already passed the minimum depth, it returns the last valid value.
* During each iteration, if we encounter an array value, all dead end values are automatically filtered out, ensuring no empty paths are included.
