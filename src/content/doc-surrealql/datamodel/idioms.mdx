...
* NONE, NULL, and arrays which are empty or contain only NONE/NULL are considered a dead end.
* An iteration with the same value as the previous one is also considered a dead end.
* If a query does not reach the minimum depth, it returns NONE.
* If it does reach the minimum depth, it returns the last valid value.
* During each iteration, all dead end values and paths are automatically filtered out whenever an array is encountered.
