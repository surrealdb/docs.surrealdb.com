...
Unfortunately, the output shows that the query stopped at a depth of one. This is because the path `->has->(?).name` only leads to a string: a dead end.
...
Enclosing `->has->(?)` in parentheses is an instruction to compute the graph traversal first as many times as needed, and only then move on to the `name` field.
