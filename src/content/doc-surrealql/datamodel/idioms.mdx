...
- **First**: Access the first element of an array.
...
Since SurrealDB is, at its core, a document database each record is stored on an underlying key-value store storage engine, with the ability to store arbitrary arrays, objects, and many other types of data.  you can access fields in a record using the dot notation. To access a field in an object, use a dot `.` followed by the field name. 

This is mostly helpful when accessing fields within a record, but can also be used to access fields within an array.

For example, using the `CREATE` statement to  a record into the `person` table:
...
To access all elements in an array or all fields in an object, use `[*]` or simply `*`. This is useful when you want to access all the elements in an array or all the fields in an object. 
...
As we saw initially to access the first element of an array, use `[0]`.

```surql
SELECT results[0].score FROM person;
```
...
To flatten nested arrays, use `...` (ellipsis). This is useful when you want to access all the elements in a nested array. 

```surql
SELECT results.score... FROM student;
```

This idiom flattens any nested arrays within `results.score`, returning a single-level array.

### Where Filter

You can also filter elements based on a condition, use `[WHERE ... ]`. This will only return the elements that match the condition.

```surql
SELECT results[WHERE score > 70].score FROM student;
```
...
```
This idiom selects elements in `results.score` where the `score` is greater than 70.
...
SurrealDB will automatically convert the idiom part `.uppercase()` to the function `string::uppercase()` when the query is executed.
...
First, Let's create a new `student` and `company` record. then relate which student works for which company.
...
```surql
CREATE company SET name = "Acme Corp", industry = "Manufacturing";
CREATE company SET name = "Globex Inc", industry = "Technology";
```

```surql
RELATE (SELECT * FROM student WHERE name = "Alice") -> works_for -> (SELECT * FROM company WHERE name = "Acme Corp") SET since = "2015-06-01";
```

Explanation:

- `(SELECT * FROM student WHERE name = "Alice")`: Selects Alice's record.
- `works_for`: The edge table representing the employment relationship.
- `(SELECT * FROM company WHERE name = "Acme Corp")`: Selects Acme Corp's record.
- `SET since = "2015-06-01"`: Adds a since field to the relationship.

Relate the other students as well:

```surql
RELATE (SELECT * FROM person WHERE name = "Bob") -> works_for -> (SELECT * FROM company WHERE name = "Acme Corp") SET since = "2017-09-15";
RELATE (SELECT * FROM person WHERE name = "Charlie") -> works_for -> (SELECT * FROM company WHERE name = "Globex Inc") SET since = "2012-04-20";
```

To navigate through graph relationships, use `->` to traverse the graph.
...
- `->works_for->company.*`: Traverses the `works_for` relationship from `person` to `company` and selects all fields of the `company`.

This idiom navigates through the `knows` relationship to another `person` and accesses their `name`.

```surql
[
  {
    "company": {
      "id": "company:z1cry7zibupizdfsbzcy", // The actual ID
      "name": "Acme Corp",
...
This idiom safely accesses `person.spouse.name` if `spouse` exists; otherwise, it returns `null`.
...
If `friends` themselves have a `friends` array, creating nested arrays:
    ...
    friends: [
        {
            ...
            name: "Frank",
            friends: [
                { id: "person:9", name: "Ivan" },
                { id: "person:10", name: "Judy" }
            ]
        },
        {
            id: "person:7",
...
To get a flat list of all friends' friends:

```surql title="Get a flat list of all friends' friends"
SELECT friends.friends... FROM person WHERE id = r'person:5';
```
