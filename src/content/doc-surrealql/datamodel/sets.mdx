...
> [!NOTE]
> Before version 3.0.0-alpha.13, sets were simply arrays that deduplicated their items. To emulate the former behaviour, add the clause `VALUE $value.distinct()` to a `DEFINE FIELD` definition.
