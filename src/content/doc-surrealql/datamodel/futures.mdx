...
> [!NOTE]
> The `future` type is only available up to SurrealDB version 3.0.0-beta. Since version 3.0.0-beta, it has been replaced by [defined fields using the `COMPUTED` clause](/docs/surrealql/statements/define/field#computed-fields). Most examples in this page include the equivalent using the `COMPUTED` clause for reference.
