...
When a query containing `THROW` is executed via the [RPC](/docs/surrealdb/integration/rpc) or [HTTP](/docs/surrealdb/integration/http) API, the client receives a structured error with `kind: "Thrown"`. This allows client code to distinguish user-thrown errors from other server errors.

For example, `THROW "Insufficient funds"` produces the following query result:

```json
{
    "status": "ERR",
    "time": "23.8µs",
    "result": "An error occurred: Insufficient funds",
    "kind": "Thrown",
    "details": null,
    "cause": null
}
...
See the [RPC protocol Error Handling](/docs/surrealdb/integration/rpc#error-handling) reference for the full error format, error kinds, and detail schemas.
