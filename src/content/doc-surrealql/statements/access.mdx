...
The `PURGE` clause completely removes grants created with a specific access method that have already been expired or revoked. In scenarios with very large amount of grants associated with an access method (e.g. when grants are automatically generated), purging inactive grants can improve the performance and experience of auditing grants with the `SHOW` clause. In some very high volume scenarios where grants are created by the hundreds of millions, purging may be bnecessary to limit the probability of collisions resulting in failure to create new grants. Note that the performance cost of validating a grant is independent from the number of existing grants, regardless of whether they are active on inactive.
...
## Example: Purge grants that have been expired

```surql
ACCESS api PURGE EXPIRED;
```

## Example: Purge grants that have been revoked for more than 90 days

```surql
ACCESS api PURGE REVOKED FOR 90d;
```

## Example: Purge all grants that have been invalid for more than a year
