...
The `KILL` statement expects the UUID of a running [live select](/docs/surrealql/statements/live_select) query to be passed. This UUID can be found in the output of the `LIVE` statement, and can thereafter be passed into a `KILL` statement once it is no longer needed.
