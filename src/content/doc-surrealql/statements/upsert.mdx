...
> [!NOTE]
> In versions of SurrealDB between 2.0.0 and 2.0.5, an UPSERT statement was treated as an "UPDATE, otherwise INSERT" statement. It has since been changed to a statement which defaults to insertion, and updates otherwise. Please see the examples below for details.
...
As the output shows, the unique index prevents the creation of a second user with the name name and class as the first.
...
To compare the performance difference between using a `WHERE` clause and a unique index yourself, here is an example that creates 20000 `user` records, after which an `UPDATE` with a `WHERE` clause and an `UPSERT` using an index is used to change the data.

```surql
DEFINE INDEX unique_key ON TABLE user FIELDS name, member_of UNIQUE;
FOR $i IN <array>0..20000 { CREATE user SET name = <string>$i, the = <string>$i; };
UPDATE user SET interests = ["music"] WHERE name = "100";
UPSERT user CONTENT { name: "100", the: "100", interests: ["music", "travel"] };
```
