...
This PR introduces a term mapping filter designed to enable lemmatization within SurrealDB.
Lemmatization is the process of reducing words to their base or dictionary form, which is particularly useful for handling irregular verbs and other terms that the default "snowball" tool cannot handle.

The mapper mechanism allows users to specify a custom dictionary file that maps terms to their base forms.
This dictionary file is then used by SurrealDB’s analyzer to standardize terms as they are indexed, improving search consistency.
...
Dictionary File Structure: The file specified in the mapper parameter should follow a specific format:

- Each line contains a pair of terms separated by a tab.
- The first term represents the canonical or base form of the word.
...
Usage: When this analyzer is applied to a text, any word that matches the mapped term in the dictionary file will be replaced by its base form before indexing.
This helps ensure consistency in search results by consolidating different forms of a word to a single, standardized entry
