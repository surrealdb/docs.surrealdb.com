...
An API endpoint can be tested using the [`api::invoke` function](/docs/surrealql/functions/database/api/#apiinvoke), which takes either the path as a single string or the path along with a request body. It can also be tested via [CURL or other means by directly using the endpoint](/docs/surrealdb/integration/http/#custom) along with the namespace and database in the headers.
