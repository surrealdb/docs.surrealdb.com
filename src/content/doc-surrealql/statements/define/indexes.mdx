...
-- Since 3.0.0-alpha.8: use FULLTEXT instead of SEARCH to benefit from concurrent full-text search
...
Since version 3.0.0-alpha.8, using `FULLTEXT ANALYZER` is the preferred method to `SEARCH ANALYZER` as `FULLTEXT` allows for more performant [concurrent full-text search](https://github.com/surrealdb/surrealdb/pull/5571), as well as the ability to [use the `OR` operator](https://github.com/surrealdb/surrealdb/pull/6179). However, the traditional `SEARCH ANALYZER` keyword remains valid.
