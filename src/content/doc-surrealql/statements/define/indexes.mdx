...
Since version 3.0.0-alpha.8, using `FULLTEXT ANALYZER` is the preferred method to `SEARCH ANALYZER` as `FULLTEXT` allows for more performant [concurrent full-text search](https://github.com/surrealdb/surrealdb/pull/5571). However, the traditional `SEARCH ANALYZER` keyword remains valid.
