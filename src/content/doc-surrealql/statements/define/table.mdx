...
COMMENT @string	
AS SELECT @projections FROM @tables [ WHERE @condition ] [ GROUP [ BY ] @groups ]
CHANGEFEED @duration [ INCLUDE ORIGINAL ]
PERMISSIONS ( NONE | FULL | FOR select @expression | FOR create @expression | FOR update @expression | FOR delete @expression )
```
