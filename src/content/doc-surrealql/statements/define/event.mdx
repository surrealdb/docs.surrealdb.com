...
Async events are processed in an interval dependant on the environment variable `SURREAL_EVENT_PROCESSING_INTERVAL` (or `--event-processing-interval` when starting the server) which is set to 5 seconds as the default. Lowering this will reduce the latency between a document change and its events, while leading to more frequent polls by the background worker.
...
The `MAXDEPTH` clause is used to set the maximum number of times that an async event can be triggered.

While the default 5 for `MAXDEPTH` can seem low, events defined on other events that lead to record creation can quickly spiral out of control even at a depth of 5.
...
While the `MAXDEPTH` in this case is only one lower than the default, the sheer number of records created results in the final `count()` score being 184528 if set to the default, compared to 20503 if lowered by one.
