...
Events can be defined on a database that are triggered after any modification to the data in a record (any operation that creates, updates, or deletes data). Each trigger is able to see the `$before` and `$after` value of the record, enabling advanced custom logic.
