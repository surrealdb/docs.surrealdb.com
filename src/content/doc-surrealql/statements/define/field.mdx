...
-- Basic syntax --
DEFINE FIELD [ OVERWRITE | IF NOT EXISTS ] @name ON [ TABLE ] @table [ @clauses ]
...
VALUE @expression
ASSERT @expression
REFERENCE [ ON DELETE REJECT | ON DELETE CASCADE | ON DELETE IGNORE | ON DELETE UNSET | ON DELETE THEN @expression ]
COMMENT @string
PERMISSIONS ( NONE | FULL | FOR select @expression | FOR create @expression | FOR update @expression )
```
