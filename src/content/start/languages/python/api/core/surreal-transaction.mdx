...
Transactions are created by calling [`.begin_transaction()`](/docs/start/languages/python/api/core/surreal-session#begin-transaction) on a session. They are **not** available on HTTP or embedded connections.
...
A transaction exposes query and CRUD methods that mirror the parent connection's interface. All operations are scoped to this transaction and are not visible outside it until committed. For full parameter tables and examples, see the [Surreal](/docs/start/languages/python/api/core/surreal) reference.

| Method | Returns | Description |
|---|---|---|
| [`.query(query, vars)`](/docs/start/languages/python/api/core/surreal#query) | [`Value`](/docs/start/languages/python/api/types/#value) | Execute a SurrealQL query within the transaction. |
...
- [SurrealSession](/docs/start/languages/python/api/core/surreal-session) — Session management reference
