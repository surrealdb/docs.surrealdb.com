...
This type is recursive: dictionaries and lists can contain nested `Value` types. See the [Data Types overview](/docs/start/languages/python/api/values/) for details on each SurrealDB-specific type.
...
When a `str` is passed, it is treated as a table name. Pass a [`RecordID`](/docs/start/languages/python/api/values/record-id) for specific record targeting, or a [`Table`](/docs/start/languages/python/api/values/table) for explicit table references.
...
- [Data Types](/docs/start/languages/python/api/values/) for the SurrealDB-specific type classes
- [Errors](/docs/start/languages/python/api/errors/) for error classes and constants
- [Surreal API reference](/docs/start/languages/python/api/core/surreal) for methods using these types
