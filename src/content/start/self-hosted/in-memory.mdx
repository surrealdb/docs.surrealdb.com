...
Since SurrealDB 3.0, in-memory storage runs on a backend called [SurrealMX](https://github.com/surrealdb/surrealmx) that also allows for [versioned queries](/docs/reference/query-language/statements/select#the-version-clause) as well as [persistent storage](/docs/reference/cli/surrealdb-cli/start#supported-parameters-for-memory-surrealmx) as a snapshot or AOL (append-only log).
