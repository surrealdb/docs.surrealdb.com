...
# Multimodal search with `jina-clip-v2` and SurrealDB
...
text_vec, image_vec = resp[0]["embedding"], resp[1]["embedding"]
```
...
```python
async def text2image(query: str, k: int = 3):
    # Embed query text
    q_vec = requests.post(
        "https://api.jina.ai/v1/embeddings",
        headers={
...
```python
async def image2text(query_image_url: str, k: int = 3):
    # Embed query image
    q_vec = requests.post(
        "https://api.jina.ai/v1/embeddings",
        headers={
