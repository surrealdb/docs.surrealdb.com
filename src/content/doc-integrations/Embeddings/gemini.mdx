...
SurrealDB exposes two ANN strategies (HNSW & M-Tree). [HNSW](https://surrealdb.com/docs/surrealdb/reference-guide/vector-search) is in-memory and fastest for RAG-style look-ups. 
...
`<|K,EF|>` is SurrealQL’s K-nearest-neighbour operator. The accompanying `vector::distance::knn()` retrieves the distance already computed by the operator. ([SurrealDB][4], [SurrealDB][5])
...
## Related resources
- [Python SDKs | Integration - SurrealDB](/docs/sdk/python)
- [Vector Search | Reference guides](/docs/surrealdb/reference-guide/vector-search)
- [Operators | SurrealQL - SurrealDB](/docs/surrealql/operators)
- [Vector functions | SurrealQL](/docs/surrealql/functions/database/vector)
- [Features | SurrealDB](/features)
