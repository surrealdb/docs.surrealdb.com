...
The following is an overview of most of the models available for Fastembed. The defaults generally seen per use case are:
...
The following example demonstrates how SurrealDB can be used to store the embeddings from the default language model for a number of phrases, after which it can be prompted to return the three closest results to a certain prompt.
...
async fn store_docs(
    ...
    db.query("INSERT INTO document $docs")
        .bind(("docs", docs))
        .await?;
    Ok(())
}
