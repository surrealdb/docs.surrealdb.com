...
# Using Nvidia Embeddings with **SurrealDB**
...
emb_resp = session.post(NVIDIA_BASE, headers=headers, json=payload).json()
vectors  = [row["embedding"] for row in emb_resp["data"]]
```
...
async def search(query: str, k: int = 3):
    ...
    q_vec = session.post(NVIDIA_BASE, headers=headers,
                         json=q_payload).json()["data"][0]["embedding"]
