...
title: SmolAgents × SurrealDB — Grocery Finder
...
* `DEFINE INDEX … HNSW` activates the **approx-nearest-neighbour** index citeturn2search1
* `<|K,EF|>` in the query performs the K-NN lookup with `K=5`, `efSearch=64` citeturn2search3
...
* SurrealDB **vector-search guide** citeturn2search1
