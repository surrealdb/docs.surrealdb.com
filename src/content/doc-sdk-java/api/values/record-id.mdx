...
A `RecordId` uniquely identifies a record in SurrealDB. It consists of a table name and an ID value. The ID can be a `long`, `String`, `UUID`, or a composite key using `Array` or `Object`. See the [SurrealQL record ID documentation](/docs/surrealql/datamodel/ids) for details on how record identifiers work in SurrealDB.
...
- [SurrealQL record IDs](/docs/surrealql/datamodel/ids) — Record identifier formats and types
