...
Additionally we will rely on an in-memory database so no external database is required.


> [!IMPORTANT]
> If you want to see the final project for this guide, you can find it in the [surrealdb-examples](https://github.com/surrealdb/examples/tree/main/java-starter) repository and follow the instructions in the `README.md` file to get the project running.

Here is the final folder structure:

```bash
my-surrealdb-project/
└─ src/me/yourname/surreal
    ├─ StartDB.java
    ├─ CreateUser.java
    ├─ QueryUser.java
    ├─ UpdateUser.java
