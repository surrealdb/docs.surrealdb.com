...
Transactions are created by calling [`.begin_transaction()`](/docs/reference/python/api/core/surreal-session#begin-transaction) on a session. They are **not** available on HTTP or embedded connections.
...
A transaction exposes query and CRUD methods that mirror the parent connection's interface. All operations are scoped to this transaction and are not visible outside it until committed. For full parameter tables and examples, see the [Surreal](/docs/reference/python/api/core/surreal) reference.
...
| [`.run(name, args, version)`](/docs/reference/python/api/core/surreal#run) | [`Value`](/docs/reference/python/api/types/#value) | Call a SurrealDB function within the transaction. |
| [`.let(key, value)`](/docs/reference/python/api/core/surreal#let) | `None` | Set a transaction-scoped variable. |
| [`.unset(key)`](/docs/reference/python/api/core/surreal#unset) | `None` | Unset a transaction-scoped variable. |
...
- [SurrealSession](/docs/reference/python/api/core/surreal-session) — Session management reference
