...
Sessions are created by calling [`.new_session()`](/docs/reference/python/api/core/surreal#new-session) on a WebSocket connection. They are **not** available on HTTP or embedded connections.
...
A session exposes the same interface as the parent connection. All methods below delegate to the underlying connection, scoped to this session's ID. For full parameter tables and examples, see the [Surreal](/docs/reference/python/api/core/surreal) reference.
...
| [`.invalidate()`](/docs/reference/python/api/core/surreal#invalidate) | `None` | Invalidate this session's authentication. |
| [`.let(key, value)`](/docs/reference/python/api/core/surreal#let) | `None` | Define a session-scoped variable. |
| [`.unset(key)`](/docs/reference/python/api/core/surreal#unset) | `None` | Remove a session-scoped variable. |
...
| [`.upsert(record, data)`](/docs/reference/python/api/core/surreal#upsert) | CRUD builder -> `dict` or `list` | Upsert a record (chain `.content/.replace/.merge/.patch`). |
| [`.delete(record)`](/docs/reference/python/api/core/surreal#delete) | CRUD builder -> `dict` or `list` | Delete records. |
| [`.insert(table, data, relation=False)`](/docs/reference/python/api/core/surreal#insert) | Insert builder -> `list[Value]` | Insert records. Pass `relation=True` or chain `.relation()` for `INSERT RELATION`. |
| [`.run(name, args, version)`](/docs/reference/python/api/core/surreal#run) | [`Value`](/docs/reference/python/api/types/#value) | Call a SurrealDB function. |
| [`.live(table, diff)`](/docs/reference/python/api/core/surreal#live) | `UUID` | Start a live query. |
| [`.kill(query_uuid)`](/docs/reference/python/api/core/surreal#kill) | `None` | Kill a live query. |
...
- [Surreal](/docs/reference/python/api/core/surreal) — Connection reference with full method documentation
- [SurrealTransaction](/docs/reference/python/api/core/surreal-transaction) — Transaction reference
- [Data types](/docs/reference/python/api/types) — Type aliases and value types
- [Errors](/docs/reference/python/api/errors) — Error classes reference
