...
<table>
	...
	<tbody>
		<tr>
			<td scope="row" data-label="Error type"><a href="/docs/reference/golang/api/errors#servererror"><code>ServerError</code></a></td>
			<td scope="row" data-label="Description">Structured error from SurrealDB v3 with kind, details, and cause chain</td>
		</tr>
		<tr>
			<td scope="row" data-label="Error type"><a href="/docs/reference/golang/api/errors#queryerror"><code>QueryError</code></a></td>
			<td scope="row" data-label="Description">Per-statement query failure returned within QueryResult</td>
		</tr>
		<tr>
			<td scope="row" data-label="Error type"><a href="/docs/reference/golang/api/errors#rpcerror"><code>RPCError</code></a></td>
...
SurrealDB v3 returns structured errors with a `Kind`, `Message`, `Details`, and an optional `Cause` chain. Use `errors.As` to extract a [`ServerError`](/docs/reference/golang/api/errors#servererror):
...
When using `Query`, individual statements can fail without causing the entire call to error. Each [`QueryResult`](/docs/reference/golang/api/types#queryresult) has an `Error` field that contains a [`QueryError`](/docs/reference/golang/api/errors#queryerror) if that statement failed.
...
The SDK defines several [sentinel errors](/docs/reference/golang/api/errors#sentinel-errors) for common failure conditions:
...
- [Errors API reference](/docs/reference/golang/api/errors) for complete error type definitions
- [Executing queries](/docs/reference/golang/concepts/executing-queries) for query-level error handling
- [Types reference](/docs/reference/golang/api/types) for `QueryResult` and `QueryError` definitions
