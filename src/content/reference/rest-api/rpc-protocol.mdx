...
This method runs a query in the same way as [`query`](#query), but answers with a sequence of frames sent while the query runs instead of one response at the end, so the first rows reach the client before the last statement has finished. Only the WebSocket `/rpc` endpoint serves it, and HTTP `POST /rpc` reports `Method not found`. The SDKs expose it as [streaming query responses](/docs/reference/javascript/concepts/executing-queries#streaming-query-responses) in JavaScript and as [stream items](/docs/reference/rust/methods/query#stream-items) in Rust.
...
Rows are provisional until their statement's `finished` frame arrives:

- A `finished` frame with an `error` retracts every row already sent for that statement.
