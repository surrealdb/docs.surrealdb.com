...
- `SURREAL_GRPC_MAX_MESSAGE_SIZE` limits every message the server receives and every message it sends. It defaults to the value of `SURREAL_HTTP_MAX_RPC_BODY_SIZE`, and the server reports it to clients as `Limits.max_message_bytes` in the `GetCapabilities` handshake, so a client knows the limit before it sends anything. The server splits query results across as many messages as they need, so the limit bounds a single request, record or live query notification rather than a whole result. A client can also set a figure of its own, which bounds the responses it accepts, as described for the [Rust SDK](/docs/reference/rust/methods/connect#connecting-over-grpc).
...
- The service's `Health` method is the gRPC health check, and the `health` [HTTP route](/docs/learn/security/authorization/capabilities#http-route-names) allows or denies it together with `GET /health`. The server does not implement the standard `grpc.health.v1.Health` service, so a probe that calls that service, such as a Kubernetes `grpc` probe, fails with `Unimplemented`. Probe [`GET /health`](/docs/reference/rest-api/http-protocol#health) or [`GET /ready`](/docs/reference/rest-api/http-protocol#ready) over HTTP instead.
- The server does not offer gRPC server reflection, so a tool such as `grpcurl` needs the `.proto` files from the [surrealdb-protocol](https://github.com/surrealdb/surrealdb-protocol) repository.
