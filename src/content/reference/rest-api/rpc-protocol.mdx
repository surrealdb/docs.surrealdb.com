...
Notice how the deleted record is returned. This differs from a [`DELETE`](/docs/reference/query-language/statements/delete) statement via the CLI or Surrealist which returns nothing unless the `RETURN BEFORE` clause is used.
