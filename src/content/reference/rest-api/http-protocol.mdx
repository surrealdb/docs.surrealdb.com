...
The GQL endpoint runs [ISO GQL](/docs/learn/querying/gql/overview) graph pattern queries against your existing tables and `RELATE` edges - `MATCH … RETURN` reads and data-modifying `INSERT`, `SET`, `REMOVE`, and `DELETE` ([GQL mutations](/docs/learn/querying/gql/mutations)).
...
> [!NOTE]
> This endpoint is **not** [ISO GQL](/docs/learn/querying/gql/overview). GQL `MATCH` queries belong on [`POST /gql`](#gql).
