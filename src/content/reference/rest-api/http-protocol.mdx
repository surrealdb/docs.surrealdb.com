...
> [!NOTE]
> This HTTP endpoint expects the HTTP body to be a JSON object (parsed as a value and bound to `$body` / `$data` in the translated statement). The body is not executed as SurrealQL; use [`/sql`](/docs/reference/rest-api/http-protocol#post-sql) or RPC when you need to run queries in the request body.
...
> [!NOTE]
> This HTTP endpoint expects the HTTP body to be a JSON object (parsed as a value, not executed as SurrealQL). Use [`/sql`](/docs/reference/rest-api/http-protocol#post-sql) or RPC to run SurrealQL in the request body.
