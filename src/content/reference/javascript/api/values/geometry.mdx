...
Geometry classes provide support for spatial and geographic data using GeoJSON-compatible structures. These types are used in location-based applications and geospatial queries.
...
Closes the line by appending the first point to the end, if it is not already closed. This is useful when constructing polygon boundaries.
