...
- `retryDelayMultiplier` - Multiply delay after each failed attempt
- `retryDelayJitter` - Random offset percentage for delays
- `retryable` - Custom predicate deciding whether an error should be retried (default: [`isRetryableConflict`](/docs/reference/javascript/api/utilities/is-retryable-conflict))
