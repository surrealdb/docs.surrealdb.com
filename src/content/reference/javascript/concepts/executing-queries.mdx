...
<table>
	...
	<tbody>
		<tr>
			<td scope="row" data-label="Method"><a href="/docs/reference/javascript/api/core/surreal-queryable#query"> <code> db.query(query, bindings?) </code></a></td>
			<td scope="row" data-label="Description">Executes raw SurrealQL statements</td>
		</tr>
		<tr>
			<td scope="row" data-label="Method"><a href="/docs/reference/javascript/api/core/surreal-queryable#select"> <code> db.select(target) </code></a></td>
			<td scope="row" data-label="Description">Selects records from the database</td>
		</tr>
		<tr>
			<td scope="row" data-label="Method"><a href="/docs/reference/javascript/api/core/surreal-queryable#create"> <code> db.create(target) </code></a></td>
			<td scope="row" data-label="Description">Creates new records</td>
		</tr>
		<tr>
			<td scope="row" data-label="Method"><a href="/docs/reference/javascript/api/core/surreal-queryable#insert"> <code> db.insert(target, data) </code></a></td>
			<td scope="row" data-label="Description">Inserts one or multiple records</td>
		</tr>
		<tr>
			<td scope="row" data-label="Method"><a href="/docs/reference/javascript/api/core/surreal-queryable#update"> <code> db.update(target) </code></a></td>
			<td scope="row" data-label="Description">Updates existing records</td>
		</tr>
		<tr>
			<td scope="row" data-label="Method"><a href="/docs/reference/javascript/api/core/surreal-queryable#upsert"> <code> db.upsert(target) </code></a></td>
			<td scope="row" data-label="Description">Inserts or replaces records</td>
		</tr>
		<tr>
			<td scope="row" data-label="Method"><a href="/docs/reference/javascript/api/core/surreal-queryable#delete"> <code> db.delete(target) </code></a></td>
			<td scope="row" data-label="Description">Deletes records from the database</td>
		</tr>
		<tr>
			<td scope="row" data-label="Method"><a href="/docs/reference/javascript/api/core/surreal-queryable#relate"> <code> db.relate(from, edge, to) </code></a></td>
			<td scope="row" data-label="Description">Creates graph relationships between records</td>
		</tr>
		<tr>
			<td scope="row" data-label="Method"><a href="/docs/reference/javascript/api/core/surreal-queryable#run"> <code> db.run(name, args?) </code></a></td>
			<td scope="row" data-label="Description">Executes a SurrealDB function or SurrealML model</td>
		</tr>
		<tr>
			<td scope="row" data-label="Method"><a href="/docs/reference/javascript/api/core/surreal-session#set"> <code> db.set(key, value) </code></a></td>
			<td scope="row" data-label="Description">Defines a parameter on the session</td>
		</tr>
		<tr>
			<td scope="row" data-label="Method"><a href="/docs/reference/javascript/api/core/surreal-session#unset"> <code> db.unset(key) </code></a></td>
...
The `.query()` method executes raw [SurrealQL statements](/docs/reference/query-language/statements/overview) against the database. You can pass bindings as a second argument to safely inject variables into the query.
...
You can also pass a [bound query](/docs/reference/javascript/concepts/bound-queries) for automatic parameterisation using the `surql` template tag.
...
The `.select()` method reads records from the database. You can pass a [`Table`](/docs/reference/javascript/api/values/table) to select all records, a [`RecordId`](/docs/reference/javascript/api/values/record-id) to select a specific record, or a `RecordIdRange` to select a range.
...
In the above example, we use the `gt()` function to create a greater than condition. This function is part of the [expression utilities](/docs/reference/javascript/api/utilities/expr) that are available in the SDK.
If you prefer to write a raw condition, you can use the `raw()` function to insert the condition directly into the query.
...
The `.relate()` method creates edges between records in SurrealDB's [graph model](/docs/reference/query-language/statements/relate). You specify the source record(s), the edge table, and the target record(s).
...
The `.run()` method executes [SurrealDB functions](/docs/reference/query-language/statements/define/function) or SurrealML models by name. You can pass arguments and optionally specify a model version.
...
On the embedded engines, `@surrealdb/node-native` and `@surrealdb/wasm-native`, results arrive in batches as the query produces them, so the first results are available before the last ones have been read. Over WebSocket and HTTP, the SDK waits for the whole response and then yields it frame by frame, so `.stream()` offers the same interface there without delivering results any earlier.

> [!NOTE]
> The embedded engines stream from SurrealDB 3.3.0. On earlier versions they held the whole result in memory and returned nothing until the last result was read, so `.stream()` gave no benefit over awaiting the query.
...
Learn more about the `jsonify` utility in the [Utilities](/docs/reference/javascript/concepts/utilities) concept page.
...
You can also set a connection-wide default with the `retry` option on [`.connect()`](/docs/reference/javascript/concepts/connecting-to-surrealdb#retrying-on-write-conflict), which `.retry()` overrides per call.

## Learn more

- [Bound queries](/docs/reference/javascript/concepts/bound-queries) for parameterised query building with `surql` and `BoundQuery`
- [Live queries](/docs/reference/javascript/concepts/live-queries) for real-time subscriptions
- [Transactions](/docs/reference/javascript/concepts/transactions) for atomic multi-query operations
- [Connecting to SurrealDB](/docs/reference/javascript/concepts/connecting-to-surrealdb#retrying-on-write-conflict) for connection-wide retry configuration
