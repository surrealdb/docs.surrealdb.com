...
Results arrive in batches as the server produces them, so the first results are available before the last ones have been read.

> [!NOTE]
> The embedded engines - `@surrealdb/node-native` and `@surrealdb/wasm-native` - stream from SurrealDB 3.3.0. On earlier versions they held the whole result in memory and returned nothing until the last result was read, so `.stream()` gave no benefit over awaiting the query. Connections over WebSocket stream on earlier versions too.
