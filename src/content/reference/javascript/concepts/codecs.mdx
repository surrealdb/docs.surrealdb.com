...
`CborCodec` outputs compact binary. Use it when you speak SurrealDB's RPC protocol or want a small, type-safe binary payload.
...
For a plain string form (good for display or simple serialisation), [`jsonify()`](/docs/reference/javascript/concepts/utilities#jsonifying-query-results) converts value instances to SurrealQL strings without the SQON JSON wrappers.
...
- [Value types](/docs/reference/javascript/concepts/value-types) for the value classes codecs operate on
- [Utilities](/docs/reference/javascript/concepts/utilities) for `jsonify()` and other SQON helpers
