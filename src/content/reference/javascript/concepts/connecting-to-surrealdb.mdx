...
<table>
	...
	<tbody>
		<tr>
			<td scope="row" data-label="Method"><a href="/docs/reference/javascript/api/core/surreal#connect"> <code> db.connect(url, options) </code></a></td>
			<td scope="row" data-label="Description">Connects to a local or remote database endpoint</td>
		</tr>
        <tr>
            <td scope="row" data-label="Method"><a href="/docs/reference/javascript/api/core/surreal#close"> <code> db.close() </code></a></td>
            <td scope="row" data-label="Description">Closes the persistent connection to the database</td>
        </tr>
        <tr>
            <td scope="row" data-label="Method"><a href="/docs/reference/javascript/api/core/surreal#ready"> <code> db.ready </code></a></td>
			<td scope="row" data-label="Description">Waits for the connection to the database to succeed</td>
		</tr>
		<tr>
			<td scope="row" data-label="Method"><a href="/docs/reference/javascript/api/core/surreal-session#use"> <code> db.use(namespace, database)</code></a></td>
...
- **Embedded** protocols using the [WebAssembly engine](/docs/reference/javascript/engines/wasm) or [Node.js engine](/docs/reference/javascript/engines/node)
...
You can directly specify the [namespace](/docs/reference/query-language/statements/define/namespace) and [database](/docs/reference/query-language/statements/define/database) to use using the `namespace` and `database` options. If you do not specify these options, the default namespace and database will be used.
Once the connection is established, you can switch the active namespace and database with the `.use()` method.

#### Authentication details

When connecting as a [system user](/docs/learn/security/authentication/users#system-users) or [token](/docs/learn/security/authentication/users#token), you can directly pass your credentials to the `authentication` option.
...
This sets the connection-wide default. It can be overridden per call with `.retry()` on a [`query()`](/docs/reference/javascript/concepts/executing-queries) or on [mutation methods](/docs/reference/javascript/concepts/executing-queries) like `.create()` and `.delete()` - including when called on a [transaction](/docs/reference/javascript/concepts/transactions), since it exposes the same query methods.
...
- **WebSocket** connections open a single long-lived stateful connection. After the initial authentication, the session duration applies. If no session duration is specified, it defaults to `NONE`, which means that the session never expires.
- **HTTP** connections are short-lived and stateless. Each request is authenticated individually with the token, which creates a short-lived session, so the token duration applies. The token duration defaults to 1 hour.

You can extend the session duration of a token or a session by setting the `DURATION` clause when creating a new access method with the [`DEFINE ACCESS METHOD`](/docs/reference/query-language/statements/define/access) statement, or when defining a new user with the [`DEFINE USER`](/docs/reference/query-language/statements/define/user) statement.
Learn more about token and session duration in our [security best practices](/docs/learn/security/best-practices/security-best-practices#expiration) documentation.
...
- [Surreal API reference](/docs/reference/javascript/api/core/surreal) for the complete connection interface
...
- [Authentication](/docs/reference/javascript/concepts/authentication) for signing in and managing credentials
- [WebAssembly engine](/docs/reference/javascript/engines/wasm) for embedded browser databases
- [Node.js engine](/docs/reference/javascript/engines/node) for embedded server-side databases
- [Error handling](/docs/reference/javascript/concepts/error-handling) for connection and reconnection errors
- [Security best practices](/docs/learn/security/best-practices/security-best-practices) for production deployments