...
SurrealDB supports multiple levels of authentication, from [system users](/docs/learn/security/authentication/authentication#system-users) to fine-grained [record-level access](/docs/learn/security/authentication/authentication#record-users). The JavaScript SDK provides methods for signing up and signing in users, managing tokens, and automatically restoring sessions on reconnect.
...
<table>
	...
	<tbody>
        <tr>
            <td scope="row" data-label="Method"><a href="/docs/reference/javascript/api/core/surreal-session#signin"> <code> db.signin(auth) </code></a></td>
            <td scope="row" data-label="Description">Signs in as a root, namespace, database, or record user</td>
		</tr>
		<tr>
			<td scope="row" data-label="Method"><a href="/docs/reference/javascript/api/core/surreal-session#signup"> <code> db.signup(auth) </code></a></td>
			<td scope="row" data-label="Description">Signs up a new record user using an access method</td>
		</tr>
        <tr>
            <td scope="row" data-label="Method"><a href="/docs/reference/javascript/api/core/surreal-session#authenticate"> <code> db.authenticate(token) </code></a></td>
            <td scope="row" data-label="Description">Authenticates the session with an existing token or token pair</td>
        </tr>
        <tr>
            <td scope="row" data-label="Method"><a href="/docs/reference/javascript/api/core/surreal-session#invalidate"> <code> db.invalidate() </code></a></td>
            <td scope="row" data-label="Description">Invalidates the current authentication, signing the user out</td>
        </tr>
        <tr>
            <td scope="row" data-label="Method"><a href="/docs/reference/javascript/api/core/surreal-session#subscribe"> <code> db.subscribe("auth", callback) </code></a></td>
...
> [!NOTE]
> When you call `.signup()` or `.signin()` manually, the `authentication` property passed to `.connect()` is no longer used for automatic re-authentication. You become responsible for handling token expiry by listening to the [`auth`](/docs/reference/javascript/api/core/surreal-session#event-auth) event.
...
The SDK exposes the current authentication tokens through the `.accessToken` property on any [`Surreal`](/docs/reference/javascript/api/core/surreal) or [`SurrealSession`](/docs/reference/javascript/api/core/surreal-session) instance. This is useful for checking whether the current session is authenticated or for forwarding tokens to other services.
...
- [SurrealSession API reference](/docs/reference/javascript/api/core/surreal-session) for authentication method signatures
...
- [Connecting to SurrealDB](/docs/reference/javascript/concepts/connecting-to-surrealdb) for connection setup and reconnection behaviour
- [Multiple sessions](/docs/reference/javascript/concepts/multiple-sessions) for isolated session management