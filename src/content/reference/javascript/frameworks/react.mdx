...
In your top-level component, wrap the root with `QueryClientProvider` and `SurrealProvider`. Pass the endpoint and any [connection options](/docs/reference/javascript/concepts/connecting-to-surrealdb#connection-options) through the `params` prop.
...
Use the `useSurrealClient()` hook to access the `Surreal` instance from any component. All [query methods](/docs/reference/javascript/concepts/executing-queries) are available on the client, including `.query()`, `.select()`, `.create()`, and more.
...
You can build an authentication layer on top of the provider using the SDK's [`.signin()`](/docs/reference/javascript/concepts/authentication#signing-in-users) and [`.signup()`](/docs/reference/javascript/concepts/authentication#signing-up-users) methods. The example below shows a minimal hook for record access authentication.
