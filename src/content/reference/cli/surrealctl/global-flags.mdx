...
Reusing the sibling's prefix would be harmful. `SURREAL_TOKEN` in a shell means a *database* JWT for one instance; reading it as a control-plane credential would send database tokens to `api.surrealdb.com` on every command.

The only `SURREAL_*` variables involved are ones `surrealctl` *sets* for the `surreal` child process during [the handoff](/docs/manage/surrealctl/install#the-surreal-handoff): `SURREAL_TOKEN`, `SURREAL_NAMESPACE` and `SURREAL_DATABASE`.
