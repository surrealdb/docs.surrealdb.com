...
`surrealctl catalog` browses the platform catalogues: the regions, instance types, versions and billing
countries SurrealDB Cloud offers. These are the slugs [`instance
create`](/docs/reference/cli/surrealctl/commands/instance#instance-create) and [`instance
update`](/docs/reference/cli/surrealctl/commands/instance#instance-update) expect.
...
These are nouns rather than `list` verbs, because a catalogue has exactly one thing you can do with it.
None takes a positional argument, none takes `--org`, and none carries any flag beyond the [list
presentation
flags](/docs/reference/cli/surrealctl/output-and-exit-codes#list-presentation-flags).

> [!IMPORTANT]
> These lists are **global**. What an organisation may actually deploy is narrower, and [`org
> plans`](/docs/reference/cli/surrealctl/commands/org#org-plans) is the organisation-scoped answer — the
> one to trust before a create.
...
This command takes no positional arguments and no options of its own. Column ids are `slug` and
`description`, with `flag` under `--wide`.
...
This command takes no positional arguments and no options of its own. Column ids are `slug`,
`category`, `cpu`, `memory`, `units`, `price_hour`, `storage` and `enabled`, with `display_name`,
`restricted` and `description` under `--wide`.
...
The `enabled` column is left unset here, because whether a type is available to you is a property of
your organisation's plan rather than of the platform — check [`org
plans`](/docs/reference/cli/surrealctl/commands/org#org-plans), or price a specific combination with
[`instance estimate`](/docs/reference/cli/surrealctl/commands/instance#instance-estimate).
...
This command takes no positional arguments and no options of its own. The columns are the same as
[`instance-types`](#catalog-instance-types).
...
This command takes no positional arguments and no options of its own. The single column id is
`version`.
...
The API's own order is newest-first and is left alone. `--sort version` sorts as strings, so `3.10.0`
would come before `3.9.0` — prefer the default order when you want the newest release.
...
- [`instance create`](/docs/reference/cli/surrealctl/commands/instance#instance-create) — where these
  slugs are used
- [`org plans`](/docs/reference/cli/surrealctl/commands/org#org-plans) — the organisation-scoped subset
- [`instance estimate`](/docs/reference/cli/surrealctl/commands/instance#instance-estimate) — pricing a
  type and region before committing to it
