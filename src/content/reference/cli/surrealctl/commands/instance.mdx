...
```bash title="Move to a larger instance type"
surrealctl instance update production --type production-4
```

A type change also resets the storage cool-off, so passing `--type` and `--storage` together fails at the storage step, in which the type is changed and the storage request is refused. The [Storage limits on Start](/docs/manage/instances/scaling#storage-limits-on-start) page lists the storage each type starts with.
...
`--namespace` and `--database` are optional here, because `surreal sql` accepts a session without them. See [the `surreal` handoff](/docs/manage/surrealctl/install#the-surreal-handoff) for how the binary is located and what is passed in the child's environment.
...
- [Installation](/docs/manage/surrealctl/install#the-surreal-handoff) - how `sql`, `import` and `export` find the `surreal` binary
