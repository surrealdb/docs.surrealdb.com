...
This command takes no positional argument, deliberately: a positional here would read as an *instance*. The organisation comes from `--org` and the [precedence chain](/docs/reference/cli/surrealctl/global-flags#the-precedence-chain).

Accepts the [list presentation flags](/docs/reference/cli/surrealctl/output-and-exit-codes#list-presentation-flags) and nothing else. Column ids are `name`, `state`, `type`, `version`, `region`, `compute_units` and `storage`, with `id`, `slug`, `host`, `access_type` and `organization_id` under `--wide`.
...
> [!WARNING] The create route accepts no idempotency key, so a retried create is a second instance and a second bill. `surrealctl` never retries it after a `502` for exactly that reason. If a create times out, run [`instance list`](#instance-list) before running it again.
...
> [!IMPORTANT] Every one of these restarts the instance. A single `instance update` that changes three fields restarts it once per field, in the order above.
...
> [!NOTE] This is the one command where `--json` writes to stdout continuously. The transitions *are* the answer, so the newline-delimited stream goes to stdout and nothing else is emitted. In text mode the run ends with a fresh detail view, except under `--until deleted`, where there is nothing left to fetch.
...
Also accepts the [list presentation flags](/docs/reference/cli/surrealctl/output-and-exit-codes#list-presentation-flags), which arrange the snapshot table. Column ids are `snapshot_id`, `started_at`, `tiers` and `on_demand`, with `valid_versions` under `--wide`.
...
This is a *database* token — the credential a SurrealDB client authenticates with. It is not a [personal access token](/docs/reference/cli/surrealctl/commands/token), which authenticates against the control plane.
...
Also accepts the [list presentation flags](/docs/reference/cli/surrealctl/output-and-exit-codes#list-presentation-flags). Column ids are the JWK member names — `kid`, `kty`, `alg` and `use`, with `crv` under `--wide` — so a member the key type does not carry renders as missing rather than empty.
...
This command does not speak SurrealQL. It resolves the instance, mints a database token, and becomes [`surreal sql`](/docs/reference/cli/surrealdb-cli/commands/sql). Everything after `--` is forwarded to that command verbatim, because the sibling's flag surface is large and moves independently.
...
`--namespace` and `--database` are optional here, because `surreal sql` accepts a session without them. See [the `surreal` handoff](/docs/reference/cli/surrealctl/install#the-surreal-handoff) for how the binary is located and what is passed in the child's environment.
...
See [`surreal import`](/docs/reference/cli/surrealdb-cli/commands/import) for the flags available after the `--`.
...
Like `import`, this refuses a missing namespace or database before minting anything. See [`surreal export`](/docs/reference/cli/surrealdb-cli/commands/export) for the flags available after the `--`.
...
Also accepts the [list presentation flags](/docs/reference/cli/surrealctl/output-and-exit-codes#list-presentation-flags). Summary rows carry `series`, `last`, `min`, `mean`, `max` and `unit`, with `samples` and `gaps` under `--wide`; sample rows carry `timestamp`, `series` and `value`, with `unit` under `--wide`.
...
Also accepts the [list presentation flags](/docs/reference/cli/surrealctl/output-and-exit-codes#list-presentation-flags). Column ids are `timestamp`, `level` and `message`, with `pod` under `--wide`.
...
Accepts the [list presentation flags](/docs/reference/cli/surrealctl/output-and-exit-codes#list-presentation-flags) and nothing else — this route takes no query parameters at all. Column ids are `instance_id`, `metric_type`, `compute_hours`, `disk_used_bytes`, `period_start` and `period_end`, with `instance_type` and `source` under `--wide`.

```bash
surrealctl instance usage production
```

For every instance in the organisation at once, use [`org usage`](/docs/reference/cli/surrealctl/commands/org#org-usage).
...
Accepts the [list presentation flags](/docs/reference/cli/surrealctl/output-and-exit-codes#list-presentation-flags) and nothing else. Column ids are `snapshot_id`, `started_at`, `tiers` and `on_demand`, with `valid_versions` under `--wide`.
...
- [Long-running operations](/docs/reference/cli/surrealctl/long-running-operations) — `--wait`, `--no-wait`, polling and exit code `10`
- [`org` commands](/docs/reference/cli/surrealctl/commands/org) — the organisation that owns these instances
- [`catalog` commands](/docs/reference/cli/surrealctl/commands/catalog) — the type, region and version slugs `create` and `update` expect
- [Installation](/docs/reference/cli/surrealctl/install#the-surreal-handoff) — how `sql`, `import` and `export` find the `surreal` binary
- [Overview](/docs/reference/cli/surrealctl/overview) — the rest of the reference
- [SurrealDB CLI](/docs/reference/cli/surrealdb-cli/overview) — the data plane: `surreal sql`, `surreal import`, `surreal export`
