...
`surrealctl config` reads and writes the CLI's own configuration file. Nothing here sends a request: the
...
Every verb works on **one profile** — whichever `--profile`, `SURREALCTL_PROFILE` and the file's own
`active_profile` settle on — and each one says which profile it touched.

## The keys

Five keys are recognised. Each is outranked by an environment variable, and by a command-line flag
above that; see the [precedence
...
The persisted context — what [`org
use`](/docs/reference/cli/surrealctl/commands/org#org-use) records — is **not** reachable from here.
There is no `config set context.org`. The two live in separate tables of the same file so that `org use`
never overwrites an `org` value you set by hand, and the hand-written one wins.
...
Keys this build has never heard of are preserved on write, so an older binary editing a newer file does
...
`key`, `value`, `set` and `description`, with `kind` under `--wide`.
...
Every key is listed whether or not it is set, and the `set` column is what distinguishes a configured
value from a default. Only a key this profile has actually set carries a value, so an unset key —
`api` above, which falls back to the built-in default — has an empty `value` cell. A note on stderr
reports the profile and the file path.
...
The output is the value alone plus a newline, with no label and no styling, so `$(…)` captures exactly
the value. An unset key writes **zero bytes**, so `[ -z "$(surrealctl config get org)" ]` and `wc -l`
...
**Warnings.** When the corresponding environment variable is exported, the command says so, because the
variable silently outranks what you just wrote:

```text
warning: SURREALCTL_JSON is set and takes precedence over this. Unset it for the configured value to apply.
```

**Refusals**, exit `2`: an unrecognised key, or a value the key's type rejects — a non-boolean for
`json`, or a URL with a path for `api`.
...
Clearing a key restores the CLI's default rather than falling back to another profile. Clearing an
already-unset key is a no-op that succeeds.
...
`credentials.json` lives beside it in the same directory. See [credential
storage](/docs/reference/cli/surrealctl/authentication#where-credentials-are-stored) for how that
directory is chosen.
...
The file is opened with `$EDITOR`. Editing it by hand is supported — [`config list`](#config-list) will
show whatever you wrote, and unknown keys survive — but a file written by a newer `surrealctl` is
...
- [Global flags](/docs/reference/cli/surrealctl/global-flags) — the flags and environment variables these
  keys sit beneath
- [`context` commands](/docs/reference/cli/surrealctl/commands/context) — switching between profiles
- [`org use`](/docs/reference/cli/surrealctl/commands/org#org-use) — remembering an organisation within a
  profile
- [Authentication](/docs/reference/cli/surrealctl/authentication#where-credentials-are-stored) — the
  credential file beside this one
