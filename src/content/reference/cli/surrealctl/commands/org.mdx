...
`surrealctl org` manages organisations. An organisation owns instances, members and billing, and almost every other command needs to know which one you mean - so this group is usually the second one you reach for after [`auth`](/docs/reference/cli/surrealctl/commands/auth).
...
Personal access tokens are read-only here: the mutating verbs answer 403, exit `4`. See [Authentication](/docs/reference/cli/surrealctl/authentication#what-a-personal-access-token-cannot-do).
...
Also accepts the [list presentation flags](/docs/reference/cli/surrealctl/output-and-exit-codes#list-presentation-flags). Column ids are `name`, `id`, `plan`, `state`, `role` and `members`, with `billing_provider`, `max_free`, `max_paid`, `privatelink`, `locked` and `archived_at` shown only under `--wide`.
...
The choice is written to the `[profile.<name>.context]` table of `config.toml`, which sits below a hand-written `org` key in the [precedence chain](/docs/reference/cli/surrealctl/global-flags#the-precedence-chain) - so setting `org` yourself is never overwritten by this command.
...
`org use` remembers an organisation *within* a profile. [`context use`](/docs/reference/cli/surrealctl/commands/context#context-use) switches the whole profile.
...
Also accepts the [list presentation flags](/docs/reference/cli/surrealctl/output-and-exit-codes#list-presentation-flags). Column ids are `name` and `permissions`.

```bash
surrealctl org roles
```

The role vocabulary is per organisation, which is why [`team invite`](/docs/reference/cli/surrealctl/commands/team#team-invite) and [`team update`](/docs/reference/cli/surrealctl/commands/team#team-update) point here rather than validating a `--role` value locally.
...
Also accepts the [list presentation flags](/docs/reference/cli/surrealctl/output-and-exit-codes#list-presentation-flags). Column ids are `resource` and `action`.

```bash
surrealctl org permissions --org acme
```

This is the authorisation model to consult when a command answers 403. [`auth scopes`](/docs/reference/cli/surrealctl/commands/auth#auth-scopes) reports the credential's own scopes, which are a different thing.
...
Also accepts the [list presentation flags](/docs/reference/cli/surrealctl/output-and-exit-codes#list-presentation-flags). Column ids are `instance_id`, `metric_type`, `compute_hours`, `disk_used_bytes`, `period_start` and `period_end`, with `instance_type` and `source` under `--wide`.

```bash title="Compute hours per instance, largest first"
surrealctl org usage --sort compute_hours --reverse
```

For one instance rather than all of them, use [`instance usage`](/docs/reference/cli/surrealctl/commands/instance#instance-usage).
...
Also accepts the [list presentation flags](/docs/reference/cli/surrealctl/output-and-exit-codes#list-presentation-flags). Column ids are `description`, `resource`, `amount`, `units` and `effective_at`, with `instance_id`, `instance_type` and `cloud_usage_units` under `--wide`.
...
Also accepts the [list presentation flags](/docs/reference/cli/surrealctl/output-and-exit-codes#list-presentation-flags). Column ids are `name`, `id`, `regions` and `instance_types`, with `description` under `--wide`.

```bash
surrealctl org plans
```

This is the organisation-scoped answer, and the one to trust before an [`instance create`](/docs/reference/cli/surrealctl/commands/instance#instance-create). The [`catalog`](/docs/reference/cli/surrealctl/commands/catalog) commands list what the platform offers globally, which is always the wider set.

## Related pages

- [`instance` commands](/docs/reference/cli/surrealctl/commands/instance) - the instances an organisation owns
- [`team` commands](/docs/reference/cli/surrealctl/commands/team) - its members
- [`catalog` commands](/docs/reference/cli/surrealctl/commands/catalog) - platform-wide regions, types and versions
