...
`Nothing was archived.` and exits `0`.

An organisation that is already archived is a no-op that exits `0`. It says so and stops before the
archive request:
...
The document is still emitted, so `org archive <name> --json` answers with the organisation in both
cases rather than changing shape with remote state the caller has not seen.
