...
> [!NOTE] [`auth scopes`](/docs/reference/cli/surrealctl/commands/auth#auth-scopes) is the one list-shaped command that carries only `--no-header`. It has no `--columns`, `--wide`, `--sort`, `--reverse` or `--limit`.
