...
`not_available` is HTTP 501, and it means nothing is broken - the feature is simply not enabled here. Every [SurrealDB Agent Memory](/docs/reference/cli/surrealctl/commands/spectron) route answers this way on a deployment without it.
...
`10` has its own code because *still running* is a different answer from *failed*, and a pipeline may reasonably poll again rather than roll back. See [Long-running operations](/docs/reference/cli/surrealctl/long-running-operations).
