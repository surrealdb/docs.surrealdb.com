...
<table>
    ...
    <tbody>
        ...
        <tr>
            <td><a href="/docs/learn/querying/gql/overview">GQL</a> (<code>POST /gql</code>)</td>
            <td><code>gql</code></td>
        </tr>
        <tr>
...
> [!NOTE]
> `mutable_permissions` is the exception to the rule above: from 3.3.0 a server enables it by default, so you do not pass the tag to turn it on. Name it in `--deny-experimental` or `SURREAL_CAPS_DENY_EXPERIMENTAL` to turn it off. Embedded engines still have it off by default and must allow it explicitly.
