...
In the `surreal start` command, the path argument is used to specify the location of the database. If no argument is given, the default of `memory` for storage [in memory](/docs/self-hosted/in-memory) is assumed. 

When using a path argument with SurrealDB, we recommend not using the `file://` prefix. Instead, please use [`surrealkv://`](/docs/architecture) or [`rocksdb://`](/docs/self-hosted/file-backed) as the path argument. If you are using the `file://` prefix, the database will start with the following warning:
