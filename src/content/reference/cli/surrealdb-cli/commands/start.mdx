...
The memory backend does not support versioned queries. From 3.3.0 it refuses the `versioned` and `retention` parameters, and the matching `SURREAL_DATASTORE_VERSIONED` and `SURREAL_DATASTORE_RETENTION` environment variables, so a server given either one fails to start. To use the [`VERSION` clause](/docs/reference/query-language/statements/select#the-version-clause), start the server on SurrealKV or RocksDB instead. <Since v="v3.3.0" />
