...
> [!NOTE]
> From **3.3.0**, [ISO GQL (Cypher Query Language)](/docs/learn/querying/gql/overview) is enabled by default and no longer uses an experimental capability. The legacy tag `gql` is still accepted for compatibility but has no effect. On **3.2.x**, use `--allow-experimental gql`.
...
Database:
      ...
      --query-timeout <QUERY_TIMEOUT>
          The maximum duration that a batch of statements can run for
