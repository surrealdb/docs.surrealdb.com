...
> Make sure you’ve [installed SurrealDB](/docs/running/installation) — it should only take a second!
...
<table>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>
            Sets the hostname or IP address to listen for <a href="/docs/reference/rest-api/postgres-protocol">Postgres wire protocol</a> connections on (for example <code>127.0.0.1:5432</code>). The listener is disabled when omitted.
            </td>
        </tr>
        <tr>
            <td colspan="2">
        ...
        <tr>
            ...
            <td>
                Persist client-attached HTTP RPC sessions in the datastore (off by default). See <a href="#durable-http-rpc-sessions">Durable HTTP RPC sessions</a>.
            </td>
        </tr>
        <tr>
            <td colspan="2">
            ...
            <td>
                Idle lifetime of a persisted HTTP RPC session before expiry (default <code>24h</code>)
            </td>
        </tr>
        <tr>
            <td colspan="2">
                `--durable-session-gc-interval`
               <label label="optional" />
            </td>
            <td>
                Interval for purging expired durable HTTP RPC sessions (default <code>60s</code>; <code>0</code> disables the sweep)
            </td>
        </tr>
    </tbody>
</table>
...
While SurrealKV supports historical/temporal querying using the `VERSION` clause when [selecting](/docs/reference/query-language/statements/select#the-version-clause) or [creating](/docs/reference/query-language/statements/create#version) data, you must explicitly opt in to this using the `surrealkv+versioned://` prefix in the path argument.
...
> [!NOTE]
> If using SurrealDB Cloud, capabilities can be set using the [Configure Instance](/docs/build/deployment/surrealdb-cloud/getting-started/create-an-instance#configure-an-instance) panel on SurrealDB Studio.
...
<table>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td><code>mutable_permissions</code></td>
        </tr>
    </tbody>
</table>
...
HTTP server:
      ...
      --client-ip <CLIENT_IP>
          ...
          - Forwarded:        RFC 7239 Forwarded header (<code>for=</code>) <Since v="v3.1.0" />
