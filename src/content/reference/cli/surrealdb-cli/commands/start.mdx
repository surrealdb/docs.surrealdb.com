...
surreal start --user root --pass secret "rocksdb://path/to/rocksdb?versioned=true&retention=30d"
surreal start --user root --pass secret "mem://tmp/data?aol=sync&snapshot=60s&sync=5s"
```
...
The memory backend does not support versioned queries. From 3.3.0 a server started on it with `versioned=true` or a non-zero `retention`, whether through the parameters or the matching `SURREAL_DATASTORE_VERSIONED` and `SURREAL_DATASTORE_RETENTION` environment variables, fails to start. `versioned=false` and a zero retention are accepted. To use the [`VERSION` clause](/docs/reference/query-language/statements/select#the-version-clause), start the server on SurrealKV or RocksDB instead. <Since v="v3.3.0" />
...
Database:
      ...
      --graph-fold-interval <GRAPH_FOLD_INTERVAL>
          The interval at which to fold graph adjacency keys into packed blocks

          [env: SURREAL_GRAPH_FOLD_INTERVAL=]
          [default: 5s]
