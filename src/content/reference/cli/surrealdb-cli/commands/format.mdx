...
> [!NOTE]
> `surreal format` arrived in 3.3.0 and is still relatively new. [`surqlfmt`](/docs/reference/cli/formatter/overview) is the more established of the two formatters and the one to prefer, particularly across a whole tree of files or in CI. Try `surreal format` where having the formatter inside the `surreal` binary is the convenience you are after, and [tell us how it goes](https://github.com/surrealdb/surrealdb/issues) - feedback on it is welcome while it settles.
...
- **Single-line comments** keep their text and are rewritten with a leading `#`, whichever of `#`, `--` or `//` the source used. Block comments written as `/* ... */` pass through unchanged. A statement that contains a comment is printed verbatim rather than reformatted, because the syntax tree the formatter rebuilds each statement from carries no comment nodes. A file that mixes comments into its statements therefore comes back partly unformatted.
...
Opt for `surreal format` for a single file on a machine that already has the `surreal` binary. Otherwise `surqlfmt` is the preferred default: it takes paths and glob patterns, checks formatting without rewriting anything, and exposes the style options this command has none of.
