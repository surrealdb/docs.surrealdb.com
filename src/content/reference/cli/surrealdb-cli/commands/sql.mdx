...
For example, to use [Surrealism](/docs/learn/extensions/plugins/overview) extensions, set the `SURREAL_CAPS_ALLOW_EXPERIMENTAL` environment variable to `surrealism`.
...
> [!NOTE]
> GQL is served by a running database instance. Enable `opengql` also on the server with [`surreal start`](/docs/reference/cli/surrealdb-cli/commands/start#experimental-capabilities), not only on the `surreal sql` client.
