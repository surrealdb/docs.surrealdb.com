...
> [!NOTE]
> GQL is served by a running database instance. Enable `gql` also on the server with [`surreal start`](/docs/reference/cli/surrealdb-cli/commands/start#experimental-capabilities), not only on the `surreal sql` client.
