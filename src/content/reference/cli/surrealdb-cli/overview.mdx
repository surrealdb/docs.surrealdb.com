...
> [!IMPORTANT]
> Before using the CLI, you will need to [install SurrealDB](/docs/self-hosted/installation). To experiment with SurrealDB before installing, see the [Surrealist sandbox](https://app.surrealdb.com/) online. To persist your Sandbox sdata while still experimenting, click on **Deploy to Cloud** in Surrealist to create a free SurrealDB Cloud instance.
...
Unless you specify otherwise, the CLI will start a database [in memory](/docs/self-hosted/in-memory) that serves at `127.0.0.1:8000` (or `http://localhost:8000`). This database has a single root user named `root` and a password `secret`.
