...
Unless you specify otherwise, the CLI will start a database [in memory](/docs/surrealdb/installation/running/memory) that serves at `127.0.0.1:8000` (or `http://localhost:8000`). This database has a single root user named `root` and a password `secret`.
