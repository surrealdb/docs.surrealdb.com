...
Resource limits for experimental [ISO GQL (OpenGQL)](/docs/learn/querying/gql/overview) `MATCH` execution. Errors name the knob when a limit is exceeded. The language can be enabled by passing in the [`--allow-experimental=opengql`](/docs/reference/cli/surrealdb-cli/commands/start#experimental-capabilities) flag when starting the database.
