...
* **SurrealDB Cloud environment variables**: environment variables that are set via the [Configure instance](/docs/build/deployment/surrealdb-cloud/getting-started/create-an-instance#configure-an-instance) sidebar for a SurrealDB Cloud instance.
...
Resource limits for experimental [ISO GQL](/docs/learn/querying/gql/overview) `MATCH` execution. Errors name the limit when it is exceeded. The language can be enabled by passing in the [`--allow-experimental gql`](/docs/reference/cli/surrealdb-cli/commands/start#experimental-capabilities) flag when starting the database.
...
Instances on SurrealDB Cloud are not started with a CLI command or environment variables. Instead, they can be set on the [Configure Instance](/docs/build/deployment/surrealdb-cloud/getting-started/create-an-instance#configure-an-instance) panel.