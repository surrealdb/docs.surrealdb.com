...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td scope="row" data-label="Notes">How many index entries one range branch of a bitmap plan, which combines several indexes into a set of candidate records, may read before that branch is abandoned. The abandoned branch's condition is then checked as an ordinary <code>WHERE</code> filter, so an unselective range cannot make the bitmap plan slower than scanning. The same budget bounds the allow-list built for a pre-filtered vector search (see <a href="#vector-search-config">Vector search config</a>).</td>
    </tr>
    <tr>
