...
<table>
  ...
  <tbody>
    <tr>
      <td scope="row" data-label="Env var"><code>SURREAL_METRICS_ENABLED</code><Since v="v3.1.0" /></td>
      <td scope="row" data-label="Default">true</td>
      <td scope="row" data-label="Allowed values">true, false</td>
      <td scope="row" data-label="Notes">When 'false', the Prometheus 'GET /metrics' route is not mounted (responses return 404) and the primary labelled execution observer is not attached. When 'true', unauthenticated scrapes receive an allowed subset of aggregate families, while the root account may scrape the full metric surface for that process.</td>
    </tr>
    <tr>
      <td scope="row" data-label="Env var"><code>SURREAL_PROCESS_METRICS_REFRESH_INTERVAL</code><Since v="v3.1.0" /></td>
      <td scope="row" data-label="Default">5</td>
      <td scope="row" data-label="Allowed values">A u64 (seconds)</td>
      <td scope="row" data-label="Notes">Cadence for refreshing the in-process snapshot that backs 'surrealdb.process.memory' and 'surrealdb.process.cpu_percent'. The background task runs only when at least one metrics reader is configured. Values shorter than 1 second are treated as one second.</td>
    </tr>
    <tr>
      <td scope="row" data-label="Env var"><code>SURREAL_TELEMETRY_PROVIDER</code></td>
      <td scope="row" data-label="Default">none</td>
      <td scope="row" data-label="Allowed values">otlp</td>
      <td scope="row" data-label="Notes">Set to 'otlp' to enable the OTLP push pipeline (metrics unless disabled with 'SURREAL_TELEMETRY_DISABLE_METRICS', logs, traces) using standard OpenTelemetry exporter environment variables.</td>
    </tr>
    <tr>
      <td scope="row" data-label="Env var"><code>SURREAL_TELEMETRY_DISABLE_METRICS</code><Since v="v2.1.3" /></td>
      <td scope="row" data-label="Default">false</td>
      <td scope="row" data-label="Allowed values">true, false</td>
      <td scope="row" data-label="Notes">When 'true', disables the OpenTelemetry metrics periodic reader (no OTLP metric export). Independent of SURREAL_METRICS_ENABLED, which controls the Prometheus '/metrics' route and whether the primary labelled execution observer is attached.</td>
    </tr>
    <tr>
      <td scope="row" data-label="Env var"><code>SURREAL_TELEMETRY_DISABLE_TRACING</code><Since v="v2.1.3" /></td>
      <td scope="row" data-label="Default">false</td>
      <td scope="row" data-label="Allowed values">true, false</td>
      <td scope="row" data-label="Notes">Whether to disable sending traces to the OTLP collector.</td>
    </tr>
    <tr>
      <td scope="row" data-label="Env var"><code>SURREAL_TELEMETRY_NAMESPACE</code></td>
      <td scope="row" data-label="Default">(ignored)</td>
      <td scope="row" data-label="Allowed values">A string</td>
      <td scope="row" data-label="Notes">Deprecated: still read from the environment but no longer applied. If set to a non-empty value, the server logs a startup warning.</td>
    </tr>
    <tr>
      <td scope="row" data-label="Env var"><code>SURREAL_TELEMETRY_RPC_LIVE_ID</code></td>
      <td scope="row" data-label="Default">(ignored)</td>
      <td scope="row" data-label="Allowed values">A string</td>
      <td scope="row" data-label="Notes">Deprecated: still read from the environment but no longer applied. If set to a non-empty value, the server logs a startup warning.</td>
    </tr>
