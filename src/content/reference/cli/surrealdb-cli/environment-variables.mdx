...
* **SurrealDB Cloud environment variables**: environment variables that are set via the [Configure instance](/docs/manage/instances/configure) sidebar for a SurrealDB Cloud instance.

> [!IMPORTANT]
> Every variable on this page belongs to the `surreal` binary and starts with `SURREAL_`. The [`surrealctl`](/docs/reference/cli/surrealctl/overview) control-plane tool reads `SURREALCTL_*` variables instead, and the two prefixes are kept apart deliberately: a database credential such as `SURREAL_TOKEN` is not a control-plane credential, and must not be sent to the SurrealDB Cloud API.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td scope="row" data-label="Env var"><code>SURREAL_TABLE_DOC_IDS_BATCH_SIZE</code><Since v="v3.3.0" /></td>
      <td scope="row" data-label="Default">1000</td>
      <td scope="row" data-label="Allowed values">A u32</td>
      <td scope="row" data-label="Notes">The number of document ids allocated at once from a table's shared document id sequence, which full-text, HNSW and DiskANN indexes use. Larger batches reduce coordination on the distributed sequence, at the cost of larger gaps in the ids when a node is lost before it uses up its current batch. Replaces <code>SURREAL_FTS_DOC_IDS_BATCH_SIZE</code>, which 3.3.0 no longer reads.</td>
    </tr>
    <tr>
      <td scope="row" data-label="Env var"><code>SURREAL_GRAPH_FOLD_BATCH</code><Since v="v3.3.0" /></td>
      <td scope="row" data-label="Default">1024</td>
      <td scope="row" data-label="Allowed values">A usize</td>
      <td scope="row" data-label="Notes">How many graph adjacency keys one background fold pass packs into blocks per transaction. This bounds the transaction size on a vertex with many edges, which is then folded over several passes. See <code>SURREAL_GRAPH_FOLD_INTERVAL</code>.</td>
    </tr>
  </tbody>
</table>
...
<tr>
      ...
      <td scope="row" data-label="Default">512 (10,000 before 3.1.0)</td>
      <td scope="row" data-label="Allowed values">A usize</td>
...
<tr>
    ...
    <tr>
      <td scope="row" data-label="Env var"><code>SURREAL_GRAPH_RESOLVE_CACHE_SIZE</code><Since v="v3.3.0" /></td>
      <td scope="row" data-label="Default">67,108,864 (64 MiB)</td>
      <td scope="row" data-label="Allowed values">A u64</td>
      <td scope="row" data-label="Notes">The maximum total size, in bytes, of the cache that maps document ids back to record keys when a traversal reads numeric-encoded graph adjacency entries. Only relevant once <code>SURREAL_GRAPH_NUMERIC_IDS</code> has written numeric blocks.</td>
    </tr>
  </tbody>
</table>
...
Server-side filesystem access for features that read paths from disk (notably the `mapper()` filter and the `segment()` tokenizer on [`DEFINE ANALYZER`](/docs/reference/query-language/statements/define/analyzer)). This is separate from `SURREAL_BUCKET_FOLDER_ALLOWLIST` (below), which gates the experimental [files](/docs/learn/schema-management/files/buckets) feature.
...
<table>
  ...
  <tr>
      ...
      <td scope="row" data-label="Notes">Directories the server may read when an analyzer uses <code>mapper('&lt;path&gt;')</code>. An empty or unset value denies every path. Each mapper file must resolve under one of the listed directories. See <a href="/docs/reference/query-language/statements/define/analyzer#mapperpath">DEFINE ANALYZER - mapper</a>.</td>
    </tr>
  <tr>
      <td scope="row" data-label="Env var"><code>SURREAL_SEGMENT_DICTIONARY_PATH</code><Since v="v3.3.0" /></td>
      ...
      <td scope="row" data-label="Notes">Directory holding the morphological dictionaries the <code>segment(&lt;language&gt;)</code> tokenizer reads, one subdirectory per language: <code>chinese</code>, <code>japanese</code>, <code>korean</code>. The released binaries already carry all three, so this is only needed to replace them with a custom or updated dictionary, or for a build made without them. Each language is overridden only if the directory holds a dictionary for it; the rest keep using the built-in one. See <a href="/docs/reference/query-language/statements/define/analyzer#segmentlanguage">DEFINE ANALYZER - segment</a>.</td>
    </tr>
  <tr>
...
<table>
  ...
  <tr>
    ...
    <tr>
      ...
      <td scope="row" data-label="Default">1000</td>
      <td scope="row" data-label="Allowed values">A usize</td>
      <td scope="row" data-label="Notes">The maximum number of idle HTTP connections to maintain. Has no effect: earlier versions read the value without applying it to the HTTP client, and 3.3.0 no longer reads it. Use <code>SURREAL_MAX_HTTP_IDLE_CONNECTIONS_PER_HOST</code> instead.</td>
    </tr>
<tr>
    ...
    <tr>
      <td scope="row" data-label="Env var"><code>SURREAL_SURREALDB_USER_AGENT</code></td>
      <td scope="row" data-label="Default">SurrealDB</td>
      <td scope="row" data-label="Allowed values">A string</td>
      <td scope="row" data-label="Notes">The USER-AGENT string used by HTTP requests. Before 3.1.0 this variable was named <code>SURREAL_USER_AGENT</code>, which later versions ignore.</td>
    </tr>
  </tbody>
</table>
...
<tr>
    ...
    <tr>
      <td scope="row" data-label="Env var"><code>SURREAL_HTTP_MAX_MCP_BODY_SIZE</code><Since v="v3.1.0" /></td>
      <td scope="row" data-label="Default">4,194,304 (4 MiB)</td>
      <td scope="row" data-label="Allowed values">A usize</td>
      <td scope="row" data-label="Notes">Maximum HTTP body size of the HTTP <code>/mcp</code> endpoint. See [Embedded MCP](/docs/build/ai-agents/mcp/embedded).</td>
    </tr>
    <tr>
    ...
    <tr>
      <td scope="row" data-label="Env var"><code>SURREAL_HTTP_MAX_GQL_BODY_SIZE</code><Since v="v3.2.0" /></td>
      <td scope="row" data-label="Default">1,048,576 (1 MiB)</td>
      <td scope="row" data-label="Allowed values">A usize</td>
      <td scope="row" data-label="Notes">Maximum HTTP body size of the HTTP <code>/gql</code> endpoint. See <a href="/docs/learn/querying/gql/via-http">GQL via HTTP</a>.</td>
    </tr>
    <tr>
    ...
    <tr>
      <td scope="row" data-label="Env var"><code>SURREAL_GRPC_MAX_MESSAGE_SIZE</code></td>
      <td scope="row" data-label="Default">The value of <code>SURREAL_HTTP_MAX_RPC_BODY_SIZE</code>, so 4,194,304 (4 MiB) unless that is raised</td>
      ...
      <td scope="row" data-label="Notes">From 3.3.0. The largest single gRPC message the server will accept or emit, also reported to clients as <code>Limits.max_message_bytes</code>. This is the gRPC counterpart of <code>SURREAL_WEBSOCKET_MAX_MESSAGE_SIZE</code>: raise it for a transaction that writes a large batch in one request.</td>
    </tr>
    <tr>
      <td scope="row" data-label="Env var"><code>SURREAL_GRPC_MAX_ATTACHED_SESSIONS</code><Since v="v3.3.0" /></td>
      <td scope="row" data-label="Default">16384</td>
      <td scope="row" data-label="Allowed values">A usize</td>
      <td scope="row" data-label="Notes">The maximum number of explicitly attached sessions the gRPC transport keeps at once. The count is for the whole process rather than per connection, because a gRPC channel reconnects transparently and one session's requests can arrive on different connections. Per-request sessions do not count towards it.</td>
    </tr>
    <tr>
      <td scope="row" data-label="Env var"><code>SURREAL_GRPC_NOTIFICATION_BUFFER</code><Since v="v3.3.0" /></td>
      <td scope="row" data-label="Default">1024</td>
      <td scope="row" data-label="Allowed values">A usize</td>
      <td scope="row" data-label="Notes">How many live query notifications can be buffered for a gRPC subscription whose client is not keeping up. When the buffer is full, the server ends that subscription with a <code>RESOURCE_EXHAUSTED</code> status instead of waiting.</td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td scope="row" data-label="Default">4,294,967,296 (4 GiB)</td>
      <td scope="row" data-label="Allowed values">A usize</td>
      <td scope="row" data-label="Notes">Maximum HTTP body size of the HTTP <code>/import</code> endpoint. Counted cumulatively across the whole request, not per chunk. Also enforced on the gRPC import stream. See [Request size limits](/docs/reference/rest-api/http-protocol#request-size-limits).</td>
    </tr>
    <tr>
...
Used by the built-in [Model Context Protocol](/docs/build/ai-agents/mcp/embedded) server (`/mcp` on `surreal start`, `surreal mcp` on stdio). Stdio namespace/database selection uses `SURREAL_MCP_NS` and `SURREAL_MCP_DB` on the [`mcp`](/docs/reference/cli/surrealdb-cli/commands/mcp) subcommand.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td scope="row" data-label="Env var"><code>SURREAL_MCP_ALLOWED_HOSTS</code> <Since v="v3.2.1" /></td>
    ...
    <tr>
      <td scope="row" data-label="Env var"><code>SURREAL_MCP_ALLOW_ALL_HOSTS</code> <Since v="v3.2.1" /></td>
...
Resource limits for [ISO GQL (Cypher Query Language)](/docs/learn/querying/gql/overview) `MATCH` execution. Errors name the limit when it is exceeded. From **3.3.0**, GQL is enabled by default; on **3.2.x**, enable it with [`--allow-experimental gql`](/docs/reference/cli/surrealdb-cli/commands/start#experimental-capabilities).
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td scope="row" data-label="Default">64</td>
      <td scope="row" data-label="Allowed values">A number of bytes, without a unit suffix</td>
    ...
    <tr>
      <td scope="row" data-label="Env var"><code>SURREAL_BITMAP_BRANCH_BUDGET</code><Since v="v3.3.0" /></td>
      <td scope="row" data-label="Default">250,000</td>
      <td scope="row" data-label="Allowed values">A usize; <code>0</code> disables the budget</td>
      <td scope="row" data-label="Notes">How many index entries one range branch of a bitmap plan, which combines several indexes into a bitmap of candidate records, may read before that branch is abandoned. The abandoned branch's condition is then checked as an ordinary <code>WHERE</code> filter, so an unselective range cannot make the bitmap plan slower than scanning. The same budget bounds the allow-list built for a pre-filtered vector search (see <a href="#vector-search-config">Vector search config</a>).</td>
    </tr>
    <tr>
    ...
    <tr>
      <td scope="row" data-label="Env var"><code>SURREAL_FAN_OUT_ROW_THRESHOLD</code><Since v="v3.3.0" /></td>
      <td scope="row" data-label="Default">4</td>
      <td scope="row" data-label="Allowed values">A usize</td>
      <td scope="row" data-label="Notes">A batch with fewer items than this is evaluated one item at a time. A larger batch has its items evaluated concurrently, but only when the evaluation is read-only. Set it above the batch size to turn concurrent evaluation off.</td>
    </tr>
     <tr>
    ...
    <tr>
      <td scope="row" data-label="Env var"><code>SURREAL_MAX_EXPRESSION_PARSING_DEPTH</code><Since v="v3.3.0" /></td>
      <td scope="row" data-label="Default">128</td>
      <td scope="row" data-label="Allowed values">A usize</td>
    ...
    <tr>
      <td scope="row" data-label="Env var"><code>SURREAL_TRANSACTION_MAX_WRITE_KEYS</code><Since v="v3.2.4" /></td>
...
<table>
  ...
  <tbody>
    <tr>
      <td scope="row" data-label="Env var"><code>SURREAL_LAZY_SURREALISM</code><Since v="v3.1.0" /></td>
      <td scope="row" data-label="Default">false</td>
      <td scope="row" data-label="Allowed values">true, false</td>
    ...
    <tr>
      <td scope="row" data-label="Env var"><code>SURREAL_SURREALISM_MAX_POOL_SIZE</code><Since v="v3.0.0" /></td>
      <td scope="row" data-label="Default">8</td>
      <td scope="row" data-label="Allowed values">A usize</td>
      <td scope="row" data-label="Notes">Per-module controller pool size ceiling for Surrealism WASM modules. Has no effect: the server does not read this variable, so the ceiling stays at 8.</td>
    </tr>
    <tr>
      <td scope="row" data-label="Env var"><code>SURREAL_SURREALISM_MAX_MEMORY</code><Since v="v3.0.0" /></td>
    ...
    <tr>
      <td scope="row" data-label="Env var"><code>SURREAL_SURREALISM_MAX_EXECUTION_TIME</code><Since v="v3.0.0" /></td>
      <td scope="row" data-label="Default">None (unlimited)</td>
    ...
    <tr>
      <td scope="row" data-label="Env var"><code>SURREAL_SURREALISM_MAX_KV_ENTRIES</code><Since v="v3.0.0" /></td>
      <td scope="row" data-label="Default">None (unlimited)</td>
      <td scope="row" data-label="Allowed values">A usize</td>
      <td scope="row" data-label="Notes">Per-module KV store entry count ceiling.</td>
    </tr>
    <tr>
      <td scope="row" data-label="Env var"><code>SURREAL_SURREALISM_MAX_KV_VALUE_BYTES</code><Since v="v3.0.0" /></td>
      <td scope="row" data-label="Default">None (unlimited)</td>
      <td scope="row" data-label="Allowed values">A usize</td>
      <td scope="row" data-label="Notes">Per-module KV store maximum value size in bytes.</td>
    </tr>
    <tr>
      <td scope="row" data-label="Env var"><code>SURREAL_SURREALISM_LOG_LEVEL</code><Since v="v3.1.0" /></td>
    ...
    <tr>
      <td scope="row" data-label="Env var"><code>SURREAL_SURREALISM_SILO_ENDPOINT</code><Since v="v3.3.0" /></td>
      <td scope="row" data-label="Default"><code>https://silo.surrealdb.com</code></td>
      <td scope="row" data-label="Allowed values">A URL</td>
      <td scope="row" data-label="Notes">The base URL that a <code>silo::</code> module executable in <a href="/docs/reference/query-language/statements/define/module">DEFINE MODULE</a> is fetched from, as <code>{endpoint}/{organisation}/{package}/{major}.{minor}.{patch}.surli</code>. A trailing slash is removed. Set it to a mirror to serve packages from somewhere else.</td>
    </tr>
  </tbody>
</table>
...
These variables tune pre-filtered vector search. When a KNN query on an HNSW or DiskANN index has `WHERE` conditions that other indexes can answer, SurrealDB first evaluates those conditions into an allow-list of matching records and restricts the vector search to it. See [How the filter is applied](/docs/learn/data-models/vector-search/similarity-search#how-the-filter-is-applied).
...
<table>
  ...
  <tbody>
    <tr>
      <td scope="row" data-label="Env var"><code>SURREAL_KNN_PREFILTER_ENABLED</code></td>
      <td scope="row" data-label="Default">true</td>
      <td scope="row" data-label="Allowed values">true, false</td>
      <td scope="row" data-label="Notes">Whether KNN queries build an allow-list from their <code>WHERE</code> conditions. When <code>false</code>, the conditions are checked for each candidate during the index traversal instead, as before 3.3.0.</td>
    </tr>
    <tr>
      <td scope="row" data-label="Env var"><code>SURREAL_KNN_PREFILTER_EXACT_THRESHOLD</code></td>
      <td scope="row" data-label="Default">2000</td>
      <td scope="row" data-label="Allowed values">A u64</td>
      <td scope="row" data-label="Notes">The allow-list size at or below which the search does not use the index graph at all, and instead scores every allowed record exactly. At this size an exact search is cheaper than a traversal and always finds the true nearest neighbours.</td>
    </tr>
    <tr>
      <td scope="row" data-label="Env var"><code>SURREAL_KNN_PREFILTER_EF_BOOST</code></td>
      <td scope="row" data-label="Default">4</td>
      <td scope="row" data-label="Allowed values">A u32</td>
      <td scope="row" data-label="Notes">The multiplier applied to the search width (<code>ef</code>) of a traversal restricted to an allow-list. A selective allow-list rejects most candidates, and a wider search recovers the recall that loses. The result is capped by <code>SURREAL_KNN_PREFILTER_EF_MAX</code> and is never lower than the <code>ef</code> the query asks for.</td>
    </tr>
    <tr>
      <td scope="row" data-label="Env var"><code>SURREAL_KNN_PREFILTER_EF_BOOST_THRESHOLD</code></td>
      <td scope="row" data-label="Default">100,000</td>
      <td scope="row" data-label="Allowed values">A u64</td>
      <td scope="row" data-label="Notes">The allow-list size above which the traversal stops multiplying <code>ef</code>, because a large allow-list rejects few candidates. Results are still restricted to the allow-list at every size.</td>
    </tr>
    <tr>
      <td scope="row" data-label="Env var"><code>SURREAL_KNN_PREFILTER_EF_MAX</code></td>
      <td scope="row" data-label="Default">1024</td>
      <td scope="row" data-label="Allowed values">A u32</td>
      <td scope="row" data-label="Notes">The ceiling for the multiplied <code>ef</code>. It never lowers the <code>ef</code> a query asks for.</td>
    </tr>
  </tbody>
</table>

The allow-list is built within the entry budget set by [`SURREAL_BITMAP_BRANCH_BUDGET`](#limits-config). When the budget is exceeded, the query falls back to checking the conditions during the traversal. A vector index created before 3.3.0 uses the allow-list only after it is rebuilt with [`REBUILD INDEX`](/docs/reference/query-language/statements/rebuild).
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td scope="row" data-label="Default">100</td>
      <td scope="row" data-label="Allowed values">A usize</td>
      <td scope="row" data-label="Notes">Number of messages that can be queued for sending via WebSocket. From 3.3.0, replies and live query notifications have separate queues on each connection, each holding this many messages, and a value below 2 is raised to 2. A connection whose notification queue is full is closed, with a close frame giving the reason, rather than dropping notifications.</td>
    </tr>
    <tr>
    ...
    <tr>
      <td scope="row" data-label="Env var"><code>SURREAL_WEBSOCKET_MAX_CONCURRENT_STREAMS</code><Since v="v3.3.0" /></td>
      <td scope="row" data-label="Default">32</td>
      <td scope="row" data-label="Allowed values">A usize</td>
      <td scope="row" data-label="Notes">Maximum number of streaming queries (the <code>query_stream</code> RPC method) running at once on a single WebSocket connection. Each open stream holds an executing query, with its read snapshot or transaction, for as long as the client keeps reading. A request over the limit is refused with an error rather than queued.</td>
    </tr>
    <tr>
      <td scope="row" data-label="Env var"><code>SURREAL_WEBSOCKET_STREAM_SEND_TIMEOUT</code><Since v="v3.3.0" /></td>
      <td scope="row" data-label="Default">30</td>
      <td scope="row" data-label="Allowed values">Seconds (integer), clamped to 1-86,400</td>
      <td scope="row" data-label="Notes">How long a streaming query may wait to hand one frame to the connection before the stream is stopped. The wait resets for every frame, so it limits a stalled client rather than the length of a stream, and it applies even when <code>SURREAL_QUERY_TIMEOUT</code> is not set.</td>
    </tr>
    <tr>
      <td scope="row" data-label="Env var"><code>SURREAL_MAX_TRANSACTIONS_PER_CONNECTION</code><Since v="v3.2.2" /></td>
      <td scope="row" data-label="Default">64</td>
      <td scope="row" data-label="Allowed values">A usize</td>
    ...
    <tr>
      <td scope="row" data-label="Env var"><code>SURREAL_MAX_TRANSACTIONS_PER_SESSION</code><Since v="v3.2.2" /></td>
      <td scope="row" data-label="Default">64</td>
      <td scope="row" data-label="Allowed values">A usize</td>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td scope="row" data-label="Env var"><code>SURREAL_GRAPH_NUMERIC_IDS</code><Since v="v3.3.0" /></td>
      <td scope="row" data-label="Default">false</td>
      <td scope="row" data-label="Allowed values">true, false</td>
      <td scope="row" data-label="Notes">Whether background graph folds write numeric-encoded adjacency blocks, which store each entry's record id as a compact table-level document id. Existing blocks are converted on their next fold, and reads handle both encodings. Size the lookup cache with <code>SURREAL_GRAPH_RESOLVE_CACHE_SIZE</code>.</td>
    </tr>
  </tbody>
</table>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td scope="row" data-label="Allowed values">A duration</td>
      <td scope="row" data-label="Notes">Version retention period as a duration string. Used by the rocksdb and surrealkv engines. From 3.3.0 the memory engine refuses a non-zero value and the server does not start. <Since v="v3.3.0" /></td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td scope="row" data-label="Notes">Whether MVCC versioning is enabled, for use with the <a href="/docs/reference/query-language/statements/select#the-version-clause"><code>VERSION</code> clause</a>. Used by the rocksdb and surrealkv engines. From 3.3.0 the memory engine does not support versioned queries, and refuses <code>true</code> so that the server does not start. <Since v="v3.3.0" /></td>
    </tr>
    <tr>
      ...
      <td scope="row" data-label="Notes">The tracked memory, in bytes, above which queries are refused with <code>The query was not executed due to the memory threshold being reached</code>. The value can be specified as bytes (b, or without any suffix), kibibytes (k, kb, or kib), mebibytes (m, mb, or mib), or gibibytes (g, gb, or gib); Kubernetes-style <code>Gi</code> and <code>Mi</code> are not accepted. Unset or <code>0</code> disables the guard, and any other value is raised to at least 1 MiB. Tracked memory is the size the allocator reserves for each live heap allocation plus the RocksDB block cache, which is less than the process's resident memory, so set the threshold below the container's memory limit and above <code>SURREAL_ROCKSDB_BLOCK_CACHE_SIZE</code>: at or below the cache size, a full cache refuses every query until the server restarts. From 3.3.0 the reserved size rather than the requested size is counted, so a threshold carried over from an earlier release refuses queries at a lighter workload and may need raising. <Since v="v3.3.0" /></td>
    </tr>
    <tr>
...
Many of the arguments passed into [the CLI](/docs/reference/cli/surrealdb-cli/commands/start) can be set using the above environment variables instead.
...
<table>
  ...
  <tr>
      ...
      <td scope="row" data-label="Command arg"><code>async-event-interval</code></td>
    ...
    <tr>
      <td scope="row" data-label="Env var"><code>SURREAL_CAPS_ALLOW_EVAL_QUERY</code><Since v="v3.2.0" /></td>
      ...
      <td scope="row" data-label="Allowed values">guest, record, system (comma-separated)</td>
      <td scope="row" data-label="Notes">Allow <code>eval::surql</code> and <code>eval::gql</code> for listed subject groups. Not enabled by <code>--allow-all</code>. Still subject to <a href="/docs/learn/security/authorization/capabilities#arbitrary-queries">arbitrary-query</a> restrictions - <code>--deny-arbitrary-query</code> blocks <code>eval</code> for that subject even when eval is allowed here. For remote clients, set on the <code>start</code> process only. See <a href="/docs/reference/cli/surrealdb-cli/commands/sql#capabilities-and-remote-connections">Capabilities and remote connections</a>.</td>
    </tr>
    <tr>
    ...
    <tr>
      <td scope="row" data-label="Env var"><code>SURREAL_CAPS_DENY_EVAL_QUERY</code><Since v="v3.2.0" /></td>
    ...
    <tr>
      ...
      <td scope="row" data-label="Notes">The method of detecting the client's IP address. <Since v="v3.1.0" /> <code>Forwarded</code> parses the RFC 7239 <code>Forwarded</code> header (<code>for=</code> parameter).</td>
    </tr>
    <tr>
...
<tr>
    ...
    <tr>
      <td scope="row" data-label="Env var"><code>SURREAL_DURABLE_SESSIONS</code><Since v="v3.2.2" /></td>
      <td scope="row" data-label="Command arg"><code>durable-sessions</code></td>
      <td scope="row" data-label="Command">`start`</td>
      <td scope="row" data-label="Default">false</td>
      <td scope="row" data-label="Allowed values">true, false</td>
      <td scope="row" data-label="Notes">Persist client-attached HTTP RPC sessions in the datastore so they survive restarts and can be resumed on any node sharing the storage. Off by default. WebSocket sessions are not durable. The persisted copy includes authentication state and is stored unencrypted. See <a href="/docs/reference/cli/surrealdb-cli/commands/start#durable-http-rpc-sessions">Durable HTTP RPC sessions</a>.</td>
    </tr>
    <tr>
      <td scope="row" data-label="Env var"><code>SURREAL_DURABLE_SESSION_GC_INTERVAL</code><Since v="v3.2.2" /></td>
    ...
    <tr>
      <td scope="row" data-label="Env var"><code>SURREAL_DURABLE_SESSION_TTL</code><Since v="v3.2.2" /></td>
    ...
    <tr>
      <td scope="row" data-label="Env var"><code>SURREAL_GRAPH_FOLD_INTERVAL</code><Since v="v3.3.0" /></td>
      <td scope="row" data-label="Command arg"><code>graph-fold-interval</code></td>
      <td scope="row" data-label="Command">`start`</td>
      <td scope="row" data-label="Default">5s</td>
      <td scope="row" data-label="Allowed values">A duration</td>
      <td scope="row" data-label="Notes">The interval between background passes that fold a vertex's per-edge graph adjacency keys into packed blocks. A pass also starts early when a write queues fold work or a traversal meets a vertex with many unfolded keys. See also <code>SURREAL_GRAPH_FOLD_BATCH</code>.</td>
    </tr>
    <tr>
...
<tr>
  ...
  <tr>
      ...
      <td scope="row" data-label="Allowed values">A duration</td>
      <td scope="row" data-label="Notes">The interval at which to refresh node registration information. The <a href="/docs/reference/rest-api/http-protocol#ready"><code>/ready</code></a> endpoint reports the node unhealthy when its cluster heartbeat is older than three times this interval (9s by default). The multiple is fixed and cannot be configured.</td>
    </tr>
    <tr>
...
<tr>
    ...
    <tr>
      ...
      <td scope="row" data-label="Default">none</td>
      <td scope="row" data-label="Allowed values">A duration</td>
      <td scope="row" data-label="Notes">The maximum duration that a batch of statements can run for. From 3.3.0 it is also a hard wall-clock limit on each HTTP <code>/sql</code>, <code>/gql</code> and <code>/graphql</code> request and on each RPC method call. An HTTP <code>/sql</code> or <code>/gql</code> request that exceeds it is answered with <code>504 Gateway Timeout</code>, and a <code>/graphql</code> request with a GraphQL error. The <code>begin</code>, <code>commit</code> and <code>cancel</code> RPC methods are exempt, but the calls made inside the transaction they control are still limited individually. <Since v="v3.3.0" /></td>
    </tr>
    <tr>
    ...
    <tr>
      <td scope="row" data-label="Env var"><code>SURREAL_RECLAIM_GRACE</code><Since v="v3.2.0" /></td>
    ...
    <tr>
      ...
      <td scope="row" data-label="Default">false</td>
      <td scope="row" data-label="Allowed values">true, false</td>
      <td scope="row" data-label="Notes">Whether strict mode is enabled on this database instance. Ignored since SurrealDB 3.0 after which strictness is defined [per database](/docs/reference/query-language/statements/define/database#defining-a-strict-database) instead of instance.</td>
    </tr>
    <tr>
      <td scope="row" data-label="Env var"><code>SURREAL_STARTUP_OPERATION_TIMEOUT</code></td>
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td scope="row" data-label="Notes">Whether to enable background WAL file flushing. 2.x only: from 3.0.0, set a duration on <code>SURREAL_DATASTORE_SYNC_DATA</code> instead.</td>
    </tr>
    <tr>
      ...
      <td scope="row" data-label="Allowed values">A usize</td>
      <td scope="row" data-label="Notes">The interval in milliseconds between background flushes. 2.x only: from 3.0.0, set a duration on <code>SURREAL_DATASTORE_SYNC_DATA</code> instead.</td>
    </tr>
    <tr>
...
<tr>
    ...
    <tr>
      ...
      <td scope="row" data-label="Default">true</td>
      <td scope="row" data-label="Allowed values">true, false</td>
      <td scope="row" data-label="Notes">Whether to sync writes to disk before acknowledgement. 2.x only: replaced in 3.0.0 by <code>SURREAL_DATASTORE_SYNC_DATA</code>.</td>
    </tr>
  </tbody>
</table>
...
Instances on SurrealDB Cloud are not started with a CLI command or environment variables. Instead, they can be set from [SurrealDB Studio](/docs/manage/instances/configure) or with [`surrealctl`](/docs/reference/cli/surrealctl/overview).