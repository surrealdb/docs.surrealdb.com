...
### OpenGQL config <Since v="v3.2.0" />

Resource limits for experimental [ISO GQL (OpenGQL)](/docs/learn/querying/gql/overview) `MATCH` execution. Errors name the limit when a threshold is exceeded. The language can be enabled by passing in the [`--allow-experimental=opengql`](/docs/reference/cli/surrealdb-cli/commands/start#experimental-capabilities) flag when starting the database.
