...
<tr>
    ...
    <tr>
      ...
      <td scope="row" data-label="Allowed values">A usize</td>
      <td scope="row" data-label="Notes">Maximum HTTP body size of the HTTP <code>/mcp</code> endpoint. See [MCP](/docs/build/ai-agents/mcp).</td>
    </tr>
    <tr>
...
Used by the built-in [Model Context Protocol](/docs/build/ai-agents/mcp) server (`/mcp` on `surreal start`, `surreal mcp` on stdio). Stdio namespace/database selection uses `SURREAL_MCP_NS` and `SURREAL_MCP_DB` on the [`mcp`](/docs/reference/cli/surrealdb-cli/commands/mcp) subcommand.
