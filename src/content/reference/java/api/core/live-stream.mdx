...
The `LiveStream` class provides a blocking interface for receiving real-time notifications from live queries. It implements `AutoCloseable`, so it can be used in a try-with-resources block. Live streams are created by calling [`.selectLive()`](/docs/reference/java/api/core/surreal#select-live) on a `Surreal` instance.
...
- [Live queries](/docs/reference/java/concepts/live-queries) — Live query concepts and patterns
