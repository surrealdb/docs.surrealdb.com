...
<table>
	...
	<tbody>
		<tr>
			<td scope="row" data-label="Method"><a href="/docs/reference/java/api/core/surreal#new-session"><code>db.newSession()</code></a></td>
...
The [`.newSession()`](/docs/reference/java/api/core/surreal#new-session) method returns a new [`Surreal`](/docs/reference/java/api/core/surreal) instance that shares the underlying connection but has its own independent state. The new session starts without a selected namespace, database, or authentication — you must configure these separately.
...
- [Surreal API reference](/docs/reference/java/api/core/surreal#new-session) for the newSession method
- [Connecting to SurrealDB](/docs/reference/java/concepts/connecting-to-surrealdb) for connection setup
- [Authentication](/docs/reference/java/concepts/authentication) for per-session authentication
