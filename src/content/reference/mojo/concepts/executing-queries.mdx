...
These build a SurrealQL statement under the hood. For example, `create("person", data)` runs `CREATE person CONTENT <data>;`. See the method reference for the full list, including [`upsert`](/docs/reference/mojo/methods/upsert), [`merge`](/docs/reference/mojo/methods/merge), [`patch`](/docs/reference/mojo/methods/patch), and [`insert_relation`](/docs/reference/mojo/methods/insert-relation).
...
Each `query` is wrapped in its own implicit transaction by the server. To run several statements atomically, use [`transaction_multi`](/docs/reference/mojo/methods/transaction-multi), or the [transactions](/docs/reference/mojo/concepts/transactions) concept page for the full picture.
