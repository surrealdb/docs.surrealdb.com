...
These build a SurrealQL statement internally. For example, `create("person", data)` runs `CREATE person CONTENT <data>;`. See the method reference for the full list, including [`upsert`](/docs/reference/mojo/methods/upsert), [`merge`](/docs/reference/mojo/methods/merge), [`patch`](/docs/reference/mojo/methods/patch), and [`insert_relation`](/docs/reference/mojo/methods/insert-relation).
