...
Apply the schema for a set of models with the `SchemaManager`. Pass an [executor](/docs/reference/php/libraries/surqlize/connections), or rely on the registered one.
...
Use `definitions()` instead of `apply()` to inspect the statements without running them. You can also apply schemas from the [CLI](/docs/reference/php/libraries/surqlize/code-generation-and-cli#schema-apply).

## Learn more

- [Models](/docs/reference/php/libraries/surqlize/models) for the `#[Schema]` attribute
- [Code generation and CLI](/docs/reference/php/libraries/surqlize/code-generation-and-cli) to apply schemas from the command line
