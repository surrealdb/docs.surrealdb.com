...
<table>
    ...
    <tbody>
        <tr>
            <td scope="row" data-label="Method"><a href="/docs/reference/php/v2/api/core#connect"> <code> $db->connect($url, $options) </code></a></td>
            <td scope="row" data-label="Description">Connects to a local or remote database endpoint</td>
        </tr>
        <tr>
            <td scope="row" data-label="Method"><a href="/docs/reference/php/v2/api/core#close"> <code> $db->close() </code></a></td>
            <td scope="row" data-label="Description">Closes the connection to the database</td>
        </tr>
        <tr>
            <td scope="row" data-label="Method"><a href="/docs/reference/php/v2/api/core#use"> <code> $db->use($namespace, $database) </code></a></td>
            <td scope="row" data-label="Description">Switches to a specific namespace and database</td>
        </tr>
        <tr>
            <td scope="row" data-label="Method"><a href="/docs/reference/php/v2/api/core#status"> <code> $db->status() </code></a></td>
...
Create a `Surreal` instance, then call `connect()` with a connection string and a [`ConnectOptions`](/docs/reference/php/v2/api/core#connectoptions) object.
...
The `status()` method returns a [`ConnectionStatus`](/docs/reference/php/v2/api/core#connectionstatus) enum with one of four values:
...
- [Surreal API reference](/docs/reference/php/v2/api/core) for the full connection interface
- [Authentication](/docs/reference/php/v2/concepts/authentication) for signing in and managing credentials
- [Error handling](/docs/reference/php/v2/concepts/error-handling) for connection and version errors
