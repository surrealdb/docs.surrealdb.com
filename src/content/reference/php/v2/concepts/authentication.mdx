...
SurrealDB supports several levels of authentication, from [system users](/docs/learn/security/authentication/users#system-users) to fine-grained [record access](/docs/learn/security/authentication/users#record-users). Version 2 of the PHP SDK represents each level as a typed credential class, so the required fields are explicit.
