...
Add your middleware through the `middleware` option on [`DriverOptions`](/docs/reference/php/v2/api/utilities#driveroptions). The list is appended to the pipeline in order.
...
See [Observability](/docs/reference/php/v2/concepts/observability) for what the telemetry step records.
...
It retries `HttpConnectionException`, `CallTerminatedException`, and `ConnectionUnavailableException`. On an [async runtime](/docs/reference/php/v2/concepts/runtimes), pass the matching scheduler so the backoff delay does not block other tasks.
...
- [Observability](/docs/reference/php/v2/concepts/observability) for the telemetry step in depth
- [Events](/docs/reference/php/v2/concepts/events) for observing requests without writing middleware
- [Runtimes and workers](/docs/reference/php/v2/concepts/runtimes) for the schedulers retry backoff uses
