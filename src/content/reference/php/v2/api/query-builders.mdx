...
| `compile()` | [`BoundQuery`](/docs/reference/php/v2/api/utilities#boundquery) | Compile to SurrealQL and bindings without running |
...
Execute a pre-built [`BoundQuery`](/docs/reference/php/v2/api/utilities#boundquery). Returns one result per statement.
...
`where()` accepts a SurrealQL string or a [`BoundQuery`](/docs/reference/php/v2/api/utilities#boundquery) fragment. Use a `BoundQuery` to keep dynamic values parameterised.
...
- [Utilities](/docs/reference/php/v2/api/utilities) for `BoundQuery` and the `Output` enum
