...
The `access` and `params` fields of the `Record` struct are the only fields of the four structs that need extra explanation.
...
Since the `params` of a `Record` struct used to sign up and sign in can be any type that implements `SurrealValue`, we can put our own struct together and pass them into these methods. On signup, the database will CREATE a `user` record that holds the name and password, and on signin it will SELECT the user that has a matching name and password.
...
The `.signup()` and `.signin()` methods return an `AccessToken`, which can be printed out if needed using the `.into_insecure_token()` or `.as_insecure_token()` methods. As the method names imply, great care should be taken with them. The `AccessToken` struct redacts the token so printing out an `AccessToken` on its own will not display it.
...
Each of the crates featured in the Rust SDK also uses the schema above. Three of them (Actix, Axum, Rocket) are web servers, while Egui is a visual UI. See the mini tutorials for each of these crates here:

* [Actix](/docs/reference/rust/frameworks/actix)
* [Axum](/docs/reference/rust/frameworks/axum)
* [Rocket](/docs/reference/rust/frameworks/rocket)
* [Egui](/docs/reference/rust/frameworks/egui)

Learn more about authentication in SurrealDB in our [security best practices](/docs/learn/security/best-practices/security-best-practices#authentication) documentation and in the [security](/docs/learn/security/authentication/users#expiration) section of the SurrealDB documentation.
...
Since the `params` of a `Record` struct used to sign up and sign in can be any type that implements `Serialize`, we can put our own struct together and pass them into these methods. On signup, the database will CREATE a `user` record that holds the name and password, and on signin it will SELECT the user that has a matching name and password.
...
The `.signup()` and `.signin()` methods return a [`Jwt`](https://docs.rs/surrealdb/latest/surrealdb/opt/auth/struct.Jwt.html), which can be printed out if needed using the `.into_insecure_token()` or `.as_insecure_token()` methods. As the method names imply, great care should be taken with them. The `Jwt` struct redacts the token so printing out a `Jwt` on its own will not display it.
