---
position: 2
title: Fetching linked records
description: All the fields of a SurrealDB linked record can be fetched and deserialised into a Rust type
---
...
To follow along interactively, connect [using SurrealDB Studio](/docs/explore/studio) or the following command to open up the CLI:
...
The following example shows a classroom joined to a few students by [record links](/docs/reference/query-language/language-primitives/record-links).
...
[The `.*` operator](/docs/reference/query-language/language-primitives/idioms#all-elements) for the `teacher` and `student` fields can be used in this case.
...
[The `.*` operator](/docs/reference/query-language/language-primitives/idioms#all-elements) for the `teacher` and `student` fields can be used in this case, but note that the `.*` must be used twice in the case of the students: once to access each member of the array, and once more to access all of its fields.
