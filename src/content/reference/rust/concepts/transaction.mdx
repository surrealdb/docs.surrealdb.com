...
To follow along interactively, connect [using SurrealDB Studio](/docs/explore/studio) or the following command to open a connection in the CLI:
...
Note that an `Ok` outer `Result` from `await`ing a query only shows that the request succeeded. The response can still include per-statement failures, where one of the SQL statements did not succeed. Collect those with [`take_errors()`](https://docs.rs/surrealdb/latest/surrealdb/struct.IndexedResults.html#method.take_errors) on the query response, or use [`check()`](https://docs.rs/surrealdb/latest/surrealdb/struct.IndexedResults.html#method.check) to fail on the first error. If the outer `Result` is `Err`, the transaction is not usable as intended.
...
To avoid the possibility of typos, the [`.set()`](/docs/reference/rust/methods/set) method can be used to set the amount to transfer.
