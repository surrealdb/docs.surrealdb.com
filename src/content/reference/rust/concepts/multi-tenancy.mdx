...
position: 11
title: Multi-tenancy
description: Multi-tenancy in the Rust SDK allows multiple connections to be used simultaneously, each tenant operating inside its own isolated namespace and database.
---
...
Before version 3.0, cloning a `Surreal<C>` created a cheap clone of the same session. Where `.clone()` is used to pass a `Surreal<C>` around and multi-tenancy is not wanted, wrap the client in a type like an `Arc` so that only the wrapper is cloned.

What that saves depends on the engine. An embedded engine builds the new session in memory, which is what the example below measures. A remote engine replays the session setup - the attach, the sign-in, and the `USE NS` and `USE DB` calls - across the connection for each clone, so every clone costs a series of network round trips rather than a copy, and the replays are written to the socket ahead of queries that are already waiting on it.

Web framework state is where this can add up. A router stores its state by cloning it into every route, and an extractor clones it again on each request, so a bare `Surreal<C>` passed to framework functions such as Axum's `with_state`, Actix's `app_data` or Rocket's `manage` opens a session per route and another per request. Wrap the client in an `Arc` first, or hold it in a static as the [framework guides](/docs/reference/rust/frameworks) do.
...
The examples below use the multi-tenancy support added in SurrealDB 3.0.
