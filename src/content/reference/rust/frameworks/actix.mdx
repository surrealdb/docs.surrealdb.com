...
Next, we will put the database client together. Actix provides an [`.app_data()`](https://actix.rs/docs/application/#state) method when starting a router that would give us access to the database inside its functions. However, for simplicity we can instead wrap the client inside a `LazyLock` to make it into a global static. A static also sidesteps a detail of `.app_data()`: the closure given to `HttpServer::new` runs once per worker thread, so a `Surreal<C>` cloned into it opens a [new session](/docs/reference/rust/concepts/multi-tenancy) per worker. Registering `web::Data::new(db)` avoids that, because `web::Data` is itself a reference count around the value.
