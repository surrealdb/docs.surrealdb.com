...
<table>
    ...
    <tbody>
        ...
        <tr>
            <td scope="row" data-label="Method"><code>.grpc(config)</code></td>
            <td scope="row" data-label="Description">Sets the gRPC message size. Returns a <code>Result</code>, as a size too small for the protocol's own framing is refused here rather than at connect time. See <a href="/docs/reference/rust/methods/connect#message-size">message size</a>.</td>
        </tr>
        <tr>
...
To make a new connection that includes SurrealKV versioning, add the "kv-surrealkv" feature flag to the `surrealdb` dependency in `Cargo.toml`, add the path to the folder containing the database inside `new()`, and call the `.versioned()` method. The memory backend does not support versioning from 3.3.0, and `Surreal::new::<Mem>(()).versioned()` fails when the connection is opened.
