...
The `.upsert()` method is followed by a second method that refers to the type of upsert to use: an upsert with `.content()`, `.merge()`, or `.patch()`.
...
The `.patch()` method uses a struct called a `PatchOp` that contains the four methods `add()`, `change()`, `remove()`, and `replace()`. Each of these methods takes different arguments depending on the operation. For example, `PatchOp::remove()` only takes a single argument (a path), while `PatchOp::replace()` takes a second value for the replacement value.
