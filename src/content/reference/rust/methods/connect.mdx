...
The two directions settle separately, because they answer to different things. A **response** is bounded by the client's figure alone, since that is the memory this client gives one message. A **request** is bounded by whichever is smaller of the client's figure and the server's, since nothing set on the client persuades a server to accept more than it allows. A request refused for size names the side that bound it, so raising the server variable is only the fix when the server is the one that bound it.
