...
A gRPC message has a size limit at each end. The server sets its own with [`SURREAL_GRPC_MAX_MESSAGE_SIZE`](/docs/reference/cli/surrealdb-cli/environment-variables) and advertises it on connection, and the SDK takes that advertised figure by default. `Config::grpc` sets the client's own figure, for a workload that writes a large batch in one transaction:
...
The two directions are limited separately, because different settings apply to each. A **response** is bounded by the client's figure alone, since that is the memory this client gives one message. A **request** is bounded by whichever is smaller of the client's figure and the server's, since no client setting can make a server accept more than it allows. The error for a request refused for size names the side that set the limit, so raising the server variable only helps when the server set it.
