...
If the input is something the database is unable to parse into a query in the first place, an `Err` is returned for the entire `.query()` call. For example, if the `string` syntax is changed to something nonsensical like `Hi how are you?`, `.query()` returns an `Err` for the whole call.
...
The `IndexedResults` struct contains helper methods such as `.check()` to check for errors, or `.take_errors()` which removes the errors from the main `IndexedResults`.
...
The [`vars!`](/docs/reference/rust/concepts/working-with-types#the-vars-macro) macro is the most direct way to set several parameters at once, as each pair is written in place rather than chained one call at a time.
...
As the `.query()` method can be used to pass any SurrealQL query on to the database, it is a convenient choice for complex queries. However, be sure to keep [the following best practices in mind](/docs/learn/security/best-practices/security-best-practices#query-safety) when doing so.

<blockquote>
When using SurrealDB as a traditional backend database, your application will usually build SurrealQL queries that may need to contain some untrusted input, such as that provided by the users of your application. To do so, SurrealDB offers the `.bind()` method on a query, which should always be used when including untrusted input into queries. Otherwise, SurrealDB will be unable to separate the actual query syntax from the user input, resulting in the well-known SQL injection vulnerabilities. This practice is known as prepared statements or parameterised queries.
</blockquote>
