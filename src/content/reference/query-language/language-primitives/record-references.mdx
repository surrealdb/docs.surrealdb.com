...
#### Permissions during a cascade

A cascading delete runs the referencing record's own delete with permissions disabled, because referential integrity has to finish whatever the caller is allowed to do to that record. A record user can therefore cause a delete in a table whose `PERMISSIONS FOR delete` would refuse them directly.

That bypass stops at the referencing record. Where deleting it also removes a vertex's graph edges, the edge cascade runs with the caller's own permissions instead, so an edge table's `PERMISSIONS FOR delete` still decides whether those edges go. A caller cannot reach an edge table they may not delete from by deleting a vertex that cascades into it.

System users are unaffected by either rule: a root, namespace or database user holding the editor role is not subject to record permissions in the first place.
...
The `ON DELETE THEN` clause allows for custom logic when a reference is deleted. This clause includes a parameter called `$this` to refer to the record in question, and `$reference` for the reference.
