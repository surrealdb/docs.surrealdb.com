...
Before 3.3.0, a closure field could not be called this way when its name matched a built-in method of another type, such as `add` for arrays and sets. The call now falls back to the closure. From 3.3.0 the closure body also runs with the session's namespace and database, so it can query the database, and a body that writes makes the statement a write, so that the change persists as described in [Closures and writes](#closures-and-writes).
