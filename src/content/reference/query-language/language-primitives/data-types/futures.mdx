...
> [!NOTE]
> The `future` type is only available up to SurrealDB 2.x. Since version 3.0.0-beta, it has been replaced by [defined fields using the `COMPUTED` clause](/docs/reference/query-language/statements/define/field#computed-fields). Most examples in this page include an equivalent `COMPUTED` clause to aid in migrating to the new implementation.
