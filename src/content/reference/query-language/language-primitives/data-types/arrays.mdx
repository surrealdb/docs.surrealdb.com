...
So `[WHERE ...]` suits a plain filter, and a closure suits work that needs one, such as a transformation, a condition that reads something other than the element itself, or a step in a longer chain.
...
The [`array::map()`](/docs/reference/query-language/functions/database-functions/array#arraymap) function provides access to each item in an array, allowing an operation to be performed on it before being passed on.
