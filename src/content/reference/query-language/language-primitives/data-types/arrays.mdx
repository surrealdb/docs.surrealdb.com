...
position: 2
title: Arrays
description: "An array is a collection of values contained inside square brackets, each of which is stored at a certain index."
---
...
Arrays are frequently encountered in SurrealDB, as [`SELECT`](/docs/reference/query-language/statements/select) statements return an array of values by default unless the `ONLY` keyword is used on an array that contains a single item.
...
[[test.results]]
error = ""Couldn't coerce value for field `employees` of `team:one`: Expected `array<record<employee>,5>` but found a collection of length `6`""
...
If a `WHERE` or `?` clause finds an item that by itself is not equal to `true` or `false`, it will check the item's [truthiness](/docs/reference/query-language/language-primitives/data-types/values#values-and-truthiness) to determine whether to pass it on or not.
...
Closures in general have inferior performance so `[WHERE ...]` is preferred when filtering on an array. This is because `[WHERE ...]` is evaluated directly, while a closure is invoked once per element, and on an array of tens of thousands the closure takes roughly an order of magnitude longer. That is the case for any method that takes one - `map()` as much as `filter()` - and is unchanged by whether it is written as a method or as `array::filter(...)`, because the cost is the closure rather than the call.

So `[WHERE ...]` suits a plain filter, and a closure is worth using where the work needs one, such as a transformation, a condition that reads something other than the element itself, or a step in a longer chain.
...
The [`array::map()`](/docs/reference/query-language/functions/database-functions/array#arraymap) function provides access to each item in an array, allowing an opearation to be performed on it before being passed on.
