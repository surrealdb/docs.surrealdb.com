...
Ranges of integers have the added convenience of being able to be used in a [FOR loop](/docs/reference/query-language/statements/for).
...
A range can be used in a `WHERE` clause in place of operators like `<` and `>`. This is useful when checking for a number that must be within a certain range, and produces shorter code.
...
The comparison form is faster, so prefer it on large tables. A pair of comparisons on an indexed field uses the index, while `IN` with a range reads every record in the table. See [numeric ranges in a `WHERE` clause](/docs/reference/query-language/statements/select#numeric-ranges-in-a-where-clause) for details.
...
This opens up a range of functional programming patterns that are made possible by SurrealDB's [array functions](/docs/reference/query-language/functions/database-functions/array), many of which can use [anonymous functions](/docs/reference/query-language/language-primitives/data-types/closures) (closures) to perform an operation on each item in the array.
...
Originally an internal syntax for mock testing, this syntax became known to the user community and is now commonly used. However, the original syntax differed from true ranges in always being inclusive, in that `1..10` was treated as "from 1 up to and including 10". A change has since been made to have the mock syntax take a true range with a syntax equivalent to that demonstrated in this page.
