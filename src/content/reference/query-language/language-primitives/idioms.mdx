...
To access an element in an array by its index, use square brackets `[]` with the index inside. For example, take a `school` record with some student results.
...
To see just the values of this object, the [`object::values()`](/docs/reference/query-language/functions/database-functions/object#objectvalues) function can be used.
...
`[$]` applies to arrays and sets. Sets are ordered, but their index operator follows sort order rather than insertion order: `[0]` returns the smallest value and `[$]` returns the greatest. You can also use [`set::last()`](/docs/reference/query-language/functions/database-functions/set#setlast) on sets. See [sets](/docs/reference/query-language/language-primitives/data-types/sets#using-the-index-operator-on-sets) for detail.
...
For example, the following creates a new `person` record and then calls `uppercase()` on its name field.
...
To learn more about string method chaining in SurrealQL, see the [string functions](/docs/reference/query-language/functions/database-functions/string#method-chaining) section.

## Graph navigation

SurrealDB can also be used in the context of graph databases, where data is stored and navigated using graph traversal idioms. The [`RELATE` statement](/docs/reference/query-language/statements/relate) is used to create relationships between records. This allows you to traverse related records efficiently without needing to pull data from multiple tables and merging that data together using SQL JOINs.
...
The keyword `AS` is necessary inside `SELECT` statements when [using an alias](/docs/reference/query-language/statements/select#basic-usage) (a new name for a field).
...
Expressions inside a destructuring operation have the same [predefined parameters](/docs/reference/query-language/language-primitives/parameters#parent-this) as any other expression, such as `$this` to the current object and `$parent` to the previous one.
...
UPDATE planet, country, state, province, city SET name = id.id();
...
The following two rules of thumb explain how the syntax inside the structure of the query works.
...
```surql
CREATE |person:1..21| RETURN NONE;
...
Idioms can combine multiple parts to navigate complex data structures.
...
- **Methods**: Use built-in methods for data manipulation within idioms.
...
An idiom is a path built from parts: fields, indexes, filters, methods, graph traversals, destructuring, optional parts and recursion. These parts can be combined in a single path to access nested fields, filter arrays or traverse graph relationships.
