...
The `array::find_index` function can also be called using the alias `array::index_of`.
...
The following example shows this function using a negative index.
...
The `array::repeat` function creates an array of a given size containing the specified value for each element. The `count` argument must be non-negative; negative values return an error.
