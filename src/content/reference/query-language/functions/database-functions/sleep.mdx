...
SurrealDB also has a [SLEEP statement](/docs/reference/query-language/statements/sleep) that accepts a datetime; however, the `sleep` function can be used in more dynamic ways such as the following example that simulates a 100ms delay between each record in a query.
...
The `sleep()` function does not interfere with operations that are underway in the background, such as a [`DEFINE INDEX`](/docs/reference/query-language/statements/define/indexes) statement using the `CONCURRENTLY` clause.
