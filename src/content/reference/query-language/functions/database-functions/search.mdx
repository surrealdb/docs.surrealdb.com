...
These functions are used in conjunction with the [`@@` operator (the 'matches' operator)](/docs/reference/query-language/language-primitives/operators#matches-a-idmatchesa) to either collect the relevance score or highlight the searched keywords within the content.
