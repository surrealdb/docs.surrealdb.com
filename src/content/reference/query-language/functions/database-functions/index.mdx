...
A few functions can be used not just on their own but with a [`GROUP BY`](/docs/surrealql/clauses/group-by) clause including as part of a [pre-computed table view](/docs/surrealql/statements/define/table#pre-computed-table-views).
