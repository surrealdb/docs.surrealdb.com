...
A few functions can be used not just on their own but with a [`GROUP BY`](/docs/reference/query-language/clauses/group-by) clause including as part of a [pre-computed table view](/docs/reference/query-language/statements/define/table#pre-computed-table-views).
