...
`eval::gql` requires the **`gql` experimental capability** (same as [`POST /gql`](/docs/reference/rest-api/http-protocol#gql)) and runs on the streaming query engine. Queries that include GQL **mutations** (`INSERT`, `SET`, `REMOVE`, `DELETE`) participate in the caller's write transaction — see [GQL mutations](/docs/learn/querying/gql/mutations).
