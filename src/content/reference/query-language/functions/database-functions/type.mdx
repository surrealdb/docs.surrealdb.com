...
12345
```
This is the equivalent of using [`<int>`](/docs/reference/query-language/language-primitives/casting#int) to cast a value to an int.
...
The `type::range` function converts a value into a [range](/docs/reference/query-language/language-primitives/data-types/ranges). It accepts a single argument, either a range or an array with two values. If the argument is an array, it will be converted into a range, similar to [casting](/docs/reference/query-language/language-primitives/casting).
...
This is similar to using [`<string>`](/docs/reference/query-language/language-primitives/casting#string) to cast a value to a string, except that an input of bytes will not fail.
