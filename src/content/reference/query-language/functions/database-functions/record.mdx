...
> [!NOTE]
> These functions were called `meta::tb()` and `meta::id()` before 2.x. Both old names still work, so an existing query does not have to change, but `record::tb()` and `record::id()` are the names to use now.
...
> [!NOTE]
> To retrieve the records that point at a record through a [reference](/docs/reference/query-language/language-primitives/record-references), define a computed field using the `<~` syntax rather than a function.
