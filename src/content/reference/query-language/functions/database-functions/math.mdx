...
* [`array::max`](/docs/reference/query-language/functions/database-functions/array#arraymax), which extracts the greatest value from an array of values
...
* [`array::min`](/docs/reference/query-language/functions/database-functions/array#arraymin), which extracts the least value from an array of values
...
Inside an array the [`array::map()`](/docs/reference/query-language/functions/database-functions/array#arraymap) function can be used to ensure that each value is the number 0 if a `NONE` or `NULL` is encountered.

Classic [array filtering](/docs/reference/query-language/language-primitives/data-types/arrays#mapping-and-filtering-on-arrays) can also be used to simply remove any `NONE` or `NULL` values before `math::sum()` is called.
