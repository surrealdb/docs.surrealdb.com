...
API functions are passed in as middleware inside a [`DEFINE API`](/docs/reference/query-language/statements/define/api) or [`DEFINE CONFIG API`](/docs/reference/query-language/statements/define/config) statement and called when a request is received.
