...
The `RELATE` statement can be used to generate graph edges between two records in the database. This allows you to traverse related records efficiently without needing to pull data from multiple tables and merge that data together using SQL JOINs.
...
By default, the edge table gets created as a schemaless table when you execute the `RELATE` statement. You can make the table schemafull by [defining a schema](/docs/reference/query-language/statements/define/table).

A common use case is to make sure only unique relationships get created. You can do that by [defining an index](/docs/reference/query-language/statements/define/indexes).
...
To enforce unidirectional relationships, you can restrict the type definition using a [`DEFINE FIELD`](/docs/reference/query-language/statements/define/field) definition.
...
There are two ways to add record data in the `RELATE` statement. Both of these queries will produce the same result. Use `CONTENT` to pass the record data as a single object, and `SET` to assign or compute each field on its own line.
...
Using the ONLY keyword returns just an object for the relation in question, instead of an array with a single object.
...
## Using [`LET`](/docs/reference/query-language/statements/let) parameters in RELATE statements

You can also use [parameters](/docs/reference/query-language/language-primitives/parameters) to specify the record IDs.
...
## Modifying output with the [`RETURN`](/docs/reference/query-language/statements/return) clause
...
You can also delete graph edges between two records in the database by using the [DELETE statement](/docs/reference/query-language/statements/delete).
...
[Destructuring](/docs/reference/query-language/language-primitives/idioms#destructuring) can also be used to pick and choose which fields to access inside a graph query. The following query will return the same output as above, except that `original_name: 'Πολιτεία'` will no longer show up.
...
Here we can use the [`array::complement`](/docs/reference/query-language/functions/database-functions/array#arraycomplement) function to return only items from one array that are not present in another array.
...
The [destructuring syntax](/docs/reference/query-language/language-primitives/idioms#destructuring) can be used to reduce some typing. Here is the same query as the last using destructuring syntax instead of one line for each field.
...
With these fields in place, a `WHERE` clause inside the arrows does refined searches on relationships of a certain type. The condition sits in parentheses, as part of the path, so it becomes the edge scan's own predicate and the edges that fail it are never followed.
...
The same condition can be written in square brackets - `->knows[WHERE has_dated]->person` - but that is the generic [array filter](/docs/reference/query-language/language-primitives/data-types/arrays#filtering-and-mapping-with-array-functions) applied after walking every edge. Both return the same records, and on a record with many edges only the parenthesised form stays fast.
...
Because the `WHERE` clause simply checks for [truthiness](/docs/reference/query-language/language-primitives/data-types/values#values-and-truthiness) (whether a value is present and not empty), these fields do not necessarily need to be booleans and can even be complex objects.
...
CREATE |city:1..6|;
UPDATE city SET name = <string>id.id() + 'ville';
...
As noted above, a `TIMEOUT` can be set for queries that may be computationally intensive. This is particularly useful when experimenting with recursive queries, which, if care is not taken, can run all the way to the maximum possible depth of 256.
...
Take the following example in which three `person` records are created, each of which likes the other two `person` records. A query on the `->likes->person` path shows that the number of records doubles each time.
...
However, clauses available in [`SELECT` statements](/docs/reference/query-language/statements/select) such as `WHERE`, `LIMIT`, `GROUP BY`, aliases and so on can be used, making a graph clause the most flexible option.
