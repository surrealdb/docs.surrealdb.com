...
However, clauses available in [`SELECT` statements](/docs/reference/query-language/statements/select) such as `WHERE`, `LIMIT`, `GROUP BY`, aliases and so on can be used, making a graph clause a most flexible option.
