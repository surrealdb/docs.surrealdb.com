...
The in-memory SurrealMX backend does not support versioned queries. From 3.3.0 it refuses the `versioned` and `retention` parameters at startup, and a `VERSION` clause sent to it returns an error saying that the underlying datastore does not support versioned queries. <Since v="v3.3.0" />
