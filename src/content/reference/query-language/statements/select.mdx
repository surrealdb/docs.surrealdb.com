...
When you are starting a new database with versioning enabled in memory, RocksDB, or beta [SurrealKV](/docs/self-hosted/file-backed), you can specify a version for each record. This is useful for time-travel queries. You can query a specific version of a record by using the `VERSION` clause. The `VERSION` clause is always followed by a [datetime](/docs/reference/query-language/language-primitives/data-types/datetimes) and when the specified timestamp does not exist, an empty array is returned.
