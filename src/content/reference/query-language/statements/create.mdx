...
If you are using [SurrealKV as the storage engine](/docs/start/architecture) with versioning enabled, when creating a record you can specify a version for each record. This is useful for time-travel queries. You can query a specific version of a record by using the `VERSION` clause.
