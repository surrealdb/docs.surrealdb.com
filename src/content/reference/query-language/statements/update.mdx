...
> [!NOTE]
> This statement can not be used to create graph relationships. For that, use the [`RELATE`](/docs/reference/query-language/statements/relate) or [`INSERT`](/docs/reference/query-language/statements/insert) statement.

> [!NOTE]
> `UPDATE` does not create records that do not exist. To update a record and create it if it does not exist, use the [`UPSERT`](/docs/reference/query-language/statements/upsert) statement.
...
> [!NOTE]
> `@target` refers to either record output including an `id` field, or a [record ID](/docs/reference/query-language/language-primitives/data-types/record-ids) on its own.

## Example usage

The examples below show how to use the `UPDATE` statement. They start by creating two `person` records with the [`CREATE`](/docs/reference/query-language/statements/create) statement so that the examples produce meaningful output.
...
The `WHERE` condition is checked before the data clause is evaluated, so a data clause with side effects only runs for the records the condition matches. The same applies to [`UPSERT`](/docs/reference/query-language/statements/upsert#evaluation-order) when it updates an existing record.
...
This has always held for ordinary fields - `SET a = a + 1, b = a + 1` assigns `b` from the old `a`. [Computed fields](/docs/reference/query-language/statements/define/field#restrictions-on-computed-fields) are now part of that image too, so a data clause and a condition both see their pre-mutation values.
...
A statement that creates a record has no earlier image, so a computed field reads `NONE` there - whether the record ID was named, generated, or reached through [`UPSERT`](/docs/reference/query-language/statements/upsert).
...
You can also specify changes to be applied to your query response, using the PATCH command, which works similarly to the [JSON Patch specification](https://jsonpatch.com/).
...
A patch is an array of operations, each naming its target with a [JSON Pointer](https://datatracker.ietf.org/doc/html/rfc6901) - a `/`-separated path into the record, so `/settings/marketing` reaches the `marketing` field inside `settings`. SurrealDB accepts the six operations of the JSON Patch specification plus `change`:

| Operation | Takes | Does |
| --- | --- | --- |
| `add` | `path`, `value` | Sets the member at `path` on an object, or inserts at that position in an array |
| `remove` | `path` | Deletes the field at `path` |
| `replace` | `path`, `value` | Overwrites the field at `path` |
| `copy` | `from`, `path` | Reads the value at `from` and writes it to `path` |
| `move` | `from`, `path` | Reads the value at `from`, writes it to `path`, and deletes `from` |
| `test` | `path`, `value` | Fails the whole patch unless the field at `path` already equals `value` |
| `change` | `path`, `value` | Applies a text diff to the string at `path` |
...
Writing to a [set](/docs/reference/query-language/language-primitives/data-types/sets) by position reaches whichever element sorts into that place, so a set is rebuilt once the patch has been applied.
...
### Read pointers and field permissions
...
Every statement that takes a `PATCH` clause behaves this way, so the same applies to [`CREATE`](/docs/reference/query-language/statements/create), [`UPSERT`](/docs/reference/query-language/statements/upsert), [`RELATE`](/docs/reference/query-language/statements/relate) and [`INSERT`](/docs/reference/query-language/statements/insert).
...
As `UPDATE` before version 2.0.0 used to create a specified record ID if it did not exist, it was used in the `.surql` files generated by the [`surreal export`](/docs/reference/cli/surrealdb-cli/commands/export) command to export existing records in a database. As of version 2.0.0, the [`INSERT`](/docs/reference/query-language/statements/insert) statement is used instead.
