...
The `Bitmap` operators appear from 3.3.0, when the planner answers a `WHERE` clause with several indexes at once instead of driving the query from one of them. `BitmapIndexScan`, `BitmapFullTextScan` and `BitmapGraphScan` each read one index into a bitmap of the matching records, `BitmapAnd`, `BitmapOr` and `BitmapAndNot` combine those bitmaps, and `BitmapResolve` fetches the records that remain. The same operators appear beneath `IndexCountScan` when a `count()` is answered from indexes alone, and beneath `KnnScan` when an index pre-filters a vector search.
