...
When an `UPSERT` updates an existing record, the `WHERE` condition is checked before the data clause is evaluated. A data clause with side effects therefore only runs for the records the condition matches, matching [`UPDATE`](/docs/reference/query-language/statements/update#evaluation-order).
...
[Unique indexes](/docs/reference/query-language/statements/define/indexes#unique-index) can be used to ensure that no field or combination of fields is ever present more than once. For example, a game might have a rule that duplicate names can exist, but not within the same class.
...
The `copy`, `move` and `test` operations read from a pointer, and a pointer into a field the caller cannot `select` fails the statement. See [read pointers and field permissions](/docs/reference/query-language/statements/update#read-pointers-and-field-permissions).
