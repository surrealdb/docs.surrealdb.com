...
A bearer access method allows generating bearer grants with an associated key that can be used to access SurrealDB as a specific [system user](/docs/learn/security/authentication/users#system-users) or [record user](/docs/learn/security/authentication/users#record-users). Bearer grants allow other systems and software to authenticate with SurrealDB using a secure and unique credential that can be [audited](/docs/reference/query-language/statements/access#show) and [revoked](/docs/reference/query-language/statements/access#revoke) at any time.
...
DEFINE ACCESS IF NOT EXISTS example ON DATABASE TYPE BEARER FOR USER;
```
...
DEFINE ACCESS OVERWRITE example ON DATABASE TYPE BEARER FOR USER;
```
