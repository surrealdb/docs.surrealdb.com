...
- You must be authenticated as a [system user](/docs/learn/security/authentication/users#system-users) at the same level or higher than the level to which you want to provide JWT access.
- [You must select a namespace or database](/docs/reference/query-language/statements/use) before you can define a JWT access method.
...
Audience values are configuration rather than secrets, so unlike a key they are shown in full by [`INFO`](/docs/reference/query-language/statements/info).
