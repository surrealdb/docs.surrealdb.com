...
- You must be authenticated as a [system user](/docs/learn/security/authentication/authentication#system-users) at the same level or higher than the level on which access is defined.
...
Learn more about [JWT access method in the documentation](/docs/reference/query-language/statements/define/access/jwt).

## Record access

A record access method allows accessing SurrealDB as a [record user](/docs/learn/security/authentication/authentication#record-users). Record users allow SurrealDB to operate as a web database by offering mechanisms to define custom signin and signup logic as well as custom table and field permissions.

Learn more about [record access method in the documentation](/docs/reference/query-language/statements/define/access/record).
...
When used in a [`DEFINE ACCESS ... TYPE JWT`](/docs/reference/query-language/statements/define/access/jwt), the `AUTHENTICATE` clause is used to validate the token claims and can be used to log or stop authentication attempts.
