...
position: 11
title: DEFINE INDEX
description: "SurrealDB uses indexes to help optimise query performance. An index can consist of one or more fields in a table and can enforce a uniqueness constraint - including HNSW and DISKANN vector indexes."
---
...
- [You must select your namespace and database](/docs/reference/query-language/statements/use) before you can use the `DEFINE INDEX` statement.
...
The created index can be tested using the [`INFO` statement](/docs/reference/query-language/statements/info).
...
#### Array-element composite indexes
...
`CONTAINSALL` / `ALLINSIDE` may still apply a residual filter above the union when intersection semantics require it. See [operators](/docs/reference/query-language/language-primitives/operators) and [`EXPLAIN`](/docs/reference/query-language/statements/explain) to confirm the planner choice.

### Count index

<Since v="v3.0.0" />

A count index maintains a running count of records on a table. It is used with `count()` and `GROUP ALL` in a query. SurrealDB stores incremental count deltas and compacts them in the background. From 3.2.5, a bare `count()` projection [implies `GROUP ALL`](/docs/reference/query-language/statements/select#bare-count-implies-group-all); the examples below keep the explicit form.
...
- `GROUP ALL` must be present (written explicitly, or [implied](/docs/reference/query-language/statements/select#bare-count-implies-group-all) by a bare `count()` projection from 3.2.5).
...
Vector search indexes in SurrealDB support efficient [k-nearest neighbors](https://en.wikipedia.org/wiki/K-nearest_neighbors_algorithm) (kNN) and [Approximate Nearest Neighbor](https://en.wikipedia.org/wiki/Nearest_neighbor_search) (ANN) operations, which are pivotal in performing similarity searches within complex, high-dimensional datasets and data types. Refer to the [Vector Search Cheat Sheet](/docs/learn/data-models/vector-search/vector-indexes#vector-search-cheat-sheet) for the parameters allowed.

> [!NOTE]
> After a large insert or update, a vector index stores the new records separately until a background process merges them into its graph. Searches return correct results throughout, but are slower until that merge completes. [`INFO FOR INDEX`](/docs/reference/query-language/statements/info#vector-index-compaction) reports this as `building.compacting`, which is separate from `building.status`.
...
<SurrealistMini orientation="horizontal" query="CREATE pts:3 SET point = [8,9,10,11];
...
Brute Force currently supports [Euclidean](/docs/reference/query-language/functions/database-functions/vector#vectordistanceeuclidean), [Cosine](/docs/reference/query-language/functions/database-functions/vector#vectorsimilaritycosine), [Manhattan](/docs/reference/query-language/functions/database-functions/vector#vectordistancemanhattan) and [Minkowski](/docs/reference/query-language/functions/database-functions/vector#vectordistanceminkowski) distance functions.

In the example below, the query searches for points closest to the vector `[2,3,4,5]` and uses [vector functions](/docs/reference/query-language/functions/database-functions/vector) to calculate the distance between two points, indicated by `<|2|>`.

<SurrealistMini orientation="horizontal" query="CREATE pts:1 SET point = [1,2,3,4];
...
The [`EXPLAIN` clause](/docs/reference/query-language/statements/select#the-explain-clause) from SurrealQL helps you understand the execution plan of the query and provides transparency around index utilization.
...
Indexes can be rebuilt using the [`REBUILD`](/docs/reference/query-language/statements/rebuild) statement. This can be useful when you want to update the index definition or when you want to rebuild the index to optimise performance.
...
DEFINE INDEX title_index ON blog FIELDS title FULLTEXT ANALYZER simple BM25(1.2,0.75) HIGHLIGHTS DEFER;
```
