...
> [!NOTE]
> After a large insert or update, a vector index keeps the new records to one side until a background process folds them into its graph. Searches stay correct throughout, but are slower while that backlog is outstanding. [`INFO FOR INDEX`](/docs/reference/query-language/statements/info#vector-index-compaction) reports this as `building.compacting`, which is separate from `building.status`.
