...
<img src="~/assets/img/image/recursive_star.png" alt="An image of a seven-pointed star created visually by relating seven records to each other and displayed inside SurrealDB Studio's graph view." />
...
> [!NOTE]
> The examples below use the [`Surreal Deal Store`](/docs/start#surreal-deal-store---there-is-a-lot-in-store-for-you-recommended) dataset.
...
```surql
DEFINE FUNCTION fn::log_access() { CREATE access_log SET at = time::now(); RETURN true; };
...
```surql
DEFINE FUNCTION fn::price_with_tax($price: number) { RETURN $price * 1.2; };
...
DEFINE FUNCTION OVERWRITE fn::price_with_tax($price: number) {
