...
<TabItem label="Bash">
```bash
# /opt/surreal/dicts must contain a subdirectory for every language in use,
# since it replaces the built-in dictionaries rather than adding to them
...
A directory set this way takes precedence over the built-in dictionaries, and it replaces them wholesale rather than adding to them: once it is set, every language is read from it, so a directory holding only `ko-dic` leaves Japanese and Chinese unavailable even though the binary carries them.
