...
The noun `한국어` is now its own token, so it matches independently of the particle attached to it.

Japanese and Chinese need no other tokenizer, since there are no spaces to split on first:
...
Each language is segmented with its own dictionary, which reaches the server one of two ways.

**From disk.** Set `SURREAL_SEGMENT_DICTIONARY_PATH` to a directory holding one subdirectory per dictionary. The names are fixed: `ko-dic` for Korean, `ipadic` for Japanese, `cc-cedict` for Chinese. Only the languages you use need to be present. The directory is read through the same [`SURREAL_FILE_ALLOWLIST`](/docs/reference/cli/surrealdb-cli/environment-variables#file-config) gate as [`mapper()`](#mapperpath), so it must be allowed too:

<Tabs syncKey="shell">

<TabItem label="Bash">
```bash
# /opt/surreal/dicts contains ko-dic/, and ipadic/ and cc-cedict/ if needed
SURREAL_SEGMENT_DICTIONARY_PATH="/opt/surreal/dicts" \
SURREAL_FILE_ALLOWLIST="/opt/surreal/dicts" \
...
$env:SURREAL_FILE_ALLOWLIST = "C:\surreal\dicts"
surreal start --user root --pass secret
```
</TabItem>
</Tabs>

**Built into the binary.** A build may instead embed a dictionary, in which case `segment` works with nothing on disk and no configuration. Dictionaries are large (Korean's is the biggest), so a build embeds only the languages it was compiled for. When `SURREAL_SEGMENT_DICTIONARY_PATH` is set it takes precedence over any embedded dictionary.

If the dictionary for a language can be found neither way, the `DEFINE ANALYZER` statement naming it fails, rather than being accepted and silently indexing unsegmented text.
