...
`BATCH` is how many values a node reserves at once. It trades coordination against gaps, and both ends of that trade are legitimate:

* A **large batch** reserves many values per round trip, so most calls to `sequence::nextval` are served from memory with no coordination at all. The cost is that a node which stops while holding a reserved range leaves the unused tail behind: another node resumes past it, so the sequence skips those values. With `BATCH 1000` and nodes coming and going, gaps of up to 999 are normal.
* **`BATCH 1`** reserves one value at a time, so nothing is ever left behind and the sequence has no gaps. The cost is a round trip to the store for every value, and on a cluster concurrent callers serialise against each other.

Values are always increasing and never repeated whichever you pick — the difference is only whether values may be skipped, and what each one costs.

Pick a large batch for surrogate keys and identifiers, where gaps do not matter and throughput does. Pick `BATCH 1` where every value must be accounted for, such as invoice or document numbering that is required to be gapless. The default of `BATCH 1000` suits the first case.
...
That makes `OVERWRITE` the way to move a sequence to a new position. Here a `START` above every value already in use is recommended:
...
`resume` on a cluster is not necessarily the value the next `sequence::nextval` returns, because a node still holding a batch it reserved earlier serves from that batch, below the bound.

> [!IMPORTANT]
> Read `resume` and reposition the sequence while nothing is drawing from it. The value is a bound on what has been issued *at the moment it is read*: if the sequence is still in use, it is stale by the time the `DEFINE SEQUENCE ... OVERWRITE` is run, and repositioning to a value the sequence has since passed hands those values out again. To resolve this, stop the writers, reposition, and then let them back.
...
> [!IMPORTANT]
> Before v3.2.5, `DEFINE SEQUENCE ... OVERWRITE` does not take effect on a running node: the node keeps serving from the position it held, and the new `START` applies only after a restart. On those versions, restart the instance after repositioning and confirm with `sequence::nextval` before resuming writes.
