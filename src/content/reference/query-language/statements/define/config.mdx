...
`DEFINE CONFIG API` is often used in conjunction with a [flag](/docs/learn/security/authorization/capabilities) or [environment variable](/docs/reference/cli/surrealdb-cli/environment-variables) to disable arbitrary queries, thereby forcing record and anonymous users to interact with the database via API endpoints alone.
...
To set the actual API endpoints and their middleware, a `DEFINE API` statement is used for each endpoint. For more details and examples, see the pages for [`DEFINE API`](/docs/reference/query-language/statements/define/api) and [API functions](/docs/reference/query-language/functions/database-functions/api).

The behaviour of API endpoints can be tested using the `api::invoke` method, or through a regular HTTP call to the endpoint that [includes the namespace and database name](/docs/reference/rest-api/http-protocol#custom-endpoint-at-apinsdbendpoint).
...
A client asks for these defaults by sending a `USE` with neither a namespace nor a database - the SDK methods `use_defaults()` in [Rust](/docs/reference/rust/methods/use-defaults) and the equivalent in the other SDKs. The defaults only fill in what is missing: a session that already selected a namespace, from a token for example, keeps that selection.

## DEFINE CONFIG GRAPHQL

The configuration set using `DEFINE CONFIG GRAPHQL` is required to enable GraphQL functionality in your database, and specifies which tables and functions should be included or excluded from the GraphQL schema.
...
You can specify a list of tables to leave out of the GraphQL schema using the `EXCLUDE` clause. Every other table in the database is included. The list of tables is specified as a comma-separated list without brackets.

```surql
DEFINE CONFIG GRAPHQL TABLES EXCLUDE audit_log, session;
```
...
The `DEFINE CONFIG GRAPHQL` statement enables and configures the GraphQL API in SurrealDB. By specifying which tables and functions are included or excluded, you can fine-tune the GraphQL schema to match your application's needs.
...
- Use GraphQL query features like `limit`, `start`, `order`, and `filter` to control the data returned.
