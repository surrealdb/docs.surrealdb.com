...
position: 19
title: DEFINE USER
description: "A DEFINE USER statement can be used to create system users on SurrealDB."
---
...
- To select the level where you want to create the user, [you may need to select a namespace and/or database](/docs/reference/query-language/statements/use) before you can use the `DEFINE USER` statement for database or namespace tokens.
...
<RailroadDiagram ast='{"type":"Diagram","padding":[10,20,10,20],"children":[{"type":"Sequence","children":[{"type":"Terminal","text":"DEFINE"},{"type":"Terminal","text":"USER"},{"type":"Optional","child":{"type":"Choice","index":1,"children":[{"type":"Terminal","text":"OVERWRITE"},{"type":"Sequence","children":[{"type":"Terminal","text":"IF"},{"type":"Terminal","text":"NOT"},{"type":"Terminal","text":"EXISTS"}]}]}},{"type":"NonTerminal","text":"@name"},{"type":"Terminal","text":"ON"},{"type":"Choice","index":1,"children":[{"type":"Terminal","text":"ROOT"},{"type":"Terminal","text":"NAMESPACE"},{"type":"Terminal","text":"DATABASE"}]},{"type":"Optional","child":{"type":"Choice","index":1,"children":[{"type":"Sequence","children":[{"type":"Terminal","text":"PASSWORD"},{"type":"NonTerminal","text":"@pass"}]},{"type":"Sequence","children":[{"type":"Terminal","text":"PASSHASH"},{"type":"NonTerminal","text":"@hash"}]}]}},{"type":"Optional","child":{"type":"Sequence","children":[{"type":"Terminal","text":"PASSSCRAM"},{"type":"NonTerminal","text":"@verifier"}]}},{"type":"Optional","child":{"type":"Sequence","children":[{"type":"Terminal","text":"ROLES"},{"type":"NonTerminal","text":"@roles"}]}},{"type":"Optional","child":{"type":"Sequence","children":[{"type":"Terminal","text":"DURATION"},{"type":"Choice","index":1,"children":[{"type":"Sequence","children":[{"type":"Terminal","text":"FOR"},{"type":"Terminal","text":"TOKEN"},{"type":"NonTerminal","text":"@duration"},{"type":"Optional","child":{"type":"Terminal","text":","}},{"type":"Optional","child":{"type":"Sequence","children":[{"type":"Terminal","text":"FOR"},{"type":"Terminal","text":"SESSION"},{"type":"NonTerminal","text":"@duration"}]}}]},{"type":"Sequence","children":[{"type":"Terminal","text":"FOR"},{"type":"Terminal","text":"SESSION"},{"type":"NonTerminal","text":"@duration"},{"type":"Optional","child":{"type":"Terminal","text":","}},{"type":"Optional","child":{"type":"Sequence","children":[{"type":"Terminal","text":"FOR"},{"type":"Terminal","text":"TOKEN"},{"type":"NonTerminal","text":"@duration"}]}}]}]}]}},{"type":"Optional","child":{"type":"Sequence","children":[{"type":"Terminal","text":"COMMENT"},{"type":"NonTerminal","text":"@string"}]}}]}]}' />
...
Note that even a root-level user can be given a limited role such as `VIEWER`. This can be useful for automated services that need to monitor each namespace and database, or for colleagues in non-technical roles.
...
| `PASSSCRAM '…'` | Set by `PASSWORD` or `PASSHASH`, if given | Stored as given, in place of one derived from `PASSWORD` |

`ALTER USER … PASSWORD '…'` regenerates both the Argon2 hash and the SCRAM verifier. `ALTER USER … PASSHASH '…'` updates the hash and **clears** any existing SCRAM verifier, unless the same statement also sets one with `PASSSCRAM`.

Users created before this release, or defined with **`PASSHASH` only**, have no SCRAM material until you set a plaintext password or import a verifier with `PASSSCRAM`. Those users can still authenticate on the Postgres port via **cleartext password** fallback (prefer TLS in production).

### Importing a SCRAM verifier with `PASSSCRAM`

The `PASSSCRAM` clause stores a precomputed SCRAM-SHA-256 verifier, so a user keeps authenticating over SCRAM without the plaintext password being known. It is usually combined with `PASSHASH`, which carries the Argon2 hash in the same way. Both values below were derived from the password `secret`:

```surql
DEFINE USER reporting ON DATABASE
	PASSHASH '$argon2id$v=19$m=19456,t=2,p=1$1JHc8xcjlKQj3BFOY9aLCA$9FlIWQO6wPfQrwVElnWRyU9KBzckbB2EnGOLfGUEzHo'
	PASSSCRAM 'SCRAM-SHA-256$4096:W5weR6PS8IhuFMeyCTr10Q==$eoonh/q2pSTydVV3MZHbqeIlCKGKFOGQSiH7IwjeFpQ=:pYX8KKM1puBfPKyERDxRJ9fYjPxKBXw0Q9elBlpngRQ='
	ROLES VIEWER;
```

The verifier uses the format PostgreSQL stores for SCRAM-SHA-256 passwords, `SCRAM-SHA-256$<iterations>:<salt>$<StoredKey>:<ServerKey>`, where the salt and both keys are base64-encoded. The iteration count must be between 4096 and 16,777,216, the salt must not be empty, and each key must decode to 32 bytes. A verifier that does not meet these rules is rejected when the statement is parsed.

This is the clause that makes users round-trip through an [export](/docs/reference/cli/surrealdb-cli/commands/export): the export writes each user's stored verifier as `PASSSCRAM` next to `PASSHASH`, so an import restores both. [`INFO`](/docs/reference/query-language/statements/info) statements show it as `PASSSCRAM '[REDACTED]'`, in the same way as the hash.
