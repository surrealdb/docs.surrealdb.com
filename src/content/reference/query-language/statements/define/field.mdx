...
A `|` vertical bar can be used to allow a field to be one of a set of types. The following example shows a field that can be a [`UUID`](/docs/reference/query-language/language-primitives/data-types/uuidsss) or an [`int`](/docs/reference/query-language/language-primitives/data-types/numbers#integer-numbers), perhaps for `user` records that have varying data due to two diffent legacy ID types.
...
One more example of a literal containing settings for a [full text search](/docs/learn/data-models/full-text-search/using-as-full-text-search-database) filter:
