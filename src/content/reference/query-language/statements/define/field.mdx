...
The check covers the whole expression, including subqueries, blocks and closures, and it follows calls to [custom functions](/docs/reference/query-language/statements/define/function). A body that calls a function which writes is refused, and the error names the function.
...
A write that cannot be resolved when the field is defined - one reached through [`eval::surql()`](/docs/reference/query-language/functions/database-functions/eval#evalsurql), a JavaScript function, or a closure that arrives as data - is still accepted at definition time. The write is refused when the field is read.

> [!NOTE]
> These checks are relaxed under `OPTION IMPORT`, so an [export](/docs/reference/cli/surrealdb-cli/commands/export) taken before the rules existed still restores.
