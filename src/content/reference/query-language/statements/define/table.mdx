...
> [!NOTE]
> The above behaviour changed in 3.3.0. Earlier versions also tried to decide at `DEFINE`/`ALTER` time whether a clause could ever reach a write, by resolving called functions against the catalog. That answer depended on which branch a call's arguments selected, so a function that conditionally wrote was refused for every caller, including guards that never took that branch. The definition-time refusals were removed and the runtime check is now the whole of the control. A guard that was refused on 3.2 is stored on 3.3 and fails when it is actually evaluated.
