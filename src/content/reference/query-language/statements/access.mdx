...
The `GRANT` clause creates and returns a grant for a certain subject using the specified access method. This subject can be a [system user](/docs/learn/security/authentication/authentication#system-users) or [record user](/docs/learn/security/authentication/authentication#record-users). Access grants can be used to access SurrealDB as that subject until they become expired or revoked.
