...
By default, the `ACCESS` statement will default to referencing access methods defined at the current level specified with the [`USE`](/docs/reference/query-language/statements/use) statement. As with other statements, access methods defined at any level can be referenced by using the `ON` clause.

Operations that either create, revoke or purge access grants using the `ACCESS` statement will be logged in the server as long as it is running with the `INFO` level (the default) or any higher verbosity level. These log lines carry a target ending in `statements::access`, such as `surrealdb_core::expr::statements::access`.
...
The `GRANT` clause creates and returns a grant for a certain subject using the specified access method. This subject can be a [system user](/docs/learn/security/authentication/users#system-users) or [record user](/docs/learn/security/authentication/users#record-users). Access grants can be used to access SurrealDB as that subject until they become expired or revoked.
...
Since the `subject` attribute of grants associated with a record is a record identifier, it can be used as a [record link](/docs/reference/query-language/language-primitives/record-links) in order to access any record fields. This can be used to filter grants associated with record users matching certain conditions based on arbitrary data.
...
To improve the output of an error message inside an `ACCESS` statement, a [`THROW`](/docs/reference/query-language/statements/throw) statement can be used.
...
As the `THROW` statement checks for the email address `me@me.com`, this can be tested using CURL at the [`signup`](/docs/reference/rest-api/http-protocol#signup) endpoint via the following command.
