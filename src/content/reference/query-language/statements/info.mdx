...
`status: 'ready'` means the index is built and queries may use it. `compacting` means it still holds record updates that have not been folded into its structure yet.

This matters for [HNSW and DISKANN](/docs/reference/query-language/statements/define/indexes#vector-search-indexes) indexes. A vector index answers a k-nearest-neighbour search over its graph **plus** any updates not yet merged into that graph, and those are compared one by one rather than navigated. While enough of them are outstanding, a query returns the same results but takes considerably longer. A background process folds them in, after which the field reads `false`.

So after a large insert or update into a vector-indexed table, `status: 'ready'` on its own does not mean the index has returned to full speed:
...
Note that `pending` reads `0` here. `pending` counts writes that arrived *while* a `CONCURRENTLY` build was running, which is a different queue — it does not report work left to compact.

To wait for a vector index to reach full query speed, poll until both conditions hold:
