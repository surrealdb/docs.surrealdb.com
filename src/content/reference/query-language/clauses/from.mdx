...
The `ONLY` keyword can be used to specify that only a single target should be retrieved as a single value instead of inside of an array. This is useful when you want to retrieve data from a single table or view. The `ONLY` keyword can be used in conjunction with the `LIMIT 1` clause to specify that only the specified value should be retrieved. 
