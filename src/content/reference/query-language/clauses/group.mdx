...
A projection made entirely of bare zero-argument [`count()`](/docs/reference/query-language/functions/database-functions/count) implies `GROUP ALL`, so `SELECT count() FROM person` and `SELECT count() FROM person GROUP ALL` are equivalent. See [Bare `count()` implies `GROUP ALL`](/docs/reference/query-language/statements/select#bare-count-implies-group-all) for the cases that stay per-row (`count(field)`, `*`, `SELECT VALUE`, `SPLIT`, and so on).
