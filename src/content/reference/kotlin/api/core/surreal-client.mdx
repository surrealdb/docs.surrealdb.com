...
The `SurrealClient` class is the main entry point for the Kotlin SDK. It connects to a SurrealDB instance, authenticates, queries, and manages data. It extends [`SurrealSession`](/docs/reference/kotlin/api/core/session) and implements `AutoCloseable`. Almost every networked method is a `suspend` function and has a `...Result` companion that returns a [`Result`](https://kotlinlang.org/api/core/kotlin-stdlib/kotlin/-result/) instead of throwing.
...
Creates a new client from a [`SurrealClientConfig`](/docs/reference/kotlin/api/core/client-config). With `autoConnect = true` (the default) the client connects lazily on the first request.
...
Returns whether the current transport supports the given [`SurrealFeature`](/docs/reference/kotlin/api/features#feature).
...
There is also an overload that accepts a `BoundQuery` built with the [`surql`](/docs/reference/kotlin/api/core/query-builder#surql) DSL.

### `.queryAs<T>(sql, vars)` {#query-as}

Runs SurrealQL and decodes the result into `T` using [`kotlinx.serialization`](/docs/reference/kotlin/concepts/serialization).
...
Decodes a [`JsonElement`](/docs/reference/kotlin/api/values/value) into `T` using the client's configured `Json` instance.
...
These methods return a [query builder](/docs/reference/kotlin/api/core/query-builder) that you refine and terminate with `await()` or [`awaitAs<T>()`](/docs/reference/kotlin/api/core/query-builder#await-as). The `what` argument accepts a [`Table`](/docs/reference/kotlin/api/values/table), [`RecordId`](/docs/reference/kotlin/api/values/record-id), or [`RecordIdRange`](/docs/reference/kotlin/api/values/record-id-range).

### `.select(what)` {#select}

Returns a [`SelectQuery`](/docs/reference/kotlin/api/core/query-builder#select-query) for reading records.
...
Returns a [`CreateQuery`](/docs/reference/kotlin/api/core/query-builder#content-builders) for inserting a record.
...
Returns an `InsertQuery` that inserts one or more records into a [`Table`](/docs/reference/kotlin/api/values/table).
...
Returns an `InsertRelationQuery` that inserts relation records into a [`Table`](/docs/reference/kotlin/api/values/table).
...
Starts a [live query](/docs/reference/kotlin/concepts/live-queries) and returns a [`LiveQuerySubscription`](/docs/reference/kotlin/api/core/live-subscription). **WebSocket only.**
...
Creates a new isolated [`SurrealSession`](/docs/reference/kotlin/api/core/session) over the same connection. **WebSocket only.**
...
- [Client configuration](/docs/reference/kotlin/api/core/client-config)
- [Session](/docs/reference/kotlin/api/core/session)
- [Query builder](/docs/reference/kotlin/api/core/query-builder)
- [Features and events](/docs/reference/kotlin/api/features)
- [Errors](/docs/reference/kotlin/api/errors)
