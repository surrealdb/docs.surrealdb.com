...
This page covers patterns for using SurrealDB when query time matters, when the database needs to react to changes on its own, or when you want to query aggregated data that is updated automatically over time.
...
To see what makes an array special in a case like this, let's compare it with a more general approach that holds the time and location inside regular fields instead of the ID itself. We'll call this table `weather2` to tell the two apart.
...
As you can see, there is nothing wrong with using an index - and this may very well fit your own use case.
...
To make the output easier to read, we'll do the following. First we'll put the two queries creating the records inside their own scope, and use the [value::chain()](/docs/reference/query-language/functions/database-functions/value#chain) function to grab the output, ignore it, and turn it into a string showing what operation has just been performed. Then we'll do the same for a regular `SELECT` query over the `weather2` data, compared to the record range query for the `weather` data. If you need the query to stop when a step does not look right, use [`.expect()`](/docs/reference/query-language/functions/database-functions/value#valueexpect) on that value instead of `.chain()` to return the original value when your closure returns `true`, otherwise failing with an error.
...
The standard record ID for SurrealDB is twenty characters long and composed of lowercase letters and numbers.
...
Technically you can work around this by setting the ID to a stringified datetime, or an array with a single datetime.
...
Another option is to set the ID of the record to be a ULID or a UUID. Both of these contain the datetime at which they were created, but are always unique and have no chance of collision.
...
To remove the precision from the original datetime, you can use the [`time::floor()`](/docs/reference/query-language/functions/database-functions/time#timefloor) function.
...
If you don't want to make a query every time you want to see the latest updates to a table, you can use a [live query](/docs/reference/query-language/statements/live-select) instead.
...
Here is one example of an event that creates an `alert` whenever a weather condition is set to critical, and also uses an [http function](/docs/reference/query-language/functions/database-functions/http) to let an external service know about it.
...
Note that you will need to use the [--allow-net](/docs/learn/security/authorization/capabilities#network) flag when starting up a SurrealDB instance to allow functions like `http::post()` to work, as they are disabled by default.
...
For a full list of aggregate functions, see [this page](/docs/reference/query-language/functions/database-functions#aggregate-functions).
