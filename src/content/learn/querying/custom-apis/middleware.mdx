...
You can add extra parameters after those two. Their values are supplied when you attach the function to `DEFINE API` (for example passing in `time::now()` to return the time at which a request was processed).
