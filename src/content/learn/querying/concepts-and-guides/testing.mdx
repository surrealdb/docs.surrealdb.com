...
While you are exploring behaviour in SurrealQL, [`.expect()`](/docs/reference/query-language/functions/database-functions/value#valueexpect) (<Since v="v3.1.0" />) on intermediate results or at the end of a method chain makes it obvious which step failed before you wrap the same logic in an SDK test or a transaction with `THROW`.
