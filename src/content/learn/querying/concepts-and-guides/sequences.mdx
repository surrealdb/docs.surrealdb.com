...
Sometimes you need a monotonic number: an order line, a ticket counter, or an audit sequence that always moves forward and never hands the same value to two different writers. SurrealDB sequences are shared, durable generators that provide this on a single node or in a cluster, without needing to construct the logic in application code.
...
If you only need a unique id and ordering is secondary, other patterns (record IDs, ULIDs, hashes) may be simpler. Sequences are the better choice when order and uniqueness are both part of the contract.
...
This produces a number that will always increase by 1, and is rolled back during a failed transaction. As such, if `fn::get_next()` returns the value 4, you can be certain that values 1, 2, and 3 also exist.
...
SurrealDB sequences use a batch idea instead: each node reserves a range of values, hands them out locally, and only talks to shared storage when the range runs out. In practice this reduces coordination traffic under load and as you scale out.
...
* [Sequence functions](/docs/reference/query-language/functions/database-functions/sequence) - `sequence::nextval` and related usage.
