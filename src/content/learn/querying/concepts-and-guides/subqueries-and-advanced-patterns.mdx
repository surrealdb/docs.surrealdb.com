...
1. **`GROUP BY role`** collapses records into one result per role. Non-aggregated fields such as `id` become arrays of the grouped record ids.
...
See also the [`GROUP` clause](/docs/reference/query-language/clauses/group#latest-record-per-group) reference for a shorter summary.
