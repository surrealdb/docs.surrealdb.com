...
A `DELETE` statement will remove the record(s) on first execution, and executing the same statement again will have no effect. A `DELETE` statement returns an empty array by default.
...
A duplicate explicit ID on `INSERT RELATION` returns an error unless you add `ON DUPLICATE KEY UPDATE`. For more, see [Explicit edge record IDs](/docs/reference/query-language/statements/relate#handling-duplicate-edge-record-ids).
