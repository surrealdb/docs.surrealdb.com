...
description: "Tips for working with data types in SurrealDB, including arrays, type safety, typed LET statements, and chaining array functions."
---
...
For general information on these data types, see the [data types](/docs/reference/query-language/language-primitives/data-types) page in the query language documentation.

## Arrays

Working with arrays is one of the most important skills when working with SurrealDB, as [`SELECT`](/docs/reference/query-language/statements/select) statements return an array of values by default unless the `ONLY` keyword is used on an array that contains a single item.
...
Once the error is located, the fix is easier to implement.
...
The [`array::map()`](/docs/reference/query-language/functions/database-functions/array#arraymap) function provides access to each item in an array, allowing an operation to be performed on it before being passed on. Other similar functions can also be used, such as [`array::filter()`](/docs/reference/query-language/functions/database-functions/array#arrayfilter) which passes on an array that only contains the items that return `true` to an expression of your choice.
...
While refining pipelines like this, [`.expect()`](/docs/reference/query-language/functions/database-functions/value#valueexpect) (<Since v="v3.1.0" />) on the result of `.filter()` or `.map()` can confirm the shape you expect before the query continues. For example, `.expect(|$records| $records.len() = 2, "Expected exactly two valid cities")`.
