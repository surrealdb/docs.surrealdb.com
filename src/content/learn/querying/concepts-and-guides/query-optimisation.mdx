...
> [!NOTE]
> An index built by SurrealDB 3.2 keeps answering queries on 3.3 without a rebuild, but it does not record the document IDs that these plans read, so it takes no part in them. An optional [`REBUILD INDEX`](/docs/reference/query-language/statements/rebuild) brings it into these plans. An index defined on 3.3 takes part from the start.
