...
This means that SurrealDB is a general-purpose database optimised for a combination of various workloads such as operational, AI and real-time workloads.
...
Read more about [SurrealDB architecture](/docs/learn/data-models/architecture) and [deployment models](/docs/manage/self-hosted/deployment-models).

## Running SurrealDB

### Using SurrealDB Cloud

The easiest way to deploy and scale up SurrealDB is by using SurrealDB Cloud, which runs, maintains and scales the database for you.

Read more about running SurrealDB [using SurrealDB Cloud ](/cloud).

### Running SurrealDB as a server

When starting the SurrealDB server, use the correct configuration options and settings. For production environments or for performance benchmarking, the `--log` command-line argument or the `SURREAL_LOG` environment variable should be set to `info` (the default option when not specified), `warn`, or `error`.
...
Read more about running SurrealDB as a [single-node server](/docs/running/file-backed) or [multi-node cluster](/docs/running/multi-node).
...
```toml
surrealdb = { version = "3", features = ["allocator", "kv-mem", "kv-surrealkv", "kv-rocksdb", "protocol-http", "protocol-ws", "rustls"] }
```
...
Read more about [running SurrealDB embedded in Rust.](/docs/build/embedding/by-language/rust)
...
SurrealDB has native built-in support for a number of different index types, without relying on external libraries or implementations.

With native support for indexes in the core database engine, SurrealDB uses indexes where possible within the SurrealQL query language, without pushing queries down to a separate indexing engine. 
...
`UPDATE`, `UPSERT` and `DELETE` statements with a `WHERE` clause use indexes in the same way as `SELECT`, so the condition can go directly on the statement. Adding `EXPLAIN` shows whether an index was chosen:

```surql
DEFINE INDEX age ON user FIELDS age;
UPDATE user SET adult = false WHERE age < 18 EXPLAIN;
```
...
[
	{
		detail: {
			plan: {
				direction: 'forward',
				from: { inclusive: false, value: NONE },
				index: 'age',
				to: { inclusive: false, value: 18 }
			},
			table: 'user'
		},
...
> [!NOTE]
> Before SurrealDB 2.3.0, only `SELECT` statements used indexes. On those versions, wrap the condition in a subquery that selects only the `id` field, such as `UPDATE (SELECT id FROM user WHERE age < 18) SET adult = false`, so that the `SELECT` can use the index.

### Index strategies explained

When using `SELECT`, `UPDATE`, `UPSERT` or `DELETE`, SurrealDB uses a query planner whose role is to identify if it can use the index to speed the execution of the query.
...
An `UPDATE` with a `WHERE` clause on the indexed field also uses the unique index, so both statements below go directly to Bob's record without a table scan. The difference is what happens when no record matches: `UPDATE` changes nothing, while `UPSERT` creates a new record.
...
-- Uses email_index to find the record
UPDATE user SET name = "Bob Bobson" WHERE email = "bob@bob.com";

-- Also uses email_index, and creates the record if no user has this email
UPSERT user SET name = "Bob Bobson", email = "bob@bob.com";
```

> [!NOTE]
> Before SurrealDB 2.3.0, `UPDATE ... WHERE` did not use indexes and performed a full table scan. On those versions, `UPSERT` is the faster way to modify a single record identified by a unique field.

### Index lookup on remote fields

SurrealDB document record IDs store both the table name and the record identifier. This design provides a straightforward and consistent way to reference records across the database. It also makes it possible to filter a table based on conditions that relate to a referenced table.

Here is a concrete example, where the statement `SELECT * FROM access WHERE user.role = 'admin'` will retrieve records from the `access` table for which the referenced record in the `user` table has the `role` field set to 'admin'.
...
To optimise this query, you can create indexes on both the `user.role` field and the `access.user` field. With these indexes, the query planner can use an index-based join strategy:
