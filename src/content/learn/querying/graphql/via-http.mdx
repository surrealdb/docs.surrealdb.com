...
SurrealDB provides an HTTP API for interacting with the database programmatically. Its RESTful interface can be used for a wide range of database operations, from querying data to modifying records and managing database structures.

## Starting a new connection

Before you can start making queries, you need to start SurrealDB with the GraphQL module enabled. You can do this by starting a new instance of SurrealDB with the [`surreal start` command](/docs/reference/cli/surrealdb-cli/commands/start).
...
curl -X POST -u "root:secret" -H "Surreal-NS: main" -H "Surreal-DB: main" -H "Accept: application/json" \
  -d '{ "query": "query { persons { name } }" }' http://localhost:8000/graphql
```

```json title="Response"
{"data":{"persons":[{"name":"Marcus"},{"name":"Simon"}]}}
```
