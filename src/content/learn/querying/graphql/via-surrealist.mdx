...
Learn more about starting a connection in the [Surrealist documentation](/docs/explore/surrealist).
...
Additionally, you can start [a serving in Surrealist](/docs/explore/surrealist) which also enables GraphQL automatically, starting a server on `http://localhost:8000` by default for a root user with username and password `root`.

<img src="~/assets/img/image/surrealist/connection.png" alt="Surrealist connection settings" />

### Preparing your database

Next, use the [SurrealQL query editor](/docs/explore/surrealist) to create some data. For example, you can create a new `user` table with fields for `firstName`, `lastName`, and `email` and add a new user to the database.
...
After you have created some data, you can start writing GraphQL queries. You can use the [Surrealist GraphQL editor](/docs/explore/surrealist) to write your GraphQL queries.
