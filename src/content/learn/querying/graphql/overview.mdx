...
> [!NOTE]
> **GraphQL is not GQL.** [ISO GQL (Cypher Query Language)](/docs/learn/querying/gql/overview) (`MATCH … RETURN …`) is a separate language on [`POST /gql`](/docs/reference/rest-api/http-protocol#gql). Do not abbreviate GraphQL to `gql` - in SurrealDB, `gql` always means ISO GQL (Cypher Query Language).
