...
To get started, [install the SurrealDB CLI](/docs/reference/cli/surrealdb-cli/overview) on your local machine.

## Getting started

After installing the SurrealDB CLI, you can start writing SurrealQL queries by running the [`surreal start`](/docs/reference/cli/surrealdb-cli/commands/start) command in your terminal. You can also add the `--help` flag to view the available options and commands.
...
To run a SurrealQL query, open up a new terminal window and run the [`surreal sql`](/docs/reference/cli/surrealdb-cli/commands/sql) command. You will now be connected to the server and able to follow up with a query. For example, to run a simple `SELECT` query, you can run the following command:
