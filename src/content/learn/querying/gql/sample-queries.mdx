...
| **REPL** | [`eval::gql("…")`](/docs/reference/query-language/functions/database-functions/eval#evalgql) in `surreal sql` or Surrealist — also needs [`--allow-eval-query`](/docs/learn/security/authorization/capabilities#eval-queries) on the process that runs the engine (embedded `surreal sql`, or `surreal start` for remote) |
