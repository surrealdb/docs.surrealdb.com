...
SurrealDB exposes two different graph query languages on **separate** endpoints. Do not abbreviate [GraphQL](/docs/learn/querying/graphql/overview) to `gql` in product identifiers - the short name **`gql`** refers to ISO GQL (Cypher Query Language) only.
...
| Syntax | `MATCH (a)-[:knows]->(b) RETURN …` | `query { persons { name } }` |
