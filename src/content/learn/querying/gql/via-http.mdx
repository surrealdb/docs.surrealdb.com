...
If you prefer the CLI or Surrealist over cURL, run GQL through [`eval::gql`](/docs/reference/query-language/functions/database-functions/eval#evalgql) inside SurrealQL. You still need [`--allow-eval-query`](/docs/learn/security/authorization/capabilities#eval-queries), as `eval::*` is denied by default even under `--allow-all`. From 3.3.0 you do not need `--allow-experimental gql` as well (required on 3.2.x only).
