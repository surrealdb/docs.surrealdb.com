...
description: "Copy-paste starter schemas for energy, finance, retail, medical, and other domains."
---

# Sample schemas by industry

These snippets are starting points that are realistic enough to learn from, short enough to paste into SurrealDB Studio or to lift into [SurrealKit](/docs/manage/schema-migration) `.surql` schema files and then reshape. They mix **tables**, **relations**, **computed** fields, **events**, and **indexes** the way a real app might.

> [!NOTE]
> Many fields use [`COMPUTED`](/docs/learn/schema-management/computed-data/computed-fields) (SurrealDB 3.0.0 onward). On older versions, replace with a [`future`](/docs/reference/query-language/language-primitives/data-types/futures) and `VALUE { … }` as in the futures documentation.
...
A project management schema that demonstrates activity scheduling, milestone tracking, and dependency management using [graph relationships](/docs/learn/data-models/graph/overview). This schema shows how to model complex project workflows with interdependent tasks and progress tracking using [`COMPUTED` fields](/docs/learn/schema-management/computed-data/computed-fields) for calculated values.
...
Industrial monitoring and control system schema for oil and gas operations. Demonstrates real-time sensor data collection, automated alert generation using [events](/docs/reference/query-language/statements/define/event), and [time-series data management](/docs/learn/data-models/time-series/overview) with composite keys. Shows how to handle flexible external data integration and [live query monitoring](/docs/reference/query-language/statements/live-select).
...
Vendor relationship and contract lifecycle management schema. Covers contract value tracking with change orders, deliverable management, and automated total commitment calculations using [`COMPUTED` fields](/docs/learn/schema-management/computed-data/computed-fields). Demonstrates complex financial calculations and status tracking across multiple related entities.
...
SAP payment terms are master data (arbitrary net days, cash discounts, calendar rules). Here you can store a normalised **`net_due`** [`duration`](/docs/reference/query-language/language-primitives/data-types/durations) for queries, and treat **`code`** as the authoritative key from SAP. Document-number **`ASSERT`** patterns below are deliberately generic (ten-digit numeric). Assertions on prefixes (for example that PO ranges must start with `45`) can be tightened once your connector documents a specific number range.

Demonstrates [literal unions](/docs/reference/query-language/language-primitives/data-types/literals), [`duration`](/docs/reference/query-language/language-primitives/data-types/durations), [graph relations](/docs/reference/query-language/statements/relate) with [`ENFORCED`](/docs/reference/query-language/statements/define/table#using-enforced-to-ensure-that-related-records-exist) endpoints, [record references](/docs/reference/query-language/language-primitives/record-references), [field assertions](/docs/reference/query-language/statements/define/field#asserting-rules-on-fields) on relation `in` / `out`, and [events](/docs/reference/query-language/statements/define/event) for lifecycle rules.
...
Incident reporting and investigation schema using [graph relationships](/docs/learn/data-models/graph/overview). Models safety events as edges between employees and projects with severity classification and role identification. Demonstrates graph-style data modelling for incident tracking and analysis.
...
Multi-currency banking system using [graph relationships](/docs/learn/data-models/graph/overview). Demonstrates polymorphic account types (JPY, EUR, CAD, USD) with different field structures, customer-bank relationships, and [unique constraint enforcement](/docs/reference/query-language/statements/define/indexes#unique-index). Shows how to model financial relationships with type-specific behaviours.
...
Traditional bank-customer schema with advanced features including [record references](/docs/reference/query-language/language-primitives/record-links#record-references), automated cent handling through [events](/docs/reference/query-language/statements/define/event), and historical interest rate tracking. Demonstrates event-driven data validation, [parameter usage](/docs/reference/query-language/statements/define/param), and complex relationship management with reference fields.
...
Secure money transfer system with credit-based limits and transaction logging. Features [custom functions](/docs/reference/query-language/statements/define/function) for atomic transfers, credit level enforcement, and audit trails. Demonstrates transaction safety, business rule enforcement, and financial data integrity.
...
RPG game system with character progression, inventory management, and quest tracking. Features polymorphic item effects, character statistics, and complex game state management. Demonstrates flexible data modelling for gaming applications with rich object structures and [relationship tracking](/docs/learn/data-models/graph/overview).
...
Astronomical observation tracking system with instrument management and data collection. Features geospatial telescope locations, flexible observation metadata, and scientific data URL management. Demonstrates [point data types](/docs/reference/query-language/language-primitives/data-types/geometries#point), complex temporal relationships, and scientific data organisation patterns.
...
Space launch monitoring system with real-time telemetry data collection. Features component-level tracking, [time-series data management](/docs/learn/data-models/time-series/overview) with composite keys, and launch lifecycle status tracking. Demonstrates high-frequency data ingestion, temporal range queries, and [live data streaming](/docs/reference/query-language/statements/live-select).
...
Military mission management system with unit tracking and operational logging. Features hierarchical command structure, real-time status updates, and audit trails. Demonstrates organisational modelling and [geospatial tracking](/docs/reference/query-language/language-primitives/data-types/geometries#point).
...
E-commerce platform schema with customer profiles, product catalogue, and shopping cart management. Features flexible address storage, multi-currency support, and timestamp tracking. Demonstrates e-commerce data modelling with [flexible object fields](/docs/reference/query-language/language-primitives/data-types/objects#flexible-objects) and relationship management.
...
Healthcare management system with patient records, encounter tracking, and clinical data management. Features vital signs [time-series data](/docs/learn/data-models/time-series/overview), medication tracking, and automated encounter lifecycle management using [events](/docs/reference/query-language/statements/define/event). Demonstrates healthcare data modelling with temporal data, clinical workflows, and medical record compliance patterns.
...
- [DEFINE FUNCTION](/docs/reference/query-language/statements/define/function)
- [DEFINE EVENT](/docs/reference/query-language/statements/define/event)
- [DEFINE PARAM](/docs/reference/query-language/statements/define/param)
