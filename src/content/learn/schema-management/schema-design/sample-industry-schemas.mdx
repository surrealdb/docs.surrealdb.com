...
> [!NOTE]
> Many fields in these schemas use [`COMPUTED` fields](/docs/reference/query-language/statements/define/field#computed-fields), available as of SurrealDB version 3.0.0-beta. For versions before this, a data type called a [`future`](/docs/reference/query-language/language-primitives/data-types/futures) was used. To use such fields in a version before 3.0.0-beta, replace `COMPUTED` with `VALUE <future>` and enter an expression to be calculated inside `{}` braces.
...
A comprehensive project management schema that demonstrates activity scheduling, milestone tracking, and dependency management using [graph relationships](/docs/learn/data-models/graph/using-as-graph-database). This schema shows how to model complex project workflows with interdependent tasks and progress tracking using [futures](/docs/reference/query-language/statements/define/field#futures) for calculated fields.
...
Incident reporting and investigation schema using [graph relationships](/docs/learn/data-models/graph/using-as-graph-database). Models safety events as edges between employees and projects with severity classification and role identification. Demonstrates graph-style data modeling for complex incident tracking and analysis.
...
Multi-currency banking system using [graph relationships](/docs/learn/data-models/graph/using-as-graph-database). Demonstrates polymorphic account types (JPY, EUR, CAD, USD) with different field structures, customer-bank relationships, and [unique constraint enforcement](/docs/reference/query-language/statements/define/indexes#unique-indexes). Shows how to model complex financial relationships with type-specific behaviors.
...
RPG game system with character progression, inventory management, and quest tracking. Features polymorphic item effects, character statistics, and complex game state management. Demonstrates flexible data modeling for gaming applications with rich object structures and [relationship tracking](/docs/learn/data-models/graph/using-as-graph-database).
...
Space launch monitoring system with real-time telemetry data collection. Features component-level tracking, [time-series data management](/docs/learn/data-models/time-series/using-as-time-series-database) with composite keys, and launch lifecycle status tracking. Demonstrates high-frequency data ingestion, temporal range queries, and [live data streaming](/docs/reference/query-language/statements/live-select).
