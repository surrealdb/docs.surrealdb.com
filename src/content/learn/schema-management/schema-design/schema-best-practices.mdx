...
When those definitions live in source control and need to roll out safely to shared databases, follow [SurrealKit schema migration](/docs/manage/schema-migration) in the **Manage** section for sync, rollouts, testing in CI, and related tooling.

## Define arrays and sets with a type and length

In addition to a type, both arrays and sets can have a required number of items built into the type definition itself. A value with more or fewer items than the stated length is rejected. The definition below pairs this with an assertion using the [`array::all()`](/docs/reference/query-language/functions/database-functions/array#arrayall) function to also ensure that every item in the `small_bytes` field is between 0 and 255.
...
> [!NOTE]
> Before SurrealDB 3.0.0, the number in `array<int, 8>` was a maximum length, so an array with fewer items passed. From 3.0.0 it is an exact length.

Learn more about [database functions](/docs/reference/query-language/functions/database-functions).
...
CREATE colour SET rgb = [0, 2];
-- Fails: must be between 0 and 255
CREATE colour SET rgb = [0, 2, 400];
```

[Learn more about assertions in `DEFINE FIELD`](/docs/reference/query-language/statements/define/field#asserting-rules-on-fields)
...
Inside a schemaless table, the only time an object will be schemafull is if the structure of the object is indicated using the [literal](/docs/reference/query-language/language-primitives/data-types/literals) syntax.
...
Not bad! The error message now includes the value and the part of it that is acceptable:
...
That doesn't mean that you have to give up the precision of a `datetime` though. By using the [`time::format()`](/docs/reference/query-language/language-primitives/formatters) function, you can keep the actual stored date as a precise SurrealQL `datetime` and then use that to output a string in any format you like.
...
If you find that parts of your table- or field-specific code are getting a bit long, it might be time to think about moving parts of it to a [database-wide parameter](/docs/reference/query-language/statements/define/param).
...
Doing so makes the code cleaner and easier to reuse in other parts of the schema.
...
Error types in programming languages often take the form of a long list of possible things that could go wrong. SurrealQL's [literal](/docs/reference/query-language/language-primitives/data-types/literals) type allows you to specify a list of all possible forms it could take, which makes it well suited to error logic.
