...
To fail the query when the pipeline does not meet your assumptions (for example while debugging), end the chain with [`.expect()`](/docs/reference/query-language/functions/database-functions/value#valueexpect) instead of casting to a string:
...
These anonymous functions provide a flexible way to define small, reusable pieces of logic that can be used throughout your queries. Using them keeps SurrealQL code modular and easier to maintain.
