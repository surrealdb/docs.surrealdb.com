...
With the above definition, the `primary` field will be set to `123.456` when a new `product` is created without a value for the `primary` field or with a value of `NONE`. When an existing `product` is updated with a value specified, the result will be the new value.
...
The `VALUE` clause differs from `DEFAULT`. A default value is only calculated if no other value is given, and otherwise the value given in a query is accepted.
...
To create a field that is calculated each time it is accessed, a [`computed field`](/docs/reference/query-language/statements/define/field#restrictions-on-computed-fields) can be used.
