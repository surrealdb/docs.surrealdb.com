...
The following sections cover what to consider when deciding what sort of record ID format to use.
...
As the output from the `SELECT` statements shows, a `WHERE` clause is needed to find two users starting at a `num` of 50, as `START 50` starts based on the order of the record IDs, which are entirely random.
...
The `START 50 LIMIT 2` query is more performant, and the entire `num` field could be removed if its only use is to return records by order of creation.
...
Take the following `event` records which can be queried as a performant record range.
...
While SurrealDB does not use auto-incrementing IDs by default, this behaviour can be achieved in a number of ways. From SurrealDB 3.0.0, the shortest is a [sequence](/docs/reference/query-language/statements/define/sequence), whose [`sequence::nextval()`](/docs/reference/query-language/functions/database-functions/sequence) function returns the next number each time it is called.

```surql
DEFINE SEQUENCE person_id START 1;
CREATE type::record("person", sequence::nextval("person_id"));
CREATE type::record("person", sequence::nextval("person_id"));
```
...
A sequence value is not returned when the transaction that took it fails, so the IDs it produces are unique and increasing but can have gaps.

Another way is to use the [`record::id()`](/docs/reference/query-language/functions/database-functions/record#recordid) function on the latest record, which returns the latter part of a record ID (the '1' in the record ID `person:1`). This can then be followed up with the [`type::record()`](/docs/reference/query-language/functions/database-functions/type#typerecord) function to create a new record ID.
