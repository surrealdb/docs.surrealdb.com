...
> [!IMPORTANT]
> Capabilities are configured per instance from **Instance settings** in [SurrealDB Studio](https://studio.surrealdb.com). See [Configure an instance](/docs/manage/instances/configure) for the available settings.
...
You can learn more about best practices when using capabilities in our [Security Best Practices](/docs/learn/security/best-practices/security-best-practices#capabilities) guide.
...
Capability flags on [`surreal start`](/docs/reference/cli/surrealdb-cli/commands/start) configure the **running database instance**. That is what enforces permissions when clients connect over HTTP, WebSocket, RPC, or [`surreal sql`](/docs/reference/cli/surrealdb-cli/commands/sql) to a remote endpoint.

[`surreal sql`](/docs/reference/cli/surrealdb-cli/commands/sql) accepts the same flags, but they apply differently:
...
When using a remote server, flags on `surreal sql` do **not** enable or disable execution-time checks such as [`eval::*`](/docs/reference/query-language/functions/database-functions/eval), arbitrary-query gates, or experimental features. They can still affect **REPL line parsing** before a query is sent.

See [Capabilities and remote connections](/docs/reference/cli/surrealdb-cli/commands/sql#capabilities-and-remote-connections) for detail.
...
<table>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td colspan="2" scope="row" data-label="Description">
                Allow the usage of one or more experimental features. Possible values are `files` and `surrealism`, separated by a comma. See <a href="/docs/reference/cli/surrealdb-cli/commands/start#experimental-capabilities">experimental capabilities</a> for which tag enables each feature. (The legacy tag <code>gql</code> is still accepted for compatibility but is unused from 3.3.0 - GQL is on by default.)
            </td>
            <td scope="row" data-label="Default">
                None
            </td>
...
SurrealDB offers [built-in functions](/docs/reference/query-language/functions/database-functions/array) to perform common operations like string manipulation, math, etc. Users can also define [their own functions](/docs/reference/query-language/statements/define/function) with custom logic.
...
SurrealDB can make outbound network connections from [`http::*`](/docs/reference/query-language/functions/database-functions/http) functions and from JWKS fetches used by [JWT access methods](/docs/reference/query-language/statements/define/access/jwt#json-web-key-set-jwks). Use `--allow-net` and `--deny-net` to control which targets those connections may reach.
...
The `--allow-arbitrary-query` and `--deny-arbitrary-query` allow database administrators to allow or deny arbitrary querying by either guest, record or system users, or a combination of those. This capability setting affects the following: [/sql endpoint](/docs/reference/rest-api/http-protocol#sql), [/key endpoints](/docs/reference/rest-api/http-protocol#get-table), [/graphql endpoint](/docs/reference/rest-api/http-protocol#graphql), [/gql endpoint](/docs/reference/rest-api/http-protocol#gql), the [Postgres wire protocol](/docs/reference/rest-api/postgres-protocol) listener, [RPC methods](/docs/reference/rest-api/rpc-protocol) `use`, `select`, `create`, `update`, `merge`, `patch`, `delete`, `relate`, `insert`, `insert_relation`, `query`, `gql`, and `graphql`, and the [`eval::*`](/docs/reference/query-language/functions/database-functions/eval) functions.

Endpoints that do not accept arbitrary queries such as [`/version`](/docs/reference/rest-api/http-protocol#version) and [authentication endpoints](/docs/reference/rest-api/http-protocol#signin) are not affected.

The `--deny-arbitrary-query` flag is often preceded with a [`DEFINE API`](/docs/reference/query-language/statements/define/api) statement to set up certain endpoints that users can use to access database resources in place of arbitrary queries.

## Eval queries

<Since v="v3.2.0" />

The [`eval::surql`](/docs/reference/query-language/functions/database-functions/eval) and [`eval::gql`](/docs/reference/query-language/functions/database-functions/eval) functions evaluate query strings at runtime. They are controlled by **`--allow-eval-query`** and **`--deny-eval-query`**, with subject groups `guest`, `record`, and `system`.
...
Configure `--allow-eval-query` on **`surreal start`** when clients connect to a remote instance. It is not required on `surreal sql` for remote connections - only for [embedded REPL sessions](/docs/reference/cli/surrealdb-cli/commands/sql#capabilities-and-remote-connections).

`eval::gql` does not need an experimental capability from **3.3.0** (on **3.2.x**, also allow `gql`). See [Eval functions](/docs/reference/query-language/functions/database-functions/eval) and [Representations and codecs](/docs/learn/querying/concepts-and-guides/representations-and-codecs).
...
A route name covers every method and path under it, so `key` covers all ten [`/key`](/docs/reference/rest-api/http-protocol#get-table) endpoints and `ml` covers both [`/ml`](/docs/reference/rest-api/http-protocol#ml-import) endpoints.

| Name | Covers |
| --- | --- |
| `health` | [`GET /health`](/docs/reference/rest-api/http-protocol#health) and the gRPC [`Health`](/docs/reference/rest-api/rpc-protocol#grpc-transport) call, but not the `/status` and `/ready` probes |
| `version` | [`GET /version`](/docs/reference/rest-api/http-protocol#version) |
| `import` | [`POST /import`](/docs/reference/rest-api/http-protocol#import) |
| `export` | [`POST /export`](/docs/reference/rest-api/http-protocol#export) |
| `signin` | [`POST /signin`](/docs/reference/rest-api/http-protocol#signin) |
| `signup` | [`POST /signup`](/docs/reference/rest-api/http-protocol#signup) |
| `key` | The [`/key`](/docs/reference/rest-api/http-protocol#get-table) table and record endpoints |
| `sql` | [`POST /sql`](/docs/reference/rest-api/http-protocol#sql) |
| `gql` | [`POST /gql`](/docs/reference/rest-api/http-protocol#gql), and the Postgres `gql` dialect |
| `graphql` | [`POST /graphql`](/docs/reference/rest-api/http-protocol#graphql) |
| `ml` | The [`/ml`](/docs/reference/rest-api/http-protocol#ml-import) import and export endpoints |
| `api` | Endpoints defined with [`DEFINE API`](/docs/reference/query-language/statements/define/api) |
| `mcp` | The [MCP server](/docs/reference/cli/surrealdb-cli/commands/mcp) endpoint |
| `rpc` | The HTTP [`/rpc`](/docs/reference/rest-api/rpc-protocol) endpoint |
| `sync` | The internal sync endpoint |
| `postgres` <Since v="v3.3.0" /> | The [Postgres wire protocol](/docs/reference/rest-api/postgres-protocol) listener |
...
ISO GQL (Cypher Query Language) is on by default from **3.3.0**, and the GQL executor is reachable from four entry surfaces. Each has its own switch, so a deployment that serves no GQL closes each surface it exposes:

| Entry surface | Switch |
| --- | --- |
| The `/gql` endpoint | `--deny-http gql` |
| The `gql` [RPC method](/docs/reference/rest-api/rpc-protocol), over WebSocket, HTTP `/rpc` and gRPC | `--deny-rpc gql` |
| [`eval::gql()`](/docs/reference/query-language/functions/database-functions/eval) inside any SurrealQL | `--deny-eval-query`, which is also the default state. `--deny-funcs` reaches it as well. |
| The [Postgres wire protocol](/docs/reference/rest-api/postgres-protocol) `dialect=gql` <Since v="v3.3.0" /> | `--deny-http gql` |
