...
To address token expiration errors you will either have to ask your end user to authenticate again with credentials to obtain a new token, rely on a persistent authenticated WebSocket session, rely on an external identity provider like [Auth0](/docs/explore/tutorials/tutorials/auth0-integration) or [AWS Cognito](/docs/explore/tutorials/tutorials/aws-cognito-integration) or, for development purposes, test the [experimental refresh token feature](/docs/reference/query-language/statements/define/access/record#with-refresh-token).
...
In that environment, you can add [`.expect()`](/docs/reference/query-language/functions/database-functions/value#valueexpect) (<Since v="v3.1.0" />) on intermediate results in a SurrealQL reproduction to see which step first diverges from what you expect, without changing schema `ASSERT` rules or production `THROW` handlers.
