...
**Example - hourly energy readings:** [`time::floor()`](/docs/reference/query-language/functions/database-functions/time#timefloor) snaps each `ts` down to the start of a duration (here `1h`), and [`GROUP BY`](/docs/reference/query-language/clauses/group) collapses every record in that window into one aggregate.
