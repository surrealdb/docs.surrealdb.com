...
The following sections work through a practical example using IoT sensor data.
...
Storing the same information as a record or as part of the ID might look similar at first glance, but internally it’s optimised for efficient range selection through [range queries](/docs/reference/query-language/language-primitives/data-types/record-ids#record-ranges) on record IDs. As all values in SurrealDB can be [compared and sorted](/docs/reference/query-language/language-primitives/data-types/values#comparing-and-ordering-values), putting the most important information into the record ID itself allows you to query only a range of record IDs instead of a whole table.

This means less effort selecting the right indexes or partitions, since the ID field already orders the data, and a range read keeps the performance of a key-value lookup regardless of size.
...
This is not the only way of doing it. You can also have the metadata in the ID and sensor data in the record itself, like in the example below. Now the record ID only contains the fields useful in a range query, while the remaining fields are kept in the record itself.
...
### IDs inside IDs
The complex record IDs above also contain record IDs of their own. This is known as a [record link](/docs/reference/query-language/language-primitives/record-links), which allows us to reduce the fields in our main time series table to only the necessary ones by offloading most of the metadata to connected tables.
...
Getting the connected data needs no table scan either, because each link points directly to a specific record on a specific table. In the example below, with time for the sensor readings set exactly to the hour (`2024-08-13T05:00:00`), the record ID is easy to predict.
...
The next step is to turn this event data into metrics.
...
Combining drop tables, table views and live queries gives an event-based, real-time solution that is easy to set up and covers capturing events, creating metrics, dropping stale data and live selects for visualisation.

#### Custom events
For something more bespoke, you can [create your own event triggers](/docs/reference/query-language/statements/define/event) based on when a record is created, updated or deleted. You can include any valid SurrealQL inside the event.
