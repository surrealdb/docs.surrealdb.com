...
Unlike in graph-only databases, **edges in SurrealDB are tables**, so you can store fields on them like any other record.
...
Some workflows rely on creating the edge first (for example, a street with predictable house IDs before the houses exist). A [`DEFINE FIELD ... VALUE`](/docs/reference/query-language/statements/define/field) can describe the path from `house` to `street` and resolve once the house exists:
