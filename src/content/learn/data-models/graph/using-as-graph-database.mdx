...
In SurrealDB, nodes are typically just records in a table—like users, posts, companies, etc. SurrealDB uses a statement called [`RELATE`](/docs/reference/query-language/statements/relate) to create this three-part structure. 
...
Since version 3.0.0-beta, a record link can be bidirectional by [defining a field](/docs/reference/query-language/statements/define/field) with the `REFERENCE` clause, allowing referenced records to define a field or use a query to track incoming references.
...
* The [page on idioms](/docs/reference/query-language/language-primitives/idioms)
...
One other advantage to strictly defining a relation table is that this information can be picked up by [Surrealist](/docs/explore/surrealist) to be displayed in its Designer view.
...
To complete this query to ensure that a record's own ID does not show up inside the list of `friends`, the [`array::complement()`](/docs/reference/query-language/functions/database-functions/array#arraycomplement) function can be used.
...
For further details on this pattern, see [this section](/docs/reference/query-language/statements/relate#bidirectional-relation-querying) in the page on the `RELATE` statement and [this section](/learn/book/chapter-07#bidirectional-querying-when-a-relationship-is-equal) of Chapter 7 of Aeon's Surreal Renaissance.
...
A [`computed field`](/docs/reference/query-language/statements/define/field#computed-fields) makes more sense in this case, as a computed field is calculated each time a record is queried, not just whenever it is created or updated.
...
Surrealist has an [Explorer view](/docs/explore/surrealist/concepts/exploring-database-records) that allows users to not just view records and their fields, but also traverse their relations one step at a time. This can be an effective tool to understand the internals of graph edges and queries on them.
...
[Recursive queries](/docs/reference/query-language/language-primitives/idioms#recursive-paths) allow traversal of a path down to a specific depth.
