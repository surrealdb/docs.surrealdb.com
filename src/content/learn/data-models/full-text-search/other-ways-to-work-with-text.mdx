...
The functions [`array::sort_natural()`, `array::sort_lexical()`, and `array::sort_natural_lexical()`](/docs/reference/query-language/functions/database-functions/array) can be used on ad-hoc data to return the same output as the `COLLATE` and `NUMERIC` clauses in a [`SELECT` statement](/docs/reference/query-language/statements/select).
