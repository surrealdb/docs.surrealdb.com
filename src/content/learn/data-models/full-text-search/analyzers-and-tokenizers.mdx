...
These all decide a boundary from the characters around it, which is enough for languages that separate words with spaces or case changes. Chinese, Japanese and Korean need a dictionary instead: `segment(chinese | japanese | korean)`, documented under [DEFINE ANALYZER](/docs/reference/query-language/statements/define/analyzer#segmentlanguage). Korean attaches grammatical particles to the word they follow, so `한국어를` only becomes searchable as `한국어` once it is segmented:
...
The server reads the dictionary from the **host filesystem** when you define the analyzer. Configure [`SURREAL_FILE_ALLOWLIST`](/docs/reference/cli/surrealdb-cli/environment-variables#file-config) so the path lies under an allowed directory, without which no filesystem paths are permitted. See [DEFINE ANALYZER - `mapper(path)`](/docs/reference/query-language/statements/define/analyzer#mapperpath) for startup examples.
