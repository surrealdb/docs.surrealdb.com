...
You may notice that the `id` field has a `users:` prefix. This is because SurrealDB uses a [id](/docs/reference/query-language/language-primitives/data-types/record-ids) to uniquely identify each record and the combination of the table name and the record id is used as the [record link](/docs/reference/query-language/language-primitives/record-links).
...
1. **Data Organization**: Data is contained in self-describing documents, typically in JSON or a similar format. Relationships, can be done within the document itself (embedding) or references using [record links](/docs/reference/query-language/language-primitives/record-links) to other documents.
