...
* A [CLI command](/docs/reference/cli/surrealdb-cli/module) compiles this code into a WASM binary.
* Statements inside SurrealDB to [allow access to the file](/docs/reference/query-language/statements/define/bucket) and to [define a module](/docs/reference/query-language/statements/define/module) holding the functions.

![A flowchart showing the steps involved to turn regular Rust code into a compiled WASM binary that can be accessed from a running SurrealDB instance.](~/assets/img/surrealdb/extensions/surrealism_flow.png)
