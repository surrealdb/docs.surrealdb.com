...
Now it's time to [start the database](/docs/reference/cli/surrealdb-cli/start) with the `surreal start` command and these two env vars.
...
You can then connect through [surrealist](/docs/explore/surrealist) or the CLI with the [surreal sql](/docs/reference/cli/surrealdb-cli/sql) command:
