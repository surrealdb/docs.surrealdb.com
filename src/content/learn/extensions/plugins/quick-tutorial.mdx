...
With this setup done, the Rust code can now be compiled to WASM via the [`module build`](/docs/reference/cli/surrealdb-cli/commands/module) command in the CLI. This is followed with the `--out` flag and the output file name/path (e.g. `demo.surli`). If you are in a separate folder, you can follow this with the path to the Rust code with the functions to expose.
...
You can then connect through [surrealist](/docs/explore/surrealist) or the CLI with the [surreal sql](/docs/reference/cli/surrealdb-cli/commands/sql) command:
...
Then we can [define a module](/docs/reference/query-language/statements/define/module) which will hold all the tests. We'll call it `mod::test` and use `AS` to connect it to the file.
