...
title: Golang | SDK | Error handling
...
The Go SDK uses Go's standard `error` interface and the `errors.As` / `errors.Is` functions for error handling. Errors fall into three main categories: structured [server errors](/docs/sdk/golang/api/errors#servererror) from SurrealDB, per-statement [query errors](/docs/sdk/golang/api/errors#queryerror), and transport-level failures.
...
<table>
	...
	<tbody>
		<tr>
			<td scope="row" data-label="Error type"><a href="/docs/sdk/golang/api/errors#servererror"><code>ServerError</code></a></td>
			<td scope="row" data-label="Description">Structured error from SurrealDB v3 with kind, details, and cause chain</td>
		</tr>
		<tr>
			<td scope="row" data-label="Error type"><a href="/docs/sdk/golang/api/errors#queryerror"><code>QueryError</code></a></td>
			<td scope="row" data-label="Description">Per-statement query failure returned within QueryResult</td>
		</tr>
		<tr>
			<td scope="row" data-label="Error type"><a href="/docs/sdk/golang/api/errors#rpcerror"><code>RPCError</code></a></td>
			<td scope="row" data-label="Description">JSON-RPC error (deprecated in favor of ServerError on v3)</td>
		</tr>
	</tbody>
</table>
...
SurrealDB v3 returns structured errors with a `Kind`, `Message`, `Details`, and an optional `Cause` chain. Use `errors.As` to extract a [`ServerError`](/docs/sdk/golang/api/errors#servererror):
...
When using `Query`, individual statements can fail without causing the entire call to error. Each [`QueryResult`](/docs/sdk/golang/api/types#queryresult) has an `Error` field that contains a [`QueryError`](/docs/sdk/golang/api/errors#queryerror) if that statement failed.
...
The SDK defines several [sentinel errors](/docs/sdk/golang/api/errors#sentinel-errors) for common failure conditions:
...
| `constants.ErrTimeout` | The operation timed out |
...
| `constants.ErrTimeout` | Yes | Transient network or server overload |
| Unmarshal errors | No | Type mismatch between expected and actual response |

## Learn more

- [Errors API reference](/docs/sdk/golang/api/errors) for complete error type definitions
- [Executing queries](/docs/sdk/golang/concepts/executing-queries) for query-level error handling
- [Types reference](/docs/sdk/golang/api/types) for `QueryResult` and `QueryError` definitions
