...
Labels are `"key=value"` strings stamped on the rows the write produces. They narrow reads (`labels` on `/query`) but never widen access - the scope predicate is applied first.
