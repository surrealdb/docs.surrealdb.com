...
This guide maps Mem0 concepts to their SurrealDB Agent Memory equivalents, then describes an incremental migration that can run both systems in parallel.
...
```python
class MemoryAdapter:
    def __init__(self, mem0_client, agent_memory_client):
        self.mem0 = mem0_client
        self.agent_memory = agent_memory_client
    
    async def add(self, content: str, user_id: str):
        # Write to both
        self.mem0.add(content, user_id=user_id)
        async with self.agent_memory.sessions.create(scopes=[f"user/{user_id}"]) as s:
            await s.remember(content)
    
    async def search(self, query: str, user_id: str, use_agent_memory: bool = False):
        if use_agent_memory:
            async with self.agent_memory.sessions.create(scopes=[f"user/{user_id}"]) as s:
...
Compare recall results between the two systems for a sample of production queries. Use the `use_agent_memory=True` flag on a percentage of traffic while monitoring for quality regressions.
