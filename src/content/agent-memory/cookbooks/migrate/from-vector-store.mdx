...
Vector stores often have minimal, inconsistent, or missing metadata. SurrealDB Agent Memory's pipeline extracts structure from content, so missing metadata matters less. If you have source information available, add it to the documents before ingestion.
