...
`persist` is the only routing control: reflect writes back against the entities its own evidence names, and returns them in `persistedAttributes`. There is no `target_entity_type` / `target_attribute_key` - you cannot pin the write to a chosen attribute. The persisted attributes appear in future `profile()` and `context()` calls, so responses include synthesised conclusions as well as raw facts.
