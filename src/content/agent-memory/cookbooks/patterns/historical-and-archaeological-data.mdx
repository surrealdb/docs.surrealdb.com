...
| The same subjects are recorded more than once | The second account disagrees with the first, and both are evidence |
...
## When the same place is recorded twice

Some corpora describe the same subjects more than once. A directory published every year, a parish register kept over decades, an excavation catalogued again by a later team. A second edition is not new material about new subjects. It is a second account of the subjects the first edition already described, and it disagrees with it.

Decide which of two questions you want to answer before you ingest the second edition.

**What changed.** Stamp each edition with its own `observedAt` and ingest as normal. A conflicting value supersedes the earlier one, and the chain links the old value to the new, so the later reading answers by default and the earlier one stays reachable with `asOf`. Use this when the subjects are stable and the change is the finding.

**What each edition attests.** Give each edition its own [dimensional scope](/docs/agent-memory/mental-model/contexts-and-scope) path, so that `1912/house/…` and `1913/house/…` are separate addresses for the same door. A grant of `1913/**` then answers for one edition. A query with no prefix reaches both. Use this when you need each edition counted on its own, or when the two are independent surveys rather than a revision of one.

A scope path that carries no edition makes the two accounts one. `house/1st-avenue-e-110` means a different household in each edition of a directory, so the second ingest reads as a correction to the first. The result stays internally consistent and gives no sign that it describes two years at once. Register the paths you intend with `agent-memory scopes create` before the first ingest.

The Calgary directories show what the second question is for. Both editions state the month they were canvassed, October 1912 and September 1913, and each is a complete canvass of the same city. Held apart, the two answer a question that neither answers alone.

| Counted over the corpus | 1912 | 1913 |
| --- | --- | --- |
| People recorded | 7,499 | 6,995 |
| Doors | 3,140 | 3,247 |
| Keeping their own house | 1,965 | 1,834 |
| In the building trades | 1,241 (17%) | 931 (13%) |
| Carpenters | 435 | 278 |
| Selling land | 130 | 91 |
| Doors marked vacant | 99 | 147 |

The above shows more doors, fewer people in them, and over a third of the carpenters gone. The city's building department reports the same months at $4,170,360 of permits in September 1912 and $362,480 in September 1913, which is the event the table describes.

Two things follow for retrieval.

- **A comparison of editions is an aggregate question.** None of the figures above needs a name, so ask it the way [Set breadth to match the question](#set-breadth-to-match-the-question) describes: fan out `/query` at a raised `k` against each edition's scope, then count in your own code.
- **A subject present in one edition and absent from the other is a finding, not a gap.** A door listed as vacant, or a person the later canvass does not reach, is what the corpus attests. Record the absence in prose so that synthesis can quote it, and label it as [certainty](#label-provenance-and-certainty) rather than leaving it to be read as missing data.
...
- **Page correctly when you list documents.** Follow `page.nextCursor` from `GET /documents?limit=100` until it is absent, rather than stopping on the first short page. See [Pagination](/docs/agent-memory/reference/rest-api#pagination).
