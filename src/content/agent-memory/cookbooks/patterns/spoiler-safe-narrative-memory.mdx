...
The reader and the permissions stay the same, and only the **`asOf`** instant changes.
...
- **Counting is yours.** SurrealDB Agent Memory stores the number you supply. Aliases, epithets, and pronouns are a counting problem to solve before the write. See [Role nouns collect unrelated people](#role-nouns-collect-unrelated-people).
...
## Role nouns collect unrelated people

A narrative refers to somebody by role before it names them, and often never names them at all: `stranger`, `friar`, `the youth`, `old man`, `landlord`. Extraction mints each of these as an entity, because in the sentence it reads they stand where a name would stand.

This is the mirror image of the problem above. A name variant splits one character into several, which a reader of the entity list can see. A role noun fuses several characters into one, and the fused entity looks ordinary, with a history like any other.

Seeding a canonical cast ahead of time does not help in this case. Extraction is shown the entities the context already holds and told to reuse their exact names, so the first `stranger` entity collects every stranger that follows it.

Some ways to work with them:

- **Look for common nouns in the entity list** after a bulk ingest. A duplicate sweep does not report these, because there is one entity rather than two, and its name is not near-identical to anything.
- **Treat a high relation count on a generic name as the signal.** An entity called `stranger` that is related to eight other people is eight people.
- **Resolve the reference in your own pipeline** where the people must be counted. Replace the role noun with the name the surrounding text supplies, or drop the sentence, before the write.
- **Keep a stoplist** of the role nouns your corpus uses, and check the entity list against it after each ingest.
...
That turns one request per character per position into one request per character per key. For the same three hundred characters and one key, that is three hundred requests.
