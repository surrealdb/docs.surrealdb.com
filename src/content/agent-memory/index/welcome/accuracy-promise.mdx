...
Calibration stores **`attribute.confidence`** (reconciler posterior) **separately** from `source.trust`. A low-confidence extraction cannot silently supersede a higher-confidence belief; it becomes **`uncertainty`** instead. See [Surface, models, and security](/docs/agent-memory/architecture/surface-security-and-models) for the model-hook story.
