...
description: "Ingesting images, audio, and video into the SurrealDB Agent Memory knowledge layer."
---
...
When a document is **reprocessed**, previous `audio_chunk` and `image_chunk` records are cleared at the start of the run before modality stages execute, so stale transcript or frame records cannot survive a profile or content change.
...
See [Configuration](/docs/agent-memory/reference/configuration#multimodal-providers-optional) for the full variable list.
...
PII redaction can be enabled per-Context and applies to all ingested content, including OCR-recognised text and speech transcripts, before chunking. See [Extraction tuning](/docs/agent-memory/reference/configuration#extraction-tuning) for the **`pii_redaction_enabled`** field.
