...
Optional **`observed_at`** (RFC 3339) stamps derived facts at a caller-supplied **known time** instead of wall-clock ingest. Use this for **narrative playback** - ingest an entire canon but answer only as far as the user has read or watched (see [Spoiler-safe narrative memory](/docs/agent-memory/cookbooks/patterns/spoiler-safe-narrative-memory)). Omitted keeps the default (ingest time).
...
**Response (`infer: none`):** `chunkId`, `sessionId`, and `turnId` for the literal memory chunk. Long text is split into multiple **`memory_chunk`** records (one embedding per sub-chunk) so vector recall covers the full passage - `chunkId` returns the first record’s id.
...
See [Sessions and turns](/docs/agent-memory/mental-model/sessions-and-turns).
...
Extraction, reconciliation, and provenance are described in [Extraction pipeline](/docs/agent-memory/reasoning/extraction-pipeline) and [Provenance and traceability](/docs/agent-memory/mental-model/provenance-and-traceability).
