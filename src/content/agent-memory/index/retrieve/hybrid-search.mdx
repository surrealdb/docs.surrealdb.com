...
The vector leg also searches **transcript segments** (`audio_chunk` records) in the same 3072-dim embedding space as text chunks, so spoken content from audio and video documents can surface as passage hits with time-coded provenance - not only the parent chunk spine.
...
**Requires server configuration:** set `SPECTRON_RERANKER_URL` and `SPECTRON_RERANKER_MODEL` at startup (see [Configuration](/docs/agent-memory/reference/configuration#reranker-optional)). Without a reranker provider, `use_reranker=true` falls through to bi-encoder ordering.
