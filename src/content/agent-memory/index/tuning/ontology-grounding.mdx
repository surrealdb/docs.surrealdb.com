...
Relation labels, attribute keys, and action verbs are counted within the write scope, ranked by frequency, and truncated to the most common. Attribute keys are drawn only from records that are still current - superseded and expired values do not keep a dead key alive in the prompt.
...
This matters more than it looks: the type is half an entity's identity, so an invented type silently produces a second entity. See [Entity matching](/docs/agent-memory/reasoning/reconciliation-and-supersession#entity-matching).
...
- [Reconciliation and supersession](/docs/agent-memory/reasoning/reconciliation-and-supersession) - exact-key entity matching, and why nicknames fragment
- [Storing memories](/docs/agent-memory/ingest/experiential/remember) - `infer` modes, including caller-supplied triples
- [Extraction pipeline](/docs/agent-memory/reasoning/extraction-pipeline) - where extraction sits in ingest
