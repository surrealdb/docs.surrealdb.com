...
**Audit note:** the trace graph may still record that a forget call occurred (who, when, which operation) without retaining the deleted content itself. Plan your compliance process around both memory erasure and trace retention policies.
...
Use scoped forget for GDPR-style deletion when a principal’s entire branch must go, rather than a single fact.
...
| Retire a scope folder in the vocabulary only | **`scope:delete`** (tombstone the scope node) | Tombstone is soft - distinct from erasing facts; see [Contexts and scope](/docs/agent-memory/mental-model/contexts-and-scope#tombstone-vs-erase) |
...
- [Reconciliation and supersession](/docs/agent-memory/reasoning/reconciliation-and-supersession) - why corrections keep history by default
- [Tri-temporal model](/docs/agent-memory/architecture/tri-temporal-model) - time-travel and belief history
- [Permissions and delegation](/docs/agent-memory/mental-model/contexts-and-scope#permissions-and-delegation) - **`memory:forget`** grants
