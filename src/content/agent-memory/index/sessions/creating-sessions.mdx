...
The ownership restriction applies to **writes** only. Reading state and retrieving context from a session may be permitted to other principals depending on the access control configuration of the context. See [Permissions and delegation](/docs/agent-memory/mental-model/contexts-and-scope#permissions-and-delegation) for details.
