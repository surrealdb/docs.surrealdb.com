...
**`GET /entities/top`** returns a ranked head, ordered by `by=coverage` (the default), `importance` or `recency`, with an optional `type` filter. The head is not the first page of a listing: `importance` is reinforced on recall, so rows move as the Context is used. Walk the whole set with `GET /entities` instead.

**`GET .../history`** returns the chain for every key on one entity, where `.../history/{key}` narrows to a single key. **`GET .../neighbourhood`** returns one hop out, and `minFacts` filters neighbours by how many facts they hold. Its `limit` has a ceiling of 100 rather than the usual 500, because each row costs its own fact count.
...
`POST .../resolve` settles one and requires the **`memory:write`** grant. The body takes `acceptedValue`, plus an optional `note` kept as the new row's source claim. The accepted value is written as a fresh assertion, and the competing rows at the same entity and key are then superseded. This is the supersession axis rather than the forget axis, so `forgotten_at` is untouched.
