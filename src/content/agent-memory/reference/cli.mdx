...
The MCP server is served at `/mcp` on the same host and port as the REST API — no CLI step is needed. Point your client at that endpoint, or run [`install-mcp`](https://github.com/supermemoryai/install-mcp). See the [MCP server install guide](/docs/agent-memory/integrations/mcp-server/install).
