...
description: "Complete reference for all seven SurrealDB Agent Memory MCP tools."
---
...
Operation failures return **`isError: true`** tool results with `structuredContent.error.status` mirroring REST (404, 403, 429, 401, 500). JSON-RPC **`error`** is reserved for protocol faults (bad params, unknown tool). Auth and missing-Context failures are masked as **401**. See [MCP tools - error handling](/docs/agent-memory/reference/mcp-tools#error-handling).
