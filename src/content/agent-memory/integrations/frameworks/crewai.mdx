...
position: 4
title: CrewAI
description: "CrewAI integration for SurrealDB Agent Memory, with memory tools and automatic per-task memory."
---
...
Package: **`agent-memory-crew-ai`** (PyPI). It pulls in CrewAI and the SurrealDB SDK (`surrealdb` v3, which bundles SurrealDB Agent Memory).
...
| Variable | Purpose |
| --- | --- |
| `AGENT_MEMORY_ENDPOINT` | Server URL |
| `AGENT_MEMORY_CONTEXT` | Context id |
| `AGENT_MEMORY_API_KEY` | Context API key (keep it in `.env`, not source) |
| `AGENT_MEMORY_DEFAULT_SCOPE` | Optional. Scope for writes and lens for reads, for example `user/tobie`. |
| `AGENT_MEMORY_TOP_K` | Optional. Memories recalled per query (default `5`). |

You can also pass any of these directly to `AgentMemory(...)` or `AgentMemoryConfig(...)` instead of using the environment.
...
from agent_memory_crewai import get_spectron_tools
...
```python
from agent_memory_crewai import get_sessionized_spectron_tools

tools = get_sessionized_spectron_tools("user-123")
```

The tools map onto SurrealDB Agent Memory operations: `agent_memory_recall`, `agent_memory_remember`, `agent_memory_context`, `agent_memory_forget`, `agent_memory_reflect`, and `agent_memory_upload`.
...
- For an MCP-native host, prefer the [MCP server](/docs/agent-memory/integrations/mcp-server/install).
- To call SurrealDB Agent Memory directly outside CrewAI, use the [Python SDK](/docs/agent-memory/integrations/sdks/python).
