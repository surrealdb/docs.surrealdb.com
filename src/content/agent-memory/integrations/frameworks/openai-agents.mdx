...
The agent decides when to `remember`, `recall`, `context`, `reflect`, or `forget`. `get_spectron_tools` builds the tools from the `AGENT_MEMORY_*` environment by default; pass `client=` for an explicit `AgentMemoryClient` or `include=` to expose a subset:

```python
from agents import Agent, Runner
from agent_memory_openai_agents_sdk import get_spectron_tools
