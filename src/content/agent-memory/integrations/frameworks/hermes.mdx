...
> [!NOTE]
> This is the hosted provider. For a persistent filesystem on a SurrealDB instance you run, with a toolset the agent calls directly, see the [SurrealFS integration for Hermes](/docs/build/integrations/ai-frameworks/hermes). Only one memory provider can be active at a time, so pick one.
