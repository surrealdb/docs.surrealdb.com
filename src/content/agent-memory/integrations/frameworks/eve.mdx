...
position: 5
title: Eve
description: "SurrealDB Agent Memory for Eve agents, via auto-memory middleware and a tool pack."
---
...
Package: **`@surrealdb/agent-memory-eve`**. It ships two layers you can use independently or together: a tool pack the model calls explicitly, and auto-memory middleware that recalls before each turn and persists after it with no tool call required.
...
```typescript
import { createSpectronClient, setSharedSpectronClient } from "@surrealdb/agent-memory-eve";
...
- To call SurrealDB Agent Memory directly outside Eve, use the [JavaScript SDK](/docs/agent-memory/integrations/sdks/javascript-and-typescript).
