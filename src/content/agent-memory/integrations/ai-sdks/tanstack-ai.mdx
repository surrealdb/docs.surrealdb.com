...
position: 2
title: TanStack AI
description: "Adding SurrealDB Agent Memory to a TanStack AI application."
---
...
To let the model decide when to use memory, expose recall as a `@tanstack/ai` tool. Build it with `toolDefinition(...).server(...)` and pass it to `chat`:
...
- [REST API](/docs/agent-memory/integrations/surfaces/rest): if you would rather call SurrealDB Agent Memory over HTTP directly
