...
const model = wrapLanguageModel({
    model: openai("gpt-4o"),
    middleware: agent-memory.middleware({ sessionId: "session-123" }),
});
...
`agent-memory.tools()` returns a Vercel AI SDK `ToolSet` the model can call during generation, bound to the same scope and session you pass:
...
const { text } = await generateText({
    model,
    tools: agent-memory.tools({ sessionId: "session-123" }),
...
```typescript
agent-memory.middleware({ scopes: "user/tobie" });           // one user
agent-memory.middleware({ scopes: ["team/eng", "user/x"] }); // OR of two
```

## Direct client access

`agent-memory.client` is the underlying `@surrealdb/memory` client for anything not wrapped here: documents, sessions, entities, `chat`, and so on. See the [JavaScript SDK](/docs/agent-memory/integrations/sdks/javascript-and-typescript) for the full surface.
