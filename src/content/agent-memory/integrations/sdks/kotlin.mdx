...
position: 6
title: Kotlin SDK
description: "Using SurrealDB Agent Memory from Kotlin applications and agents on JVM, Android, and iOS."
---
...
Construct an [`AgentMemory`](/docs/agent-memory/reference/sdk-kotlin) client with a context id, an API key, and your endpoint. Authentication uses the **`Authorization: Bearer`** header on every request.
...
- [Kotlin SDK reference](/docs/agent-memory/reference/sdk-kotlin) for package layout and the full surface
- [SurrealDB Kotlin SDK](/docs/reference/kotlin) for the database client in the same package
- [REST API](/docs/agent-memory/reference/rest-api) for the underlying HTTP surface
