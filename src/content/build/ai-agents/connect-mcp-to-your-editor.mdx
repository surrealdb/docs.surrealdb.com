...
For the full tool list, limits, and security model, see [Embedded MCP](/docs/build/ai-agents/mcp/embedded). This page focuses on getting a working setup you can verify in a few minutes. If your database runs on SurrealDB Cloud, the hosted [SurrealDB MCP Server](/docs/build/ai-agents/mcp) is a shorter path — one URL, no local process.
...
| Database runs on a remote host you operate | **`surreal start`** → **`/mcp`** (HTTP) | `url` + auth headers |
| Database runs on SurrealDB Cloud | **[SurrealDB MCP Server](/docs/build/ai-agents/mcp)** (hosted) | `url` + OAuth sign-in |
...
> [!WARNING]
> Stdio MCP runs every tool call with owner-level access. Use it only on a machine you trust. Do not expose `surreal mcp` to untrusted users. See the [security checklist](/docs/build/ai-agents/mcp/embedded#security-checklist) before production HTTP deployments.
...
Replace the header value with Base64-encoded credentials for your SurrealDB user (for example `root:secret` during local dev). In production, prefer a least-privilege `DEFINE USER` and TLS in front of the server — see [MCP security](/docs/build/ai-agents/mcp/embedded#security-checklist).
...
Once connected, the assistant can use the published tools documented on the [MCP overview](/docs/build/ai-agents/mcp/embedded#published-tools). Typical workflows:
...
| `403 Forbidden: Host header is not allowed` (HTTP) | The `/mcp` transport only accepts loopback hosts by default. Set `SURREAL_MCP_ALLOWED_HOSTS` to your FQDN (include `localhost` if you still need it), or `SURREAL_MCP_ALLOW_ALL_HOSTS=true` behind a trusted proxy. Requires SurrealDB 3.2.1+. See [MCP configuration](/docs/build/ai-agents/mcp/embedded#configuration). |
| Query timeout | Default tool timeout is 60 seconds (`SURREAL_MCP_QUERY_TIMEOUT_SECS`). Large scans may need a narrower query. |
| Result truncated | Output is capped (default 256 KiB). Paginate or aggregate in SurrealQL. See [MCP configuration](/docs/build/ai-agents/mcp/embedded#configuration). |
...
- [Embedded MCP](/docs/build/ai-agents/mcp/embedded) — transports, tools, environment variables, security
- [SurrealDB MCP Server](/docs/build/ai-agents/mcp) — the hosted server for SurrealDB Cloud
