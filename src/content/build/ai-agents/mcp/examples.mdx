...
These are prompts to try once you have [connected your AI tool](/docs/build/ai-agents/mcp). Each one shows what the assistant does behind the prompt, so you know what to expect before you ask.
...
It lists the roles the organisation offers, sends the invitation with the role you chose, and reports current members and any invitations still outstanding.
...
- **Let it check before it deploys.** Readiness and cost checks cost nothing and stop a deploy failing halfway.
...
- [SurrealDB MCP Server](/docs/build/ai-agents/mcp) — the full tool list and what each group does
- [Agent Skills](/docs/build/ai-agents/agent-skills) — teach your assistant idiomatic SurrealQL to go with these tools
- [Agent rules](/docs/build/ai-agents/agent-rules) — project rules that keep generated queries consistent
