...
- **Give agents long-term memory** — create and manage [SurrealDB Agent Memory](/docs/agent-memory) contexts, then store and recall memories from them.
...
SurrealDB Agent Memory contexts work the same way. Your assistant can `remember`, `recall`, `reflect`, `forget`, `upload`, and `inspect` against a context you own, described in the [SurrealDB Agent Memory MCP tools reference](/docs/agent-memory/integrations/mcp-server/tools-reference).
