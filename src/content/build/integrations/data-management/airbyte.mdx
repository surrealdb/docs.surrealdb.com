...
- A token or user credentials with [`DEFINE TABLE`](/docs/reference/query-language/statements/define/table), [`DEFINE INDEX`](/docs/reference/query-language/statements/define/indexes), [`UPSERT`](/docs/reference/query-language/statements/upsert), [`SELECT`](/docs/reference/query-language/statements/select), and [`REMOVE`](/docs/reference/query-language/statements/remove) permissions.
...
Upon a successful test, you can start syncing data from your sources into SurrealDB tables. Each stream will be output into its own table in SurrealDB. Each table will contain 3 fields:

- `_airbyte_raw_id`: a uuid assigned by Airbyte to each event that is processed. The field type in SurrealDB is string. The connector uses this as the ID of each record in the destination SurrealDB table.
- `_airbyte_extracted_at`: a timestamp representing when the event was pulled from the data source. The field type in SurrealDB is datetime.
- `_airbyte_data`: a JSON blob holding the event data. The field type in SurrealDB is object.
