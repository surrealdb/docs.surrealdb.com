...
Transactions are backed by [`SurrealDB.beginTransaction()`](/docs/surrealql/statements/begin). If any operation inside the callback throws, the transaction is cancelled and changes are discarded.
...
Records use Better Auth's default string identifiers. The adapter stores them as SurrealDB [record IDs](/docs/surrealql/datamodel/ids) and deserialises them back to strings when reading. Numeric and UUID identifier strategies are **not** supported.
