...
Nothing stores a path, so moving or renaming a folder is a single `UPDATE` and every descendant's path follows automatically. A folder is a record with no content, `hash` is maintained by an event, and the HNSW and BM25 indexes behind the two search modes are part of the same schema.
...
The prompt is cut down to search terms before it reaches the index: stopwords out, a dozen terms at most. Every word in a whole paragraph is another `OR` branch, and matching records are read back in full to be ranked.
...
Turns are filed under `/home/<agent>/memories/<profile>/`. The path has two segments, because there are two ways for memories to end up in one database that should not mix.
...
The tree is on the left and the open file is on the right. Text is editable and saves back to the table, markdown renders with a source toggle, agent-authored HTML renders in a sandboxed iframe, images display, and the search box is the same hybrid search behind the agent's own tool calls.
