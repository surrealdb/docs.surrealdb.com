...
SurrealDB ≥ v1.5 has built-in HNSW vector indexes, queried with SurrealQL's `<| K |>` operator and [`vector::distance::*` functions](/docs/reference/query-language/functions/database-functions/vector).

## Define the one-time vector table and index
...
1. `SurrealDocumentWriter` embeds and inserts snack facts into `snack_mem` with an HNSW index.
...
<table>
    ...
    <tbody>
        <tr>
            ...
            <td>Keep vectors, relational data, graph edges and real-time queries in one DB.</td>
        </tr>
        <tr>
...
- [Vector Search reference guide](/docs/learn/data-models/vector-search/overview)
