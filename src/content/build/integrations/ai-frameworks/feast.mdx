...
<table>
    ...
    <tbody>
        <tr>
            <td><strong><code>DEFINE INDEX … HNSW</code></strong> – build an approximate-NN index in one DDL line (<a href="/docs/reference/query-language/statements/define/indexes" rel="noopener noreferrer">SurrealDB</a>)</td>
            <td>Millisecond-latency k-NN retrieval without an extra service</td>
        </tr>
        <tr>
            <td><strong><code>vector::similarity::cosine()</code></strong> and [other vector functions](/docs/reference/query-language/functions/database-functions/vector)</td>
            <td>Compute similarity scores directly in the query plan</td>
        </tr>
        <tr>
            <td><strong><code>&lt;|K|&gt;</code></strong> operator for "return <em>K</em> [nearest neighbours](/docs/reference/query-language/language-primitives/operators)"</td>
...
With these few steps, Feast can serve scalar features and high-dimensional embeddings straight out of SurrealDB, with zero micro-services, one binary. Happy feasting!

## Resources

- [DEFINE INDEX statement](/docs/reference/query-language/statements/define/indexes)
- [Vector functions](/docs/reference/query-language/functions/database-functions/vector)
- [Operators](/docs/reference/query-language/language-primitives/operators)
