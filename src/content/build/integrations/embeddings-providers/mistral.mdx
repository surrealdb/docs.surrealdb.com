...
2. **Fast vector storage** - **SurrealDB** (≥ v1.5) ships an in-memory HNSW index, queried with the `<|K,EF|>` operator in SurrealQL.

The sections below connect the two, from installation through ingestion and search to a complete script.
...
Bulk-inserting sends every document in one SurrealQL call and one network round trip, which is **much faster** than inserting records one at a time.
...
Create a table called `document` to store documents and embeddings. The `HNSW` index is one way to [maintain performance](/docs/reference/query-language/language-primitives/operators#hnsw-method) if the dataset becomes quite large; otherwise, it can be left out.
...
These statements can all be put inside a single `.query()` call in the Rust SDK, followed by a line to check for any errors.
...
There are a [number of ways](/docs/reference/rust/concepts/flexible-typing) to work with or avoid structs when using the Rust SDK, including creating structs: one to represent the input into a `.create()` statement, which will implement `Serialize`, and another that implements `Deserialize` to show the result.
