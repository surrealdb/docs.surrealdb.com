...
The Hierarchical Navigable Small World (HNSW) index trades a tiny bit of accuracy for dramatic speed‑ups compared with brute-force scanning; ideal once your table grows beyond a few thousand records. Because HNSW lives in RAM today, index creation is instant but counts against your memory budget. 
...
This keeps the SurrealDB side simple while you experiment with different quantisers. For more information about vector search in SurrealDB, see the [Vector Search Reference guides](/docs/learn/data-models/vector-search/overview).
...
This guide demonstrates how to store OpenAI embeddings as [SurrealDB vectors](/docs/learn/data-models/vector-search/overview) via the Rust SDK for the purposes of semantic search.
...
Then use it to generate an OpenAI embedding with [`text-embedding-3-small`](https://platform.openai.com/docs/guides/embeddings/embedding-models), and print it with a `println!` statement.
