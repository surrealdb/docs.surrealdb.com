...
The `2.0.0` release of SurrealDB includes [many new features, improvements, and bug fixes](/releases#v2-0-0). It also includes some breaking changes that you should be aware of when upgrading.
...
- The underlying approach for storing record IDs and ranges has changed
  ...
  - [Ranges are now their own value rather than being available only as record ID ranges](https://github.com/surrealdb/surrealdb/pull/4506)

### SurrealQL
- The `UPDATE` statement no longer creates records if these are missing. Instead, use the new [UPSERT](/docs/reference/query-language/statements/upsert) statement for this behaviour.
- The `file://` connection protocol has been deprecated in favour of the more explicit `rocksdb://` protocol.
- The `DEFINE SCOPE` statement has been dropped in favour of the new [`DEFINE ACCESS TYPE RECORD`](/docs/reference/query-language/statements/define/access/record) statement
...
- Some functions have been renamed for clarity
  - `meta::tb()` -> [`record::tb()`](/docs/reference/query-language/functions/database-functions/record)
  - `meta::id()` -> [`record::id()`](/docs/reference/query-language/functions/database-functions/record)
  - `string::endsWith()` -> [`string::ends_with()`](/docs/reference/query-language/functions/database-functions/string)
  - `string::startsWith()` -> [`string::starts_with()`](/docs/reference/query-language/functions/database-functions/string)

### Authentication and headers
- Authentication is now enabled by default as you previously would with `--auth`. The [`--unauthenticated`](/docs/reference/cli/surrealdb-cli/commands/start#unauthenticated-mode) flag is now required in order to provide the previous default behaviour.
...
- Headers used to communicate via HTTP with SurrealDB now require the `surreal-` prefix. For example, the legacy `ns` and `db` headers are now `surreal-ns` and `surreal-db`.

## Upgrading your data

A new [`surreal fix`](/docs/reference/cli/surrealdb-cli/commands/fix) command has been implemented to automatically change the format of your stored data. The command is followed by a path to the data. For example:
...
> [!IMPORTANT]
> Run `surreal fix` with a 2.x binary. From SurrealDB 3.0.0 the command exits with `Fix is not implemented` and changes nothing, so data going from 1.x to 3.x is fixed with 2.x first and then moved to 3.x by following [Migrating from 2.x to 3.x](/docs/build/migrating/from-old-surrealdb-versions/2x-to-3x).
...
-  If you have used the now deprecated [`DEFINE TOKEN`](/docs/reference/query-language/statements/define/token) command to define a token on a Scope with the also deprecated [`DEFINE SCOPE`](/docs/reference/query-language/statements/define/scope) command, you will have to update your access management rules to use the new [`DEFINE ACCESS`](/docs/reference/query-language/statements/define/access) which supports creating permissions using [TYPE JWT](/docs/reference/query-language/statements/define/access/jwt) and [TYPE RECORD](/docs/reference/query-language/statements/define/access/record) rules.
