...
The following chart shows PostgreSQL data types along with the equivalent or near-equivalent [SurrealQL data type](/docs/reference/query-language/language-primitives/data-types) for each.
...
As SurrealQL is inspired by SQL, many queries between it and PostgreSQL are identical.
...
If a record ID is known ahead of time and you are using a version of SurrealDB before 3.0, be sure to query by the record ID itself instead of using a `WHERE` clause in SurrealQL. This will avoid a full table scan if the field is not indexed.
