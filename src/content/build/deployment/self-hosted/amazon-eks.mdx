...
position: 4
title: Amazon EKS
description: Amazon Elastic Kubernetes Service (Amazon EKS) is a managed service that eliminates the need to install.
---

# Deploy on Amazon Elastic Kubernetes Service (EKS)

> [!NOTE]
> This guide deploys a **highly available cluster** on EKS and connects SurrealDB to it. For a **single-node RocksDB** deployment on Kubernetes, start with [Deploy on Kubernetes](/docs/build/deployment/self-hosted/kubernetes). For managed deployments, see [SurrealDB Cloud](/docs/build/deployment/surrealdb-cloud/what-is-surrealdb-cloud).

<br />

## What is Amazon EKS?

[Amazon Elastic Kubernetes Service (Amazon EKS)](https://docs.aws.amazon.com/eks/) is a managed service that eliminates the need to install, operate, and maintain your own Kubernetes control plane on Amazon Web Services (AWS). This guide covers a highly available SurrealDB deployment on EKS.
...
> [!IMPORTANT]
> This guide was tested in `eu-west-1` (Ireland region) and follows best practices for scalability and high availability. It will provision up to 12 Amazon Elastic Compute Cloud (Amazon EC2) instances, several Amazon Elastic Block Storage (Amazon EBS) drives, and up to three Amazon Elastic Loadbalancers (Amazon ELB). The forecasted cost to run this guide is $5 USD per hour.
...
- Create the necessary node groups for the data store according to [best practices](https://docs.pingcap.com/tidb-in-kubernetes/stable/deploy-on-aws-eks)
...
The following instructions install the TiDB operator in your EKS cluster.
...
The following script installs SurrealDB on your EKS cluster with a public endpoint exposed via an Application Load Balancer (ALB).
...
![AWS volume](~/assets/img/aws-volumes.png)
