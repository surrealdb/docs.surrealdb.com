...
> [!NOTE]
> For a **single-node RocksDB** deployment on Kubernetes (including a cluster you create with AKS), start with [Deploy on Kubernetes](/docs/build/deployment/self-hosted/kubernetes). For a local open-source distributed-storage playground only, see [Run a multi-node cluster](/docs/running/multi-node).
...
| Single SurrealDB pod, RocksDB on a volume | [Deploy on Kubernetes](/docs/build/deployment/self-hosted/kubernetes) on an AKS cluster you manage |
