...
## Audit log knobs <Edition value="enterprise" />

Controls the [Enterprise audit log pipeline](/docs/build/deployment/observability/audit-logging). Setting `SURREAL_AUDIT_SINK=none` (the default) leaves the pipeline disabled entirely; no observer is registered and the cost is zero.
...
<table>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td scope="row" data-label="Purpose"><code>drop</code> (single non-blocking <code>try_send</code>) or <code>block</code> (bounded busy-yield retry). See <a href="/docs/build/deployment/observability/audit-logging#overflow-semantics">overflow semantics</a>. Neither offers a lossless guarantee.</td>
        </tr>
        <tr>
...
## Slow-query log knobs <Edition value="enterprise" />

Controls the [Enterprise slow-query log pipeline](/docs/build/deployment/observability/slow-query-logging). Setting `SURREAL_SLOW_QUERY_SINK=none` (the default) leaves the pipeline disabled entirely.
...
## SurrealDS networking <Edition value="enterprise" />
...
Pair this with the alert hints in the [metrics reference](/docs/build/deployment/observability/metrics#alert-hints) and the alerting rules on `surrealdb_audit_dropped`, `surrealdb_audit_append_errors` and `surrealdb_audit_queue_depth` listed in the [Compliance checklist](#compliance-checklist) below.
...
Filter on `namespace` and `database` at the proxy and strip the `user` label before exposing to a customer. See the [Recommended cloud whitelist](/docs/build/deployment/observability/metrics#recommended-cloud-whitelist) for the suggested customer-visible subset.

</TabItem>
</Tabs>

## Compliance checklist <Edition value="enterprise" />
...
For the full pipeline details (record shape, rotation, hash chain, redaction) see the [Audit logging](/docs/build/deployment/observability/audit-logging) reference.
