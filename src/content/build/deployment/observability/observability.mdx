...
SurrealDB contains a large number of flags for the [surreal start](/docs/reference/cli/surrealdb-cli/commands/start) command and [environment variables](/docs/reference/cli/surrealdb-cli/environment-variables) to modify the output from the database server. A quick overview of some of these flags is as follows:
