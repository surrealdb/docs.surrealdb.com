...
On the **Start** plan, **resize** when monitoring shows a steady mismatch between provisioned resources and workload — for example, sustained high CPU or memory after optimising queries. In SurrealDB Studio or the Cloud dashboard, open the instance, choose a new **instance type** or storage size, and apply the change. A short period of **elevated latency or reconnects** may take place while the platform adjusts capacity, so plan changes outside peak traffic when you can.

## Scaling Scale clusters

On the **Scale** plan, clusters run on [SurrealDS](https://surrealdb.com/platform/surrealds) with a minimum of **three compute units**. Increase compute units to add query nodes and adjust **storage** as the dataset grows. Horizontal scale improves throughput and fault tolerance, allowing the cluster to keep serving when one node is unavailable.
