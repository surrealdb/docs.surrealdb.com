...
| Multi-node | SurrealDS (<Edition value="enterprise" />, coming soon) | Horizontal | Replication and consensus | Where enabled on the storage tier | Large-scale production workloads | Self-hosted Enterprise (when available); Cloud (Dedicated tiers today) |
...
SurrealKV remains **beta**. For production on disk today, **prefer RocksDB** unless you are deliberately evaluating SurrealKV. It is nonetheless a serious storage path inside the project: features such as temporal reads via the [`VERSION`](/docs/reference/query-language/statements/select#the-version-clause) clause were exercised on SurrealKV first and have since been extended to [SurrealMX](/docs/running/in-memory) and RocksDB where the engine supports them.
...
> [!NOTE]
> **SurrealDS has not yet reached general availability.** The distributed storage layer described below is in development for <Edition value="enterprise" />. For managed multi-node clusters today, use [SurrealDB Cloud](/docs/build/deployment/surrealdb-cloud/what-is-surrealdb-cloud) Dedicated tiers.

For high availability and horizontal scalability, <Edition value="enterprise" /> deployments will be able to use **SurrealDS** as the distributed transactional storage layer behind SurrealDB query nodes.
...
When SurrealDS is available, operational signals will be documented under [Enterprise observability](/docs/manage/observability/enterprise-observability) (including the `surrealdb.ds.*` metric family).
...
**RocksDB** — Persistent embedded storage for write-heavy, SSD-backed workloads. See [File-backed storage](/docs/running/file-backed).
...
**Use multi-node with SurrealDS when** (<Edition value="enterprise" />, coming soon)

- High availability is required
- Workloads need horizontal scaling
- Infrastructure spans multiple nodes or availability zones

Until SurrealDS is available, use [SurrealDB Cloud](/docs/build/deployment/surrealdb-cloud/what-is-surrealdb-cloud) Dedicated tiers or contact SurrealDB for Enterprise early-access programmes.
