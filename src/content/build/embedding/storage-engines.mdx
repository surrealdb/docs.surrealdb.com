...
Typical options you will see in SurrealDB deployments include RocksDB for mature local disk storage, SurrealMX (`memory`) for in-process or server in-memory workloads, and SurrealKV (beta) where you want SurrealDB’s own LSM-backed path (including features that evolve with the product). In-memory modes are ideal for tests and scratch work and embedded deployments unless you enable Redis-like SurrealMX persistence. See [Deployment](/docs/build/deployment) for how engines map to hosting models.
