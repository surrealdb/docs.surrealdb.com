...
We do not implement the parsing of Record IDs in the Python SDK, as that would mean that we need to be able to parse any SurrealQL value, which comes with a cost. Instead you can send it over as a string with `StringRecordId`, allowing the server to handle the parsing.

```python
StringRecordID("person:john")
```