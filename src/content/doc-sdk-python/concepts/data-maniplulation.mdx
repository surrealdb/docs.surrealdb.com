...
person = await db.select(RecordID('person', 'h5wxrf2ewk8xjxosxtyc'))
```
...
people = await db.insert_relation('likes', [
	{
    	"in": RecordID('person', 'tobie'),
    	"out": RecordID('post', 123),
	},
...
await db.update('person', {name: "Jaime"})

# Update a record with a specific ID
await db.update(RecordID('person', 'tobie'), {
	"name": 'Tobie',
	"settings": {
		"active": true,
		"marketing": true,
