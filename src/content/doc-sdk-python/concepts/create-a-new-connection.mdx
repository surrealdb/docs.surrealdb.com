...
First, you need to initialize a new instance of SurrealDB and connect it to a database endpoint using the `.connect()` method.
...
<table>
	...
	<tbody>
		<tr>
			<td scope="row" data-label="Method"><a href="#connect"> <code> db.connect() </code></a></td>
		...
		<tr>
			<td scope="row" data-label="Method"><a href="#use"> <code> db.use(namespace,database)</code></a></td>
...
db = SurrealDB('http://127.0.0.1:8000/rpc')
db.connect()

# Connect to a remote endpoint with ws protocol
db = SurrealDB('wss://cloud.surrealdb.com/rpc')
db.connect()
```
...
An alternative to using `.connect()` and `.close()` is using Python's context manger to automatically open and close connection when exiting the context.
