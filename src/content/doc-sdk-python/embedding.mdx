...
The file-based database can be created using the `rocksdb://` or `surrealkv://` endpoint with a path to the database file.
...
async def main():
    ...
    # Create a file-based database (can use "rocksdb://mydb" or "surrealkv://mydb")
...
# File-based database
with Surreal("rocksdb://mydb") as db:
    db.use("test", "test")
...
### File-Based (`rocksdb://` or `surrealkv://`)
...
### Use Embedded (`memory`, `mem://`, `rocksdb://`, or `surrealkv://`) when:
...
- **`persistence.py`** - Demonstrates file-based persistent storage using `rocksdb://` or `surrealkv://` URLs, showing how data persists across multiple connections and can be updated over time. Run with: `python examples/embedded/persistence.py`
...
### File-Based Database
```python
# Persistent storage using RocksDB file path
db = AsyncSurreal("rocksdb://path/to/database")
...
- **Development**: Use `rocksdb://` for persistence
- **Production (single-node)**: Use `rocksdb://` with backups
