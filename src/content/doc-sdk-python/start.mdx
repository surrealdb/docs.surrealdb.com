...
This guide uses the `SurrealDB` class, but this example would also work with `AsyncSurrealDB` class, with the addition of `await` in front of the class methods.
...
# Import the SurrealDB class
from surrealdb import SurrealDB

# Using a context manger to automatically connect and disconnect
with SurrealDB("ws://localhost:8000/rpc") as db:
    db.signin({"root", "root"})
...
You can also find starter kits on our [awesome surreal repo](https://github.com/surrealdb/awesome-surreal?tab=readme-ov-file#starter-kits).