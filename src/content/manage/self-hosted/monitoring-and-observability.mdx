...
For **server logs** (levels, JSON, files, sockets), see [Logging](/docs/build/deployment/observability/logging). For **metrics and distributed traces**, use the [Observability overview](/docs/build/deployment/observability/overview). There you can find details on how to configure a collector with [OTLP (push)](/docs/build/deployment/observability/telemetry) (`SURREAL_TELEMETRY_PROVIDER=otlp` and your endpoint), and use [Prometheus (pull)](/docs/build/deployment/observability/observability) for `/metrics`, naming, and scrape authentication.
