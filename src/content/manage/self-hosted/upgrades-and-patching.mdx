...
Across **major versions**, on-disk formats may change. When documentation requires it, run [`surreal fix`](/docs/reference/cli/surrealdb-cli/fix) to migrate data between layouts, and follow [Migrating from older SurrealDB versions](/docs/build/migrating/from-old-surrealdb-versions/overview) plus any linked guides (for example between specific major lines).
...
**Before any upgrade**, take a fresh backup—[`surreal export`](/docs/reference/cli/surrealdb-cli/export) for a logical copy and/or a storage snapshot—so you can revert if migration or client incompatibility surfaces after deploy.
