...
Reliable backups are central to operating self-hosted SurrealDB. The CLI provides logical backups as SurrealQL using [`surreal export`](/docs/reference/cli/surrealdb-cli/commands/export), which dumps namespaces, databases, schemas, and data in a form you can version, diff, and store outside the database.
...
To **restore**, use [`surreal import`](/docs/reference/cli/surrealdb-cli/commands/import) against a running instance, pointing at the correct endpoint, credentials, and target namespace or database.
