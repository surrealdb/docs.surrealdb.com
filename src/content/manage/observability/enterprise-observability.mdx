...
- **SurrealDS cluster instruments** (`surrealdb.ds.*`) — consensus, networking, recovery, and garbage collection when the distributed datastore path (SurrealDS, coming soon) is active.
...
| SurrealDS `surrealdb.ds.*` cluster metrics (SurrealDS coming soon) | Not emitted | Emitted when the DS runtime is active and a metrics reader exists [E] |
...
Legacy series names were collapsed into labelled families in SurrealDB 3.1. The same migration table applies to Community and Enterprise; see [Observability (metrics and Prometheus)](/docs/manage/observability/observability) for the summary, [Telemetry (OTLP)](/docs/manage/observability/telemetry) for wire-format and deprecation notes, and the [Metrics reference](/docs/manage/observability/metrics#migration-from-30) for the full migration table.
...
- [Audit logging](/docs/manage/observability/audit-logging) — record shape, rotation, overflow semantics, hash chaining, redaction.
- [Slow-query logging](/docs/manage/observability/slow-query-logging) — triage pipeline mirroring the audit observer.
- [Configuration](/docs/manage/observability/configuration) — all `SURREAL_AUDIT_*` and `SURREAL_SLOW_QUERY_*` variables and the compliance checklist.
