...
position: 3
title: OTLP (push)
description: OTLP (push) — OpenTelemetry export, intervals, backward-compatible instruments, and differences before and after SurrealDB 3.1.
---

# OTLP (push)

This page covers push export using the OpenTelemetry Protocol (OTLP): your server sends signals to a collector. For pull scraping of `GET /metrics` (Prometheus), see [Prometheus (pull)](/docs/build/deployment/observability/observability).
...
For async runtime introspection (tasks, polls, scheduling), you can use the [Tokio console](/docs/build/deployment/observability/tokio-console) which is separate from OTLP.

For server logging (levels, JSON, files, sockets, slow-query logging), see [Logging](/docs/build/deployment/observability/logging).

</TabItem>

<TabItem label="SurrealDB 3.1 and later">

From SurrealDB 3.1, OpenTelemetry is the single source of truth for metrics and for audit and slow-query logs: one meter provider and one logger provider route every measurement to multiple exporters, including the Prometheus text exposition at [`GET /metrics`](/docs/build/deployment/observability/observability). An OTLP subscriber receives the same metric and log surface that a Prometheus operator sees (subject to the `/metrics` allowlist for anonymous scrapes), plus tracing.

For Prometheus (pull) — allowlists, naming, migration from older series, and multi-tenant guidance — see [Prometheus (pull)](/docs/build/deployment/observability/observability).
...
| Logs | Unified logger provider | Audit records (scope `surrealdb.audit`), slow-query records (scope `surrealdb.slow_query`). Severity mapping is summarised on [Prometheus (pull)](/docs/build/deployment/observability/observability). |
...
When `SURREAL_TELEMETRY_PROVIDER` is unset (or any value other than `otlp`), OTLP exporters are not built; metrics and logs still flow through their other paths (Prometheus `/metrics` and durable file sinks for audit and slow-query).
...
For OpenTelemetry log and trace verbosity, file output, sockets, and other logging flags used alongside this stack, see [Logging](/docs/build/deployment/observability/logging) and [`surreal start`](/docs/reference/cli/surrealdb-cli/commands/start).

</TabItem>

</Tabs>

For async runtime debugging with the Tokio console (tasks, poll histograms), see [Tokio console](/docs/build/deployment/observability/tokio-console). That workflow sits beside OTLP and is not version-split in the tabs above.

For diagrams, histogram bucket rules, deprecated variables, and the full OTLP signal matrix in prose form, see [TELEMETRY.md](https://github.com/surrealdb/surrealdb/blob/main/doc/TELEMETRY.md) in the SurrealDB repository alongside [OBSERVABILITY.md](https://github.com/surrealdb/surrealdb/blob/main/doc/OBSERVABILITY.md) for `/metrics`, allowlists, and metric catalogues.
