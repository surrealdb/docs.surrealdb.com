...
This page covers push export using the OpenTelemetry Protocol (OTLP): your server sends signals to a collector. For pull scraping of `GET /metrics` (Prometheus), see [Prometheus (pull)](/docs/manage/observability/observability).
...
From SurrealDB 3.1, OpenTelemetry is the single source of truth for metrics and for audit and slow-query logs: one meter provider and one logger provider route every measurement to multiple exporters, including the Prometheus text exposition at [`GET /metrics`](/docs/manage/observability/observability). An OTLP subscriber receives the same metric and log surface that a Prometheus operator sees (subject to the `/metrics` allowlist for anonymous scrapes), plus tracing.

For Prometheus (pull) — allowlists, naming, migration from older series, and multi-tenant guidance — see [Prometheus (pull)](/docs/manage/observability/observability).
...
| Logs | Unified logger provider | Audit records (scope `surrealdb.audit`), slow-query records (scope `surrealdb.slow_query`). Severity mapping is summarised on [Prometheus (pull)](/docs/manage/observability/observability). |
