...
- **stdout** carries the command's data payload and nothing else.
...
`not_available` means a feature is not enabled for this deployment. It does not indicate a fault.

## Exit codes

The exit codes below are a stable contract; error text is not.
...
`10` has its own code because "still running" is a genuinely different answer from "failed". A pipeline may reasonably poll again rather than roll back.
...
Run `surrealctl status` early in a job. It takes about one second, and it names which credential is in use, whether the API answers, and which organisation resolved, all before a later step fails for one of those reasons.
...
Your script needs to know what each command does when it runs again:
