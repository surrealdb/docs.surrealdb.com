...
jobs:
  report:
    ...
    steps:
      - run: curl -sSf https://install.surrealdb.com/surrealctl | sh
...
> [!IMPORTANT] A personal access token reads the control plane and cannot write to it. A job that creates, updates, or deletes needs a login session on a dedicated machine account. See [What a personal access token cannot do](/docs/manage/surrealctl/authentication#what-a-personal-access-token-cannot-do).
