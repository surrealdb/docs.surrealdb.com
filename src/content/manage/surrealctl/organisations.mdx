...
> [!NOTE] `SURREALCTL_ORG` outranks the value `org use` remembered. If a remembered organisation seems to be ignored, check whether the variable is exported. `org use` warns you about it at the time.
...
> [!IMPORTANT] Under `--json`, money stays in integer minor units — cents, or millicents where the API uses them. Do not treat `amount` as a decimal. Divide before you display.
