...
position: 1
title: surrealctl
description: "What the surrealctl control-plane CLI does. When to use it instead of SurrealDB Studio or the surreal binary, plus a sixty-second quickstart."
---
...
The two tools work together. `surrealctl` never speaks SurrealQL. When you run `surrealctl instance sql`, it resolves the instance, mints a database token, and hands off to the `surreal` binary - the same binary you would run by hand, with the endpoint and credentials already filled in. The same applies to `surrealctl instance import` and `surrealctl instance export`.

These three commands are the only ones that need `surreal`.
...
`surrealctl` deliberately does not do three things:
