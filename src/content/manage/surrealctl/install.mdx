...
`surrealctl` keeps four paths, all in one directory under `$XDG_CONFIG_HOME/surrealctl` or `~/.config/surrealctl`. XDG applies on macOS too, because this is a terminal tool that people symlink into a dotfiles repository.
