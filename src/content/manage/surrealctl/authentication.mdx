...
> [!NOTE] `auth scopes` returns an empty list for a login session. Identity scopes are not the authorisation model for a session — your role in the organisation is. Run `surrealctl org permissions` to see what you may do.
...
> [!WARNING] Revocation is not instant. A revoked token can continue to work for several minutes while the platform's exchange cache expires. Rotate a leaked token and then confirm with `surrealctl token list`.
