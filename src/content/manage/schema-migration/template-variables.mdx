...
Place `surrealkit.toml` in the root of your project. It is read automatically. The same file holds the [`[typegen]` section](/docs/manage/schema-migration/typegen) and, since 1.0, [schema modules and targets](/docs/manage/schema-migration/modules-and-targets). See [Configuration](/docs/manage/schema-migration/configuration) for the full key reference.

> [!WARNING]
> `[variables]` is read from the working directory only, while `[schema.*]` and `[target.*]` are found by walking up from it. Running a command from a subdirectory of your project therefore applies your modules but leaves every template variable undefined. Run project commands from the directory that holds `surrealkit.toml`.
...
**Working directory:** `[variables]` is read from the current directory, not found by walking up the tree. See the warning above.
