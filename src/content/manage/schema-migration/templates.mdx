...
`surrealkit.toml` holds template variables, type generation settings, and any [schema modules and targets](/docs/manage/schema-migration/modules-and-targets) the project declares. See [Configuration](/docs/manage/schema-migration/configuration) for the full key reference. `init` never overwrites an existing one.
