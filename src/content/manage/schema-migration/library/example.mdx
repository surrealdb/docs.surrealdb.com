...
If a process is killed mid-rollout, the `__rollout` record can be left in a `running_*` state. Inspect it with `rollout.status(&db).await?` and recover it with the CLI [`rollout repair`](/docs/manage/schema-migration/rollouts) command or `Rollout::abandon(&db, "20260604__add_account").await?`.
</content>
