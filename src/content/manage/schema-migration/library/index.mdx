...
Each file is applied once and re-applied only when its content changes, tracked in `__seed`, so this is safe to call on every application start. Use the [`Seed`] builder for runtime control, including `force` and embedded seed files. See [Seeding](/docs/manage/schema-migration/seeding).
...
Records in `__entity` are partitioned per [schema module](/docs/manage/schema-migration/modules-and-targets). Apply a named module with `Sync::embedded(BILLING).module("billing")?`, and pass the module to `rollout::run_baseline` and `rollout::run_abandon_rollout`, both of which gained a module argument in 1.0.
...
- [Upgrading to SurrealKit 1.0](/docs/manage/schema-migration/upgrading#if-you-use-the-rust-library): renamed and removed library items
