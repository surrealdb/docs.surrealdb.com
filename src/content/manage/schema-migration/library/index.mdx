...
Rows in `__entity` are partitioned per [schema module](/docs/manage/schema-migration/modules-and-targets). Apply a named module with `Sync::embedded(BILLING).module("billing")?`, and pass the module to `rollout::run_baseline` and `rollout::run_abandon_rollout`, both of which gained a module argument in 1.0.
