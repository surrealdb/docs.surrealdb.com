...
SurrealKit protects against this: running `surrealkit sync` against a database that already contains SurrealKit metadata (the `__entity` table) will require you to pass `--allow-shared-prune` explicitly before it will prune anything. The flag exists to make you stop and confirm, so do not pass it routinely.

## Next steps

- [Sync](/docs/manage/schema-migration/sync): full Sync reference
- [Rollouts](/docs/manage/schema-migration/rollouts): full Rollouts reference
