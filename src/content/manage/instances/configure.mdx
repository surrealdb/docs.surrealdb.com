...
**Allow and deny lists** apply to capabilities with many members: RPC methods, HTTP endpoints, functions, and [network access](/docs/manage/instances/network-access). Each list starts from a default and takes exceptions:
...
To add more disk storage:

1. Open **Instance settings → Compute**.
2. Set the value under **Storage capacity**.
3. Select **Save changes**.
...
Change the type and the storage in separate saves. The storage slider goes up to the maximum of the type the instance is on now, so the maximum of a new type is only available after the type change is saved. A type change also sets the storage to the starting storage of the new type, and the six-hour wait starts again from that change. The [Storage limits on Start](/docs/manage/instances/scaling#storage-limits-on-start) page lists the starting and maximum storage of each type.

> [!IMPORTANT]
> Storage expansion is unavailable on free instances. Move to a paid instance type to change storage capacity. The move happens in place, and the instance keeps its data.

Expect a brief reconnect while the instance restarts on the new size. For when to resize and what to watch afterwards, see [Scaling](/docs/manage/instances/scaling).
...
> [!WARNING]
> Do not treat the snapshots of an instance as the safety net for deleting it. If you might need the data later, [restore a backup into a new instance](/docs/manage/instances/backups) or take a [logical export](/docs/manage/instances/import-and-export) first. Confirm the export loads cleanly before you delete.
