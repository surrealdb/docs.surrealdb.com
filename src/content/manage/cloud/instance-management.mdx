...
When you **create an instance**, you choose a **plan** (Start or Scale), a **region** close to your applications, and an **instance type** (CPU, memory, and storage profile). Start provisions a single-node instance with dedicated storage. Scale provisions a multi-node cluster on SurrealDS for fault-tolerant, horizontally scalable workloads. Options such as network access mode depend on your organisation and plan.

Give each instance a clear name so teams can find it in the organisation view. See [Cloud architecture](/docs/build/deployment/surrealdb-cloud/architecture) for how plans map to topology.
