...
A [`DateTime`](/docs/languages/javascript/api/values/datetime) represents a point in time with nanosecond precision. Unlike JavaScript's native `Date` which is limited to millisecond precision, `DateTime` preserves the full precision of SurrealDB timestamps. A [`Duration`](/docs/languages/javascript/api/values/duration) represents a span of time using SurrealQL duration syntax.
...
A [`Decimal`](/docs/languages/javascript/api/values/decimal) represents an arbitrary-precision decimal number. This avoids the floating-point precision issues that occur with JavaScript's `number` type.
...
A [`Range`](/docs/languages/javascript/api/values/range) represents a bounded or unbounded range of values. Ranges are used in SurrealQL for selecting slices of records by ID or filtering numeric and temporal values. A `RecordIdRange` is a specialization for querying a range of records from a table.
