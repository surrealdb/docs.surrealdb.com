...
<table>
	...
	<tbody>
		<tr>
			<td scope="row" data-label="Method"><a href="/docs/languages/javascript/api/core/surreal#newsession"> <code> db.newSession() </code></a></td>
			<td scope="row" data-label="Description">Creates a new isolated session on the connection</td>
		</tr>
		<tr>
			<td scope="row" data-label="Method"><a href="/docs/languages/javascript/api/core/surreal-session#forksession"> <code> session.forkSession() </code></a></td>
			<td scope="row" data-label="Description">Creates a copy of a session, inheriting its state</td>
		</tr>
		<tr>
			<td scope="row" data-label="Method"><a href="/docs/languages/javascript/api/core/surreal-session#closesession"> <code> session.closeSession() </code></a></td>
			<td scope="row" data-label="Description">Destroys a session and releases its resources</td>
		</tr>
		<tr>
			<td scope="row" data-label="Method"><a href="/docs/languages/javascript/api/core/surreal-session#reset"> <code> session.reset() </code></a></td>
...
Sessions support all the same query methods as the main `Surreal` instance, including `.query()`, `.select()`, `.create()`, `.update()`, `.delete()`, and [more](/docs/languages/javascript/concepts/executing-queries).
...
> [!NOTE]
> Using a session after it has been closed throws an [`InvalidSessionError`](/docs/languages/javascript/api/errors#invalidsessionerror).
...
Sessions that were authenticated via `.signin()` or `.signup()` rely on the [`auth`](/docs/languages/javascript/api/core/surreal-session#event-auth) event for re-authentication. See [Authentication](/docs/languages/javascript/concepts/authentication#listening-to-authentication-events) for details.
...
- [SurrealSession API reference](/docs/languages/javascript/api/core/surreal-session) for the full session interface
- [Surreal.newSession()](/docs/languages/javascript/api/core/surreal#newsession) for session creation details
- [Authentication](/docs/languages/javascript/concepts/authentication) for session-level authentication
- [Transactions](/docs/languages/javascript/concepts/transactions) for atomic operations within sessions
