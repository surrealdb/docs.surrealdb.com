...
Before you can use the SDK in your Vue application, you need to configure the codec options to mark all returned [value classes](/docs/languages/javascript/concepts/value-types) as raw. This prevents Vue's reactivity system from being applied to them, and allows you to use them in your components without any issues.
...
The `params` option accepts the same values as [`.connect()`](/docs/languages/javascript/concepts/connecting-to-surrealdb#connection-options), including `namespace`, `database`, and [`authentication`](/docs/languages/javascript/concepts/authentication#providing-credentials-on-connect).
...
Register the TanStack Query plugin in your application entry point, then call `provideSurreal()` in your root component. Pass the endpoint and any [connection options](/docs/languages/javascript/concepts/connecting-to-surrealdb#connection-options) through the `params` field.
...
Use the `useSurrealClient()` composable to access the `Surreal` instance from any descendant component. All [query methods](/docs/languages/javascript/concepts/executing-queries) are available on the client, including `.query()`, `.select()`, `.create()`, and more.
...
You can build an authentication layer on top of the composable using the SDK's [`.signin()`](/docs/languages/javascript/concepts/authentication#signing-in-users) and [`.signup()`](/docs/languages/javascript/concepts/authentication#signing-up-users) methods. The example below shows a minimal composable for record access authentication.
