...
- The [JavaScript SDK installed](/docs/start/languages/javascript/installation) in your project
...
Follow the [installation guide](/docs/start/languages/javascript/installation) for more information on how to install the SDK in your project.

## Value classes

Before you can use the SDK in your Vue application, you need to configure the codec options to mark all returned [value classes](/docs/start/languages/javascript/concepts/value-types) as raw. This prevents Vue's reactivity system from being applied to them, and allows you to use them in your components without any issues.
...
The `params` option accepts the same values as [`.connect()`](/docs/start/languages/javascript/concepts/connecting-to-surrealdb#connection-options), including `namespace`, `database`, and [`authentication`](/docs/start/languages/javascript/concepts/authentication#providing-credentials-on-connect).
...
Register the TanStack Query plugin in your application entry point, then call `provideSurreal()` in your root component. Pass the endpoint and any [connection options](/docs/start/languages/javascript/concepts/connecting-to-surrealdb#connection-options) through the `params` field.
...
Use the `useSurrealClient()` composable to access the `Surreal` instance from any descendant component. All [query methods](/docs/start/languages/javascript/concepts/executing-queries) are available on the client, including `.query()`, `.select()`, `.create()`, and more.
...
You can build an authentication layer on top of the composable using the SDK's [`.signin()`](/docs/start/languages/javascript/concepts/authentication#signing-in-users) and [`.signup()`](/docs/start/languages/javascript/concepts/authentication#signing-up-users) methods. The example below shows a minimal composable for record access authentication.
...
- [Connecting to SurrealDB](/docs/start/languages/javascript/concepts/connecting-to-surrealdb) for connection protocols and reconnection behavior
- [Authentication](/docs/start/languages/javascript/concepts/authentication) for signing in, signing up, and token management
- [Executing queries](/docs/start/languages/javascript/concepts/executing-queries) for query builders and raw SurrealQL
- [Live queries](/docs/start/languages/javascript/concepts/live-queries) for real-time subscriptions
- [JavaScript SDK API reference](/docs/start/languages/javascript/api/) for the complete method reference
