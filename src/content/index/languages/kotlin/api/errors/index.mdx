...
The Kotlin SDK raises exceptions that all extend the sealed base class `SurrealException`. Because the hierarchy is sealed, you can branch over it exhaustively with `when`. See [Error handling](/docs/languages/kotlin/concepts/error-handling) for usage patterns and the [`Result` variants](/docs/languages/kotlin/concepts/executing-queries#result-variants) that avoid exceptions altogether.
...
Raised when a feature is invoked that the current transport does not support — for example a [live query](/docs/languages/kotlin/concepts/live-queries) or [transaction](/docs/languages/kotlin/concepts/transactions) over HTTP. Extends the base [`SurrealException`](#surreal-exception). Guard against it with [`.supports()`](/docs/languages/kotlin/api/core/surreal-client#supports).
...
- [Error handling](/docs/languages/kotlin/concepts/error-handling)
