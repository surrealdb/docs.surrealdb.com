...
The Go SDK uses Go's standard `error` interface and the `errors.As` / `errors.Is` functions for error handling. Errors fall into three main categories: structured [server errors](/docs/languages/golang/api/errors#servererror) from SurrealDB, per-statement [query errors](/docs/languages/golang/api/errors#queryerror), and transport-level failures.
...
<table>
	...
	<tbody>
		<tr>
			<td scope="row" data-label="Error type"><a href="/docs/languages/golang/api/errors#servererror"><code>ServerError</code></a></td>
			<td scope="row" data-label="Description">Structured error from SurrealDB v3 with kind, details, and cause chain</td>
		</tr>
		<tr>
			<td scope="row" data-label="Error type"><a href="/docs/languages/golang/api/errors#queryerror"><code>QueryError</code></a></td>
			<td scope="row" data-label="Description">Per-statement query failure returned within QueryResult</td>
		</tr>
		<tr>
			<td scope="row" data-label="Error type"><a href="/docs/languages/golang/api/errors#rpcerror"><code>RPCError</code></a></td>
...
SurrealDB v3 returns structured errors with a `Kind`, `Message`, `Details`, and an optional `Cause` chain. Use `errors.As` to extract a [`ServerError`](/docs/languages/golang/api/errors#servererror):
...
When using `Query`, individual statements can fail without causing the entire call to error. Each [`QueryResult`](/docs/languages/golang/api/types#queryresult) has an `Error` field that contains a [`QueryError`](/docs/languages/golang/api/errors#queryerror) if that statement failed.
...
The SDK defines several [sentinel errors](/docs/languages/golang/api/errors#sentinel-errors) for common failure conditions:
...
- [Errors API reference](/docs/languages/golang/api/errors) for complete error type definitions
- [Executing queries](/docs/languages/golang/concepts/executing-queries) for query-level error handling
- [Types reference](/docs/languages/golang/api/types) for `QueryResult` and `QueryError` definitions
