...
Then pass the connection to [`FromConnection`](/docs/languages/golang/api/core/db#fromconnection) to create a [`*DB`](/docs/languages/golang/api/core/db):
...
1. **Namespace and database** -- the last values passed to [`.Use()`](/docs/languages/golang/api/core/db#use)
2. **Authentication** -- the last token from [`.SignIn()`](/docs/languages/golang/api/core/db#signin), [`.SignUp()`](/docs/languages/golang/api/core/db#signup), or [`.Authenticate()`](/docs/languages/golang/api/core/db#authenticate)
3. **Connection variables** -- all variables set with [`.Let()`](/docs/languages/golang/api/core/db#let) (and removed with [`.Unset()`](/docs/languages/golang/api/core/db#unset))
4. **[Live queries](/docs/languages/golang/concepts/live-queries)** -- all active live queries are re-subscribed, and notification routing is restored

> [!NOTE]
> If the authentication token has expired by the time reconnection occurs, re-authentication will fail. The application is responsible for handling token expiry, for example by using [refresh tokens](/docs/languages/golang/concepts/authentication#using-refresh-tokens).
...
- [Connecting to SurrealDB](/docs/languages/golang/concepts/connecting-to-surrealdb) for standard connection setup
- [Live queries](/docs/languages/golang/concepts/live-queries) for live query setup that persists across reconnections
- [DB API reference](/docs/languages/golang/api/core/db) for `FromConnection` usage
