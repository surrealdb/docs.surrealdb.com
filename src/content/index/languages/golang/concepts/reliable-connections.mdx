...
Then pass the connection to [`FromConnection`](/docs/start/languages/golang/api/core/db#fromconnection) to create a [`*DB`](/docs/start/languages/golang/api/core/db):
...
1. **Namespace and database** -- the last values passed to [`.Use()`](/docs/start/languages/golang/api/core/db#use)
2. **Authentication** -- the last token from [`.SignIn()`](/docs/start/languages/golang/api/core/db#signin), [`.SignUp()`](/docs/start/languages/golang/api/core/db#signup), or [`.Authenticate()`](/docs/start/languages/golang/api/core/db#authenticate)
3. **Connection variables** -- all variables set with [`.Let()`](/docs/start/languages/golang/api/core/db#let) (and removed with [`.Unset()`](/docs/start/languages/golang/api/core/db#unset))
4. **[Live queries](/docs/start/languages/golang/concepts/live-queries)** -- all active live queries are re-subscribed, and notification routing is restored

> [!NOTE]
> If the authentication token has expired by the time reconnection occurs, re-authentication will fail. The application is responsible for handling token expiry, for example by using [refresh tokens](/docs/start/languages/golang/concepts/authentication#using-refresh-tokens).
...
- [Connecting to SurrealDB](/docs/start/languages/golang/concepts/connecting-to-surrealdb) for standard connection setup
- [Live queries](/docs/start/languages/golang/concepts/live-queries) for live query setup that persists across reconnections
- [DB API reference](/docs/start/languages/golang/api/core/db) for `FromConnection` usage
