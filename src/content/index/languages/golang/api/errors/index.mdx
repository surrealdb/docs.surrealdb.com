...
Represents a per-statement error within a query result. Returned in the `Error` field of [`QueryResult`](/docs/start/languages/golang/api/types#queryresult) when an individual statement fails.
...
- [Error handling](/docs/start/languages/golang/concepts/error-handling) for error handling patterns and retry guidance
- [Types reference](/docs/start/languages/golang/api/types) for [`QueryResult`](/docs/start/languages/golang/api/types#queryresult) containing `QueryError`
- [DB reference](/docs/start/languages/golang/api/core/db) for methods that return these errors
- [Session reference](/docs/start/languages/golang/api/core/session) for `ErrSessionClosed` and `ErrSessionsNotSupported`
- [Transaction reference](/docs/start/languages/golang/api/core/transaction) for `ErrTransactionClosed` and `ErrTransactionsNotSupported`
