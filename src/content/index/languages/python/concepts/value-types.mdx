...
<table>
	...
	<tbody>
		<tr>
			<td scope="row" data-label="Class"><a href="/docs/languages/python/api/values/record-id"><code>RecordID(table_name, identifier)</code></a></td>
		</tr>
		<tr>
			<td scope="row" data-label="Class"><a href="/docs/languages/python/api/values/table"><code>Table(table_name)</code></a></td>
		</tr>
		<tr>
			<td scope="row" data-label="Class"><a href="/docs/languages/python/api/values/duration"><code>Duration(elapsed)</code></a></td>
		</tr>
		<tr>
			<td scope="row" data-label="Class"><a href="/docs/languages/python/api/values/datetime"><code>Datetime(dt)</code></a></td>
		</tr>
		<tr>
			<td scope="row" data-label="Class"><a href="/docs/languages/python/api/values/range"><code>Range(begin, end)</code></a></td>
		</tr>
		<tr>
			<td scope="row" data-label="Class"><a href="/docs/languages/python/api/values/geometry"><code>GeometryPoint(...)</code></a></td>
		</tr>
	</tbody>
</table>
...
| `dict` | `object` |
| `list` | `array` |
| [`RecordID`](/docs/languages/python/api/values/record-id) | `record` |
| [`Table`](/docs/languages/python/api/values/table) | `table` |
| [`Duration`](/docs/languages/python/api/values/duration) | `duration` |
| [`Datetime`](/docs/languages/python/api/values/datetime) | `datetime` |
| [`Range`](/docs/languages/python/api/values/range) | `range` |
| [`Geometry*`](/docs/languages/python/api/values/geometry) | `geometry` |

## Working with record identifiers

A [`RecordID`](/docs/languages/python/api/values/record-id) uniquely identifies a single record in a table by combining a table name with an identifier value. You can construct one directly or parse it from a string.
...
See the [RecordID reference](/docs/languages/python/api/values/record-id) for the complete API, including equality checks and Pydantic support.

## Working with durations

A [`Duration`](/docs/languages/python/api/values/duration) represents a time span with nanosecond precision. The most common way to create one is by parsing a human-readable string using SurrealDB's duration syntax.
...
See the [Duration reference](/docs/languages/python/api/values/duration) for all unit properties and methods.

## Working with datetime values

A [`Datetime`](/docs/languages/python/api/values/datetime) wraps an ISO 8601 datetime string for use with SurrealDB's `datetime` type. It preserves the original string representation through serialization and deserialization.
...
See the [Datetime reference](/docs/languages/python/api/values/datetime) for the full API.

## Working with ranges

A [`Range`](/docs/languages/python/api/values/range) represents a bounded interval with inclusive or exclusive endpoints. Each bound is wrapped in a `BoundIncluded` or `BoundExcluded` to specify whether the boundary value is part of the range.
...
See the [Range reference](/docs/languages/python/api/values/range) for the full API and bound types.
...
More complex types compose from simpler ones — a `GeometryLine` is built from a list of points, a `GeometryPolygon` from a list of lines, and so on. See the [Geometry reference](/docs/languages/python/api/values/geometry) for constructors and examples of all seven types.
...
- [Data manipulation](/docs/languages/python/concepts/data-manipulation) for using types in queries
