...
<table>
	...
	<tbody>
		<tr>
			<td scope="row" data-label="Method"><a href="/docs/languages/python/api/core/surreal#signin"><code>db.signin(vars)</code></a></td>
			<td scope="row" data-label="Description">Signs in as a root, namespace, database, or record user</td>
		</tr>
		<tr>
			<td scope="row" data-label="Method"><a href="/docs/languages/python/api/core/surreal#signup"><code>db.signup(vars)</code></a></td>
			<td scope="row" data-label="Description">Signs up a new record user via an access method</td>
		</tr>
		<tr>
			<td scope="row" data-label="Method"><a href="/docs/languages/python/api/core/surreal#authenticate"><code>db.authenticate(token)</code></a></td>
			<td scope="row" data-label="Description">Authenticates the connection with an existing JWT token</td>
		</tr>
		<tr>
			<td scope="row" data-label="Method"><a href="/docs/languages/python/api/core/surreal#invalidate"><code>db.invalidate()</code></a></td>
			<td scope="row" data-label="Description">Invalidates the current authentication session</td>
		</tr>
		<tr>
			<td scope="row" data-label="Method"><a href="/docs/languages/python/api/core/surreal#info"><code>db.info()</code></a></td>
...
The `.signin()` method authenticates the connection. The fields you pass determine the authentication level. The method returns [Tokens](/docs/languages/python/api/types/#tokens) on success, which contain the JWT access token and optional refresh token.

Refer to the [API reference](/docs/languages/python/api/core/surreal#signin) for the full list of parameters at each level.
...
The `.signup()` method registers a new record user through a [record access method](/docs/reference/query-language/statements/define/access/record) and returns [Tokens](/docs/languages/python/api/types/#tokens). Signup is only available for record-level access.
...
The return value is a [Value](/docs/languages/python/api/types/#value) containing the fields of the authenticated user's record. If no record user is authenticated, the method returns `None`.
...
- [Surreal API reference](/docs/languages/python/api/core/surreal) for complete method signatures and parameters
- [Tokens type reference](/docs/languages/python/api/types/#tokens) for the structure of the returned tokens
...
- [Connecting to SurrealDB](/docs/languages/python/concepts/connecting-to-surrealdb) for connection setup and protocol options
