...
All errors raised by the Python SDK extend [`SurrealError`](/docs/languages/python/api/errors/#surrealerror), so you can catch every SDK error with a single `except` clause. Server-originated errors use the [`ServerError`](/docs/languages/python/api/errors/#servererror) subtree with structured kinds, details, and cause chains. SDK-side errors cover connection, parsing, and feature support failures.

See the [Errors reference](/docs/languages/python/api/errors/) for the complete error hierarchy and all available properties.
...
<table>
	...
	<tbody>
		<tr>
			<td scope="row" data-label="Error class"><a href="/docs/languages/python/api/errors/#surrealerror"><code>SurrealError</code></a></td>
			<td scope="row" data-label="Description">Base class for all SDK errors</td>
		</tr>
		<tr>
			<td scope="row" data-label="Error class"><a href="/docs/languages/python/api/errors/#servererror"><code>ServerError</code></a></td>
			<td scope="row" data-label="Description">Structured server error with kind, details, and cause</td>
		</tr>
		<tr>
			<td scope="row" data-label="Error class"><a href="/docs/languages/python/api/errors/#notallowederror"><code>NotAllowedError</code></a></td>
			<td scope="row" data-label="Description">Thrown when permission is denied</td>
		</tr>
		<tr>
			<td scope="row" data-label="Error class"><a href="/docs/languages/python/api/errors/#notfounderror"><code>NotFoundError</code></a></td>
			<td scope="row" data-label="Description">Thrown when a resource is not found</td>
		</tr>
		<tr>
			<td scope="row" data-label="Error class"><a href="/docs/languages/python/api/errors/#connectionunavailableerror"><code>ConnectionUnavailableError</code></a></td>
			<td scope="row" data-label="Description">Thrown when no connection is active</td>
		</tr>
		<tr>
			<td scope="row" data-label="Error class"><a href="/docs/languages/python/api/errors/#unsupportedfeatureerror"><code>UnsupportedFeatureError</code></a></td>
...
You can check whether an error is a `ServerError` and then inspect its kind using the constants defined on [`ErrorKind`](/docs/languages/python/api/errors/#errorkind).
...
For fine-grained control, catch the specific error subclass you need. The SDK maps server error kinds to dedicated Python classes such as `ValidationError`, [`NotAllowedError`](/docs/languages/python/api/errors/#notallowederror), and [`NotFoundError`](/docs/languages/python/api/errors/#notfounderror).
...
A [`ConnectionUnavailableError`](/docs/languages/python/api/errors/#connectionunavailableerror) is raised when you try to perform an operation before establishing a connection.
...
An [`UnsupportedFeatureError`](/docs/languages/python/api/errors/#unsupportedfeatureerror) is raised when you attempt to use a feature that requires a specific connection type. For example, sessions and transactions require a WebSocket connection.
...
An [`UnsupportedEngineError`](/docs/languages/python/api/errors/#unsupportedengineerror) is raised when the URL scheme is not recognized.
...
- [Errors reference](/docs/languages/python/api/errors/) for complete error hierarchy
- [ErrorKind constants](/docs/languages/python/api/errors/#errorkind) for error kind matching
- [Authentication](/docs/languages/python/concepts/authentication) for auth-related error patterns
- [Connecting to SurrealDB](/docs/languages/python/concepts/connecting-to-surrealdb) for connection error patterns
