...
When a `str` is passed, it is treated as a table name. Pass a [`RecordID`](/docs/languages/python/api/values/record-id) for specific record targeting, or a [`Table`](/docs/languages/python/api/values/table) for explicit table references.
