...
`SurrealValue` is the universal value type used to represent any data that can be sent to or received from SurrealDB. It is most often used when binding parameters to [raw queries](/docs/languages/swift/methods/query-raw) or when working with dynamically typed data.
...
A `SurrealRecordID` identifies a single record by its table and id. It is used by the record-targeted overloads of [`select`](/docs/languages/swift/methods/select), [`create`](/docs/languages/swift/methods/create), [`update`](/docs/languages/swift/methods/update) and [`delete`](/docs/languages/swift/methods/delete).
...
[`queryRaw`](/docs/languages/swift/methods/query-raw) returns an array of `RPCQueryResult`, one per statement in the query. Each result carries a `status` and a `result`.
