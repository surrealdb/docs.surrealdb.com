...
In addition to the high-level CRUD methods, the Swift SDK lets you build queries with a type-safe DSL. You can use either compile-time expression macros or the programmatic `SurrealDSL` builder, then run the result with [`query`](/docs/languages/swift/methods/query).
...
Run a query with the [`query`](/docs/languages/swift/methods/query) method:
...
For arbitrary SurrealQL that the DSL does not model, use [`queryRaw`](/docs/languages/swift/methods/query-raw) with bound parameters.
