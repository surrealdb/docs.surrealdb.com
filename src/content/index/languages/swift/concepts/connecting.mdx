...
The Swift SDK provides two transports: an HTTP client and a WebSocket client. Both share the same querying and CRUD API; the WebSocket client additionally supports [live queries](/docs/languages/swift/concepts/live-queries).
...
`SurrealWebSocketClient` connects over WebSocket and is required for [live queries](/docs/languages/swift/concepts/live-queries). It can also automatically reconnect when a connection is dropped.
