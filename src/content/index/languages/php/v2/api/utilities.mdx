...
| `events` | `?EventDispatcherInterface` | new dispatcher | PSR-14 dispatcher for [domain events](/docs/languages/php/v2/concepts/events) |
| `logger` | `?LoggerInterface` | null logger | PSR-3 logger; enables the [logging middleware](/docs/languages/php/v2/concepts/middleware#logging) |
| `tracer` | `?Tracer` | no-op tracer | Tracing backend; enables [telemetry](/docs/languages/php/v2/concepts/observability) |
| `meter` | `?Meter` | no-op meter | Metrics backend; enables [telemetry](/docs/languages/php/v2/concepts/observability) |
...
| `middleware` | `list<MiddlewareInterface>` | `[]` | Extra [middleware](/docs/languages/php/v2/concepts/middleware) appended to the pipeline |
| `pingInterval` | `int` | `30` | WebSocket ping interval in seconds |

The `engines` and `*Factory` options are the transport-level extension seams. The [runtime presets](/docs/languages/php/v2/concepts/runtimes) configure them for you, so you only set them when supplying a transport of your own.
