...
Sign in with a [credential object](/docs/languages/php/v2/concepts/authentication#credential-types) or an array. Returns a [`Tokens`](#tokens) object.
...
Execute a [`BoundQuery`](/docs/languages/php/v2/api/utilities#boundquery), returning one result per statement.
...
These methods start a fluent [query builder](/docs/languages/php/v2/api/query-builders). Call `execute()` to run it.
...
Iterating the result blocks the process while it waits for messages. See [running without blocking the application](/docs/languages/php/v2/concepts/live-queries#running-without-blocking-the-application) for consuming live queries in a worker.
...
- [Data types](/docs/languages/php/v2/api/data-types) for the value classes
- [Utilities](/docs/languages/php/v2/api/utilities) for `BoundQuery`, enums, and driver options
- [Connecting to SurrealDB](/docs/languages/php/v2/concepts/connecting-to-surrealdb) for the connection guide
