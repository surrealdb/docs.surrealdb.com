...
<table>
    ...
    <tbody>
        <tr>
            <td scope="row" data-label="Method"><a href="/docs/languages/php/v2/api/query-builders#run"> <code> $db->run($surql, $bindings) </code></a></td>
            <td scope="row" data-label="Description">Executes raw SurrealQL statements</td>
        </tr>
        <tr>
            <td scope="row" data-label="Method"><a href="/docs/languages/php/v2/api/query-builders#select"> <code> $db->select($target) </code></a></td>
            <td scope="row" data-label="Description">Selects records from the database</td>
        </tr>
        <tr>
            <td scope="row" data-label="Method"><a href="/docs/languages/php/v2/api/query-builders#create"> <code> $db->create($target) </code></a></td>
            <td scope="row" data-label="Description">Creates a new record</td>
        </tr>
        <tr>
            <td scope="row" data-label="Method"><a href="/docs/languages/php/v2/api/query-builders#insert"> <code> $db->insert($target, $data) </code></a></td>
            <td scope="row" data-label="Description">Inserts one or many records</td>
        </tr>
        <tr>
            <td scope="row" data-label="Method"><a href="/docs/languages/php/v2/api/query-builders#update"> <code> $db->update($target) </code></a></td>
            <td scope="row" data-label="Description">Updates existing records</td>
        </tr>
        <tr>
            <td scope="row" data-label="Method"><a href="/docs/languages/php/v2/api/query-builders#delete"> <code> $db->delete($target) </code></a></td>
            <td scope="row" data-label="Description">Deletes records from the database</td>
        </tr>
        <tr>
            <td scope="row" data-label="Method"><a href="/docs/languages/php/v2/api/query-builders#relate"> <code> $db->relate($from, $edge, $to) </code></a></td>
...
Every builder method returns a builder object. Call `execute()` to run it and get the result of the single statement, or `compile()` to get the [`BoundQuery`](/docs/languages/php/v2/api/utilities#boundquery) without running it.
...
The `select()` method reads records. Pass a [`Table`](/docs/languages/php/v2/api/data-types#table) to read all records, or a [`RecordId`](/docs/languages/php/v2/api/data-types#recordid) to read one. Chain `fields()`, `where()`, `start()`, `limit()`, and `fetch()` to refine the query.
...
- [Query Builders API reference](/docs/languages/php/v2/api/query-builders) for every builder method
- [Live queries](/docs/languages/php/v2/concepts/live-queries) for real-time subscriptions
- [Transactions](/docs/languages/php/v2/concepts/transactions) for atomic multi-statement operations
