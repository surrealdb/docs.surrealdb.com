...
Additional sessions are managed through the [`ConnectionController`](/docs/languages/php/v2/api/core#connectioncontroller), which you reach with `connection()`.
...
The same call accepts a transaction id as a third argument, so a session can run statements inside an explicit [transaction](/docs/languages/php/v2/concepts/transactions).

## Learn more

- [Surreal API reference](/docs/languages/php/v2/api/core#connectioncontroller) for the session methods
- [Transactions](/docs/languages/php/v2/concepts/transactions) for running statements atomically
- [Connecting to SurrealDB](/docs/languages/php/v2/concepts/connecting-to-surrealdb) for the default session and WebSocket setup
