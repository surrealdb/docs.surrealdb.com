...
Use [`subscribe()`](/docs/languages/php/v2/api/core#subscribe) for quick hooks into the connection lifecycle. Pass an event name and a listener; the method returns a closure that removes the listener when called.
...
> [!NOTE]
> `RpcRequestSent` and `RpcResponseReceived` fire for every call, so keep their listeners cheap. For tracing and metrics, prefer the dedicated [telemetry](/docs/languages/php/v2/concepts/observability) seam, which is built on this same pipeline.
...
- [Surreal API reference](/docs/languages/php/v2/api/core#subscribe) for the `subscribe()` signature
- [Observability](/docs/languages/php/v2/concepts/observability) for tracing and metrics
- [Connecting to SurrealDB](/docs/languages/php/v2/concepts/connecting-to-surrealdb) for the lifecycle these events track
