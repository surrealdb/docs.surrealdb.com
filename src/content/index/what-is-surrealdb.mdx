...
SurrealDB is a [multi-model database](/blog/what-are-multi-model-databases) built in Rust designed to unify multiple data models into a single, powerful engine. It combines [document](/docs/learn/data-models/document/overview), [graph](/docs/learn/data-models/graph/overview), [time-series](/docs/learn/data-models/time-series/overview), [relational](/docs/explore/tutorials/tutorials/define-a-schema), [geospatial](/docs/learn/data-models/geospatial/overview) and key-value data types into one query language, [SurrealQL](/docs/reference/query-language), with powerful search and retrieval ([vector](/docs/learn/data-models/vector-search/overview), [full-text](/docs/learn/data-models/full-text-search/overview), hybrid), and [real-time and event-driven capabilities](/docs/learn/querying/real-time/real-time-best-practices), enabling developers to build applications faster and more efficiently.
...
SurrealDB is source-available (see the code [here on GitHub](https://github.com/surrealdb/surrealdb)) and is also available as a cloud managed service through [SurrealDB Cloud](/docs/manage/instances).
...
- **Native ACID compliance:** [snapshot isolation](/docs/transactions-and-isolation) on every transaction, with immediate consistency after a commit, and [locked reads](/docs/transactions-and-isolation#locked-reads-with-for-update) when a decision rests on a record the transaction does not write. Opt in to eventual consistency in certain cases if desired.
