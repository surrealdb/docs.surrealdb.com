...
SurrealMX does not support [versioned queries](/docs/reference/query-language/statements/select#the-version-clause). From 3.3.0 a server started on it with `versioned=true` or a non-zero `retention` fails to start. Earlier 3.x releases supported versioned queries on SurrealMX. For versioned queries, use a [file-backed](/docs/running/file-backed) server on SurrealKV or RocksDB. <Since v="v3.3.0" />
