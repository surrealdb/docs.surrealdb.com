...
Since SurrealDB 3.0, in-memory storage runs on a backend called [SurrealMX](https://github.com/surrealdb/surrealmx) that also allows for [persistent storage](/docs/reference/cli/surrealdb-cli/commands/start#supported-parameters-for-memory-surrealmx) as a snapshot or AOL (append-only log).

SurrealMX does not support [versioned queries](/docs/reference/query-language/statements/select#the-version-clause). From 3.3.0 it refuses the `versioned` and `retention` parameters, so a server started with either one fails to start. Earlier 3.x releases supported versioned queries on SurrealMX. For versioned queries, use a [file-backed](/docs/running/file-backed) server on SurrealKV or RocksDB. <Since v="v3.3.0" />

SurrealMX uses a commit pipeline that splits the commit process into discrete, narrowly-scoped stages. Multiple transactions can then progress through different stages of the pipeline concurrently, much like instruction pipelining in a CPU.

Opting in to persistent storage is done by adding a path to the area to store the data as well as parameters to customise the type of storage and its behaviour. For example, the following command will start an in-memory server that stores its data in snapshots taken every 60 seconds.
