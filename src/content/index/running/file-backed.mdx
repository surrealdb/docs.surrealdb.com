...
A single node of SurrealDB can be run with data persisted to the filesystem. This configuration can be done using either RocksDB or SurrealKV as the backend. Both use the same [transaction and isolation semantics](/docs/transactions-and-isolation) as every other SurrealDB deployment.
...
- MVCC-backed concurrent reads and writes on disk (isolation semantics are defined by the [query layer](/docs/transactions-and-isolation), not by the engine alone).
