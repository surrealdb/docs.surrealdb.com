...
Both the SurrealDB database server and the [command-line tool](/docs/reference/cli/surrealdb-cli/overview) are packaged as a single executable file. You can install via the [install script](/docs/self-hosted/installation/windows#installing-surrealdb-using-the-install-script), [Chocolatey](https://chocolatey.org/), or [Scoop](https://scoop.sh/).
