...
- **Built-in Versioning**: Start SurrealKV [in versioned mode](/docs/reference/cli/surrealdb-cli/start#surrealkv-beta) to track and access historical versions of your data.
...
A number of parameters can be used on startup such as `versioned` to set whether to use versioning or not, and `sync` to specify when to flush the database to the file system. For more details on these parameters, see [this page](/docs/reference/cli/surrealdb-cli/start#supported-parameters-for-surrealkv) for the `start` command or [this page](/docs/reference/cli/surrealdb-cli/environment-variables#surrealkv-environment-variables) to set the same parameters using environment variables.
...
SurrealKV does have some important limitations to consider, many of which pertain to SurrealKV [when versioning is enabled](/docs/reference/cli/surrealdb-cli/start#surrealkv-beta):
