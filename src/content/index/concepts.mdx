...
SurrealDB is designed to run in many different ways and environments. Due to the [separation of the storage and compute layers](/docs/architecture), you can use it as an embedded database, a vertically scalable single-node server, or a horizontally scalable multi-node cluster — with the same SurrealQL and APIs throughout. In-memory operation (including [SurrealMX](/docs/running/in-memory)), browser persistence via [IndexedDB](https://developer.mozilla.org/en-US/docs/Web/API/IndexedDB_API), on-disk [RocksDB](/docs/running/file-backed) for write-heavy server deployments, and optional beta [SurrealKV](/docs/running/file-backed) for embedded and local-first deployments cover most local and single-node cases. For distributed production, use [SurrealDB Cloud](/docs/build/deployment/surrealdb-cloud/what-is-surrealdb-cloud) or self-hosted highly available clusters on [Amazon EKS](/docs/build/deployment/self-hosted/amazon-eks), [Google GKE](/docs/build/deployment/self-hosted/google-gke), and [Azure AKS](/docs/build/deployment/self-hosted/azure-aks). See [Deployment](/docs/build/deployment) for a full comparison and guide.
