...
Let's go ahead and list info about the SurrealDB instance and also about the current namespace from the [`demo dataset`](/docs/start).
...
Next, we can create our own namespace called `acme` using [`DEFINE NAMESPACE`](/docs/reference/query-language/statements/define/namespace) and list the namespaces in the SurrealDB instance.
...
Now that we have the `acme` namespace in the SurrealDB instance we can switch to use that namespace via the [`USE`](/docs/reference/query-language/statements/use) statement.
