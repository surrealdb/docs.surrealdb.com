...
SurrealDB is designed to run in many different ways and environments. Due to the [separation of the storage and compute layers](/docs/architecture), you can use it as an embedded database, a vertically scalable single-node server, or a horizontally scalable multi-node cluster — with the same SurrealQL and APIs throughout. In-memory operation (including [SurrealMX](/docs/self-hosted/in-memory)), browser persistence via [IndexedDB](https://developer.mozilla.org/en-US/docs/Web/API/IndexedDB_API), on-disk [RocksDB](/docs/self-hosted/file-backed), and optional beta [SurrealKV](/docs/self-hosted/file-backed) cover most local and single-node cases; distributed production deployments use SurrealDS (<Edition value="enterprise" />) or [SurrealDB Cloud](/docs/build/deployment/surrealdb-cloud/what-is-surrealdb-cloud). See [Deployment](/docs/build/deployment) for a full comparison and guide.
