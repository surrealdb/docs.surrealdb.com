...
For example, you have a table called `users` in your application, the document model database will store user data (e.g., name, age, profile settings, recent activity) in a single JSON-like document. 
...
For example, if you wanted to associate a person with an article they wrote, you could do this by assigning the person's ID to the `author` field of the article document.
...
When thinking in a document model database, you will often find that the concepts are similar to those in SurrealDB. Below is a mapping of the most common concepts between MongoDB as an example of a document model database and SurrealDB.
...
## Benefits of Using Document Model Databases
...
In SurrealDB, you can create a database, specify a namespace (like a schema in relational parlance), and then store collections of documents (often referred to as “tables”) without strict schema definitions. Let’s walk through an example.

### Setting Up a Sample Namespace and Table

```surql
-- Use or create a namespace for our data.
...
In a document model, you’ll typically query by top-level fields or nested fields. SurrealDB supports standard filters:
...
For nested structures, SurrealDB allows queries on nested fields:

```surql
-- SELECT * FROM users WHERE addresses[*].city = "Springfield";
```

The `addresses[*].city` syntax indicates that SurrealDB will inspect each element in the `addresses` array and match if the city is `"Springfield"`.

### Creating Indexes

Indexing in document databases is often essential for performance, especially when you’re dealing with large datasets. SurrealDB lets you create indexes in a manner somewhat reminiscent of relational databases. For instance:

```surql
-- DEFINE INDEX on users FIELDS email UNIQUE;
```

This creates a unique index on the `email` field in the `users` table. Now queries that filter by `email` can use this index to speed up lookups. You can also index nested fields, though the syntax will vary depending on how SurrealDB manages advanced indexing for arrays and objects. */}
