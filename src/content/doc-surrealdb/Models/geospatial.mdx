...
Historically, geospatial capabilities were mostly the domain of specialized systems such as PostGIS (an extension of PostgreSQL) or dedicated GIS software. More recently, however, general-purpose or multi-model databases, including SurrealDB, have begun offering geospatial features to unify location-related data with other data models (relational, document, graph, vector, etc.) under one roof.
...
We’ll explore these concepts through examples in SurrealDB—a multi-model database that offers flexible schemas, document storage, graph capabilities, real-time queries, and more.
