...
If we now wanted to use environment variables instead of the `--allow-net` and `--allow-funcs` flags, we would use the `SURREAL_CAPS_ALLOW_NET` and `SURREAL_CAPS_ALLOW_FUNC` environment variables.

As the `--allow-net` flag was passed in without a following value, the same will be the case with the `SURREAL_CAPS_ALLOW_NET` environment variable, becoming `SURREAL_CAPS_ALLOW_NET=`. The `--allow-funcs` flag can also be used on its own to allow execution of all functions, but in this case is followed by a string to indicate which exact functions are allowed to be executed. As such, the `SURREAL_CAPS_ALLOW_FUNC` environment variable must also be followed by a string, becoming `SURREAL_CAPS_ALLOW_FUNC="array, string, crypto::argon2, http::get"`.

The command would then look like the following in Linux:

```bash
export SURREAL_CAPS_ALLOW_NET
