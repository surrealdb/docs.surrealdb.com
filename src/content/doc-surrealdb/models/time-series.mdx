...
Our [pre-computed table views](/docs/surrealql/statements/define/table#pre-computed-table-views) are most similar to event-based, incrementally updating, materialised views. Practically, this means our downsampled metrics will always be up to date as it incrementally updates in near real-time when we add more records to the sensor\_readings table.
...
1. Define a event to trigger when a record is added to the sensor\_readings table.
