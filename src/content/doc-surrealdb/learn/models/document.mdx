...
In this guide, we will explore how to think using a document database model, in **SurrealDB**. We will also explore CRUD statements and how to query data in SurrealDB.
...
The above query returns all documents in the `users` table, much like a traditional SQL `SELECT` statement:
...
For more on the SurrealQL equivalent to MongoDB data types and how to automatically import MongoDB data into a SurrealDB instance, see these pages from the Surreal Sync migration tool:
