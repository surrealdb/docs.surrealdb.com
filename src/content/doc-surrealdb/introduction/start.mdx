...
Let's now create a blog article record, which links to the `person` and `account` tables. In the following example we link to the person record directly in the article table's `author` field, and we link to the account record with a subquery which gives us the ID for ACME Inc.
