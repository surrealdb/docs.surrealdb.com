...
When we say [multi-model](/blog/what-are-multi-model-databases), we mean that SurrealDB can store and query data in a variety of ways. You can use SurrealDB as a [Graph](/docs/surrealdb/reference-guide/graph_relations), [Key-Value](/docs/surrealdb/cli/start#surrealkv-beta), [Document](/docs/surrealdb/reference-guide/graph_relations), [Time-Series](/blog/its-about-time-time-series-in-surrealdb), or [Vector](/docs/surrealdb/reference-guide/vector-search) database and it is all designed to be queried with our [SurrealQL](/docs/surrealql) query language. 
...
- [Surreal Cloud](#get-started-via-surreal-cloud)
- [Install on your machine](#install-on-your-machine)
- [SDK](#getting-started-with-the-an-sdk)
...
- [Create a Cloud Account](/docs/surrealdb/reference-guide/full-text-search)
- [Create a Cloud Instance](/docs/cloud/getting-started/create-an-instance)
- [Connect to your Cloud Instance](/docs/cloud/getting-started/connect-to-an-instance)

## Getting started via an SDK 

SurrealDB offers SDKs for a variety of programming languages. You can use these SDKs to interact with SurrealDB from your application. This allows you use methods to interact with SurrealDB. Depending on the language you are using, you can use the SDK to start the database, run queries, and more. Follow the links below to learn more about the different SDKs.
...
## Get started in-memory or on-disk

SurrealDB can also be run locally on your machine in-memory or on-disk. This is useful if you want to test out SurrealDB or if you want to run SurrealDB without the need for an internet connection. You can also use this method in a production environment if you want to run SurrealDB in a local network. 

You can either [run single node in-memory](//docs/surrealdb/installation/running/file), [run multi-node scalable cluster](/docs/surrealdb/installation/running/memory) or [run multi-node, scalable cluster](/docs/surrealdb/installation/running/tikv).
...
After you have installed SurrealDB, you can start the database using the [SurrealDB CLI](/docs/surrealdb/cli/start) using the `surreal start` command, the major differentiator between starting in-memory or on-disk is the suffix for on-disk to assign the backend engine.


<Tabs groupId="node-package-manager">
  <TabItem value="bun" label="In-Memory">
    ```bash
    surreal start memory
    ```
  </TabItem>
  <TabItem value="npm" label="Single Node On-Disk SurrealKV" default>
    ```bash
    surreal start -u root -p root  surrealkv://mydb
    ```
  </TabItem>
  <TabItem value="yarn" label="Single Node On-Disk rocksDB">
    ```bash
    surreal start -u root -p root rocksdb://mydb
    ```
  </TabItem>
  <TabItem value="pnpm" label="Multi-Node Scalable Cluster">
    ```bash
...
To learn more about the different backend engines, you can refer to the [running](/docs/surrealdb/installation/running) documentation.
...
By completing this guide you have successfully set up a SurrealDB database and ran some SurrealQL queries. To learn more about the ways you can query SurrealDB check the [querying](/docs/surrealdb/querying) section, for more examples and guides, refer to the [SurrealQL documentation](/docs/surrealql). 
