...
> [!IMPORTANT]
> Capabilities can be enabled and configured in SurrealDB Cloud. This is done by clicking on the "Configure instance" button in the SurrealDB Cloud dashboard. Learn more about capabilities in SurrealDB Cloud [in the Cloud documentation](/docs/cloud/getting-started/create-an-instance#configure-an-instance).
...
You can learn more about best practices when using capabilities in our [Security Best Practices](/docs/surrealdb/security/security-best-practices#capabilities) guide.
...
The `--deny-arbitrary-query` flag is often preceded with a [`DEFINE API`](/docs/surrealql/statements/define/api) statement to set up certain endpoints that users can use to access database resources in place of arbitrary queries.