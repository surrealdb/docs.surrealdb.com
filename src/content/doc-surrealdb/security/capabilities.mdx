...
> [!WARNING]
> SurrealDB currently does not perform reverse DNS lookups to prevent http functions directly accessing an IP address even when the related hostname is listed within `--deny-net`. This is an issue when SurrealDB is configured with allow network access by default e.g `--allow-net --deny-net www.google.com` or when the IP is in the allowlist, but the hostname is in the denylist e.g `--allow-net 172.217.169.14 #google IP --deny-net www.google.com` 

It is recommended that you deny by default by defining specific `--allow-net` targets and provide additional layers of network security within your infrastructure e.g firewalls.
