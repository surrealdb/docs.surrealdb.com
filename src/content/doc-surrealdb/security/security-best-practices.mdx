...
This guide outlines some key security best practices for using SurrealDB. While SurrealDB offers powerful and flexible features to help you meet your desired security standards, your use of those features will ultimately determine whether or not you meet them.
...
The best way to protect tokens against stealing is to not store them at all. If your use case supports it, use the token in memory to [`authenticate`](/docs/sdk/javascript/api/core/surreal-session) a persistent session using the WebSocket protocol and destroy the token from memory after the session is established. When the session expires, ask your users to sign in again with their credentials and establish a new authenticated session with SurrealDB. Your use case may even support not using a token at all by directly authenticating the session with user credentials using [`signin`](/docs/sdk/javascript/api/core/surreal-session#signin).
...
- Encode or at least sanitize all [untrusted input](/docs/surrealdb/security/security-best-practices#content-safety) before showing it on the page.
