...
## Vector Search Cheatsheet

Indexes: M-Tree, HNSW, brute force
...
|---------------|----------|-----------|-------------------------|--------------
| DIMENSION     | yes      |           |                         | Size of the vector
| DIST          | no       | EUCLIDEAN | EUCLIDEAN, COSINE, MANHATTAN | Distance function
| TYPE          | no       | F64       | F64, F32, I64, I32, I16 | Vector type
| CAPACITY      | no       | 40        |                         | Max number of records that can be stored in the index
| DOC_IDS_ORDER | no       | 100       |
| DOC_IDS_CACHE | no       | 100       |
| MTREE_CACHE   | no       | 100       |
...
| DIST          | no       | EUCLIDEAN | EUCLIDEAN, COSINE, MANHATTAN | Distance function
| TYPE          | no       | F64       | F64, F32, I64, I32, I16 | Vector type
| EFC           | no       | 150       |                         | EF construction
| M             | no       | 12        |                         | max connections per element
| M0            | no       | 24        |                         | max connections in the lowest layer
| LM            | no       | 0.40242960438184466f |              | multiplier for level generation
...
FROM pts
WHERE point
    <|2|>  -- return 2, in this case using the default euclidean function
    $vector;
```
...
| vector::distance::minkowski(point, $vector, 3) |
...
| <\|2, CHEBYSHEV\|>      |
| <\|2, HAMMING\|>        |
| <\|2, 10\|>             | couldn't figure this out

### Questions

- <|2, 10|> what it the second parameter, and how to use it?
- should the query always use the same distance function as the one used when creating the index?

### Tips

- Verify index utilization in queries using the [`EXPLAIN` clause](/docs/surrealql/statements/select#the-explain-clause). E.g: `SELECT id FROM pts WHERE point <|10, EUCLIDEAN|> [2,3,4,5] EXPLAIN;`
