...
TODO: Checking with Micha on why some of these don't work now
...
> [!NOTE]
> Many fields in these schemas use [`COMPUTED` fields](/docs/surrealql/statements/define/field#computed-fields), available as of SurrealDB version 3.0.0-beta.1. For versions before this, a data type called a [`future`](/docs/surrealql/datamodel/futures) was used. To use such fields in a version before 3.0.0-beta.1, replace `COMPUTED` with `VALUE <future>` and wrap the expression in `{}` braces.
