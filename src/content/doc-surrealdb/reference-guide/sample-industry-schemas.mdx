...
> [!NOTE]
> Many fields in these schemas use [`COMPUTED` fields](/docs/surrealql/statements/define/field#computed-fields), available as of SurrealDB version 3.0.0-alpha.7. For versions before this, a data type called a [`future`](/docs/surrealql/datamodel/futures) was used. To use such fields in a version before 3.0.0-alpha.7, replace `COMPUTED` with `VALUE <future>` and enter an expression to be calculated inside `{}` braces.
