...
Database testing by users is most frequently done using an SDK, particularly for the languages in which SurrealDB can be used [in embedded mode](/docs/surrealdb/embedding). This allows testing to be conducted in just a few lines of code.
...
Testing can also be done through direct SurrealQL queries to the database. While the CLI and Surrealist are the most convenient tools to use, sending in queries via the [HTTP endpoints](/docs/surrealdb/integration/http) may be more convenient when needing to test output depending on certain environments such as queries as a record user vs. as a database-level system user or a root user.
...
One or more `.surql` files can be validated by the CLI using the [`surreal validate`](/docs/surrealdb/cli/validate) command.
