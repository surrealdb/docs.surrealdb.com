...
In order to allow querying the created table using GraphQL, you will need to explicitly enable graphql using the [`DEFINE CONFIG graphql`](/docs/surrealql/statements/define/config) statement. This will allow you to query the table using GraphQL on a per-database basis.
...
To use the GraphQL API, you first need to enable it using the `DEFINE CONFIG graphql` statement. This will allow you to query the table using GraphQL on a per-database basis.

To do this, you can send a `POST` request to the `/sql` endpoint with a JSON body containing the `DEFINE CONFIG graphql` statement. For example:
