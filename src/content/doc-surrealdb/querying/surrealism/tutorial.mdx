...
To allow these functions to be accessed from SurrealDB, first we can add Surrealism to Cargo.toml. To use the `SurrealValue` trait above 

We'll also add `surrealdb-types`, which provides access to the `SurrealValue` trait above. Adding the entire `surrealdb` crate is not necessary.
