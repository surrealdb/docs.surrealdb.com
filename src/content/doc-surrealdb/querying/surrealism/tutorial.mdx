...
To allow these functions to be accessed from SurrealDB, first we can add Surrealism to Cargo.toml. We'll also add `surrealdb-types`, which provides access to the `SurrealValue` trait above. Adding the entire `surrealdb` crate is not necessary, which is a nice plus for compilation speed.

```toml
surrealism = "0.1.7"
surrealdb-types = "0.8.2"
```
...
```
SURREAL_CAPS_ALLOW_EXPERIMENTAL=files,surrealism SURREAL_BUCKET_FOLDER_ALLOWLIST="/Users/my_name/my_rust_code/" cargo run -- sql --ns ns --db db --user root --pass strongpassword
```
