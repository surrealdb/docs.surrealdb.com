...
When a query fails, the response includes structured error information with a machine-readable `kind`, optional `details`, and an optional `cause` chain:

<Tabs groupId="http-sql">
<TabItem value="V2" label="V2.x+" default>
```bash title="Request"
curl -X POST -u "root:secret" -H "surreal-ns: main" -H "surreal-db: main" -H "Accept: application/json" -d "SELECT * FROM nonexistent" http://localhost:8000/sql
```
</TabItem>

<TabItem value="V1" label="V1.x">
```bash title="Request"
curl -X POST -u "root:secret" -H "NS: main" -H "DB: main" -H "Accept: application/json" -d "SELECT * FROM nonexistent" http://localhost:8000/sql
```
</TabItem>
