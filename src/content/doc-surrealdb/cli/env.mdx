...
<tr>
    ...
    <tr>
      ...
      <td scope="row" data-label="Default">64</td>
      <td scope="row" data-label="Notes">Threshold for deciding whether to wait for more transactions. Waits up to SURREAL_ROCKSDB_GROUPED_COMMIT_TIMEOUT to collect a larger batch if fewer than this number and more transactions are pending. This is NOT a hard requirement. Under low load, smaller batches will be committed immediately to maintain low latency. Under high load, batches naturally form and can exceed this threshold. Only used when SURREAL_SYNC_DATA is enabled and SURREAL_ROCKSDB_BACKGROUND_FLUSH is disabled.</td>
    </tr>
    <tr>
