...
import LightLogo from "@img/light/fly.png";
import DarkLogo from "@img/fly-icon.png";
...
We will store the username and password for the root user in secrets. Feel free to pass on any other options here. You can use `surreal start -h` to see which [environment variables](/docs/surrealdb/installation/running/env_vars) can be passed to surreal.
