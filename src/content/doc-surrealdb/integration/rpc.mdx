...
> [!NOTE]
>Version 2 significantly enhances data manipulation and retrieval methods by adding an optional object parameter. This enables direct control over statement clauses like `WHERE`, `LIMIT`, `TIMEOUT`..., previously only available through direct SQL execution.
>*   **Method Consolidation:**  The `merge`, `patch`, and `insert_relation` methods from v1 have been consolidated into the `update` and `insert` methods respectively.
