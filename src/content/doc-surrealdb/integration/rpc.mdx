...
SurrealDB returns structured, machine-readable errors with a `kind`, optional `details`, and optional cause chaining. This section describes the wire format so that SDKs and clients can parse and handle errors consistently.

### RPC error response format

When an RPC request fails, the response uses the following shape:
...
|-------|------|-------------|
| `code` | number | Legacy JSON-RPC 2.0 error code. Kept for backward compatibility. Clients should prefer `kind`. |
| `kind` | string | Machine-readable error category. See the [error kinds reference](#error-kinds) below. |
| `message` | string | Human-readable error description. |
| `details` | object (optional) | Structured details specific to the error kind. Schema depends on `kind`. Omitted when not available. |
| `cause` | object (optional) | The underlying error that led to this one. Same shape as the top-level error object (recursive). Omitted when not available. |

Older server versions may only include `code` and `message`; in that case, clients can derive the category from the [legacy code reference](#legacy-error-codes) below.

### Query result error format

When executing multi-statement queries (e.g. via the `query` method), each statement result can independently succeed or fail. For a failed statement, the result object looks like:

```json
{
    "status": "ERR",
    "time": "50.5µs",
    "result": "The table 'nonexistent' does not exist",
    "kind": "NotFound",
    "details": { "Table": { "name": "nonexistent" } },
    "cause": null
}
```

In query results, the error message is in the `result` field (not `message`), and `kind`, `details`, and `cause` are flattened into the top-level object alongside `status` and `time`.
...
<table>
    <thead>
        <tr>
            ...
            <th scope="col">Description</th>
            <th scope="col">Example scenarios</th>
        </tr>
    </thead>
    <tbody>
        <tr>
            <td scope="row" data-label="Kind"><code>Validation</code></td>
            <td scope="row" data-label="Description">Invalid input, parse error, bad parameters</td>
            <td scope="row" data-label="Example scenarios">Malformed query, invalid RPC params, empty namespace/database</td>
        </tr>
        <tr>
            <td scope="row" data-label="Kind"><code>Configuration</code></td>
            <td scope="row" data-label="Description">Feature or configuration not supported</td>
            <td scope="row" data-label="Example scenarios">Live queries disabled, bad GraphQL config</td>
        </tr>
        <tr>
            <td scope="row" data-label="Kind"><code>Thrown</code></td>
            <td scope="row" data-label="Description">User-thrown error from SurrealQL</td>
            <td scope="row" data-label="Example scenarios"><code>THROW "custom error"</code> in a query</td>
        </tr>
        <tr>
            <td scope="row" data-label="Kind"><code>Query</code></td>
            <td scope="row" data-label="Description">Query execution failure</td>
            <td scope="row" data-label="Example scenarios">Query timeout, cancelled, not executed</td>
        </tr>
        <tr>
            <td scope="row" data-label="Kind"><code>Serialization</code></td>
            <td scope="row" data-label="Description">Serialization or deserialization error</td>
            <td scope="row" data-label="Example scenarios">Encoding/decoding failure</td>
        </tr>
        <tr>
            <td scope="row" data-label="Kind"><code>NotAllowed</code></td>
            <td scope="row" data-label="Description">Permission or authorization error</td>
            <td scope="row" data-label="Example scenarios">Invalid credentials, expired token, method/function blocked</td>
        </tr>
        <tr>
            <td scope="row" data-label="Kind"><code>NotFound</code></td>
            <td scope="row" data-label="Description">Resource not found</td>
            <td scope="row" data-label="Example scenarios">Table, record, namespace, database, or RPC method not found</td>
        </tr>
        <tr>
            <td scope="row" data-label="Kind"><code>AlreadyExists</code></td>
            <td scope="row" data-label="Description">Duplicate resource</td>
            <td scope="row" data-label="Example scenarios">Record, table, namespace already exists</td>
        </tr>
        <tr>
            <td scope="row" data-label="Kind"><code>Connection</code></td>
            <td scope="row" data-label="Description">Client connection error</td>
            <td scope="row" data-label="Example scenarios">SDK-side: uninitialised or already connected</td>
        </tr>
        <tr>
            <td scope="row" data-label="Kind"><code>Internal</code></td>
            <td scope="row" data-label="Description">Unexpected server error</td>
            <td scope="row" data-label="Example scenarios">Catch-all for unknown/unexpected failures</td>
        </tr>
    </tbody>
</table>

Unknown `kind` values from newer server versions should be treated as `Internal` by clients.

### Key details schemas

The `details` object structure depends on `kind`. Below are examples for common patterns.

**Authentication errors (kind: NotAllowed):**

```json
{ "details": { "Auth": { "TokenExpired": null } } }
{ "details": { "Auth": { "InvalidAuth": null } } }
{ "details": { "Auth": { "NotAllowed": { "actor": "user:john", "action": "edit", "resource": "table:secrets" } } } }
```

**Resource errors (kind: NotFound):**
...
For backward compatibility, the numeric `code` field is preserved. New clients should use `kind` for error categorization. Older servers may not include `kind`; in that case, derive the category from this table.

<table>
    <thead>
        <tr>
            <th scope="col">Code</th>
            <th scope="col">Legacy meaning</th>
            <th scope="col">Kind</th>
        </tr>
    </thead>
    <tbody>
        <tr>
            <td scope="row" data-label="Code">-32700</td>
            <td scope="row" data-label="Legacy meaning">Parse error</td>
            <td scope="row" data-label="Kind">Validation</td>
        </tr>
        <tr>
            <td scope="row" data-label="Code">-32600</td>
            <td scope="row" data-label="Legacy meaning">Invalid request</td>
            <td scope="row" data-label="Kind">Validation</td>
        </tr>
        <tr>
            <td scope="row" data-label="Code">-32601</td>
            <td scope="row" data-label="Legacy meaning">Method not found</td>
            <td scope="row" data-label="Kind">NotFound</td>
        </tr>
        <tr>
            <td scope="row" data-label="Code">-32602</td>
            <td scope="row" data-label="Legacy meaning">Method not allowed</td>
            <td scope="row" data-label="Kind">NotAllowed</td>
        </tr>
        <tr>
            <td scope="row" data-label="Code">-32603</td>
            <td scope="row" data-label="Legacy meaning">Invalid params</td>
            <td scope="row" data-label="Kind">Validation</td>
        </tr>
        <tr>
            <td scope="row" data-label="Code">-32604</td>
            <td scope="row" data-label="Legacy meaning">Live query not supported</td>
            <td scope="row" data-label="Kind">Configuration</td>
        </tr>
        <tr>
            <td scope="row" data-label="Code">-32605</td>
            <td scope="row" data-label="Legacy meaning">Bad live query config</td>
            <td scope="row" data-label="Kind">Configuration</td>
        </tr>
        <tr>
            <td scope="row" data-label="Code">-32606</td>
            <td scope="row" data-label="Legacy meaning">Bad GraphQL config</td>
            <td scope="row" data-label="Kind">Configuration</td>
        </tr>
        <tr>
            <td scope="row" data-label="Code">-32000</td>
            <td scope="row" data-label="Legacy meaning">Internal error</td>
            <td scope="row" data-label="Kind">Internal</td>
        </tr>
        <tr>
            <td scope="row" data-label="Code">-32002</td>
            <td scope="row" data-label="Legacy meaning">Invalid auth</td>
            <td scope="row" data-label="Kind">NotAllowed</td>
        </tr>
        <tr>
            <td scope="row" data-label="Code">-32003</td>
            <td scope="row" data-label="Legacy meaning">Query not executed</td>
            <td scope="row" data-label="Kind">Query</td>
        </tr>
        <tr>
            <td scope="row" data-label="Code">-32004</td>
            <td scope="row" data-label="Legacy meaning">Query timed out</td>
            <td scope="row" data-label="Kind">Query</td>
        </tr>
        <tr>
            <td scope="row" data-label="Code">-32005</td>
            <td scope="row" data-label="Legacy meaning">Query cancelled</td>
            <td scope="row" data-label="Kind">Query</td>
        </tr>
        <tr>
            <td scope="row" data-label="Code">-32006</td>
            <td scope="row" data-label="Legacy meaning">Thrown error</td>
            <td scope="row" data-label="Kind">Thrown</td>
        </tr>
        <tr>
            <td scope="row" data-label="Code">-32007</td>
            <td scope="row" data-label="Legacy meaning">Serialization error</td>
            <td scope="row" data-label="Kind">Serialization</td>
        </tr>
        <tr>
            <td scope="row" data-label="Code">-32008</td>
            <td scope="row" data-label="Legacy meaning">Deserialization error</td>
            <td scope="row" data-label="Kind">Serialization</td>
        </tr>
    </tbody>
</table>
...
> Older server versions may only return `code` and `message` in the error object. See the [Error Handling](/docs/surrealdb/integration/rpc#error-handling) section for the full format and legacy code mapping.
