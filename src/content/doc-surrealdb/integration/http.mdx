...
HTTP endpoints can return errors in two different shapes depending on the type of failure.

### Query result errors

Endpoints that execute queries (`/sql`, `/key/*`) return HTTP 200 even when individual statements fail. Each statement result in the response array can independently succeed or fail. Failed statements include `status: "ERR"` along with structured error fields:

```json title="Example: query referencing a nonexistent table"
[
    {
        "status": "ERR",
        "time": "50.5µs",
...
|-------|------|-------------|
| `status` | string | `"ERR"` for failed statements. |
| `time` | string | Execution time for this statement. |
| `result` | string | Human-readable error message. |
| `kind` | string | Machine-readable error category (e.g. `"NotFound"`, `"Validation"`, `"NotAllowed"`). |
| `details` | object (optional) | Structured details specific to the error kind. |
| `cause` | object (optional) | The underlying error that led to this one (same shape, recursive). |

For the full list of error kinds and detail schemas, see the [RPC protocol Error Handling](/docs/surrealdb/integration/rpc#error-handling) reference.

### Authentication and request-level errors

Authentication endpoints (`/signin`, `/signup`) and requests with missing or invalid headers return non-200 HTTP status codes with a JSON body:

```json title="Example: invalid credentials on /signin"
{
    "code": 403,
...
Authentication error responses intentionally omit detailed cause information to avoid leaking internal details to unauthenticated users. See [Security Troubleshooting](/docs/surrealdb/security/troubleshooting) for debugging guidance.
...
When a query fails, the response still returns HTTP 200 but the statement result includes structured error information:
...
For the full list of error kinds and detail schemas, see the [Error Responses](#error-responses) section above or the [RPC protocol Error Handling](/docs/surrealdb/integration/rpc#error-handling) reference.
