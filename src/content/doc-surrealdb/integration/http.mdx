...
Query parameters can be provided via URL query parameters. These parameters will securely replace any parameters that are present in the query.

This practise is known as prepared statements or parameterised queries, and [should be used](/docs/surrealdb/reference-guide/security-best-practices#query-safety) whenever untrusted inputs are included in a query to prevent injection attacks:
