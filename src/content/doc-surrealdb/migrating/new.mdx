...
Many futures can be automatically be converted from a future to a computed field, which effectively will replace the above code with the following.
...
<details>
<summary>**Click to open**: List of new vs. old function names</summary>
...
> * **Severity**: Unlikely break.
> * **Action**: Add `=` to any existing mock ranges to keep them as inclusive ranges. 
> * **Action**: Change any queries that depend on the specific mock type of value being returned.
...
```surql
CREATE |person:1..10|; -- Now creates from person:1 to person:9
UPDATE |person:5..=9| SET name = "Billy"; -- Use = now for an inclusive range
```

Mocks are also no longer a value, when encountering `|person:1..5|` within an expression it will immediately be evaluated to an array of record ids.
...
Identifiers can be escaped in SurrealQL with `` or  ⟨ . Identifiers which are escaped this way now support escape sequences like newlines \n, unicode escape sequences `\u{AB1234}` and others also supported in strings. 
...
In addition to the new functions above, SurrealDB 3.0 also allows you to use [Surrealism](/docs/surrealdb/extensions) extensions plugins to write your own.
...
These are the parts I'm not quite certain about.

### Concurrent indexing

Indexes are now built and rebuilt concurrently by default.

CHECK: I think this is the case but also see that CONCURRENTLY is still a keyword. Does it just do nothing now?
