...
### `GROUP` and `SPLIT` cannot be used together

> * **Action**: Put either the `GROUP` or `SPLIT` section of such queries into a subquery.

#### Description

`GROUP` and `SPLIT` are different operations, as `GROUP` reduces the output of a query while `SPLIT` is a post-processing clause that expands it. Previous versions allowed both to be used together, resulting in inconsistent output.

A query with both clauses will no longer parse. If you have any queries that use both, determine whether you want to `SPLIT` the output of a `GROUP` query, or `GROUP` the output of `SPLIT` query, and put the first query into a subquery.

Example:

```surql
-- Original query no longer parses
SELECT age, emails FROM FROM user SPLIT emails GROUP BY age;

-- Fix by choosing to split, then grouping the output
SELECT age, emails FROM (SELECT * FROM user SPLIT emails) GROUP BY age;

-- Or choose to group, then split the output
