...
## `$this` `$parent` `$self` parameters

- Severity: Can break.

This will break when the parameters are used in unorthodox positions or when used from within a parenthesized expression

### Description

The `$this` `$parent` and `$self` special parameters refer to the current or parent document of a query.
The meaning of what the _current_ and _parent_ document are have changed in 3.0.

Before 3.0 each parenthesis pair `()` would create a new document.
So to refer to the document of a select statement when parenthesising an expression you would have to use `$parent`.
```surql
    CREATE table SET a = 42;
    SELECT 2 * ($parent.a + 1) as calc FROM table;
```

In 3.0 only when issueing a query which actually has a document is the value of `$this`, `$self` and `$parent` updated.
So the above query would have to be manually changed to the following to keep the same behavior.
```surql
    SELECT 2 * ($this.a + 1) as calc FROM table;
    // Or equivalent
    SELECT 2 * (a + 1) as calc FROM table;
```

Besides the changes with `()` `$this` and other special parameters will also only have a value when within a context 
where a document is present. 
See below for examples of places where `$this` no longer has a value.
```surql
// $this has no defined value since there is no single record document and usage will throw an error.
SELECT $this FROM foo GROUP BY field;

// The `FROM` clause is computed before the rest of the SELECT clause and so there is no valid document.
// in this context
SELECT field FROM $this;

// `$this` here is valid but referes to the top level select's document.
SELECT (SELECT foo FROM type::table($this)) FROM table;
```

### Mitigation

For cases where the query is affected by the changes to `()` fixing this issue will require manually checking the usages 
of `$parent` and changing them to `$this` or ommiting it where required.

For the cases where the document is no longer present there is no direct fix, although the document in those cases was ill-defined 
and queries which relied on documents being present where they shouldn't have been would probably have had strange undefined behavior.
These types of queries should probably be redesigned to not require the value that previously was in the special parameters.
