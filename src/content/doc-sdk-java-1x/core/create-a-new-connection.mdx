...
The `.connect()` method accepts a `String` pointing to the desired local or remote instance. Since the Java SDK supports running embedded databases, you can also specify an embedded endpoint such as `memory://` and `surrealkv://`. See the [SurrealDB CLI start](/docs/surrealdb/cli/start) documentation for server configuration options.
...
Depending on the complexity of your use case, you can switch to a specific [namespace](/docs/surrealql/statements/define/namespace) and [database](/docs/surrealql/statements/define/database) using the `.useNs()` and `.useDb()` methods. This is particularly useful if you want to switch to a different setup after connecting. You can also stay in the same namespace but switch to a different database. 
