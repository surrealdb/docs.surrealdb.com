    ...
    visit(tree, "element", (node) => {
      if (node.tagName === "blockquote") {
        ...
        if (match) {
          ...
          const heading = match[3] ?? kind[0].toUpperCase() + kind.slice(1);
          text = text.slice(match[1].length);
          node.properties.className = [`kind-${kind}`];
          // Strip the tag
          node.children[1].children[0].value =
            node.children[1].children[0].value.slice(match[1].length);
              ...
              children: [
                {
                  ...
                  children: fromHtml(kindMap[kind].icon, { fragment: true })
                    .children,
                },
                {
                  type: "text",
