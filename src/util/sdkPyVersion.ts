...
export function isVersionedSdkPyPath(pathname: string): boolean {
    return (
        pathname.startsWith("/docs/1.x/sdk/python") || pathname.startsWith("/docs/2.x/sdk/python")
    );
}
...
export function getSdkPyRelativePath(pathname: string): string {
    const versionedMatch = pathname.match(/^\/docs\/(?:1\.x|2\.x)\/sdk\/python(.*)$/);
    if (versionedMatch) {
 ...
 */
export function toVersionedPath(pathname: string, version: SdkPyVersion): string {
    if (version === "latest") {
        return pathname.replace(/^\/docs\/(?:1\.x|2\.x)\/sdk\/python/, "/docs/sdk/python");
    }
