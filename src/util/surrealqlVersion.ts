...
export function versionMatches(
    ...
    targetVersion: SurrealQLVersion
): boolean {
    ...
    if (targetVersion === '3.x') {
        ...
        return (
            /^v?2\./.test(versionString) ||
            compareVersions(versionString, 'v3.0.0') >= 0
        );
    }
