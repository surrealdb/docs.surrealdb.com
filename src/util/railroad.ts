...
type RRConstructors = {
  ...
  Terminal: (text: string, opts?: { href?: string; title?: string; cls?: string }) => unknown;
  NonTerminal: (text: string, opts?: { href?: string; title?: string; cls?: string }) => unknown;
  Comment: (text: string, opts?: { href?: string; title?: string; cls?: string }) => unknown;
  Skip: () => unknown;
};
...
export type RailroadNode =
  | { type: "Terminal"; text: string; href?: string; title?: string; cls?: string }
  | { type: "NonTerminal"; text: string; href?: string; title?: string; cls?: string }
  | { type: "Comment"; text: string; href?: string; title?: string; cls?: string }
  | { type: "Skip" }
  ...
  | { type: "ZeroOrMore"; child: RailroadNode; repeat?: RailroadNode; skip?: "skip" }
  | { type: "Diagram"; children: RailroadNode[]; padding?: number | [number] | [number, number] | [number, number, number, number] }
  | { type: "ComplexDiagram"; children: RailroadNode[]; padding?: number | [number] | [number, number] | [number, number, number, number] };

export type Padding = number | [number] | [number, number] | [number, number, number, number];

function buildNode(node: RailroadNode): unknown {
  switch (node.type) {
    case "Terminal":
      return Terminal(node.text, { href: node.href, title: node.title, cls: node.cls });
    case "NonTerminal":
      return NonTerminal(node.text, { href: node.href, title: node.title, cls: node.cls });
    case "Comment":
      return Comment(node.text, { href: node.href, title: node.title, cls: node.cls });
    case "Skip":
      return Skip();
    case "Sequence":
    ...
    case "OneOrMore":
      return OneOrMore(buildNode(node.child), node.repeat ? buildNode(node.repeat) : undefined);
    case "ZeroOrMore":
      return ZeroOrMore(
...
export function diagramFromAst(ast: RailroadNode): RRDiagram {
  ...
  throw new Error("diagramFromAst expects a root of type 'Diagram' or 'ComplexDiagram'");
}
