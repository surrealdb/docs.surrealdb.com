...
As of SurrealDB 3.0.4, imports via the [`surreal import`](/docs/surrealdb/cli/import) and [`/import`](/docs/surrealdb/integration/http#import) HTTP endpoint require the automatically generated `OPTION IMPORT` line to be present in order to disable events, live queries, field processing, and result output for optimal import performance. If side effects are desired when importing data, remove the line and use this endpoint instead.
