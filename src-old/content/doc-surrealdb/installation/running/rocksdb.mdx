...
A number of parameters can be used on startup such as `sync` to set when to flush the database to the file system. For more details on these parameters, see [this page](/docs/surrealdb/cli/start#supported-parameters-for-rocksdb) for the `start` command or [this page](/docs/surrealdb/cli/env#surrealkv-environment-variables) to set the same parameters using environment variables.