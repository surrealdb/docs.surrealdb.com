...
Since SurrealDB is a database that is designed to be used in a distributed environment, it is important to secure the database and the data that is stored in it. SurrealDB provides a number of methods for [authenticating users](/docs/surrealdb/security/authentication) and securing the database.

In your SurrealDB database, you can configure authentication using the [`DEFINE USER`](/docs/surrealql/statements/define/user) or [`DEFINE ACCESS`](/docs/surrealql/statements/define/access) statements. See the [security best practices](/docs/surrealdb/security/security-best-practices) for guidance on managing credentials and token expiration.
